...
- [ ] 如目标分支为 `master`，已确认聚合 `Candidate Quality` 及其中的 `Dependency Security` 组件通过，或已先执行 `npm run check:dependency-security`
...
- [ ] 如目标分支为 `master`，已明确合并后立即把最新 `origin/master` 回灌 `dev` 的执行人和时机
