...
| M5 | 文件上传与内容增强 | 文件上传（图片/文档）、Rust 扩展集成、图片处理、MarkdownEditor 图片上传集成 | 可上传图片/文档；自动生成缩略图；MarkdownEditor 支持图片上传；文件去重功能 | 🚧 进行中 (2025.12.21~) |
...
> **当前进度**：✅ M4（前端框架与认证）已于 2025.12.21 完成，🚧 M5（文件上传与内容增强）于 2025.12.21 开始。
>
> **M4 完成情况**：论坛应用完整实现（列表/详情/发帖/评论/编辑/删除/排序/搜索/点赞）、MarkdownEditor 富文本编辑器、速率限制中间件、审计日志系统、日志系统完整文档。详见 [2025.12 开发日志](./changelog/2025-12.md) 第四阶段总结。
>
> **M5 进度说明**：本阶段重点是文件上传功能（支持图片和文档）、Rust 扩展集成（图片水印）、图片处理（缩略图/多尺寸/压缩）、MarkdownEditor 图片上传集成。详细方案见 [文件上传设计方案](./features/file-upload-design.md)。
