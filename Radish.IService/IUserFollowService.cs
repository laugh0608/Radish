...
public interface IUserFollowService : IBaseService<UserFollow, UserFollowVo>
{
    ...
    Task<bool> UnfollowAsync(long followerUserId, long targetUserId, long tenantId, string? operatorName);

    /// <summary>获取关注状态</summary>
    Task<UserFollowStatusVo> GetFollowStatusAsync(long currentUserId, long targetUserId);

    Task<UserFollowStatusVo> GetFollowStatusAsync(long currentUserId, long targetUserId, long tenantId);

    /// <summary>分页获取我的粉丝列表</summary>
    Task<VoPagedResult<UserFollowUserVo>> GetMyFollowersAsync(long userId, int pageIndex, int pageSize);

    Task<VoPagedResult<UserFollowUserVo>> GetMyFollowersAsync(long userId, long tenantId, int pageIndex, int pageSize);

    /// <summary>分页获取我的关注列表</summary>
    Task<VoPagedResult<UserFollowUserVo>> GetMyFollowingAsync(long userId, int pageIndex, int pageSize);

    Task<VoPagedResult<UserFollowUserVo>> GetMyFollowingAsync(long userId, long tenantId, int pageIndex, int pageSize);

    /// <summary>分页获取我的关注动态流</summary>
    Task<VoPagedResult<PostVo>> GetMyFollowingFeedAsync(long userId, int pageIndex, int pageSize);

    Task<VoPagedResult<PostVo>> GetMyFollowingFeedAsync(long userId, long tenantId, int pageIndex, int pageSize);
    ...
    Task<VoPagedResult<PostVo>> GetMyDistributionFeedAsync(long userId, long tenantId, string streamType, int pageIndex, int pageSize);

    /// <summary>获取我的关系链汇总</summary>
    Task<UserFollowSummaryVo> GetMyFollowSummaryAsync(long userId);

    Task<UserFollowSummaryVo> GetMyFollowSummaryAsync(long userId, long tenantId);
}
