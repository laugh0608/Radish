...
public class SqlSugarAop
{
        ...
        string sql, SugarParameter[] p, ConnectionConfig config)
    {
        try
        {
            NormalizeDateTimeParametersForPostgreSql(config, p);
    ...
    private static void NormalizeDateTimeParametersForPostgreSql(ConnectionConfig config, SugarParameter[]? parameters)
    {
        if (config.DbType != SqlSugar.DbType.PostgreSQL || parameters == null || parameters.Length == 0)
        {
            return;
        }
        ...
        {
            parameter.Value = NormalizeDateTimeValue(parameter.Value);
        }
    }
    ...
    private static object? NormalizeDateTimeValue(object? value)
    {
        return value switch
        {
            ...
            DateTimeOffset?[] nullableDateTimeOffsets => nullableDateTimeOffsets
                ...
                .ToArray(),
            object[] objectValues => objectValues.Select(NormalizeDateTimeValue).ToArray(),
            IEnumerable<DateTime> dateTimeValues => dateTimeValues.Select(NormalizeDateTime).ToArray(),
            IEnumerable<DateTimeOffset> dateTimeOffsetValues => dateTimeOffsetValues.Select(item => item.ToUniversalTime()).ToArray(),
            _ => value
        };
    }
