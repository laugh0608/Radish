...
- ✅ 如果因为沙盒限制、PATH 缺失、权限隔离、网络/证书/文件系统限制等原因，导致无法判断代码是否真实可编译、可测试、可运行验证，则允许 AI 直接申请提权，改用系统环境执行**编译、测试、必要验证命令**
- ✅ 提权申请时必须明确说明用途，仅限“构建 / 测试 / 必要验证”，不得扩大为安装依赖、长期运行服务或其他高风险操作
...
**关键配置**:
- `Snowflake.WorkId`: 宿主差异配置（每环境唯一，0-30）
...
- 描述使用短句或短条目即可，避免写成长篇变更日志
- 纯单点修复可省略；跨文件、跨层或带行为变化的提交应优先补充描述
- 描述内容聚焦“做了什么 / 为什么需要 / 是否有边界”
