...
class RunnerTests: XCTestCase {
  ...
  func testPendingAuthCallbackIsConsumedOnce() throws {
    let callbackStore = RadishNativeAuthCallbackStore()

    XCTAssertTrue(
      callbackStore.receive(
        try XCTUnwrap(
          URL(string: "radish://oidc/callback?code=native-code&state=native-state")
        )
      )
    )
    XCTAssertEqual(
      try decode(callbackStore.take()),
      [
        "type": "login",
        "code": "native-code",
        "state": "native-state",
    ...
    XCTAssertNil(callbackStore.take())
  }
