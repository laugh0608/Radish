...
- forum feed、forum detail、评论分页、子评论分页、评论发布 / 回复、作者跳转与 detail 原地登录续接；评论 / 回复失败重试复用 `forum-comment:` 提交意图 key
...
- 聊天、完整通知中心、完整商城工作台、完整资产中心、完整创作器、购物车、退款、权益使用
- “移动版 WebOS”
...
24. 在通知列表中标记单条未读通知为已读，确认该条状态局部更新；失败时保留未读状态并显示错误；打开未读 forum 通知时，标记已读失败也不应阻断详情打开
25. 在我的 profile 中打开“查看最近访问”，确认公开帖子 / 文档 / 商品记录可打开对应详情，forum / docs 优先展示公开路径，空态 / 错误态 / 刷新 / 加载更多和返回 profile 正常
26. 在 forum 页发布一条纯文本帖子，确认分类、标题、正文和标签校验生效，成功后刷新列表并打开新帖子详情；详情公开链接应使用 `Post.PublicId`，发布失败应保留当前输入
27. 在商品详情购买 `1` 件商品，确认匿名态会登录回流，登录态会展示当前余额、先检查购买资格、要求支付口令，成功后刷新余额并进入订单详情；余额读取失败、余额不足、支付口令失败或接口失败应展示明确错误
28. 在我的 profile 中打开“查看商城订单”和“查看背包”，确认订单详情、背包来源订单 / 商品可按来源返回；订单详情扣款流水入口按订单 ID 筛选，背包发放确认可返回订单详情，不出现取消订单、退款、权益激活或道具使用入口
29. 在我的 profile 中打开“查看胡萝卜资产”，确认余额概要、累计统计、最近流水、空态 / 错误态和返回 profile 正常；从订单详情进入时应保留 `Order #orderId` 筛选上下文，不出现转账、打赏、调账或支付入口
30. 在我的 profile 中打开“查看经验记录”，确认等级概要、升级进度、冻结状态、最近流水、空态 / 错误态和返回 profile 正常，不出现经验调整、冻结治理或管理员复核入口
31. 关闭并重启应用，确认会话恢复或匿名回落符合当前 token 状态，复访入口仍符合最近状态
