...
- shop 公开商品列表与商品详情：发现页可进入原生公开商城列表，列表项和商城精选商品可打开商品详情，并通过 Android 返回键回到原来源；匿名态购买会登录回流，登录态可在详情页完成单商品购买
...
- 已登录态纯文本发帖：论坛页读取顶级分类，登录用户可发布纯文本帖子，成功后刷新列表并打开新帖子详情，详情公开链接使用 `Post.PublicId`；发布失败保留草稿输入；当前不开放富文本、附件、投票、抽奖、草稿箱或编辑
- 已登录态单商品购买：商品详情支持购买资格检查、支付口令输入和购买 `1` 件商品，成功后进入订单详情确认结果；匿名态购买会先登录并回到当前商品
- 已登录态商城订单 / 背包入口：我的页可打开订单列表、订单详情和背包，订单详情可按订单 ID 查看扣款流水并进入背包发放确认，背包权益 / 道具可查看来源订单或来源商品；当前不开放购物车、取消订单、退款、权益激活或道具使用
...
27. 在商品详情购买 `1` 件商品，确认匿名态会登录回流，登录态会先检查购买资格、要求支付口令，成功后进入订单详情；余额不足、支付口令失败或接口失败应展示明确错误
...
- P3-8-D “Flutter 纯文本发帖”已完成代码与自动化验证：论坛页读取顶级分类，登录态用户可发布纯文本帖子，成功后刷新列表并打开新帖子；本轮不扩展富文本、附件、投票、抽奖、草稿箱或编辑
