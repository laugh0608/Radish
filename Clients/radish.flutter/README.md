...
- 当前为 Android 起步、iOS 已建立平台基座的原生客户端；Windows / macOS / Linux 在共享 UI 和平台门禁通过后分别产品化
- `discover / forum / docs / profile` 四个高价值入口的首批真实只读页面
- 最小登录、退出、会话恢复与 Android / iOS 系统浏览器 OIDC 回调；session token 与 OIDC authorization attempt 通过 Android Keystore / iOS Keychain 支撑的安全存储持久化，Forum / Docs recent、recent profile 与 pending post-login target 使用非敏感跨平台偏好；Android 首次升级按“新 owner 缺失 -> 读取旧值 -> 写入并回读确认 -> 清除旧值”幂等迁移。请求前统一处理临近过期 token，同一 token 并发 refresh 合并，首次 `401` 只强制 refresh 后重试一次，只有 `invalid_grant` 清除会话
...
- iOS 真机、Apple Team、正式签名、provisioning、archive、更新与分发门禁，以及 Windows / macOS / Linux 平台工程
...
Flutter 不引入移动端专属 BFF，`HttpRadishApiClient` 直接消费 Gateway/API 返回的 Radish 响应信封。
...
12. `P8-A desktop platform readiness` 已关闭：macOS 本机工具链完整，Windows11 ARM64 日常 VM 尚缺 Flutter / Dart、CMake / Ninja / clang-cl 与 Visual Studio C++ Desktop workload，CleanBase 未启动；Debian VM 待实时工具链审计。当前 desktop 仍为 `unsupported` 内存壳，下一步先冻结 P8-B macOS 平台基座方案
...
Android 与 iOS 平台目录已经生成，Windows / macOS / Linux 尚未生成。P7-B 使用 `flutter_secure_storage 10.3.1`：Android 采用 RSA-OAEP + AES-GCM 并禁用应用数据自动备份，iOS 使用 Keychain 与 `first_unlock_this_device` 可访问级别；非敏感回访状态继续由 `shared_preferences` 承担。锁定插件已有三桌面实现，但当前 `RadishPlatformKind` 会把三者统一归为 `unsupported`，回退到内存 session / auth / follow-up，因此不能直接生成 runner 后宣称产品可用。P8 按 macOS、Windows、Linux、成组门禁推进；Tauri 和 WebOS 不作为桌面安装包替代路线。
...
15. 在问题帖详情确认回答区位于正文之后、轻回应之前；匿名态可读取已有回答；已登录态发布纯文本回答后应局部更新回答区和回答数，发布失败应只在回答输入区提示并复用失败重试 key
16. 在帖子详情确认轻回应位于回答区之后、评论区之前；匿名态可读取最近轻回应；已登录态发布一句轻回应后应保持当前位置，不刷新正文、回答或评论，新轻回应应显示在轻回应墙顶部并出现局部成功提示
17. 匿名态在帖子详情发起登录，取消登录后应出现显式提示；重试登录成功后应回到原帖子上下文，并可继续发布回答、轻回应或评论
18. 进入 profile，确认最近 forum 阅读、最近文档、公开主页复访、商城订单 / 背包、胡萝卜资产和经验记录入口是正式用户文案，可回到对应详情、公开主页或只读列表；已有公开资料刷新时应保留公开资料、公开帖子、公开评论、我的轻回应与复访区块，刷新失败只显示“刷新资料失败”局部提示
19. 在我的 profile 中确认“最近阅读”展示最近多条 forum 阅读上下文；重复打开同一帖子或评论应前置并去重；点击任意条目应回到对应帖子详情，返回后仍留在 profile
20. 在 profile 中确认“最近公开帖子”可继续加载更多；点击任意帖子应回到对应帖子详情
21. 在 profile 中确认“最近公开评论”可继续加载更多；点击任意评论应回到对应帖子，并自动定位到目标根评论或子评论位置，而不是停在帖子顶部
22. 在我的 profile 中确认“我的轻回应”只在已登录我的主页展示，公开主页不展示；若有多页轻回应，可继续加载更多；点击任意条目应回到对应帖子详情
23. 已登录态执行退出，确认浏览器登出回调后回到匿名态，后续重新进入 profile 或 detail 登录入口仍可用
24. 接收账号登录 Flutter 后，确认壳层出现通知入口；点击后应展示最近站内通知，选择 forum 通知可打开对应帖子，并在通知包含 `commentId` 时落到目标评论上下文；系统等不可跳通知保持只读展示
25. 在通知列表中标记单条未读通知为已读，确认该条状态局部更新；失败时保留未读状态并显示错误；打开未读 forum 通知时，标记已读失败也不应阻断详情打开
26. 在我的 profile 中打开“查看最近访问”，确认公开帖子 / 文档 / 商品记录可打开对应详情，forum / docs 优先展示公开路径，空态 / 错误态 / 刷新 / 加载更多和返回 profile 正常
27. 在 forum 页发布一条纯文本帖子，确认分类、标题、正文和标签校验生效，成功后刷新列表并打开新帖子详情；详情公开链接应使用 `Post.PublicId`，发布失败应保留当前输入
28. 在商品详情购买 `1` 件商品，确认匿名态会登录回流，登录态会展示当前余额、先检查购买资格、要求支付口令，成功后刷新余额并进入订单详情；余额读取失败、余额不足、支付口令失败或接口失败应展示明确错误
29. 在我的 profile 中打开“查看商城订单”和“查看背包”，确认订单详情、背包来源订单 / 商品可按来源返回；订单详情扣款流水入口按订单 ID 筛选，背包发放确认可返回订单详情，不出现取消订单、退款、权益激活或道具使用入口
30. 在我的 profile 中打开“查看胡萝卜资产”，确认余额概要、累计统计、最近流水、空态 / 错误态和返回 profile 正常；从订单详情进入时应保留 `Order #orderId` 筛选上下文，不出现转账、打赏、调账或支付入口
...
- P3-8-D “Flutter 公开商品详情”当前已从只读浏览推进到登录态单商品购买：匿名态购买会登录回流，登录态可检查购买资格、输入支付口令并购买 `1` 件商品，成功后进入订单详情；本轮不扩展完整商城、购物车、退款或权益激活
- P3-8-D “Flutter 登录态通知列表”已完成代码与自动化验证：已登录壳层展示最近站内通知，forum 通知可打开对应帖子 / 评论并返回打开前 tab，系统等不可跳通知只读展示，单条未读通知可显式标记已读，未读 forum 通知打开详情前也会尝试标记已读；本轮不扩展完整通知中心、批量已读、删除、系统推送、聊天通知或商城通知
- P3-8-D “Flutter 登录态商城订单 / 背包入口”已完成代码与自动化验证：我的页可打开订单列表、订单详情和背包，背包来源订单 / 商品可返回原上下文；本轮不扩展取消订单、退款、权益激活或道具使用
...
- P3-8-D “Flutter 论坛详情评论发布与回复”已完成代码与自动化验证：已登录用户可在原生帖子详情发布根评论、回复根评论或子评论，提交后局部更新评论区，失败只在评论输入区提示；P3-10 后续治理已补 `clientSubmissionId` 失败重试复用；当前已补作者根评论编辑与 `forum-comment-edit:` 失败重试复用；本轮不扩展子评论编辑、点赞、投票、审核治理或富文本评论
- P3-10 “Flutter 问答回答承接”已完成代码与自动化验证：问题帖详情展示回答区，登录态用户可发布纯文本回答，提交成功局部更新回答区，失败重试复用 `forum-answer:` 提交意图 key；本轮不扩展回答采纳、回答编辑、富文本回答、附件回答或问题发帖模式
- P3-8-D “Flutter 论坛评论登录回流与来源回归”已完成代码与自动化验证：匿名态从评论区发起登录后会回到当前评论输入区；从公开主页评论上下文进入详情并回复后，Android Back 仍返回公开主页来源
- P3-8-D “Flutter 公开商城列表入口”已完成代码与自动化验证：发现页可进入公开商城列表，列表项可打开商品详情并返回商城列表；购买能力仅在登录态商品详情内承接，不在列表页扩展购物车、订单或背包入口
...
- P3-8-D “Flutter 登录态最近访问”已完成代码与自动化验证：我的页可打开最近访问列表，公开帖子 / 文档 / 商品记录可回到对应原生详情，forum / docs 优先使用公开标识打开和展示路径；本轮不扩展清空、删除、推荐系统或完整浏览历史治理
- P3-8-D “Flutter 纯文本发帖”已完成代码与自动化验证：论坛页读取顶级分类，登录态用户可发布纯文本帖子，成功后刷新列表并打开新帖子；P3-10 后续治理已补 `clientSubmissionId` 失败重试复用；当前已补作者帖子正文编辑与 `forum-post-edit:` 失败重试复用；本轮不扩展富文本、附件、投票、抽奖、草稿箱、分类 / 标签编辑或回答编辑
...
- 通知入口只验证站内最近通知读取、单条标记已读、未读 forum 通知打开前已读回流和 forum 来源回流，不代表完整通知中心、批量已读、删除或系统通知栏推送已纳入 Android MVP
