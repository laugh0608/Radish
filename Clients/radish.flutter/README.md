...
- 已登录态通知列表：原生壳层会读取当前用户最近站内通知，展示标题、内容、类型、已读状态和时间；forum 通知可复用 forum detail handoff 打开 `postId / commentId`，系统等不可跳通知保持只读展示；未读通知支持单条显式标记已读
- 已登录态最近访问入口：我的页可打开最近访问列表，承接公开帖子 / 文档 / 商品详情；当前不开放清空、删除或推荐系统
- 已登录态纯文本发帖：论坛页读取顶级分类，登录用户可发布纯文本帖子，成功后刷新列表并打开新帖子；当前不开放富文本、附件、投票、抽奖、草稿箱或编辑
- 已登录态单商品购买：商品详情支持购买资格检查、支付口令输入和购买 `1` 件商品，成功后进入订单详情确认结果；匿名态购买会先登录并回到当前商品
- 已登录态商城订单 / 背包入口：我的页可打开订单列表、订单详情和背包，背包权益 / 道具可查看来源订单或来源商品；当前不开放购物车、取消订单、退款、权益激活或道具使用
...
24. 在通知列表中标记单条未读通知为已读，确认该条状态局部更新；失败时保留未读状态并显示错误
25. 在我的 profile 中打开“查看最近访问”，确认公开帖子 / 文档 / 商品记录可打开对应详情，空态 / 错误态 / 刷新 / 加载更多和返回 profile 正常
26. 在 forum 页发布一条纯文本帖子，确认分类、标题、正文和标签校验生效，成功后刷新列表并打开新帖子详情
27. 在商品详情购买 `1` 件商品，确认匿名态会登录回流，登录态会先检查购买资格、要求支付口令，成功后进入订单详情；余额不足、支付口令失败或接口失败应展示明确错误
28. 在我的 profile 中打开“查看商城订单”和“查看背包”，确认订单详情、背包来源订单 / 商品可按来源返回，不出现取消订单、退款、权益激活或道具使用入口
29. 在我的 profile 中打开“查看胡萝卜资产”，确认余额概要、累计统计、最近流水、空态 / 错误态和返回 profile 正常，不出现转账、打赏、调账或支付入口
...
- P3-8-D “Flutter 登录态通知列表”已完成代码与自动化验证：已登录壳层展示最近站内通知，forum 通知可打开对应帖子 / 评论并返回打开前 tab，系统等不可跳通知只读展示，单条未读通知可显式标记已读；本轮不扩展完整通知中心、批量已读、删除、系统推送、聊天通知或商城通知
...
- P3-8-D “Flutter 登录态最近访问”已完成代码与自动化验证：我的页可打开最近访问列表，公开帖子 / 文档 / 商品记录可回到对应原生详情；本轮不扩展清空、删除、推荐系统或完整浏览历史治理
...
- 通知入口只验证站内最近通知读取、单条标记已读和 forum 来源回流，不代表完整通知中心、批量已读、删除或系统通知栏推送已纳入 Android MVP
