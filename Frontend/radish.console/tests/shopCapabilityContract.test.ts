...
test('Console 商品表单应消费服务端能力元数据', () => {
  ...
  const displaySource = readFileSync(resolve(consoleRoot, 'src/pages/Products/productDisplay.ts'), 'utf8');
  const serviceSource = readFileSync(resolve(consoleRoot, '../../Radish.Service/ProductService.cs'), 'utf8');
  ...
  assert.match(displaySource, /voConfigurationRequirementKeys/);
  assert.match(displaySource, /voUnavailableReasonKey/);
  assert.match(serviceSource, /GetConfigurationRequirementKeys/);
  assert.match(serviceSource, /GetUnavailableReasonKey/);
