 ...
 * 获取系统设置列表
 */
export async function getSystemConfigs(t?: TFunction): Promise<SystemConfigVo[]> {
  const response = await apiGet<SystemConfigVo[]>('/api/v1/SystemConfig/GetSystemConfigs', { withAuth: true });

  if (!response.ok || !response.data) {
    throw createApiResponseError(response, translateFallback(t, 'systemConfig.feedback.loadListFailed', '获取系统设置失败'));
  }
 ...
 * 获取配置分类列表
 */
export async function getConfigCategories(t?: TFunction): Promise<string[]> {
  const response = await apiGet<string[]>('/api/v1/SystemConfig/GetConfigCategories', { withAuth: true });

  if (!response.ok || !response.data) {
    throw createApiResponseError(response, translateFallback(t, 'systemConfig.feedback.loadListFailed', '获取配置分类失败'));
  }
 ...
 * 根据ID获取配置详情
 */
export async function getConfigById(id: number, t?: TFunction): Promise<SystemConfigVo> {
  const response = await apiGet<SystemConfigVo>(`/api/v1/SystemConfig/GetConfigById?id=${id}`, { withAuth: true });

  if (!response.ok || !response.data) {
    throw createApiResponseError(response, translateFallback(t, 'systemConfig.feedback.loadDetailFailed', '获取配置详情失败'));
  }
...
export async function createConfig(
  configData: ConfigRequest & { category: string; key: string; name: string },
  t?: TFunction,
  ...
  if (!response.ok || !response.data) {
    throw createApiResponseError(response, translateFallback(t, 'systemConfig.feedback.updateFailed', '系统设置不支持通过 Console 新增'));
  }
 ...
 * 更新系统设置覆盖值
 */
export async function updateConfig(id: number, configData: ConfigRequest, t?: TFunction): Promise<SystemConfigVo> {
  const response = await apiPut<SystemConfigVo>(`/api/v1/SystemConfig/UpdateConfig?id=${id}`, configData, { withAuth: true });

  if (!response.ok || !response.data) {
    throw createApiResponseError(response, translateFallback(t, 'systemConfig.feedback.updateFailed', '更新系统设置失败'));
  }
...
export async function restoreConfigDefault(
  ...
  const response = await apiPut<SystemConfigVo>(
    ...
    {
      reason: request?.reason || translateFallback(t, 'systemConfig.reason.restoreDefault', '恢复默认值'),
  ...
  if (!response.ok || !response.data) {
    throw createApiResponseError(response, translateFallback(t, 'systemConfig.feedback.updateFailed', '恢复默认失败'));
  }
 ...
 * 获取系统设置变更历史
 */
export async function getConfigChangeLogs(id: number, take = 20, t?: TFunction): Promise<SystemConfigChangeLogVo[]> {
  ...
  if (!response.ok || !response.data) {
    throw createApiResponseError(response, translateFallback(t, 'systemConfig.feedback.historyLoadFailed', '获取系统设置变更历史失败'));
  }
 ...
 * 兼容旧删除接口，后端语义为恢复默认
 */
export async function deleteConfig(id: number, t?: TFunction): Promise<boolean> {
