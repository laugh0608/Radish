...
test('消息工作区应以服务端状态驱动分区、权限、动作和实时刷新', () => {
  ...
  const readSurfaceSource = readClientSource('src/apps/chat/useActiveChatReadSurface.ts');
  ...
  assert.match(readSurfaceSource, /advanceChannelReadState\(channelKey, target\)/);
...
test('消息移动端应在列表与会话详情之间切换并隐藏详情底部导航', () => {
  ...
  assert.match(chatSource, /onBackToList=\{hasRoutedChannel \? onBackToConversationList : \(\) => setActiveChannel\(null\)\}/);
