...
test('parsePublicForumRoute 应解析公开论坛发帖入口与受控作者态意图', () => {
  ...
  assert.deepEqual(parsePublicForumRoute('/forum/compose/', '?category=0'), {
    kind: 'compose',
    categoryId: null,
  });
  ...
  assert.deepEqual(parsePublicForumRoute('/forum/post/2042219067430928384', '?intent=edit'), {
    kind: 'detail',
    postId: '2042219067430928384',
    intent: 'edit',
  });
  assert.deepEqual(parsePublicForumRoute('/forum/post/2042219067430928384', '?intent=history'), {
    kind: 'detail',
    postId: '2042219067430928384',
    intent: 'history',
  });
