...
public class ProductVo
{
    ...
    public string VoCategoryId { get; set; } = string.Empty;
    ...
    public string? VoBenefitValue { get; set; }

    /// <summary>售价（胡萝卜）</summary>
    public long VoPrice { get; set; }

    /// <summary>原价（胡萝卜）</summary>
    public long? VoOriginalPrice { get; set; }

    /// <summary>是否有折扣</summary>
    public bool VoHasDiscount => VoOriginalPrice.HasValue && VoOriginalPrice.Value > VoPrice;

    /// <summary>折扣百分比</summary>
    /// <remarks>例如 80 表示 8 折</remarks>
    public int? VoDiscountPercent => VoHasDiscount && VoOriginalPrice > 0
        ? (int)(VoPrice * 100 / VoOriginalPrice.Value)
        : null;

    /// <summary>库存类型</summary>
    public StockType VoStockType { get; set; }

    /// <summary>库存数量</summary>
    public int VoStock { get; set; }

    /// <summary>已售数量</summary>
    public int VoSoldCount { get; set; }

    /// <summary>每人限购数量</summary>
    public int VoLimitPerUser { get; set; }

    /// <summary>是否有库存</summary>
    public bool VoInStock => VoStockType == StockType.Unlimited || VoStock > 0;
    ...
    public string VoDurationDisplay => VoDurationType switch
    {
        DurationType.Permanent => "永久",
        DurationType.Days => $"{VoDurationDays}天",
        DurationType.FixedDate => VoExpiresAt?.ToString("yyyy-MM-dd") ?? "未知",
        _ => "未知"
    };
    ...
    public bool VoIsOnSale { get; set; }

    /// <summary>是否启用</summary>
    public bool VoIsEnabled { get; set; }

    /// <summary>上架时间</summary>
    public DateTime? VoOnSaleTime { get; set; }
