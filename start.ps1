...
function Show-Menu {
    ...
    Write-Host "  8. Start Gateway + Auth + API"
    Write-Host "  9. Start Frontend + Console"
    Write-Host
    Write-Host "[Start ALL]"
    Write-Host " 10. Start ALL (Gateway + API + Auth + Frontend + Console)"
    Write-Host
...
function Start-Backend {
    ...
    try {
        ...
        Invoke-Step "dotnet clean ($Configuration)" { dotnet clean $projectPath -c $Configuration }
        Invoke-Step "dotnet restore" { dotnet restore $projectPath }
        Invoke-Step "dotnet build ($Configuration)" { dotnet build $projectPath -c $Configuration }
        Invoke-Step "dotnet run (http only)" { dotnet run --project $projectPath -c $Configuration --launch-profile http }
    }
...
function Start-BackendNoBuild {
    ...
    try {
        $env:ASPNETCORE_URLS = "http://localhost:5100"
        Invoke-Step "dotnet run API (no-build, http only)" { dotnet run --no-build --project $projectPath -c $Configuration --launch-profile http }
    }
    finally { Pop-Location }
}
...
function Start-Gateway {
    Push-Location $repoRoot
    try { dotnet run --project (Join-Path $repoRoot "Radish.Gateway/Radish.Gateway.csproj") --launch-profile https }
    finally { Pop-Location }
}

function Start-GatewayNoBuild {
    Push-Location $repoRoot
    try { dotnet run --no-build --project (Join-Path $repoRoot "Radish.Gateway/Radish.Gateway.csproj") --launch-profile https }
    finally { Pop-Location }
}

function Start-Frontend {
    Push-Location $repoRoot
    try { npm run dev --prefix $clientPath }
    finally { Pop-Location }
}

function Start-Console {
    Push-Location $repoRoot
    try { npm run dev --prefix $consolePath }
    finally { Pop-Location }
}
...
function Start-Auth {
    ...
    try {
        ...
        Invoke-Step "dotnet clean ($Configuration)" { dotnet clean $authProjectPath -c $Configuration }
        Invoke-Step "dotnet restore" { dotnet restore $authProjectPath }
        Invoke-Step "dotnet build ($Configuration)" { dotnet build $authProjectPath -c $Configuration }
        Invoke-Step "dotnet run (http only)" { dotnet run --project $authProjectPath -c $Configuration --launch-profile http }
    }
    finally { Pop-Location }
}
...
function Start-AuthNoBuild {
    ...
    try {
        $env:ASPNETCORE_URLS = "http://localhost:5200"
        Invoke-Step "dotnet run Auth (no-build, http only)" { dotnet run --no-build --project $authProjectPath -c $Configuration --launch-profile http }
    }
    finally { Pop-Location }
}
...
function Run-Tests {
    Push-Location $repoRoot
    try { Invoke-Step "dotnet test (Radish.Api.Tests)" { dotnet test $testProjectPath -c $Configuration } }
    finally { Pop-Location }
}
...
function Start-All {
    Write-Host "[Combo] ALL: Gateway + API + Auth + Frontend + Console..."
    Build-All
    Start-BackgroundShell "Gateway running at https://localhost:5000" "dotnet run --no-build --project Radish.Gateway/Radish.Gateway.csproj --launch-profile https"
    Start-BackgroundShell "Auth running at http://localhost:5200" "dotnet run --no-build --project Radish.Auth/Radish.Auth.csproj --launch-profile http"
...
function Start-FrontendConsole {
    Write-Host "[Combo] Frontend + Console..."
    Start-BackgroundShell "Frontend running at http://localhost:3000" "npm run dev --prefix Frontend/radish.client"
    Start-Console
}
...
switch ($choice) {
    ...
    "6"  { Start-DbMigrate }
    "7"  { Run-Tests }
    "8"  { Start-GatewayAuthApi }
    "9"  { Start-FrontendConsole }
    "10" { Start-All }
    default { Write-Error ('Unknown choice: ' + $choice); exit 1 }
}
