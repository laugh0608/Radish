...
- 当前部署口径：开发运行使用 IDE / `dotnet run` / `npm run dev`；本地容器验证使用 `Deploy/docker-compose.local.yaml`；测试与生产共用 `Deploy/docker-compose.yaml` 并通过 `Deploy/.env` 中的 `RADISH_IMAGE_TAG` 与 `RADISH_PUBLIC_URL` 区分；所有容器编排都会先执行 `dbmigrate apply` 初始化共享业务库
