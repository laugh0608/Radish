...
- 初始化入口：`Frontend/radish.client/src/i18n.ts`。
- 在 `main.tsx` 中全局挂载：
  - `import './i18n';`
...
- 推荐封装一个统一设置 `Accept-Language` 的请求 helper；当前新代码优先复用 `@radish/http`、业务 API helper 或等价的 `requestJson<T>()` 封装，不在页面组件中分散设置：
...
if (!parsed.ok || !parsed.data) {
    throw new Error(parsed.message || t('error.common.internal_error'));
}
setUser(parsed.data);
```
...
- 使用规范：
  - `Code`：面向前端/日志的**业务错误码**，建议大驼峰命名，如：`Auth.InvalidCredentials`、`User.NotFound`。
...
2. 在 `Frontend/radish.client/src/i18n.ts` 的 `resources.zh` 与 `resources.en` 中各自添加对应值。
...
- **统一错误码与 key 注册表**：在 `Docs/` 或代码中维护一份错误码清单，避免不同模块使用重复/冲突的 `Code` 或 `MessageKey`。
