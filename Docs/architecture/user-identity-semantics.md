...
3. **私有登录凭证**：`Email` 是当前登录凭证；`LoginName` 仅作为历史内部字段保留，不进入注册、登录或普通展示链路。
...
| `LoginName` | 历史登录账号 | 是 | 否 | B6 后不再作为注册 / 登录凭证和普通展示字段；仅允许作为历史内部兼容字段 |
...
`LoginName` 是历史私有登录字段。B6 后新注册、登录、Bootstrap、OIDC / CurrentUser 和普通前端状态不再依赖它；如短期保留数据库列，只允许作为内部兼容字段，不应继续进入新功能。

历史规则如下，主要用于旧字段兼容和清理判断：
...
用户本人页面和 Console 可查看邮箱，但应按页面上下文做必要脱敏。历史内部 `LoginName` 如短期保留，只允许出现在迁移或排障上下文中，不作为普通资料字段。日志、审计和错误信息中默认避免输出完整邮箱；如确需排障，应使用结构化字段并控制日志级别和访问权限。
...
- 补规范化字段或等价唯一索引：当前登录凭证侧优先是 `EmailNormalized`；`LoginNameNormalized` 只在保留历史内部字段并需要兼容旧数据时评估。首批已通过统一小写保存、查询规范化和注册唯一性检查承接运行时语义，后续如需要大小写保留展示值，再补独立规范化列。
...
- Console 支持按邮箱、公开索引、`PublicId` 和内部 ID 排障搜索；历史 `LoginName` 只在旧数据兼容排障中保留可选入口。（首批已展示公开句柄，搜索治理继续后置）
- 高风险账号字段变更进入二次确认与审计。
