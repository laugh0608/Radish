...
需要监听上传进度时允许使用 `XMLHttpRequest`，正式实现统一维护在 `Frontend/radish.client/src/api/chunked-upload.ts`，不得在各页面复制。该例外仍必须遵守统一客户端契约：

1. 通过 `getApiClientConfig()` 获取 `baseUrl`、timeout、token、当前语言和消息翻译器。
2. 表单同时提交 `sessionId`、`chunkIndex` 与 `chunkData`，分片索引和内容必须与服务端会话契约一致。
3. 设置 `Accept: application/json`、`Accept-Language` 与可选 Bearer token，并支持 timeout、`AbortSignal`、网络失败和取消语义；分片上传属于非幂等写入，不得自动重试。
4. 成功与失败响应都按 `MessageModel` 解析，保留 HTTP status、`Code / MessageKey / MessageArguments / TraceId`，再构造统一的 `ApiResponseError`；不得只拼接 HTTP 状态文本。
5. 创建会话、查询、合并和取消等不需要上传进度的请求继续使用 `@radish/http`。
...
- Markdown / 富文本正文的新写入与已迁移数据统一保存为 `attachment://{id}` 协议，而不是完整 URL；历史 `/uploads/**` 与旧域名绝对 URL 在完成迁移前只保留兼容读取。
...
- 采用 `AttachmentId / attachment://` 的数据更换域名时无需更新附件类字段；历史直链先按上传专题盘点迁移，再更新公开入口、反向代理和 OIDC 配置。
- 用户上传根目录不得通过 `/uploads/**` 静态直出；业务附件只通过 `/_assets/attachments/*`、受控下载或临时 token 路由读取，统一执行启用、删除、公开性和权限判定。
- `/uploads/DefaultIco/**` 只承载随版本发布的可信默认图标，不得扩展为用户文件的旁路入口。
...
Radish 项目采用**日历版本号（Calendar Versioning）**规范，版本号直观反映发布时间，便于追踪和管理。

### 版本号格式

标准版本号格式为：`vYY.M.RELEASE`

- **YY（年份）**：两位年份
  - 示例：26 代表 2026 年

- **M（月份）**：月份数字（1-12，不补零）
  - 示例：1 代表 1 月，12 代表 12 月

- **RELEASE（发版序号）**：当月发版序号
  - 从 1 开始递增
  - 每月重置为 1
  - 示例：v26.1.1 → v26.1.2 → v26.1.3

### 版本号示例

**典型发布流程：**
```
v26.1.1 - 2026年1月第1版（项目初始版本）
v26.1.2 - 2026年1月第2版（添加用户认证功能）
v26.2.1 - 2026年2月第1版（添加多租户支持）
v26.3.1 - 2026年3月第1版（首个正式发布版本）
v26.3.2 - 2026年3月第2版（添加 SignalR 实时通信功能）
v26.3.3 - 2026年3月第3版（修复登录超时 Bug）
v26.4.1 - 2026年4月第1版（添加 Rust 原生扩展支持）
v27.1.1 - 2027年1月第1版（新年度首版）
```
...
对于当日发布的热更新、紧急修复或阶段性 PR 合并，可使用扩展格式：`vYY.M.RELEASE.DDXX`

- **DD**：两位日期（01-31）
- **XX**：当日更新序号（01-99）
- 示例：
  - `v26.1.1.2601` - v26.1.1 版本在26日的第1次热更新
  - `v26.2.1.1203` - v26.2.1 版本在12日的第3次更新
  - `v26.3.2.0105` - v26.3.2 版本在1日的第5次阶段性合并
...
| `-release` | 正式发布版本 | `v26.3.1-release` |

**组合使用：**
- `v26.1.1.1203-dev` - 开发环境的热更新版本
- `v26.2.1-test` - 2月第1版的测试部署版本
- `v26.3.1-release` - 3月第1版的正式发布

**注意事项：**
- 热更新后缀和版本标识可组合使用
- 当前 GitHub Actions 镜像发布仅响应 `v*-dev`、`v*-test`、`v*-release` 三类 tag；不带环境后缀的 tag 不参与当前 GHCR 发布轨道
- 若热更新也需要触发镜像发布，请使用如 `v26.1.1.3003-test`、`v26.1.1.3003-release` 这类带环境后缀的 tag
- 正式发布时应使用 `-release` 标识
- 生产环境部署应固定使用明确的 `-release` tag
...
```bash
# 创建正式版本标签
git tag -a v26.1.1-release -m "Release v26.1.1: 项目初始版本"
git push origin v26.1.1-release

# 创建开发版本标签
git tag -a v26.1.2-dev -m "Dev: 添加用户认证功能"
git push origin v26.1.2-dev

# 创建测试版本标签
git tag -a v26.1.2-test -m "Test: 准备测试部署镜像"
git push origin v26.1.2-test

# 创建热更新标签
git tag -a v26.1.1.3003 -m "Hotfix: 修复关键登录 Bug（30日第3次更新）"
git push origin v26.1.1.3003
```
...
**配置文件：**
- 后端版本号：`Directory.Build.props` 中的 `<Version>` 标签（所有 .NET 项目共享）
- 前端版本号：各 `package.json` 中的 `version` 字段（不带 v 前缀，如 `26.1.1`）
- Rust 扩展：`Lib/radish.lib/Cargo.toml` 中的 `version` 字段
- 保持所有组件版本号同步更新

**版本兼容性：**
- 同一年度内的版本更新尽量保持向后兼容
- 跨年度版本可能包含破坏性变更，需提供迁移指南
- 重大架构变更应在 Release Notes 中明确标注
...
两者独立管理：
- 项目版本 v26.1.1 可能同时包含 API v1 和 API v2
- API 版本更新不一定触发项目版本号变更
- 项目重大版本更新通常伴随 API 版本升级
...
- Rust 原生扩展：已迁移到根目录 `Lib/radish.lib/`（统一 Rust 扩展库，构建后拷贝到 `Radish.Api/bin/<Configuration>/net10.0/`）；与其他前端项目保持一致的目录结构。
...
## BaseService 与 BaseRepository 使用约束

- 简单 CRUD 默认由 Controller 注入 `IBaseService<TEntity, TVo>`，不要为每个实体创建只转发基础方法的专属 Service。
- 只有在多实体事务、复杂业务验证、外部服务协作或专属查询确有需要时，才定义 `IService` 并实现自定义 Service；实现应继承 `BaseService<TEntity, TVo>` 复用基础能力。
- Repository 只返回实体，Service 对外必须映射为 DTO / Vo；Controller 禁止直接注入 `IBaseRepository<TEntity>`。
- Service 严禁直接访问 `_repository.Db.Queryable` 或 `_repository.DbBase.Queryable`；优先扩展或复用 Repository 查询能力。
- 去重、求和、最大值、最小值、平均值、批量查询和分页排序应在数据库侧完成，禁止先加载全量数据再做内存聚合，也要避免循环单条查询造成 N+1。
- 业务实体优先采用软删除；物理删除基础方法虽为兼容保留，但不得成为新业务的默认路径。
- 公共方法签名以当前接口与实现源码为准；完整能力清单、软删除字段、查询示例和自定义 Service 场景见 [BaseService 与 BaseRepository 专题指南](./base-service-repository-guide.md)。

## 实体与视图模型规范

### 软删除实体规范

- 需要软删除的业务实体必须实现 `IDeleteFilter`，包含 `IsDeleted`、`DeletedAt` 与 `DeletedBy`。
- 新记录默认 `IsDeleted = false`；软删除时记录时间与操作者，恢复时清理删除状态。
- 常规查询应依赖统一过滤能力排除已删除记录，不要在各业务查询中重复拼接口径不一的过滤条件。
- 业务删除优先调用 `SoftDeleteByIdAsync` / `SoftDeleteAsync`，需要恢复时调用对应 Restore 方法。
- 完整接口、实体示例与字段说明见 [BaseService 与 BaseRepository 专题指南](./base-service-repository-guide.md#软删除实体规范)。
...
- 参考实现：`Frontend/radish.client/public/webos.html`（Nebula OS 示例）展示了开机过渡、可拖拽窗口、多任务任务栏、Start 菜单与预置 Terminal/Editor/Browser 等应用图标；任何新页面都应优先在该场景中定义窗口外观与交互再迁移至 React 组件，保持动画节奏、图标尺寸与控件布局一致。
