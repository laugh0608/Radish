...
- `Radish.Api`
  ...
  - 日志：宿主调用 `builder.Host.AddSerilogSetup(runtimeLogging)`，由 `Radish.Extension.Log` 统一配置输出目标。Serilog 默认读取 appsettings，写入控制台与 `Logs/{ProjectName}/` 目录（普通日志 -> `Log.txt`，SqlSugar AOP 日志 -> `AopSql/AopSql.txt`），内部基于 `LogContextTool.LogSource` 区分日志类型并通过 `WriteTo.Async()` 异步落盘，避免阻塞请求线程；项目名解析当前优先依赖 `ContentRootPath` / `AppContext.BaseDirectory`，避免开发态把多个宿主统一误判到 `Logs/Radish/`。SQL AOP 生成策略由共享配置 `SqlAopLog` 控制，普通诊断默认关闭且仅允许显式 Development 诊断；慢操作 / 连接有独立开关，仅输出操作、参数数量和耗时，不输出 SQL 或参数值。Service AOP 已移除，事务层不重复记录最终异常。新日志调用优先 `ILogger<T>`，已有 Serilog 调用按统一日志专题迁移；候选入口与 DbMigrate 诊断规则见[日志系统](/guide/logging)及[统一事件契约](/features/unified-logging-contract)。
  - API 文档：开发环境把 Scalar UI 映射到 `/scalar`（`/api/docs` 重定向到 `/scalar`）；`IApiVersionDescriptionProvider` 按实际 Controller 版本动态注册 OpenAPI 文档，当前只有 `/openapi/v1.json`。如需定制交互，可在 `Radish.Api/wwwroot/scalar/config.js` 中追加 JS 配置并在 `MapScalarApiReference` 中调用 `WithJavaScriptConfiguration`。
  - API 版本控制：采用 **URL 路径版本控制**（`/api/v{version}/[controller]/[action]`），基于 `Asp.Versioning.Mvc` (8.1.0) 实现；Controller 通过 `[ApiVersion("x.0")]` 特性声明版本，未指定版本时默认使用 v1.0。当前生产 Controller 全部归属 v1；v2 及更高版本仅在存在真实产品或迁移契约时启用，不以配置读取、性能基准或演示端点占位。OpenAPI 与 Scalar 只展示实际存在的版本。详细规范见 [开发规范](./specifications.md) 的"API 版本控制规范"章节。
...
- 配置与服务访问：Program.cs 依次调用 `hostingContext.Configuration.ConfigureApplication()` → `builder.ConfigureApplication()` → `app.ConfigureApplication()` → `app.UseApplicationSetup()`，把 Configuration/HostEnvironment/RootServices 注入到 `Radish.Common.Core.App`；在非 DI 管道下可使用 `App.GetService*`、`App.GetOptions*` 手动解析服务或配置。应用预启动且 `InternalServices` 尚未建立时，非强制解析返回空，`App.CurrentUser` 回退匿名上下文；只有显式 `mustBuild` 的强制读取才抛出启动前不可用错误。常规字符串读取仍统一使用 `AppSettings.RadishApp("Section", ...)`，批量强类型配置通过 `ConfigurableOptions + AddAllOptionRegister` 自动绑定 `IConfigurableOptions`。
- `Radish.Service`
...
- 绝对时刻、系统业务日、API `DateOnly` 与物理 `date` 列的完整契约见 [时间语义与业务自然日](/guide/time-semantics)。
