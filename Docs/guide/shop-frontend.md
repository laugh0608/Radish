...
### 7.1.1 正式 Web 与 WebOS 集成方式

商城系统当前同时存在正式 Web 路由和 WebOS 历史工作台入口。正式 Web 承接普通浏览器里的公开商品浏览、登录后购买、订单和背包；WebOS 商城窗口继续作为 `/desktop` 保留入口维护。
...
正式 Web 路由当前承担浏览器主路径：公开浏览使用 `/shop`、`/shop/products`、`/shop/product/:productId`，登录后继续购买使用 `/shop/product/:productId?intent=purchase`，订单和背包使用 `/shop/orders`、`/shop/order/:orderId`、`/shop/inventory`，资产入口使用 `/me/assets` 与 `/me/assets/transactions`。WebOS 工作台内的商城窗口继续保留应用内路由，供 `/desktop` 历史入口和窗口恢复使用：
...
公开商品详情页当前提供复制 canonical 公共链接入口，复制 `https://radishx.com/shop/product/:productId` 这类规范 URL；继续购买时使用 `/shop/product/:productId?intent=purchase` 登录回流，购买成功后进入 `/shop/order/:orderId`。订单列表、订单详情和背包的正式 Web 契约见 [商城正式 Web 回流与 WebOS 深链保留](/guide/shop-web-return-paths)。
...
7. 购买成功：
   - 显示成功通知
   - 后端返回有效订单 ID 时进入订单详情
   - 订单详情可回到商品详情；已发放权益可在背包从来源回到订单 / 商品
8. 购买失败：
   - 显示错误通知
   - 提示失败原因
