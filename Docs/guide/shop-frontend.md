...
- 商品评价 contract / client 位于 `@radish/http`，Client 的商城 API 只做薄封装，不自建请求协议。
...
- 匿名用户可以读取综合评分、五星分布和稳定分页评价；公开列表只包含当前可见评价。
- 登录用户通过本人评价接口读取 Completed 购买资格、已有评价和当前版本；不能用前端订单列表推断资格。
- 创建 / 编辑提交 `Rating / Comment / ExpectedVersion`，删除提交评价 LongId 与 `expectedVersion`；所有 LongId 保持字符串。
- 编辑草稿离开前必须确认；结构化 `409` 保留星级和评论草稿，刷新权威评价后由用户决定是否重新提交。
- 评价 unavailable / stale 只降级评价区，不覆盖商品主体和购买动作；真实零评价与读取失败必须分开表达。
- `ProductReview` 举报复用共享举报弹窗并回到父商品；治理隐藏和申诉恢复后的列表、总数、均分与五星分布都以重新读取结果为准。
- 不提供评价媒体、回复、有用 / 点赞、追评、标签、推荐排序、独立评价中心或实时刷新。
...
- Tauri 暂时弃用，只保留历史验证资产，不进入当前商城开发、UI 或验证门禁。
