...
> 最后更新：2026-08-09
> 状态：权限映射与种子 V1 已收口；角色聚合写入进入 R2-C03 能力门禁
...
- 深层写入动作必须在回调传递和 handler 入口双层复核权限；详情 footer、抽屉、弹窗、上传、导入、导出和恢复默认等入口不能只依赖按钮隐藏或禁用
...
角色授权页当前采用整页覆盖保存，前端需要把授权资源集合、角色 ID 和接口预览相关 ID 全部按字符串处理。

保存交互边界：

- 保存按钮在请求进行中必须进入禁用 / 保存中状态，并阻止重复提交。
- 保存 handler 入口必须复核当前是否可保存；如果按钮因权限、并发状态或数据态禁用，直接调用 handler 也不能继续提交。
- 同一份 `expectedModifyTime` 只能用于一次保存请求；重复请求可能被后端判定为并发冲突，前端不应让用户在保存中继续触发。
...
`2026-08-09` 的代码事实审计确认，上述前端保存约束与资源映射已经存在，但不能据此把角色授权聚合描述为完整闭环：

- `/roles/:roleId/permissions` 当前误以 `roles.edit` 作为路由门槛，导致已有的 view-only 页面状态不可达；路由读取应服从 `roles.view`，写入继续要求 `roles.edit`。
- `System / Admin` 会按角色名获得默认全权限，但普通角色 CRUD 尚未固定内建角色、保留名与唯一性边界。
- 当前 `expectedModifyTime` 来自活动角色资源关联的最大修改时间，纯删除 / 清空授权不能形成可靠单调版本；读取比较与多次关联写入也尚未形成事务内 CAS。
- 普通并发异常尚未稳定映射为结构化 `409`。

因此 V1 的“已收口”只指权限键、URL 映射、种子与消费链路，不代表角色治理写入已满足并发与聚合安全。下一步按 [R2-C03 readiness 记录](/records/f4-r-r2-c03-console-settings-permissions-readiness-audit-2026-08-09)完成内建角色保护、命令 DTO、单调版本、事务内 CAS 和稳定错误契约后，再进入危险确认设计。
...
5. 新增详情页、抽屉或深层动作时，先确认页面级访问权限、按钮级操作权限、回调传递和 handler 入口校验是否一致
