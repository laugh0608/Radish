...
- Console 管理员调账只能绑定刚取得的权威用户余额快照，不再在调账表单内接受第二份用户 ID；目标变化时必须放弃旧草稿。
- `AdminAdjustBalance` 必须提交 `ExpectedVersion + IdempotencyKey`。同键同载荷重放返回既有流水号，异载荷复用、处理中状态和余额版本变化均返回结构化冲突；余额、Main `CoinTransaction` 与幂等成功记录共同提交或回滚。
- 管理流水固定使用 `CreateTime desc + Id desc` 稳定分页，页码必须为正数且 `pageSize <= 100`；余额响应包含权威用户显示名与版本，供 Console 提交前确认目标。
- 商城购买扣款流水以 `TransactionType=CONSUME`、`BusinessType=Order`、`BusinessId=OrderId` 关联订单；Console 可从订单详情跳到对应流水筛选结果，用于资产一致性排查。
- 论坛“胡萝卜 +1”统一定义为基于现有账本的内容赞赏：发送者扣除 `1 胡萝卜`、作者收到 `1 胡萝卜`，不创建新积分或风评值；服务端、Pencil、正式 Web 与 Gateway 成组验收均已完成。
