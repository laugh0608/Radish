...
B6 后这些字段的消费者关系不变，但字段语义已经收敛：`name` 表示 `DisplayName`，`preferred_username` 表示 `DisplayHandle`，二者都不再表示私有 `LoginName`。官方客户端只能把它们作为界面展示兜底，不得恢复为登录凭证或公开搜索输入。
...
截至 `2026-04-04`，这两类边界已经结合仓库资产侧与当前生产环境事实完成当前轮次确认，因此不再构成“是否允许启动”的默认阻塞项；后续若部署形态变化，仍需回到仓库外边界清单补事实。
