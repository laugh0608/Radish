...
- `LoginName` 已从当前实体、注册 / 登录链路、Bootstrap、普通 DTO、Console 系统设置和新库 schema 退场；旧本地 SQLite 不做兼容迁移，按 B6 要求删除并重新初始化。
- 创建用户时由服务端分配 `User.PublicId` 与 `User.PublicIndex`，前端不计算公开索引，也不把公开索引用作登录凭证。
- `PublicIndex` 自动分配会读取 `UserIdentity.PublicIndex.ReservedIndexes` 与 `UserIdentity.PublicIndex.VanityRules`，跳过显式保留号和命中靓号规则的号码；配置 JSON 非法、越界、重复或包含未知规则键时，后端应暴露配置错误。
...
- 当前只允许以下白名单路径，避免登录回调被外部开放重定向利用：
  ...
  - `/me`、`/me/assets`、`/me/assets/transactions`：用于我的状态、资产和资产流水登录回流，不接受 query 或 hash
  - `/me/content`：用于我的内容登录回流，仅保留 `tab=posts|comments|quick-replies` 和正整数 `page`
  - `/me/history`、`/me/experience`：用于最近访问和经验记录登录回流，仅保留正整数 `page`
  - `/me/attachments`：用于附件库登录回流，仅保留 `businessType=All|General|Post|Comment|Avatar|Document`、`keyword` 和正整数 `page`
  - `/messages`：用于消息复访登录回流，可保留合法 `channelId/messageId`，其中存在 `messageId` 时必须同时存在合法 `channelId`
  - `/pet`：用于电子宠物登录回流，不接受 query 或 hash
  - `/forum/compose`：用于作者发帖回流，可保留正整数 `category`
  - `/forum/post/:postId`：仅用于公开详情参与回流，`postId` 必须是 `pst_...` PublicId 或正整数旧 ID，且必须携带 `intent=comment|quickReply|answer|edit|history`；可选 `commentId` 必须是正整数，且只允许搭配 `comment|quickReply`
  - `/shop/product/:productId`：仅用于商品购买回流，必须携带 `intent=purchase`
  - `/shop/orders`、`/shop/inventory`、`/shop/order/:orderId`：用于订单、背包和订单详情登录回流，不接受额外 query
  - `/docs/mine`、`/docs/compose`、`/docs/edit/:documentId`、`/docs/revisions/:documentId`：用于文档作者工作流登录回流，不接受 query 或 hash
