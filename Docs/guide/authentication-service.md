...
当前注册页的 `Username` 语义已经收敛为私有 `LoginName`，不是公开昵称。注册与登录遵循以下口径：

- 注册必填 `LoginName`、`Email`、密码与确认密码。
- `LoginName` 长度默认 `3-32`，由 `UserIdentity.LoginName.MinLength` / `UserIdentity.LoginName.MaxLength` 系统设置治理；只允许字母和数字，且至少包含一个字母；提交后统一转为小写保存。
- `admin`、`administrator`、`system`、`root`、`support`、`official`、`bot`、`moderator`、`radish` 等系统保留登录名不可注册。
- `Email` 必填，按邮箱格式校验，长度不超过 `254`，提交后统一转为小写保存；同一邮箱只能绑定一个有效用户。
- 登录框允许输入 `LoginName` 或 `Email`，服务端按原值与小写规范化值匹配可登录用户，并保持统一的失败提示。
- 当前阶段 `User.UserName` 数据库列仍承接公开展示名，注册时默认等于登录名；公开页面应展示 `DisplayName#PublicIndex` 或等价 `DisplayHandle`，不得把 `LoginName` 或 `Email` 当作公开昵称。
