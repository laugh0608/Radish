...
4. 点击任一摘要卡的主区域，确认页面会优先平滑滚动到本页对应区块，并把当前公开分发页来源写回到对应 `section` 状态；点击摘要卡里的“直接进入公开页”动作时，再确认会跳转到对应公开入口。
...
- 四张公开摘要卡默认优先服务“先在本页继续看一眼”，主区域会滚动定位到对应区块，同时仍保留明确的“直接进入公开页”动作。
