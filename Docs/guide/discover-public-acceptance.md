...
9. 从 `/forum`、`/docs`、`/leaderboard`、`/shop` 任一公开页顶部点击“社区发现”，确认能稳定回到 `/discover`，且不触发整页跳桌面或异常丢失公开壳层样式。
10. 从 `/discover` 进入 forum 帖子详情或 docs 详情后，确认详情页返回动作会优先回到 `/discover`，而不是回退成专题默认列表。
