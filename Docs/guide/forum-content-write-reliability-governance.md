...
> 状态：`创建链路、首批编辑重试幂等与 Flutter 发帖 / 评论承接已完成`
>
...
`WOG-1` 至 `WOG-6` 首轮写操作治理已经阶段收束。下一批不继续扩展 Flutter 转账、完整移动商城或服务端强制 `idempotencyKey`，而是回到 P3-10 后续产品 / 治理增量选择，优先治理论坛内容发布、重复提交、短窗口刷屏和编辑历史边界。
...
当前代码已按确认方案覆盖创建链路和首批编辑重试幂等：`PublishPostDto`、`CreateCommentDto`、`CreateAnswerDto`、`UpdatePostDto` 与 `UpdateCommentDto` 新增可选 `ClientSubmissionId`，服务端新增 `ContentSubmissionRecord` 作为内容提交意图记录，Web 论坛发帖、评论、回答、帖子编辑和评论编辑会按提交意图生成并复用 key。Flutter 原生纯文本发帖、根评论和评论回复也已复用同一 `clientSubmissionId` 字段和 `forum-post:` / `forum-comment:` 前缀规则。帖子 / 评论编辑历史继续使用既有 `PostEditHistory` / `CommentEditHistory`，不新增通用编辑历史表。
...
| Flutter 发帖 / 评论 | `Clients/radish.flutter` 原生纯文本发帖和根评论 / 回复已生成并提交 `clientSubmissionId` | 失败后直接重试复用同一 key；成功、草稿 / 回复对象变化或账号变化后生成新 key |
...
- Redis 分布式锁、队列、Outbox 或完整可观测性平台。
...
Web 论坛创建和首批编辑路径已传 key；Flutter 当前已有的纯文本发帖、根评论和评论回复也已传 key。旧客户端或尚未接入的客户端不传 key 时走内容指纹短窗口保护。若 Flutter 后续新增问答回答、帖子编辑或评论编辑入口，继续复用同一字段和前缀规则，但需按对应入口范围单独评审。
...
该项作为后续治理方向保留，本次代码批次先通过 `ContentSubmissionRecord` 和内容指纹短窗口处理重复提交，不实现独立频率限制平台。

建议首批分层：

| 类型 | 限制维度 | 默认语义 |
| --- | --- | --- |
| 发帖 | `TenantId + UserId + PostType` | 限制连续发帖，不影响评论其他帖子 |
| 评论 / 回复 | `TenantId + UserId + PostId + CommentScope` | 限制同帖刷屏，不影响跨帖子互动 |
| 回答 | `TenantId + UserId + PostId` | 限制同一问答帖重复回答 |
| 编辑 | `TenantId + UserId + TargetType + TargetId` | 沿用既有编辑次数 / 评论时间窗，补提交重试去重 |

管理员和系统任务不走普通用户发布频率限制，但必须保留现有权限、审计和编辑历史语义。

首批不引入 Redis 计数。数据库记录是短窗口和重试判断的真值；多实例、高频压力或真实刷屏问题出现后，再按 Redis 与缓存治理专题评审缓存 / 限流辅助。
...
| 频率限制 | `操作过于频繁，请稍后再试`，并返回可展示的等待秒数 |
| 编辑无变化 | `内容没有变化，无需保存` |

如果项目当前没有统一 `409 Conflict` 封装，首批可沿用现有业务响应路径，但测试必须区分“重复提交 / 频率限制 / 参数错误 / 权限不足”。
...
3. 前端契约
   ...
   - `radish.flutter` 纯文本发帖、根评论和评论回复请求生成并复用 `clientSubmissionId`。
   - 网络失败重试不生成新 key。
4. 数据库结构
...
- 不启动完整反垃圾、用户信誉、审核队列、Redis 分布式锁、Outbox 或 E2E 平台。
...
5. Web 首批传 key，Flutter 已承接当前存在的纯文本发帖、根评论和评论回复入口。

完成本批验证后，后续再评审独立频率限制、Flutter 问答回答 / 编辑入口是否需要新增产品能力承接，以及下一批 P3-10 产品 / 治理增量，不顺带扩大资产幂等范围。
