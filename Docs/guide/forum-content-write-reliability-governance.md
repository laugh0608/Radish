...
> 状态：`创建链路、首批编辑重试幂等、Flutter 发帖 / 评论承接与创建类限频已完成`
>
...
当前代码已按确认方案覆盖创建链路、首批编辑重试幂等和创建类短窗口频率限制：`PublishPostDto`、`CreateCommentDto`、`CreateAnswerDto`、`UpdatePostDto` 与 `UpdateCommentDto` 新增可选 `ClientSubmissionId`，服务端新增 `ContentSubmissionRecord` 作为内容提交意图记录，Web 论坛发帖、评论、回答、帖子编辑和评论编辑会按提交意图生成并复用 key。Flutter 原生纯文本发帖、根评论和评论回复也已复用同一 `clientSubmissionId` 字段和 `forum-post:` / `forum-comment:` 前缀规则。发帖、评论 / 回复和回答已基于近期成功的 `ContentSubmissionRecord` 做创建类短窗口限频；帖子 / 评论编辑历史继续使用既有 `PostEditHistory` / `CommentEditHistory`，不新增通用编辑历史表。
...
6. Web 首批传 key，Flutter 已承接当前存在的纯文本发帖、根评论和评论回复入口。

完成本批验证后，后续再评审 Flutter 问答回答 / 编辑入口是否需要新增产品能力承接，以及下一批 P3-10 产品 / 治理增量，不顺带扩大资产幂等范围。
