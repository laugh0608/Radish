...
- `/forum`、`/forum/category/:categoryId`、`/forum/search`、`/forum/post/:postId` 的移动阅读布局、分类上下文、搜索过滤、列表卡片密度、分页或返回链路改动后
...
3. 直接打开 `/forum/search`，确认页面会进入公开搜索页，而不是静默退回全部帖子列表；首屏会展示只读搜索说明、关键词输入、时间范围和排序过滤。
4. 在窄屏下检查分类页、全部帖子页与搜索页首屏，确认头部说明、分类 rail、搜索过滤区、列表卡片标题、分类、标签、作者、时间、统计和最近互动没有重叠、截断异常或明显挤压。
5. 切换分类与“最新 / 最热”，确认列表刷新后 URL 会稳定回写到 `/forum` 或 `/forum/category/:categoryId`，并继续带上分页 / 排序查询参数；刷新页面后状态仍能恢复。
6. 在 `/forum/search` 输入关键词，并分别切换时间范围、排序和分页，确认 URL 会稳定回写 `q / sort / range / start / end / page`，刷新后仍能恢复搜索上下文。
7. 切到第 `2` 页及之后任意一页，分别从分类列表和搜索结果进入帖子详情，确认返回后不会丢失原来的分类 / 搜索 / 排序 / 分页状态。
8. 打开一条普通帖子详情，确认详情页首批仍只承载正文、轻回应墙展示与评论阅读，没有误带入桌面工作台交互。
9. 在帖子详情页检查“返回列表”按钮、浏览 / 轻回应 / 讨论摘要胶囊、评论排序按钮与“加载更多”按钮，确认窄屏下纵向节奏自然，没有横向撑破。
10. 分别检查一条长标题帖子和一条多标签帖子，确认分类页、搜索结果卡片与详情页都没有出现文本溢出、标签顶破或关键信息被完全挤没。
11. 抽查一个不存在或不可访问的 `categoryId`，确认公开壳层会给出只读状态页，而不是静默退回全部帖子或空白页。
12. 再检查一条问答 / 投票 / 抽奖帖子，确认结构化状态标签、正文和只读详情块在公开壳层下仍可阅读，未误暴露编辑 / 提交类操作。

## 5. 预期结果

- `/forum`、`/forum/category/:categoryId`、`/forum/search` 与 `/forum/post/:postId` 都以公开内容壳层直达，不绕回桌面入口。
...
- 公开壳层没有误暴露标签独立直达、发帖、评论提交、投票提交或其他桌面交互动作。
...
- 验收人：<name>
- 验收范围：forum 公开移动入口
- 执行入口：/forum、/forum/category/:categoryId、/forum/search、/forum/post/:postId
- 结果：通过 / 阻塞
