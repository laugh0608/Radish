...
10. 从 `/discover` 进入任一帖子详情后，点击详情页返回按钮，确认会优先回到 `/discover`，而不是回退成 forum 默认列表。
11. 从 `/u/:id` 的帖子 / 评论入口进入任一帖子详情后，点击详情页返回按钮，确认会优先回到原公开个人页，而不是丢失来源上下文。
12. 在帖子详情页检查“返回列表”按钮、浏览 / 轻回应 / 讨论摘要胶囊、评论排序按钮与“加载更多”按钮，确认窄屏下纵向节奏自然，没有横向撑破。
13. 分别检查一条长标题帖子和一条多标签帖子，确认分类页、标签页、搜索结果卡片与详情页都没有出现文本溢出、标签顶破或关键信息被完全挤没。
14. 抽查一个不存在或不可访问的 `categoryId`，确认公开壳层会给出只读状态页，而不是静默退回全部帖子或空白页。
15. 抽查一个不存在或不可访问的 `tagSlug`，确认公开壳层会给出只读状态页，而不是静默退回全部帖子或空白页。
16. 通过帖子卡片或帖子详情里的“问答 / 投票 / 抽奖”徽标进入 `/forum/question`、`/forum/poll` 与 `/forum/lottery`，确认会落到对应公开类型列表，而不是停留在静态徽标。
17. 再检查一条问答 / 投票 / 抽奖帖子，确认结构化状态标签、正文和只读详情块在公开壳层下仍可阅读，未误暴露编辑 / 提交类操作。
