...
2. 再按改动主题补专题回归：
   ...
   - React 复用路线历史结论、Capacitor Android 已清理代码的 spike 记录或 Tauri 历史验证相关问题，优先参考 [React 复用路线 Capacitor / Tauri Spike 记录（2026-05-04）](/records/react-capacitor-tauri-spike-record-2026-05-04)、[Tauri + WebOS 桌面安装包第二轮分发评估清单（2026-05-05）](/records/tauri-webos-desktop-distribution-evaluation-2026-05-05) 与 [多端客户端路线评估方案](/planning/multiplatform-client-route-evaluation)；当前 Tauri 冻结且不再绑定 WebOS
...
| docs 正式 Web Author 协作 | `/docs/mine`、`/docs/compose`、`/docs/edit/:id`、`/docs/revisions/:id` 的 Owner / Invitee / Editor、共享草稿、邀请响应、CAS 保存、提交 / 撤回、冲突恢复或正式版本回看改动后 | `npm run test --workspace=@radish/http`、`npm run test --workspace=radish.client`、client type-check / build；涉及后端契约时补 `WikiAuthoringControllerTest / WikiAuthoringServiceTest / WikiAuthoringRepositoryTest` | [作者协作使用说明](/guide/docs-author-collaboration)、[权威设计](/features/wiki-author-contribution-collaboration-design) | Author 入口只承接创作、协作、保存和提交；审核、发布及其他治理动作归 Console，公开 `/docs` 保持只读 |
| Wiki 附件隐私与生命周期 | `WikiAttachmentReference`、Wiki 私有默认、正文 / 草稿 / revision 引用、动态 ACL、访问令牌、清理或历史回填改动后 | Main migration doctor / apply / verify、SQLite / PostgreSQL migration / Repository / Service / 并发测试、`Radish.Api.Attachment.*.http`、`Radish.Api.Wiki.http`、`@radish/http` 与 client 定向测试 | [Wiki 附件隐私与生命周期权威闭环](/features/wiki-attachment-privacy-lifecycle-design)、[文件上传设计](/features/file-upload-design) | `Attachment.IsPublic` 不是 Wiki 可见性真相；通用 Document 不属于 Wiki 产品域，令牌必须先做当前 Wiki ACL 再消费，清理必须覆盖正文、草稿和 revision 权威引用 |
...
| Flutter Android MVP / 条件维护壳层 | `Clients/radish.flutter` 下 Android 壳层、docs / forum / discover / profile 原生页面、handoff、Android Back、OIDC 回调或本地复访状态改动后 | `flutter test test/<topic>_test.dart`、`flutter test test/smoke_test.dart`、`flutter analyze`、必要时补 `.\gradlew.bat :app:testDebugUnitTest` | [Flutter Android RC 分发前置清单](/records/flutter-android-rc-distribution)、[Flutter Android MVP 第七批首个主链路变更回归记录（2026-05-01）](/records/flutter-android-mvp-regression-record-2026-05-01)、[Flutter Android MVP docs 详情只读上下文补强记录（2026-05-01）](/records/flutter-android-mvp-docs-detail-readonly-context-record-2026-05-01)、[Flutter Android MVP forum detail 来源上下文与错误态补强记录（2026-05-01）](/records/flutter-android-mvp-forum-detail-readonly-context-record-2026-05-01)、[Flutter Android MVP profile 空态人称与文档口径复核记录（2026-05-02）](/records/flutter-android-mvp-profile-empty-copy-record-2026-05-02)、[Flutter Android MVP profile 公开主页长文本窄屏显示复核记录（2026-05-02）](/records/flutter-android-mvp-profile-long-text-record-2026-05-02)、[Flutter Android MVP 内测分发前置整理记录（2026-05-01）](/records/flutter-android-internal-rc-prep-record-2026-05-01)、[Flutter Android MVP RC 补验评估记录（2026-05-02）](/records/flutter-android-mvp-rc-supplemental-assessment-2026-05-02)、[Flutter Android MVP 第八至第二十批验证索引（2026-05-02）](/records/flutter-android-mvp-validation-index-2026-05-02)、[Flutter Android MVP 正式域名临时 smoke 记录（2026-05-02）](/records/flutter-android-mvp-radishx-smoke-record-2026-05-02)、[Flutter Android MVP RC 验收记录（2026-05-04）](/records/flutter-android-mvp-rc-acceptance-record-2026-05-04)、`Clients/radish.flutter/README.md` | 只有实际修改 Flutter 或触发其阻断、安全、认证兼容维护时才跑本行验证；纯 Web 改动不要求 Flutter 跟跑。涉及 Android 原生 handoff、OIDC callback、签名或 release 构建时再补 Gradle 单测、签名检查与 release APK 真机复核 |
| Tauri 冻结实验壳 | `Clients/radish-tauri`、Tauri 配置、`tauriBridge`、桌面 OIDC loopback、deep link 或历史 Tauri + WebOS 验证路径被明确修改后 | `npm run type-check --workspace=radish.client`、`npm run test --workspace=radish.client`、`npm run build --workspace=radish.client`、`cargo build`；只有明确触达 release exe 或 installer 时才补 `cargo build --release` / `cargo tauri build` | [React 复用路线 Capacitor / Tauri Spike 记录（2026-05-04）](/records/react-capacitor-tauri-spike-record-2026-05-04)、[Tauri + WebOS 桌面安装包第二轮分发评估清单（2026-05-05）](/records/tauri-webos-desktop-distribution-evaluation-2026-05-05)、[多端客户端路线评估方案](/planning/multiplatform-client-route-evaluation)、[前端多壳层策略](/frontend/shell-strategy)、`Clients/radish-tauri/README.md` | Tauri 不进入当前候选 CI 必需矩阵；历史验证事实保留，只有明确桌面原生需求与维护预算成立后才重新定义默认入口、签名、自动更新、SmartScreen、托盘、菜单、卸载 / 升级、`radish://` 协议注册清理和正式分发验证 |
...
| 论坛核心主链 / 分类标签 / 帖子版本恢复 | 分类、标签、发帖、帖子列表 / 详情、帖子编辑、Revision / CAS、旧历史兼容或恢复改动后 | `Radish.Api.Forum.Core.http`，涉及服务端版本契约时补 Main migration doctor / apply / verify、Repository / Service / Controller / 并发测试 | [论坛应用功能说明](/features/forum-features)、[论坛内容版本完整性与作者恢复](/features/forum-content-version-recovery-design)、[论坛编辑历史（兼容）](/features/forum-edit-history) | 新版本真相只认完整 Revision；旧 EditHistory 不得作为可恢复快照，恢复必须创建新版本并通过 CAS |
| 论坛评论 / 回复 / 评论版本恢复 | 评论树、回复、评论编辑、Revision / CAS、评论时间窗、旧历史兼容或恢复改动后 | `Radish.Api.Forum.Comment.http`，涉及服务端版本契约时补 Main migration doctor / apply / verify、Repository / Service / Controller / 并发测试 | [论坛内容版本完整性与作者恢复](/features/forum-content-version-recovery-design)、[论坛编辑历史（兼容）](/features/forum-edit-history)、[`GetCommentTree` 兼容入口退场清单](/records/comment-tree-compat-retirement-checklist) | 根评论与子评论共用版本契约；普通作者仍受时间窗和编辑次数约束，旧 EditHistory 只保留受权兼容证据 |
...
| 文档审核 / Wiki / Console Documents | `AdminGetReviewQueue / AdminGetDraftById / AdminReviewDraft`、`console.docs.review`、草稿 / 正文双版本校验、审核证据或发布 / 访问 / 回滚等既有治理改动后 | `Radish.Api.Wiki.http`、Console test / type-check / build、`npm run check:console-permissions`；涉及服务端边界时补 `WikiAuthoring*`、`WikiDocumentManagementServiceTests` 与 `AuthorizationBoundaryTests` | [作者协作使用说明](/guide/docs-author-collaboration)、[Console 权限覆盖矩阵](/guide/console-permission-coverage-matrix) | RequestChanges / Reject / Apply 与 Publish 分离；未审核、驳回及 Applied 但未发布正文均不得进入公开读取面 |
