...
| Wiki 附件隐私与生命周期 | `WikiAttachmentReference`、`Wiki / Document` 私有默认、正文 / 草稿 / revision 引用、动态 ACL、访问令牌、清理或历史回填改动后 | Main migration doctor / apply / verify、SQLite / PostgreSQL migration / Repository / Service / 并发测试、`Radish.Api.Attachment.*.http`、`Radish.Api.Wiki.http`、`@radish/http` 与 client 定向测试 | [Wiki 附件隐私与生命周期权威闭环](/features/wiki-attachment-privacy-lifecycle-design)、[文件上传设计](/features/file-upload-design) | `Attachment.IsPublic` 不是 Wiki 可见性真相；令牌必须先做当前 Wiki ACL 再消费，清理必须覆盖正文、草稿和 revision 权威引用 |
