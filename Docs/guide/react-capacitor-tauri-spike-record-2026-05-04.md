...
当前第 1-6 项均已完成。Tauri 默认入口切到 WebOS 桌面工作台的代码与命令级构建已完成；系统浏览器登录 / 登出已从 `radish://` 优先切换为 loopback 回跳，但仍缺 Tauri GUI 启动下的人工验收、真实窗口生命周期人工观察、installer bundle 与自动更新链路验证。
...
5. Tauri 桌面壳命令级构建复杂度低于 Capacitor Android 登录链路；桌面登录已优先采用系统浏览器 + loopback callback，避免依赖 Windows 注册表，但仍需要 GUI 人工验证浏览器登录态复用、回跳、端口占用提示与登出回到 WebOS 的完整体验
...
4. Tauri 尚不足以直接切为桌面主线：默认入口已切到 WebOS 桌面工作台，但仍缺真实 GUI 启动、登录 / 登出回跳、installer、自动更新与签名分发验证。
5. 最终多端开发口径收束为三条线：
