...
2. 复用公开内容入口，Tauri 原生壳根路径默认进入 `/docs`
...
- 不改变 WebOS 桌面入口
- 不引入完整移动端账号中心
- 不执行 Tauri GUI 启动或人工登录验收，本轮只做命令级构建与桥接逻辑验证
...
- 工程：`Frontend/radish.client/src-tauri`
- `identifier`：`com.radish.client.tauri.spike`
- `productName`：`Radish React Tauri Spike`
- `frontendDist`：`../dist`
...
- Tauri 环境下根路径 `/` 会与 Capacitor 一样收口到 `/docs`
- Tauri 环境下登录使用系统浏览器打开 `/connect/authorize`，`redirect_uri` 切换为 `radish://oidc/callback`
- Tauri 环境下登出使用系统浏览器打开 `/connect/endsession`，`post_logout_redirect_uri` 切换为 `radish://oidc/logout-complete`
- Tauri deep link 进入前端后会转换为既有浏览器回调路径：`radish://oidc/callback?...` -> `/oidc/callback?...`
- Rust 壳层会记录 `Focused`、`Resized` 与 `CloseRequested` 窗口事件，并通过 `radish-deep-link` 事件把 deep link 透传给前端
...
- `npm run test --workspace=radish.client`：Tauri 桥接后通过，93 个测试通过，新增 `tauriBridge` deep link 转换测试
...
- Tauri release 可执行文件：`Frontend/radish.client/src-tauri/target/release/radish-client-tauri-spike.exe`
...
4. `src-tauri` 可通过 `cargo build`
5. `src-tauri` 可通过 `cargo build --release`
6. Windows release exe 可生成，并能明确记录产物大小

当前第 1-6 项均已完成。Tauri GUI 启动、真实窗口生命周期人工观察、系统浏览器登录 / 登出回跳人工验收、installer bundle 与自动更新链路仍未纳入本轮命令级验证。
...
5. Tauri 桌面壳命令级构建复杂度低于 Capacitor Android 登录链路，但真实桌面登录仍需要人工验证系统浏览器与 `radish://` scheme 注册是否按预期回跳
6. Tauri 当前只验证 `cargo build` / `cargo build --release` 生成 exe，尚未验证 `cargo tauri build` installer、自动更新、代码签名、Windows SmartScreen、托盘与后台驻留策略
7. Tauri release exe 体积约 `9.15 MiB`，但正式分发体积还需要在 installer bundle 后重新记录
...
3. Tauri 桌面壳首轮命令级 spike 成立：React / `@radish/http` / `@radish/ui` / Vite `dist` 复用成本低，窗口生命周期和 deep link 桥接可用 Rust 原生壳集中承接。
4. Tauri 尚不足以直接切为桌面主线：仍缺真实 GUI 启动、登录 / 登出回跳、installer、自动更新与签名分发验证。
5. React 复用路线若继续，应优先补 Tauri 桌面人工验收和分发链路，而不是回头扩大 Capacitor Android 登录态能力。
