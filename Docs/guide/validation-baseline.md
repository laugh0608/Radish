...
当前已通过 GUI 启动、WebOS 桌面布局、窗口生命周期观察、系统浏览器登录 / 登出 `127.0.0.1:48801` loopback 回跳人工验收，测试后暂未发现问题；但 installer bundle、代码签名、自动更新、Windows SmartScreen、托盘、菜单和正式分发体积仍未纳入已完成验收，不能用 `cargo build --release` 结论替代。当前 `Clients/radish-tauri` 仍保留 `Radish Tauri Spike` / `com.radish.tauri.spike` 身份，在 installer、卸载、升级、签名与 deep link 注册路径完成验证前，不应提前切换为正式桌面包身份。
