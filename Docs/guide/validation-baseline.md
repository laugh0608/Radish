...
- `collect:m14-host-record`
  - 汇总 `validate:baseline:host` 与 `check:host-runtime` 已落盘的 Markdown 报告，默认输出 `.tmp/m14-host-maintenance-record.md`
  - 适合把启动前与启动后的 `M14` 结论沉淀成维护记录
- `collect:change-regression-record`
  - 汇总批次级自动化报告，生成一份可直接贴进 PR 或回归记录的 Markdown
  - 默认读取 `.tmp/validate-ci-report.md` 并输出 `.tmp/change-regression-record.md`
  - `baseline / host / M14` 报告需显式传入，避免误吃历史 `.tmp` 文件
  - 会复用当前 changed files 判定后端与身份语义影响面，把命中原因与失败归类一并写进记录
...
- `check:host-runtime`
  ...
  - 当前三个宿主都保持“`/health` 最小探活、`/healthz` 结构化明细”的分层语义
  - 如需在失败时顺手带出失败宿主的 `/healthz` 关键摘要，可执行 `npm run check:host-runtime -- --details`
  - 支持 `--report / --report-file <path>`，报告摘要字段和 `validate:baseline:host` 保持同一套分诊口径
- `validate:identity`
...
- 公开个人页：`/u/:id` 直链在公开壳层下打开时，`id` 可以是 `usr_...` PublicId 或旧 LongId 字符串，均不得被前端数值化；旧 LongId 加载成功后应 replace 到 `/u/usr_...`，PublicId 路由刷新仍应正常读取同一用户
- 分页规范化：`/u/:id` 的 `tab / page` 会稳定回写到 URL；若直接打开越界页码，当前会 replace 收口到最后一页，而不是停留在空页 URL
- forum 作者跳转：公开 forum 列表、帖子详情与评论作者入口跳到 `/u/:id` 时，优先使用 `User.PublicId`；缺少公开标识时仍不因大整数用户 ID 被前端当作 `number` 而打开失败
...
- 公开个人页分享：`/u/:id` 顶部复制入口应优先复制 `/u/usr_...` canonical 链接，不包含 `/desktop`、窗口参数或临时来源状态；嵌入浏览器 / 权限受限上下文应走同步 textarea fallback，避免误报复制失败

当前批次与公开榜单首批直接相关的人工确认面：

- 公开榜单：`/leaderboard` 与 `/leaderboard/:type` 直链在公开壳层下打开时，会直接进入榜单阅读页，而不是回到桌面 `Shell`
- 类型切换：切换不同榜单类型后，URL 会稳定回写到对应规范路径；默认经验榜单收口到 `/leaderboard`，其他榜单继续落到 `/leaderboard/:type`，刷新后仍能恢复当前榜单
- 经验展示：默认经验榜单页 `/leaderboard` 当前会额外补一块只读的“经验体系公开展示”说明区；兼容直链 `/leaderboard/experience` 会自动收口回 `/leaderboard`，且不会把“我的经验明细 / 经验交易记录”搬进公开壳层
...
- 公开分发页：`/discover` 直链在公开壳层下打开时，会直接进入公开社区分发页，而不是回到桌面 `Shell`
- 聚合跳转：forum / docs / leaderboard / shop 四个区块里的卡片与“查看全部”入口会继续落在 Public Content Shell，不会跳桌面或带出多窗口语义
- 来源返回：从 `/discover` 进入 `/forum/post/:postId`、`/docs/:slug` 或 `/shop/product/:productId` 后，详情页返回动作会优先回到 `/discover`，而不是退回各专题默认列表
- 统一入口：forum / docs / leaderboard / shop / `u/:id` 顶部的“社区发现”轻入口会稳定回到 `/discover`，不会异常跳桌面或丢失公开壳层样式
- 真实链接：公开内容卡片和用户内容入口保留真实公开 `href`；普通点击可保留来源返回，新标签打开和复制链接不应丢到桌面或带入来源状态
- 只读边界：公开分发页当前只承载公开内容分发与只读跳转，不扩展登录后工作台动作、桌面治理入口或其他重交互能力

当前批次与 forum 公开详情轻参与直接相关的人工确认面：
...
- 目录直链：`/docs` 直链在公开壳层下打开时，会直接进入公开文档目录，而不是回到桌面 `Shell`
- 详情直链：`/docs/:slug` 直链在公开壳层下打开时，会直接进入公开文档正文阅读，而不是要求先进入桌面文档应用
...
- 复制结果：点击分享入口后，会复制当前公开文档的 canonical 公共链接，而不是桌面窗口路由、内部 API 地址或兼容重写前的旧地址；若详情由兼容路径或非 canonical slug 打开，当前也会先收口后再复制
...
- `validate:ci` 失败：优先拆成 `Repo Hygiene changed-only`、`Frontend Lint changed-only`、`Baseline Quick`、条件 `validate:backend`、条件 `validate:identity` 五类
- 若需要把本轮判断回写到 PR 或维护记录，优先直接复用 `check:backend-impact / check:identity-impact` 的命中原因类别，以及 `Repo Quality 故障分诊手册` 的失败归类
