...
> 自 **2026-03-07** 起，Radish 正式启动“身份语义收敛”专项：协议层仍保留 Claim 显式映射，但运行时代码将逐步统一到 `CurrentUser` / `ICurrentUserAccessor`。本节保留现有协议与兼容说明；实际迁移策略以 [身份语义收敛与 Claim 治理设计](/architecture/identity-claim-convergence) 和 [身份语义收敛迁移计划](/guide/identity-claim-migration) 为准。
