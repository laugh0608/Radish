...
4. 使用当前环境有效账号登录；仅当本地或受控测试环境显式开启开发默认用户种子时，才可使用开发演示账号 `test / test123456`
...
> 兼容说明：历史版本的自建 JWT 仅将用户 Id 写入 `jti`、租户 Id 写入 `TenantId`。当前长期协议承诺已经收敛到 `sub / name / preferred_username / role / scope / tenant_id`；`jti` 与 `TenantId` 只保留输入兼容，不再作为新签发 Token 的输出承诺。
...
3. 选择 **oauth2** 认证方式
4. 点击 **Authorize** 按钮
5. 使用当前环境有效账号登录；仅当本地或受控测试环境显式开启开发默认用户种子时，才可使用开发演示账号 `test / test123456`
