...
> B6 后登录凭证固定为邮箱 + 密码，`displayName` 来自公开展示名，`displayHandle` 来自 `DisplayName#PublicIndex`。`LoginName` 只作为历史内部字段保留，不进入新签发 Token。
