...
- 与现有页面/按钮权限关系：
  ...
  - 普通角色必须同时拥有 `console.access` 与至少一个真实 Console 页面访问权限，才会看到并允许进入 Console
...
1. **Console 入口**
   ...
   - WebOS 显示 Console 图标、Console SPA 本身放行，除 `Admin/System` 外还依赖至少一个真实 Console 页面访问权限
...
- `console.access`
  - 默认授予 `System/Admin`
  - 其他角色通过显式授权或权限快照联动补齐
  - 若其他角色没有任何真实 Console 页面访问权限，则不应仅保留该入口权限
