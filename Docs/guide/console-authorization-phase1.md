...
1. **授权配置仍依赖种子与数据库直改**
   - 已有 `ApiModule + RoleModulePermission + ConsolePermissions + VoPermissions` 链路
   - 但缺少面向 Console 管理者的授权配置入口

2. **角色管理只管理角色基础信息，不管理授权**
   - 当前 `Roles` 页面能增删改查角色
   - 但无法为角色配置 Console 页面、按钮与接口权限

3. **前端可见性与接口最终授权仍未完全统一**
   - 文档目标是支持“非默认角色通过数据库授权派生 Console 权限”
   - 但现有大量 Console 接口仍使用 `SystemOrAdmin`
   - 导致未来即使前端可见，也可能卡在后端接口层

因此，当前需要进入 `Console-ext` 的第一阶段：在不重做整套权限平台的前提下，补齐一套**最小可用、可落地、可维护**的授权配置能力。
