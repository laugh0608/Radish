...
- 与现有页面/按钮权限关系：
  - `console.access` 只代表“允许进入 Console”
...
- **Console 入口权限独立于菜单权限**：`console.access` 只负责进入 Console，不参与菜单树生成
...
1. **Console 入口**
   - 不属于普通菜单项
   - 由独立权限 `console.access` 控制
   - WebOS 显示 Console 图标、Console SPA 本身放行，都依赖这个权限
