...
5. 从 `/shop/products` 点击商品进入 `/shop/product/:productId`，确认详情页只展示商品信息、说明和返回桌面工作台的导向，不会误弹购买确认、举报或登录提示；再点击详情页返回按钮，确认会优先回到原商品列表上下文，而不是一律退回默认列表，且返回文案应显示为“返回商品列表”。
6. 从 `/discover` 的商城推荐区点击商品进入 `/shop/product/:productId`，确认详情页返回按钮会优先回到 `/discover`，而不是丢失到商城默认列表；此时返回文案应显示为“返回社区发现”。
7. 直接刷新 `/shop/product/:productId`，确认匿名详情读取仍然可用，不会因为前端带了 `withAuth` 或把外部 `productId` 当成 `number` 而失败。
8. 在详情页点击“进入 WebOS 工作台”或同类导向按钮，确认会返回桌面入口；再从浏览器回退，确认还能回到当前公开详情或商品列表上下文。
9. 人工抽查一个不存在或已失效的 `productId`，确认公开壳层会给出只读的“商品不可访问”状态，而不是跳回论坛、桌面或空白页。
