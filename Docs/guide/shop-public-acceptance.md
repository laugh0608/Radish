...
5. 从 `/shop/products` 点击商品进入 `/shop/product/:productId`，确认详情页只展示商品信息、说明和返回桌面工作台的导向，不会误弹购买确认、举报或登录提示；再点击详情页返回按钮，确认会优先回到原商品列表上下文，而不是一律退回默认列表。
6. 从 `/discover` 的商城推荐区点击商品进入 `/shop/product/:productId`，确认详情页返回按钮会优先回到 `/discover`，而不是丢失到商城默认列表。
