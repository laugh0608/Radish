...
通知中心是 Radish WebOS 中的一个**独立应用**，作为窗口应用运行。

Flutter 移动端当前只承接登录态站内通知的只读列表和 forum 来源回流，不复刻 WebOS 通知中心应用。
...
2. **Dock 固定入口**：在 Dock 中点击通知图标快速打开通知中心（固定在第一个位置）
...
- Flutter 当前不提供标记已读、全部已读、删除、通知设置、SignalR 实时同步或系统通知栏推送入口。
- WebOS 通知中心仍是完整站内通知管理入口；Flutter 只承担移动端快速查看和 forum 回流。
...
### 组件位置
- **应用组件**: `Frontend/radish.client/src/apps/notification/NotificationApp.tsx`
