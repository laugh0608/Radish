...
Console 前端也必须按同一权限边界执行：普通设置编辑、favicon 上传、favicon 恢复默认和编辑抽屉打开都要求 `console.system-config.edit`，且必须复核当前设置 `VoIsEditable`。按钮隐藏或禁用只是展示层约束，handler 入口仍需返回明确错误提示，不能继续调用保存或上传逻辑。
...
- 高危设置要求二次确认和填写原因。
- 敏感设置默认脱敏，不允许复制明文。
- 移动视图下品牌卡、设置项说明和当前值展示必须可收缩换行；横向滚动只允许存在于表格局部区域，不能让系统设置主内容区被长 URL 或说明文本撑宽。

设置项控件应按类型选择：开关、数字输入、滑块、枚举下拉、文本输入、JSON 编辑器或只读说明。

### 8.1 多语言展示边界

Console 的界面动作、状态、风险等级、生效方式、筛选和校验提示由 `settings.*` 中英文资源提供。设置定义的展示遵循稳定键优先：

- `voKey` 是设置定义本地化的稳定标识，名称、说明和影响摘要分别查找 `systemConfig.definition.<voKey>.name / description / impact`。
- 分类优先按已登记的稳定设置键前缀映射；没有登记的 key 保留服务端 `voCategory`，不能根据中文名称猜测分类。
- 缺少本地资源时保留服务端 `voName / voDescription / voImpactSummary`，不编造翻译，也不影响编辑权限、风险判断或保存控制流。
- 当前值、默认值、JSON 内容、修改原因和历史审计内容属于配置型或人工内容，保持原文；日期与数字只按当前 locale 格式化展示。
- 当前语言仍是浏览器设备偏好 `radish_lang`，不写入 `SystemConfig`。系统设置中心不承接在线词元编辑、翻译资源发布或语言包管理。

新增注册设置时，如果需要双语展示，应在 Console 中英文资源中同步登记稳定 `voKey`；服务端定义文本继续作为无资源版本、未知 key 和运维排障时的安全回退。
