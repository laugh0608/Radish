...
截至 `2026-04-04`，启动评审、仓库外兼容边界确认、首轮实施与官方顺序真实回归都已完成，Phase 4 当前不再停留在“等待启动”或“继续验证中”的状态：

- **正式结论已更新为：首轮实施与官方回归已完成。**
- **当前结论已更新为：本轮无需回滚，转入稳定维护。**

当前仓库内已经完成的动作包括：

- `AccountController / AuthorizationController` 已停止 `ClaimTypes.NameIdentifier / ClaimTypes.Name / ClaimTypes.Role / TenantId` 等历史双写输出；
- `UserInfoController` 已在保持 `sub / name / email / role / tenant_id` 对外结构稳定的前提下完成最小对齐；
- `radish-client / radish-console` 的 Token 直读规则已提取为可测试解析模块并锁定“标准优先 + 输入兼容 fallback”；
- `Radish.Api.AuthFlow.http`、`Scalar` 联调提示与鉴权文档已同步到当前回归口径。
- `radish-client / radish-console / Radish.Api.AuthFlow.http / radish-scalar` 当前均已按官方顺序完成真实端到端回归，授权流程与权限链路均正常，当前未发现需要恢复历史双写字段的阻塞事实。

因此当前状态建议更新为：

`首轮实施、官方顺序真实回归与回滚窗口验证已完成；下一步转入稳定维护与防回归治理准备`
