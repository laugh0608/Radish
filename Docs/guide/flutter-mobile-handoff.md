# Flutter 移动端 handoff 与回流说明

本文说明 Flutter Native 已成立的原生导航、外部来源、登录回流、公开链接、登录态轻写入与复访承接边界。Android 已有真实 AVD 运行证据；iOS 已完成 P7-B 平台 owner，并通过 P7-C iPhone / iPad Simulator 运行态，当前结论为 `Simulator Go`。P7-D readiness 已确认真机 / 分发仍为 `No-Go`，完整客户端范围仍以 `Clients/radish.flutter/README.md` 为准。

## 总体边界

Flutter Native 是 Web 之外唯一正式原生安装包产品线，但保持次级优先、mobile-first、desktop stage-gated，也不复刻 WebOS。Android 已验收 MVP 包含公开内容阅读、原生返回、登录续接、个人复访、基础个人资料编辑、通知列表与单条已读、forum 评论 / 问答回答 / 轻回应 / 纯文本发帖写入、作者帖子正文编辑、作者根评论编辑、公开商品浏览、登录态单商品购买、订单 / 背包查看、胡萝卜资产与经验记录只读查看、公开详情链接复制和 docs 原生阅读链路；iOS Simulator 已验证相同 Dart UI 的系统浏览器 OIDC、Keychain / preferences、phone / tablet 三档、旋转、真实键盘和四主题，但没有真机、Apple Team、正式签名、provisioning、TestFlight 或 App Store 结论。

当前不包含：

- 完整通知中心
- 系统通知栏推送
- 完整商城工作台
- 购物车、取消订单、退款、权益激活、道具使用
- 转账、打赏、调账、经验调整、冻结治理、管理员复核
- 完整资产中心、完整浏览历史治理
- 头像上传、完整账号设置、密码修改、关注管理
- 富文本发帖、附件发帖、富文本回答、富文本评论、回答采纳、子评论编辑、回答编辑、评论审核治理、点赞、投票、完整编辑治理
- 系统分享 SDK、海报生成、分享统计
- 桌面工作台多窗口能力

## iOS 分发前置边界

P7-D readiness 已确认：现有 iOS identity、OIDC scheme、Keychain entitlements、AppIcon 与 Release configuration 具备继续产品化的基础，但不能直接进入签名和上传。

- testing / production distribution build 必须显式使用项目所有者确认的 HTTPS Gateway；localhost、保留示例域名、非法或缺失 define 必须 fail closed，开发证书 opt-in 不得进入分发候选。
- Auth 登录页允许注册，但当前没有账号删除能力；外部 TestFlight / App Store 前必须建立用户可在 App 内发起的账号删除全链路。
- 服务端与 Web 已有 ContentModeration / UserBlock 契约，Flutter 尚未提供 UGC 举报和用户屏蔽 / 解除入口；P7-D1 只复用既有权威 API，不新增移动专属治理真相。
- Flutter 当前可用站内胡萝卜余额购买并解锁数字主题 / 权益，但没有 StoreKit。项目所有者必须先裁决 StoreKit 全链路或 iOS external distribution commerce gate；readiness 不自动选择。
- Apple Team、证书、provisioning、真机、archive、TestFlight、App Store metadata、privacy answers 与 export compliance 都是后续独立授权事项，不能由 Simulator `Go` 推导。

## API 响应与失败态

Flutter 通过 `HttpRadishApiClient` 复用 Radish API 的 `MessageModel` 响应信封，不引入移动端专属 BFF。

- 成功响应必须能读取 `isSuccess / IsSuccess / success / Success` 和 `responseData / ResponseData / data / Data`。
- HTTP `401 / 403 / 404 / 5xx`、空响应、非 JSON 响应和缺少成功标记的 2xx JSON 都应转成明确的 `RadishApiClientException`。
- 携带 bearer token 的请求在发送前统一解析当前 session；access token 临近过期时先 refresh，首次 `401` 只允许强制 refresh 后重试一次，不建立业务 Repository 各自续签或无限重试。
- 同一旧 access token 的并发 refresh 合并为一个请求，并以 session epoch 隔离迟到结果；只有授权端明确返回 `invalid_grant` 才清除本地会话，临时网络 / 服务失败保留已有会话并展示可恢复 issue。
- 业务页面只在本地任务区域展示失败，例如购买面板、余额区域、订单刷新区或背包来源区；失败不应清空已加载详情、来源 tab、订单 / 背包上下文或登录恢复状态。
- 接口返回格式异常、登录失效、权限不足和接口不存在属于可见错误态，不应被当作“无数据”或默认成功。

## 统一登录边界

- Flutter 使用系统浏览器承接 Gateway / Auth 的 OIDC Authorization Code + PKCE 登录，不在 App 内复制账号密码表单、Cookie 或授权页面。
- App 只保存当前 pending login attempt 所需的 state / verifier 与最终 token session；两者必须进入 Android Keystore / iOS Keychain 支撑的安全存储，不得写入普通 `shared_preferences`。浏览器取消、回调 state 不匹配或尝试过期必须显示可恢复错误，不接受未知 callback。
- Native authorization attempt 自打开系统浏览器起有效 `15` 分钟，用于覆盖系统浏览器交互、开发证书确认与 Activity / Flutter owner 重建；超时、redirect 不一致、state 不匹配和重放仍 fail closed，成功或校验失败后一次性消费。
- Forum / Docs recent、recent profile 与 pending post-login target 属于非敏感设备偏好，Android / iOS 共用 Dart `shared_preferences` owner；Android 旧 MethodChannel / `SharedPreferences` 状态只在新 owner 缺失时迁移，写入并回读确认后才清除旧值。
- 本地 Android 使用 `https://localhost:5000` 时，App API client 可按开发环境规则接受本地证书；iOS 只有显式传入 `RADISH_ALLOW_LOCAL_DEVELOPMENT_CERTIFICATES=true` 且 Gateway 为 loopback 时才允许。系统浏览器仍可能单独显示开发证书警告，该环境行为不改变 production 证书或统一登录边界。

## 榜单到公开主页

原生 `榜单` tab 当前承载经验榜只读入口。

- 经验榜条目可打开原生公开主页。
- Android Back 返回榜单，而不是退到默认 profile tab 或退出应用。
- 商品榜、我的经验明细、经验交易记录和管理员调整能力不进入当前 Flutter 批次。

## 公开主页来源返回

原生公开主页是 Flutter 当前个人链路的复访承接点。

- 公开资料、公开统计、公开帖子和公开评论分别使用现有 Public API，并统一传递 `identifier`；不得回退到要求本人或治理权限的旧 `userId` 私有路径。
- 从发现页、forum 作者入口或榜单打开公开主页时，壳层会记录打开前 tab。
- Android Back 优先返回原来源 tab，而不是强制停留在 profile tab。
- 公开主页继续打开帖子、评论或轻回应详情后，详情页返回会回到公开主页；再次 Android Back 仍应回到最初来源 tab。
- 最近公开主页入口可保留最近一次公开用户，但该本地复访状态不等于完整浏览历史中心。

## 通知列表与 Forum 回流

已登录壳层会读取当前用户最近站内通知，作为移动端通知列表。

- 列表展示通知标题、内容、类型、已读状态和时间。
- forum 通知点击后复用 forum detail handoff 打开目标帖子。
- 通知携带 `commentId` 时，详情页继续尝试定位目标评论上下文。
- 系统通知等不可跳通知保持只读展示，不伪造目标详情。
- 从通知进入详情后，Android Back 返回打开通知前所在 tab，不强制留在 forum tab。
- 未读通知支持单条显式标记已读，成功后只更新本地该条状态；失败在通知 sheet 内提示并保留原状态。
- 未读 forum 通知被打开时也会先尝试单条标记已读；标记失败只提示，不阻断帖子 / 评论详情打开。
- 该入口不代表完整通知中心、系统通知栏推送、批量已读、删除通知或通知设置已纳入 Android MVP。

## 最近访问

已登录我的页提供最近访问入口，用于承接用户在移动端回看公开内容。

- 数据来源为 `User/GetMyBrowseHistory`。
- 当前展示公开帖子、文档和商品浏览记录。
- 列表覆盖加载、空态、错误态、刷新、加载更多和返回 profile。
- 点击记录会打开既有原生 forum detail、docs detail 或 shop product detail。
- forum / docs 记录优先使用 `targetSlug` / `PublicId` 作为打开目标和可见路径，旧 `routePath` 只作为缺少公开标识时的兼容回退。
- 当前不提供清空、删除、推荐排序、跨端浏览历史治理或完整时间线。

## 个人资料编辑

已登录我的页当前提供“编辑资料”入口，用于维护移动端基础个人资料。

- 读取接口为 `User/GetMyProfile`，保存接口为 `User/UpdateMyProfile`。
- 当前可编辑展示名（接口字段仍兼容 `userName`）、邮箱、年龄和地址；保存成功后刷新原生公开资料摘要。
- 地址更新语义固定为：字段省略或 `null` 保持原值，空字符串清空，非空字符串修剪后写入；Flutter 与 Web 编辑器都不得把用户主动清空折叠成“未提供”。
- 编辑对话框覆盖加载、字段校验、保存中、保存失败提示和取消返回。
- forum 作者展示名按 `DisplayName -> UserName 兼容字段 -> User-{id}` 口径回读当前资料，Console 用户详情也展示同一展示名称；`UserRealName` 不再作为当前资料展示来源。
- 头像上传、密码修改、完整账号设置、关注管理和资料治理不进入当前 Flutter 边界。

## Forum 纯文本发帖

Forum tab 当前开放已登录态的纯文本帖子发布。

- 分类来源为 `Category/GetTopCategories`，提交接口为 `Post/Publish`。
- 发布前校验标题、分类、正文和 `1-5` 个标签。
- 成功后刷新论坛列表，并通过既有 forum detail handoff 打开新帖子。
- 新帖详情读取服务端返回的 `Post.PublicId` 后，公开链接展示为 `/forum/post/:publicId`，不把内部帖子 ID 作为复制口径。
- 匿名态从发帖表单提交时，会保留页面存活期间的标题、分类、标签和正文草稿，发起登录后回到发帖表单继续发布。
- 失败在发帖面板内展示错误，不重置标题、标签和正文输入，也不误触发详情跳转。
- 发帖请求会生成 `forum-post:` 前缀的 `clientSubmissionId`；同一草稿失败后直接重试复用同一个 key，成功、草稿变化或账号变化后生成新 key。
- 当前不扩展 Markdown / 富文本切换、附件、投票、抽奖、草稿箱、编辑、点赞或完整创作器。

## Forum Detail 原生阅读布局

Forum detail 保持“正文 -> 回答区（仅问题帖）-> 轻回应 -> 评论区”的同一连续阅读轴。

- compact / medium 使用单主轴与“社区 / 本帖”紧凑入口，正文、互动和目标评论定位不拆成独立页面。
- `1440px` expanded 代表尺寸使用 `220 / 820 / 250` 社区导航—连续正文—线程索引三栏；较窄 expanded 保留主阅读与线程索引，并把社区 / 来源上下文放回正文。
- 三档布局共享同一 detail、comment、quick reply controller 和 handoff target，不复制数据状态或改变 Android Back 来源返回。
- loading、empty、unavailable 与能力停止线复用 Flutter 共享状态原语；不因布局调整新增后端接口或 Web 治理能力。

## Forum 评论发布与回复

Forum detail 当前在公开阅读基础上开放已登录态的根评论发布与评论回复。

- 匿名态可读取帖子正文、轻回应与评论区；从评论区发起登录后，OIDC 回跳会回到当前帖子评论输入上下文。
- 已登录态可发布根评论，也可回复根评论或子评论。
- 提交成功只局部更新评论区：根评论前插到评论列表，回复更新对应根评论的回复数和子评论列表。
- 提交失败只在评论输入区提示，不刷新正文、轻回应或来源 tab。
- 评论和回复请求会生成 `forum-comment:` 前缀的 `clientSubmissionId`；同一帖子、同一回复目标、同一内容失败后直接重试复用同一个 key。
- 从公开主页评论上下文进入详情并回复后，Android Back 仍返回原公开主页来源。
- 发帖编辑器、点赞、投票、审核治理、富文本评论和完整社区治理仍不进入当前 Flutter 边界。

## Forum 作者编辑

Forum detail 当前承接作者自己的帖子正文编辑和作者根评论编辑。

- 作者帖子正文编辑复用 `Post/Update`，只开放当前移动端已有的正文编辑能力，不扩展富文本、附件、投票、抽奖或问答模式切换。
- 作者根评论编辑复用 `Comment/Update`，只覆盖根评论；子评论编辑继续后置单独评审。
- 帖子详情和评论节点必须消费服务端 `voContentRevision`；编辑提交当前 `expectedContentRevision`，成功后使用响应返回的新 revision 局部更新节点，冲突不得被本地覆盖或盲重试吞掉。
- 编辑请求分别生成 `forum-post-edit:` 与 `forum-comment-edit:` 前缀的 `clientSubmissionId`；同一编辑目标、同一编辑内容失败后直接重试复用同一个 key。
- 成功、编辑目标变化、编辑内容变化或账号变化后生成新 key。
- 保存成功后局部刷新帖子正文或评论节点；失败时保留编辑输入和当前详情上下文。
- 帖子 / 评论编辑历史仍由 Web 正式页面和 WebOS 历史入口承接；Flutter 当前不提供历史查看或版本回滚。
- 回答编辑、子评论编辑、管理员覆盖、审核治理和完整编辑历史治理仍不进入当前 Flutter 边界。

## Forum 问答回答

Forum detail 当前在问题帖正文后开放已登录态的纯文本回答。

- 匿名态可读取问题帖正文、已有回答、轻回应与评论区；从回答区发起登录后，OIDC 回跳会回到当前帖子回答输入上下文。
- 已登录态可发布纯文本回答，提交接口复用 `Question/Answer`。
- 提交成功使用接口返回的问答详情局部更新回答区和回答数，不刷新正文、轻回应、评论或来源 tab。
- 提交失败只在回答输入区提示，不清空当前回答输入。
- 回答请求会生成 `forum-answer:` 前缀的 `clientSubmissionId`；同一用户、同一问题帖、同一内容失败后直接重试复用同一个 key。
- 回答采纳、回答编辑、Markdown / 富文本回答、附件回答和问题发帖模式仍不进入当前 Flutter 边界。

## Forum 轻回应登录回流

Forum detail 当前按“正文 -> 回答区（仅问题帖）-> 轻回应 -> 评论区”展示。

- 匿名态可读取最近轻回应。
- 已登录态可发布一句最多 `10` 字的轻回应；Flutter 输入计数与服务端权威上限保持一致。
- 从轻回应区点击“登录后发布”后，浏览器 OIDC 登录成功会回到当前帖子轻回应区，并提示可继续发布。
- 发布成功只刷新轻回应墙与局部成功提示，不刷新正文或评论阅读位置。
- 轻回应删除、举报、审核台能力、点赞、投票和编辑治理仍不进入当前 Flutter 边界。

## Docs 原生阅读与内链

Docs detail 当前承载公开文档的原生只读阅读。

- `discover` 文档精选、docs 列表、docs 搜索结果和 profile 最近文档都可进入原生 docs detail，并按来源返回。
- 公开文档正文中的 `/docs/:slug`、完整公开 URL、`docs/:slug`、`./:slug` 与普通相对 slug 文档链接会继续打开原生 docs detail。
- Docs tab 内联正文、直达 handoff 与文档内链 route 共用同一 reader controller / surface 契约；每个 route 保持独立 controller 和真实返回栈。
- 同 slug 刷新保留旧正文并局部表达 refreshing / stale；切换 slug 立即清空上一篇正文，迟到响应不会回写当前 target。
- compact 为目录 / 正文互斥单任务，medium 为 `200–280px` 目录 + 正文，expanded 为 `280px` 目录 + `24px` 间距 + `904px` 阅读轴；直达 handoff 不为布局额外请求目录。
- 页内锚点、附件路径、外部链接和非 docs 链接仍按文本展示，不在 Flutter 内扩外部浏览器跳转或附件治理。
- 16 位以上纯数字旧 long slug 不作为公开可见 slug 展示或复制，避免把兼容路径当作普通用户可读地址。

## Shop 公开浏览

Flutter 当前承接公开商城的移动浏览入口。

- 发现页可进入原生公开商城列表。
- 发现页通过从属商城入口进入原生公开商城列表，商城列表项可打开原生公开商品详情。
- Android Back 返回打开前来源：发现页来源回发现页，列表来源回商城列表。
- 匿名态商品详情展示公开信息，并在点击购买时发起登录回流。

## Shop 单商品购买、订单与背包

已登录用户可从原生商品详情发起单商品购买；我的页当前继续承接订单、背包和钱包等私域复访入口，商城入口归属可在后续移动信息架构批次中评估。

- 商品详情先调用 `Shop/CheckCanBuy/{productId}` 检查购买资格。
- 商品详情购买面板会读取并展示当前胡萝卜余额，购买成功后刷新余额，便于用户在同一商品上下文核对扣款前后状态。
- 购买接口为 `Shop/Purchase`，Flutter 当前固定购买 `1` 件商品，并要求输入支付口令。
- 成功后打开订单详情，确认订单号、状态、商品快照和扣款结果。
- 失败态覆盖购买资格、支付口令、接口失败、登录失效、权限不足、接口不存在、非 JSON 响应和返回格式异常。
- 我的页可打开订单列表、订单详情和背包。
- 订单列表和订单详情读取当前用户订单、订单状态、商品快照和金额等信息。
- 订单详情可按 `BusinessType=Order / BusinessId=OrderId` 打开胡萝卜流水筛选，不依赖详情额外返回扣款流水 ID。
- 订单详情刷新失败时保留当前订单上下文，只提示刷新失败。
- 背包展示当前用户权益和道具，并可从来源订单或来源商品继续打开只读详情。
- 背包来源订单 / 商品 ID 继续按规范 LongId 字符串读取和传递，不通过 `int.tryParse` 或其他数值化路径进入 Dart `int`。
- 订单详情可进入背包确认权益 / 道具发放结果；从该入口进入背包时返回文案指向订单详情。
- 背包来源订单或来源商品不存在 / 加载失败时展示对应错误态，并保留返回背包能力。
- 从订单 / 背包进入来源页后，Android Back 返回原订单或背包上下文。
- Flutter 不承担购物车、取消订单、退款、权益激活或道具使用；这些能力仍由 WebOS 私域商城和后续专门治理链路承接。

## 胡萝卜资产只读

已登录我的页当前提供胡萝卜资产只读入口。

- 资产页读取可用余额、冻结余额、累计获得、累计支出和最近流水。
- 从订单详情进入资产页时，可按 `Order #orderId` 保留扣款流水筛选上下文；无结果或刷新失败时仍展示筛选条件和上次可用余额。
- 最近流水用于移动端快速核对，不承担财务对账后台职责。
- Flutter 不开放转账、打赏、调账、支付或冻结治理。

## 经验记录只读

已登录我的页当前提供经验记录只读入口。

- 经验页读取等级、当前经验、总经验、升级进度、冻结状态和最近经验流水。
- 最近流水用于用户自查经验来源，不承担管理员复核或配置治理职责。
- Flutter 不开放经验调整、冻结治理或管理员复核。

## 公开详情链接复制

Flutter 原生 forum detail、docs detail 与 shop detail 使用与 Web 公开路由一致的复制口径。

- 复制内容由当前 `RADISH_GATEWAY_BASE_URL` 派生的 Gateway Base URL 加 Web 公开路径组成。
- forum detail 只有在 `Post.PublicId` 可用时展示可复制的 `/forum/post/:publicId`；旧 long `postId` 仍可作为打开兼容 fallback，但不作为复制口径。
- docs detail 使用 `/docs/:slug`；16 位以上纯数字旧 long slug 不生成公开复制链接。
- shop detail 使用 `/shop/product/:productId`。
- 来源返回、目标评论、当前 tab 等应用内上下文不写入复制链接。
- 当前只提供“展示完整公开链接 + 复制到剪贴板”，不接系统分享 SDK、海报生成或分享统计。

## 标识与内部接口

- 外部来源优先携带 `postPublicId`。
- 旧 `postId` 只作为字符串 fallback。
- 最近访问中的 forum / docs 打开目标优先使用公开标识，旧 `routePath` 只作兼容回退。
- 进入详情并读取成功后，评论、轻回应和定位类内部接口使用真实 `VoId`。
- 不应把公开标识误传给需要内部 ID 的接口。

## 验证要点

- 经验榜用户条目打开公开主页后，Android Back 回榜单。
- 从 discover 或 forum 作者入口打开公开主页后，Android Back 回原来源 tab。
- 从公开主页打开帖子 / 评论详情并返回后，再按 Android Back 仍回到公开主页原来源 tab。
- 从非 forum tab 打开通知列表并选择 forum 通知进入详情后，返回回到原 tab；系统等不可跳通知保持只读展示。
- 在通知列表中标记单条未读通知为已读后，该条状态应局部更新；失败时应保留原状态并显示错误。
- 打开未读 forum 通知时，即使标记已读失败，也应继续进入目标详情并保留来源返回。
- 通知携带 `commentId` 时，应落到评论上下文或给出可理解的定位失败提示。
- 已登录态从 forum tab 发布纯文本帖子后，应刷新列表并打开新帖子详情；失败时不丢失输入。
- 匿名态从发帖表单提交并完成登录回调后，应回到发帖表单并保留页面存活期间的草稿。
- 新帖子详情应使用详情接口返回的 `Post.PublicId` 展示公开链接；失败发帖不应生成公开链接或成功态。
- 匿名态从评论区发起登录后，应回到当前帖子评论输入上下文；已登录态发布根评论或回复后，只局部更新评论区。
- 作者编辑帖子正文或根评论失败后，应保留编辑内容；重试应复用同一 `clientSubmissionId`，成功后局部刷新详情或评论节点。
- 问题帖应展示已有回答；匿名态从回答区发起登录后应回到回答输入上下文；已登录态发布回答后，只局部更新回答区并复用失败重试 key。
- 匿名态从轻回应区发起登录后，应回到当前轻回应区并可继续发布。
- 发现页进入公开商城列表和商品详情后，Android Back 应回到原来源；匿名态购买登录后应回到当前商品详情。
- 登录态在商品详情购买 `1` 件商品后，应进入订单详情并展示明确结果状态；余额不足、购买资格失败或支付口令失败应在购买区提示。
- 商品详情购买区应展示当前余额；余额读取失败时只提示余额区错误，不阻断商品详情阅读，购买成功后应刷新余额。
- 我的页进入订单列表、订单详情、背包、来源订单或来源商品后，Android Back 应回到原订单 / 背包 / profile 上下文。
- 背包来源订单 / 商品 ID 应按字符串 LongId 承接，非规范来源 ID 不应被数值化后误打开来源页。
- 订单详情进入扣款流水应保留 `Order #orderId` 筛选；进入背包发放确认后，应能从背包返回订单详情。
- 订单详情刷新失败、背包来源加载失败和钱包订单筛选刷新失败都应保留可返回的原上下文。
- 我的页进入胡萝卜资产和经验记录后，应能读取概要、分页或最近流水、空态 / 错误态，并可返回 profile。
- 我的页进入最近访问后，应能读取公开帖子 / 文档 / 商品记录并打开对应详情；空态、错误态、刷新和加载更多状态应明确。
- 最近访问中的 forum / docs 可见路径应优先展示公开路径，不能把内部 LongId 当作普通公开路由展示。
- 我的页编辑个人资料后，应刷新公开资料摘要；保存失败应保留编辑对话框和当前资料展示，不污染已加载资料。
- forum / docs / shop detail 展示的公开链接应能完整复制，长链接在窄屏下不撑开布局。
- docs 正文内链命中公开 docs 路径或相对 slug 时，应继续打开原生 docs detail；附件、锚点和外部链接不应误当作原生文档跳转。
