...
6. 切到第 `2` 页及之后任意一页，确认分页按钮在窄屏下仍可点击，刷新页面后页码状态仍能恢复。
7. 在未登录状态打开用户榜单，确认不会误显示“我的排名”；在已登录状态打开同一榜单，确认“我的排名”只在用户榜单中出现，且商品榜单不会误显示该信息。
8. 点击一条用户榜单项，确认能跳到 `/u/:id`，而不是只能打开桌面 `profile` 窗口。
9. 打开 `/leaderboard/hot-product`，确认商品榜单当前只做公开展示，不会误跳商品详情、商城购买、订单或背包链路。
10. 再从公开个人页返回榜单或浏览器回退，确认不会异常跳回桌面入口，榜单上下文保持稳定；若公开个人页由榜单进入，返回按钮文案会按来源显示为“返回榜单”而不是泛化的“返回上一入口”。
