...
2. 在窄屏下检查首屏，确认标题、只读标识、榜单切换按钮、统计胶囊，以及当前榜单对应的只读说明区没有重叠、截断或横向撑破。
3. 打开默认经验榜单页 `/leaderboard`，确认首屏会展示增强后的“经验体系公开展示”说明区，除排行依据、等级含义与只读边界外，还会明确提示公开路由只保留阅读 / 比较 / 公开个人页跳转，以及“我的经验明细”“经验交易记录”等动作继续留在桌面工作台。
4. 兼容直链打开 `/leaderboard/experience`，确认页面会自动收口回 `/leaderboard`，且经验体系说明区与只读边界保持一致。
5. 切换至少两类非默认用户榜单，再刷新页面，确认 URL 会同步落到 `/leaderboard/:type`，刷新后仍能恢复当前榜单。
6. 打开任一非经验用户榜单，确认首屏会展示轻量的“公开用户榜单阅读提示”，明确当前路由只保留公开比较、公开个人页跳转与只读边界，不会把账号明细页带进公开壳层。
7. 打开 `/leaderboard/hot-product`，确认首屏会展示轻量的“公开商品榜单阅读提示”，明确这里只做只读展示与公开比较，不会误跳购买、订单或背包流程。
8. 切到第 `2` 页及之后任意一页，确认分页按钮在窄屏下仍可点击，刷新页面后页码状态仍能恢复。
9. 在未登录状态打开用户榜单，确认不会误显示“我的排名”；在已登录状态打开同一榜单，确认“我的排名”只在用户榜单中出现，且商品榜单不会误显示该信息。
10. 点击一条用户榜单项，确认能跳到 `/u/:id`，而不是只能打开桌面 `profile` 窗口。
11. 打开 `/leaderboard/hot-product`，确认商品榜单当前只做公开展示，不会误跳商品详情、商城购买、订单或背包链路。
12. 再从公开个人页返回榜单或浏览器回退，确认不会异常跳回桌面入口，榜单上下文保持稳定；若公开个人页由榜单进入，返回按钮文案会按来源显示为“返回榜单”而不是泛化的“返回上一入口”。

## 5. 预期结果

- `/leaderboard` 与 `/leaderboard/:type` 以公开内容壳层直达，不绕回桌面入口。
- 公开榜单在手机上保持“榜单切换清晰、排行信息可扫读、分页可继续”的阅读节奏。
- 默认经验榜单页 `/leaderboard` 会额外给出更清晰的经验体系公开展示说明；兼容直链 `/leaderboard/experience` 会自动收口回 `/leaderboard`，但不会把“我的经验明细 / 经验交易记录”搬进公开壳层。
- 非经验榜单也会给出更轻量的只读阅读提示：用户榜单强调公开比较与公开个人页跳转，商品榜单强调只读展示与不带购买链路。
- 榜单类型和分页状态都能被 URL 保存，刷新后不会丢失上下文。
- 用户榜单可稳定跳到公开个人页，不依赖桌面窗口系统。
- 从榜单进入公开个人页后，返回按钮文案会按真实来源显示为“返回榜单”，便于用户判断当前回跳目标。
...
- 验收人：<name>
- 验收范围：公开榜单首批
- 执行入口：/leaderboard、/leaderboard/experience、/leaderboard/:type、/u/:id
- 结果：通过 / 阻塞
