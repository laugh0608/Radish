...
> 版本：v1.20 | 最后更新：2026-09-19 | 状态：Console 权限治理、页面类型视觉基座、内容治理案件与申诉、外部 LongId 字符串安全、核心治理域中英文资源、文档审核、角色权限矩阵和系统设置 PC / mobile 治理入口已收口，后续新增或明显改动页面继续按页面类型与稳定契约对齐
...
- 当前接入对象包含：`Post`、`Comment`、`PostAnswer`、`PostQuickReply`、`ChatMessage`、`Product`、`ProductReview`
- 服务端已建立案件、追加式证据、目标动作、申诉、事件和用户唯一当前治理状态；同一目标的多份举报聚合到开放案件，原决定、申诉结果、目标动作和用户限制分别保留权威事实
- `console.moderation.view / review / appeal / action` 已分离；Review 不自动获得申诉正文，Appeal 不自动获得纠正执行权，Action 只能执行已登记的案件动作或申诉纠正
- 正式页面已迁移到 Case / Appeal API，旧 `Review / ApplyUserAction` 等举报单写入口已经删除；案件与申诉工作区不再读取旧动作流水推断当前状态
- 用户详情页可带目标用户跳转到内容治理工作台；`Moderation` 通过 URL 状态承接用户过滤，并可在同一治理应用切换案件与申诉
- 案件队列、本人举报和治理日志的筛选与摘要使用服务端查询和批量装配；关键词、状态、目标用户、来源举报单和动作类型等条件不得回退到前端或逐案查询
- 审核台承接人工案件处理、禁言 / 封禁联动、申诉复核、七类目标纠正和治理时间线；批量治理、敏感词、自动策略、多级仲裁仍后置
...
- 文档治理页当前在既有 `/documents` 内承接待审队列、正式正文 / 草稿证据、RequestChanges / Reject / Apply 和审核时间线；`console.docs.review` 与 Publish、访问策略、归档、删除、恢复、回滚、导入 / 导出权限保持分离。
- 案件处理使用 Case 版本保护；并发冲突必须刷新案件后重新确认，前端应保留尚未提交的决定、内部备注和动作草稿，不得自动覆盖新版本。

**当前页面体验边界**：

- Console 仍是独立后台入口，不嵌入 WebOS 窗口，也不复刻公开内容壳层。
- 当前视觉方向采用 `Case Desk`：低饱和暖灰 / 纸色底、轻侧栏、克制边框、清晰按钮层级和可扫描的工作台信息密度。
- 普通列表页优先使用“紧凑指标条 + 筛选 / 工具条 + 表格”；`ConsoleResourceList` 的说明与权限上下文放在列表后的折叠区；设置 / 配置页优先使用“分组导航 + 设置列 + 影响范围摘要”；治理页才使用“队列 / 详情 / 动作”的工作台结构。
- `AdminLayout` 在移动端继续保持 Console 独立后台形态：顶部品牌栏与“总览 / 治理 / 交易 / 权限 / 更多”五项胶囊导航承接跨模块入口；单任务详情可隐藏底部导航。业务页需要自行把表格、工具条和详情摘要重排为连续列表、按需筛选或 Bottom Sheet，不得依赖桌面横向表格。
...
- Console 当前独立维护 `users / moderation / orders / products / settings / documents` 等业务域中英文资源；页面按稳定枚举、Code 或 key 解析系统词元和错误分支，配置型与人工内容保持原文，日期、数字与英文数量规则按当前 locale 展示。
