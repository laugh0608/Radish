...
> 版本：v1.18 | 最后更新：2026-07-20 | 状态：Console 权限治理、页面类型视觉基座、外部 LongId 字符串安全、核心治理域中英文资源、结构化错误消费、系统设置 / 商品管理版本语义、用户与商城排障回流、文档审核治理和系统设置治理入口已收口，后续新增或明显改动页面继续按页面类型与稳定契约对齐
...
- 服务端已建立案件、追加式证据、事件和用户唯一当前治理状态；同一目标的多份举报聚合到开放案件，决定、目标动作和用户限制分别保留权威结果
- `console.moderation.view / review / action` 已分离；案件决定携带用户动作时同时要求 Action，结案后纠正动作只允许 Action 权限
- 当前正式页面仍是迁移期举报审核、手动治理和动作日志工作流；F4-I-C 将其迁移到 Case API，并在消费者清零后删除旧 `Review / ApplyUserAction` 写入口
