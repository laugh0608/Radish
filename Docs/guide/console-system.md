...
> 版本：v1.14 | 最后更新：2026-06-16 | 状态：Console 权限治理、首批页面类型视觉基座、外部 LongId 字符串安全、角色授权保存守护、用户排障到内容治理、商品 / 订单 / 流水排障回流、移动壳层、会话刷新窗口和系统设置治理入口已收口，后续新增或明显改动页面按页面类型小步对齐
...
- 系统设置页当前只展示代码注册的设置定义，保存覆盖值时写入修改原因 / 确认参数和变更审计；Low / Medium 可按后端规则编辑，High / Critical 不开放编辑
