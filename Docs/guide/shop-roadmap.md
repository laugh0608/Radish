...
> 文档定位：本文属于阶段性路线文档，记录商城模块当时的里程碑与待办拆分，不作为当前默认说明书入口。
>
> 日常协作优先查看 [Guide 手册索引](/guide/) 与 [shop-system](/guide/shop-system)。

> 入口页：[商城系统设计方案](/guide/shop-system)
> **当前状态**：✅ M9 核心功能已完成（2026-01-14），剩余优化功能和测试放到最后完成

## 8.1 实施概览

### 8.1.1 里程碑定位

商城系统作为 Radish 项目的 **M9 里程碑**，已于 2026-01-14 完成核心功能。
...
- ⚠️ **P4 阶段**：管理功能（后端 API 完整，前端移至 M10）

**增量优化**（待实施，非阻塞）：
- ⏳ 前端管理界面（移至 M10：Console 项目）
- ⏳ 定时任务（订单超时、权益过期）
- ⏳ 单元测试和集成测试（后置到当前 `M13：验证基线与回归资产工程化` 统一处理）
- ⏳ 压力测试和性能优化（后置到验证 / 观测基线稳定后的后续阶段）

---

## 8.2 阶段划分

### 8.2.1 P1 阶段：核心购买流程 ✅ 已完成
...
**预计工作量**：3-4 天
**实际工作量**：1 天（2026-01-13）

---

### 8.2.2 P2 阶段：权益系统 ✅ 已完成
...
- ⏳ 订单超时取消任务
- ⏳ 权益过期检查任务
- ⏳ 单元测试（UserInventoryService、UserBenefitService）
- ⏳ 集成测试（完整购买流程）
...
### 8.2.4 P4 阶段：管理功能 ⚠️ 后端已完成，前端移至 M10
...
- ✅ 统计报表数据准确（后端 API）
- ⏳ 管理界面友好易用（前端待实现，移至 M10）

**实际工作量**：1 天（后端 API，2026-01-13）
**待实现**：前端管理界面（radish.console，移至 M10：后台管理Console项目）

**决策说明**（2026-01-15）：
- M9 商城系统核心功能（购买流程、权益系统、用户前端）已完成
- 管理后台前端涉及完整的 Console 应用架构（认证、路由、UI 框架）
- 为避免重复工作，将管理界面统一放到 M10（后台管理Console）实施
