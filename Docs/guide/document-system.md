...
3. 公开 docs 壳层当前继续保持只读阅读边界，不扩展分享入口、编辑、发布、回收站、版本历史等桌面治理能力；
4. 在最小纯逻辑回归基线之上，继续按批次补组件交互与验收清单；
5. 评估分享入口是否需要进入下一阶段；
...
- 以上判断针对的是桌面文档应用本体；第二开发阶段中的公开内容壳层文档阅读当前已完成“目录 + 搜索 + 正文阅读”首批入口收口，但仍作为 `Phase 2-2` 的只读阅读入口维持稳定维护，不与桌面治理链路混为一体。

建议的后续顺序：

1. 先维持公开内容壳层下文档目录、公开搜索与正文阅读入口的稳定维护；
