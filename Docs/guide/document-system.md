...
4. 公开 docs 壳层当前继续保持只读阅读边界，不扩展编辑、发布、回收站、版本历史等桌面治理能力；
5. Flutter 原生 docs detail 当前复用公开 docs 路由口径，正文中的 `/docs/:slug`、完整公开 URL、`docs/:slug`、`./:slug` 与普通相对 slug 会继续打开原生 docs 详情；页内锚点、附件路径和非 docs 链接仍按文本展示；
6. Flutter 原生 docs detail 的公开链接复制使用当前 Gateway Base URL 加 `/docs/:slug`，不复制 Flutter 内部 handoff、来源 tab 或旧 long 兼容路径；
7. 在基础纯逻辑回归基线之上，继续按批次补组件交互与验收清单；
8. 如后续使用反馈出现目录治理瓶颈，再评估批量重排或拖拽式目录治理；
9. 桌面文档应用当前以稳定维护为主，不再单列新的桌面治理主线开发任务。
...
- 以上判断针对的是桌面文档应用本体；第二开发阶段中的公开内容壳层文档阅读当前已完成“目录 + 搜索 + 正文阅读 + 复制公开链接”首批入口收口，但仍作为 `Phase 2-2` 的只读阅读入口维持稳定维护，不与桌面治理链路混为一体。
