...
2. 在现有回归基线上继续维持组件交互与验收清单；公开 docs 壳层的 UI / 路由验收当前统一收口到 [docs 公开阅读首批人工验收清单](/guide/docs-public-acceptance)；
...
>
> 公开内容壳层下 `/docs`、`/docs/search`、`/docs/:slug` 的目录阅读、搜索回跳、分享入口与来源返回，不再混在本节桌面治理模板里，统一改看 [docs 公开阅读首批人工验收清单](/guide/docs-public-acceptance)。
