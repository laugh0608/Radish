...
1. 在 `Phase 2-2` 中为文档阅读补公开内容壳层首批入口：`/docs` 与 `/docs/:slug`，只做目录浏览与正文阅读；
2. 在最小纯逻辑回归基线之上，补组件交互与验收清单；
3. 评估正文搜索、分享入口是否需要进入下一阶段；
4. 如后续使用反馈出现目录治理瓶颈，再评估批量重排或拖拽式目录治理；
5. 桌面文档应用当前以稳定维护为主，不再单列新的桌面治理主线开发任务。
...
- 以上判断针对的是桌面文档应用本体；第二开发阶段中的公开内容壳层文档阅读，当前作为 `Phase 2-2` 的新入口继续推进，不与桌面治理链路混为一体。

建议的后续顺序：

1. 先补公开内容壳层下的文档目录与正文阅读入口；
