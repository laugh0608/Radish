...
2. 公开 docs 壳层当前继续保持只读阅读边界，不扩展编辑、发布、回收站、版本历史等桌面治理能力；
3. 在最小纯逻辑回归基线之上，继续按批次补组件交互与验收清单；
4. 评估正文搜索、分享入口是否需要进入下一阶段；
5. 如后续使用反馈出现目录治理瓶颈，再评估批量重排或拖拽式目录治理；
6. 桌面文档应用当前以稳定维护为主，不再单列新的桌面治理主线开发任务。
...
- 以上判断针对的是桌面文档应用本体；第二开发阶段中的公开内容壳层文档阅读，当前已完成首批目录与正文阅读入口收口，但仍作为 `Phase 2-2` 的只读阅读入口维持稳定维护，不与桌面治理链路混为一体。

建议的后续顺序：

1. 先维持公开内容壳层下文档目录与正文阅读入口的稳定维护；
2. 在现有回归基线上补齐组件交互与验收清单；
3. 再评估正文搜索、分享入口等下一阶段能力；
