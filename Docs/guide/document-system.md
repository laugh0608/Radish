...
1. 打开文档应用，确认固定文档目录树可加载，正文渲染、目录展开折叠与窄窗口布局正常。
2. 新建一篇在线文档并保存为草稿，确认标题、Slug、父级与排序建议值可正常写入。
3. 编辑该在线文档的正文与元信息后再次保存，确认详情刷新后内容与目录关系同步更新。
4. 将草稿发布，再执行一次撤稿或归档，确认列表筛选、状态标识与详情展示一致。
5. 导入一个 Markdown 文件生成在线文档，再执行一次导出，确认导入导出链路可用。
6. 对在线文档进行至少一次修改后查看版本历史，再执行一次回滚，确认会产生新的回滚版本且正文恢复正确。
7. 删除该在线文档并到回收站中查看，再执行恢复，确认恢复后可重新在正常列表中访问。
