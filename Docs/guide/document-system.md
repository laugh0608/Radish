...
- 左侧：加宽后的导航侧栏，承载搜索、状态筛选、目录树与检索结果；
- 中部：文档详情或编辑器主工作区；
- 右侧：管理员版本历史与版本详情面板（宽屏下显示）；
...
- **文档 App 的 A 阶段可视为已基本收口**；
- 后续不建议继续投入较重的目录治理开发（如拖拽排序、批量重排），除非已有明确真实使用痛点；
...
- [ ] **新建在线文档**：管理员可成功创建在线文档，创建后默认进入草稿状态；
...
- [ ] **前端回归基线**：`npm run test --workspace=radish.client` 通过；
