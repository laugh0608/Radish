...
3. docs 公开分享入口首批当前已补到详情阅读页：首批只提供“复制当前公开链接”的最小能力，复制结果会优先收口到 canonical 公共路径，不扩展海报、分享统计或其他治理语义；
4. 公开 docs 壳层当前继续保持只读阅读边界，不扩展编辑、发布、回收站、版本历史等桌面治理能力；
5. 在最小纯逻辑回归基线之上，继续按批次补组件交互与验收清单；
6. 如后续使用反馈出现目录治理瓶颈，再评估批量重排或拖拽式目录治理；
7. 桌面文档应用当前以稳定维护为主，不再单列新的桌面治理主线开发任务。
