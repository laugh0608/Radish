...
- 文档 App 已完成浅色化与侧栏/主内容区重排，提升目录浏览与阅读空间。
...
截至当前收尾版本，文档 App 已完成以下 A 阶段核心闭环：
...
- 文档 App 浅色 UI 与 Markdown 正文 / 代码样式已完成第二轮收口；
- 最小前端纯逻辑回归基线。

基于以上结果，当前判断为：

- **文档 App 的 A 阶段主链路已基本闭环，但仍需按验收清单完成一次完整联调确认**；
- 目录治理能力先维持当前父级禁选、回收站与排序建议方案，不继续提前扩成拖拽式重排；
- 更适合进入“验收 + 试点 + 下一阶段评估”节奏，而不是继续扩写文档 App 本体。
...
- [ ] **固定文档展示**：启动后能在文档 App 中看到来自 `Docs/` 的固定文档，且正文渲染正常；
...
- [ ] **前端回归基线**：`npm run test --workspace=radish.client` 通过；同时确认文档 App 侧栏、浅色正文与代码高亮视觉协调；
...
- 上述验收清单中的核心链路项已通过；
- 没有新增阻断级缺陷；
- 文档 App 在实际使用中未出现明显目录治理瓶颈；
- 已明确下一阶段要优先做的是“迁移试点 / 搜索评估 / 分享入口”中的哪一项。
