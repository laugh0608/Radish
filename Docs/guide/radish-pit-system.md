...
> 版本：v1.1 | 最后更新：2026-07-16 | 状态：已实现，持续维护
...
```text
HTTP status + Code + MessageKey + MessageArguments + MessageInfo + TraceId
```
...
`Code` 和 HTTP status 决定控制流，`MessageKey` 与可选 `MessageArguments` 驱动 client 展示格式化，`MessageInfo` 仅作为服务端安全兜底。页面不得匹配中英文消息文本，也不得使用动态参数决定业务分支。
