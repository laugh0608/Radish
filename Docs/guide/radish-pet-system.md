...
> 状态：Phase B 登录态 Web 主链路已上线并进入稳定维护；F4-H-B 公开名片服务端契约已完成、正式页面待 F4-H-C
>
...
- 用户可修改宠物名称和公开展示开关。
- 公开展示开关已接入 `User/GetPublicProfile` 的服务端字段白名单投影；正式 `/u/:id` 页面尚未消费 `VoPet`，在 F4-H-C 完成前不能把开启开关理解为已经在页面上可见。
