...
**防刷机制**：
- 同一用户对同一内容的点赞 / 评论奖励按自然业务事实生成 `RewardBusinessKey`，由 `CoinTransaction.TenantId + RewardBusinessKey` 唯一约束保护，重复触发时返回既有奖励事实，不重复增加余额
- 帖子点赞、评论点赞的作者奖励与点赞者奖励分别使用独立业务键；点赞者每日奖励上限仍按成功奖励流水汇总判断
- 评论创建奖励、评论被回复奖励、神评 / 沙发基础奖励、点赞加成奖励和保留奖励都应使用稳定业务键表达唯一性
- 点赞奖励每日上限：`50 胡萝卜`（防止刷赞）
- 系统检测异常行为（短时间大量操作），自动冻结账户

经验奖励与胡萝卜奖励保持同一思路：需要一次性发放的经验流水使用 `ExpTransaction.RewardBusinessKey`，由 `TenantId + RewardBusinessKey` 唯一约束保护。普通每日统计和历史 `idx_dedup` 仍用于既有经验查询和日粒度约束，但不能替代奖励业务键。
...
| **内容赞赏** | 固定 `1 胡萝卜` | 论坛“胡萝卜 +1”，发送者与作者之间免手续费流转；详见[专题设计](/features/forum-content-reward) |
