...
> 版本：v1.5 | 最后更新：2026-05-31 | 状态：用户侧购买、订单、背包来源、公开商城到工作台深链、Flutter 登录态订单 / 背包只读承接和构建拆包治理已完成当前阶段目标，公开详情分享与 Console 治理转入稳定维护
...
- ✅ Flutter 移动端已补登录态订单列表、订单详情、背包和来源订单 / 商品只读查看；移动端不承接购买、支付口令、权益激活或道具使用
...
- 公开商城仍保持只读边界；购买仍只属于登录后的 WebOS 私域商城，订单和背包查看同时由 WebOS 私域商城与 Flutter 登录态只读页承接。

### 1.8 Flutter 移动端只读承接现状
...
- Flutter 当前不承接购买、取消订单、支付口令校验、权益激活、道具使用、退款或商城 Console 治理。
