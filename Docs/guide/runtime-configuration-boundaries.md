...
| Console 系统设置 | 已注册的运营参数，例如 favicon、账号身份长度、帖子 / 评论 / 轻回应长度 | 仅限 Low / Medium 且受后端确认规则限制 | `DataBases/SystemConfigs/system-configs.json` 覆盖值 + 代码默认值 |
...
邮箱、登录凭证、高风险账号字段、安全会话、奖励数值、审核阈值和资产相关设置仍需逐项评审，不直接开放编辑。
