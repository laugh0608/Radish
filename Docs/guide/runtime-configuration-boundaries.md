...
| Console 系统设置 | 已注册的运营参数，例如 favicon、账号身份长度与改名频率、PublicIndex 保留号规则、帖子 / 评论 / 轻回应长度、轻回应返回条数、频率治理参数和神评稳定性参数 | 仅限 Low / Medium 且受后端确认规则限制 | `DataBases/SystemConfigs/system-configs.json` 覆盖值 + 代码默认值 |
...
| `UserIdentity.PublicIndex.ReservedIndexes` | `[1314,5200]` | Medium | 普通注册 / Bootstrap 自动分配时跳过的显式 PublicIndex 保留号 |
| `UserIdentity.PublicIndex.VanityRules` | `{"repeatedDigits":true,"ascendingSequence":true,"descendingSequence":true,"palindrome":true}` | Medium | 普通注册 / Bootstrap 自动分配时跳过的 PublicIndex 靓号规则 |
...
轻回应功能开关 `ForumQuickReply.Enable`、神评任务启停 / 调度 / 扫描 / 触发数量门槛、邮箱、登录凭证、人工指定 PublicIndex、高风险账号字段、安全会话、奖励数值、审核阈值和资产相关设置仍需逐项评审，不直接开放编辑。
