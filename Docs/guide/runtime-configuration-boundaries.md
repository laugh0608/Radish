...
| Console 系统设置 | 已注册的运营参数，例如 favicon、账号身份长度、帖子 / 评论 / 轻回应长度、轻回应返回条数、频率治理参数和神评稳定性参数 | 仅限 Low / Medium 且受后端确认规则限制 | `DataBases/SystemConfigs/system-configs.json` 覆盖值 + 代码默认值 |
