...
| 文件 | 处理 |
| --- | --- |
| `Radish.Auth/Views/Authorization/Consent.cshtml` | 三次具体化授权确认页：主标题带请求应用名，首屏说明当前账号和返回位置；账号、应用、开发方和返回目标合并成可核对信息区；权限列表隐藏原始 `scope`，改展示用途和后果；风险提示改为“不会获得密码 / 支付口令 / 验证码”等明确边界。 |
...
- `E7-D` 已按用户截图反馈完成紧凑授权卡与具体授权决策三次收口，下一步不直接进入 `P3-12-F`。
