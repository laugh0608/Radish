...
5. [Flutter Android RC 分发前置清单](/records/flutter-android-rc-distribution) 已明确内测 RC 与外部分发阻塞条件的边界
...
- 后续若准备正式 release 包，应先补齐测试 Gateway、测试对象与反馈闭环，再按 [Flutter Android RC 分发前置清单](/records/flutter-android-rc-distribution) 执行 testing Gateway release APK 构建与真机验收
...
- 上述动作统一留到正式 release 包发布前，再按 [Flutter Android RC 分发前置清单](/records/flutter-android-rc-distribution) 补齐
