...
>
> 状态：首批代码与 Gateway PC / mobile smoke 已完成
>
...
`2026-06-23` 已完成首批代码实现：

- 新增 `/workbench` 正式 Web 功能总入口，按静态能力清单展示公开、登录态、Console 和 WebOS 桌面版兼容入口。
- 公共壳层“工作台”默认指向 `/workbench`。
- `/desktop` 继续保留为“WebOS 桌面版”兼容入口，只作为功能地图中的历史入口出现。
- 不新增后端 API，不探测功能可用性，不迁移 WebOS Dock、窗口系统、桌面背景、窗口几何记忆或 `openApp` 语义。

已完成验证：

- `npm run test --workspace=radish.client -- ./tests/entryRoute.test.ts ./tests/realUsagePathContracts.test.ts`
- `npm run type-check --workspace=radish.client`
- `npm run build --workspace=radish.client`
- `git diff --check`
- `npm run check:host-runtime`

`2026-06-23` 在用户确认前后端已启动后，已完成 Gateway 真实页面 smoke：

- PC `1920x1080 @ DPR 2` 与移动 `390x844 @ DPR 3` 覆盖 `/workbench`、`/discover`、`/messages`、`/me`、`/shop/orders`、`/docs/mine`、`/desktop`。
- `/workbench` 功能地图标题、公共壳层“工作台”入口和静态能力清单链接均正常渲染，无横向溢出。
- `/discover` 页头“工作台”真实点击可跳转到 `/workbench`。
- `/messages`、`/me`、`/shop/orders`、`/docs/mine` 在未登录状态下进入 `Radish 统一登录`，无空白页或横向溢出。
- `/desktop` 保持 WebOS 桌面版兼容入口，可在 PC 与移动视口打开。
- 页面 console `error/warn` 为空；Playwright CLI wrapper 因本机 `npx` 包准备未及时返回，实际 smoke 使用 Codex 内置浏览器 + CDP 设备指标覆盖完成。
...
阶段验收时，在用户明确说明前后端已启动后，覆盖 Gateway PC / mobile 页面 smoke：
