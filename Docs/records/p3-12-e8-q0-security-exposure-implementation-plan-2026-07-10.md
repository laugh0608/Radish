...
> 状态：`Q0-A 至 Q0-D 已于 2026-07-11 完成代码、静态验证与成组运行态补验`
>
...
2. `Q0-B`：从生产 API 删除性能测试、敏感配置、事务演示、测试写入与手动推送入口，同时清理失效模型、服务链、资源、HttpTest 和架构口径。
...
- `npm audit --omit=dev --json` 实施前返回 3 个 High，并以非零状态退出。
- `dotnet package list --project Radish.slnx --vulnerable --include-transitive --format json --no-restore` 实施前列出多个项目中的 `Microsoft.OpenApi 2.0.0` High，但命令状态仍为 `0`。
...
Q0-A 实施过程中，仓库协作规则已同步调整为“包安装、依赖更新和项目启动必须先获得当前任务的用户明确授权”。本批实际流程为：

1. AI 先说明具体命令、目标版本和 lockfile 影响，并修改 `Frontend/radish.console/package.json`。
2. 用户明确授权当前两条依赖更新命令后，由 AI 执行：
...
3. AI 复核 `package-lock.json`、完整依赖树、安全审计与定向回归，再形成批次结论。

.NET 依赖直接编辑 csproj，不执行 `dotnet add package`；后续通过允许的 build / restore 流程验证。

### 3. dotnet 命令修正

修改 `Scripts/dotnet-command.mjs` 与 Windows 入口 `Scripts/dotnet-local.ps1`：
...
- `check:repo-quality-contract` 必须拒绝两个 .NET 包装入口再次出现 `NuGetAudit=false` 或全局 `NoWarn=NU1903`。
...
- job 安装 Node 24 / .NET 10，执行 `npm ci`、`dotnet restore Radish.slnx --disable-parallel`、`npm run check:dependency-security`。
...
- `Radish.Api/Controllers/AppSettingController.cs`
- `Radish.Api/Controllers/TransactionController.cs`
- `TenantController.AddTest` 与 `UserController.TestPushUnreadCount` 两个测试 Action
- `Radish.Model/ViewModels/WeatherForecastVo.cs`
- 仅由 Weather 演示控制器使用的 `GetByIdResultVo`、`TestResultVo`、`CacheTestResultVo`、`ScopeTestResultVo`、`AppTestResultVo`、`ServiceObjTestResultVo`
- `RedisConfigVo`、`TransactionTestResultVo`、`TenantAddTestResultVo`、`TestPushResultVo`
- 只为事务传播演示存在的 `IDepartmentService`、`DepartmentService` 与 `IUserService.TestTranPropagationUser` 实现链
...
- 删除 AppSetting 后，OpenAPI / Scalar 只按实际 Controller 版本生成文档；当前不再生成空的 v2 文档或保留固定 v2 示例。
...
- 明确拒绝 `RustTestController`、`WeatherForecastController`、`AppSettingController`、`TransactionController` 和后续同名回归。
- 在保留的 Controller 上精确拒绝 `TenantController.AddTest` 与 `UserController.TestPushUnreadCount` Action 回归。
- 不采用“所有含 Test/Demo 字样都禁止”的宽泛规则，避免误伤测试辅助类型；当前只守护已裁决退出的生产面。

### 4. Q0-B 验证

- 目标源码 / OpenAPI 文案扫描不再命中已退出的 Controller、Action、事务演示或敏感配置返回契约。
- `dotnet build Radish.slnx -c Debug`
- `dotnet test Radish.Api.Tests`
- `npm run check:repo-hygiene:changed`
- `git diff --check`

运行态补验时检查 `/scalar` 只展示实际 v1 文档且不再出现已退出标签；当前没有生产 v2 Controller，不应生成 `/openapi/v2.json`。
...
- 删除 4 个 Controller、2 个测试 Action、11 个仅供演示或敏感配置返回使用的 Vo。
- 删除只为事务传播演示存在的 2 个 Service 类型，并从 `IUserService` / `UserService` 清理无调用方测试方法与依赖。
- 2 个 API 资源文件、2 个 HttpTest 文件。
- Scalar 描述、framework、specifications、i18n 和 Rust 指南相关口径。
- 新增生产 API 表面测试。
...
## 九、Q0-A 实施结论（2026-07-11）

Q0-A 已完成，退出条件成立：

| 范围 | 结果 |
| --- | --- |
| .NET 依赖 | `Microsoft.AspNetCore.OpenApi 10.0.9`、`Microsoft.OpenApi 2.7.5` 已解析；NuGet High / Critical 为 `0` |
| npm 生产依赖 | `react-router / react-router-dom 7.18.1`、`ws 7.5.11` 已解析；`npm audit --omit=dev` 全等级漏洞为 `0` |
| 审计语义 | Node 与 PowerShell 两个 .NET 包装入口均不再注入 `NuGetAudit=false` 或 `NoWarn=NU1903` |
| 安全门禁 | workflow 与 `master` ruleset 模板新增 `Dependency Security` required check；审计失败、源不可用、JSON 异常或 High / Critical 均阻断 |
| 门禁契约 | workflow、ruleset、CI-only 与本地 `validate:ci` 的有意差异已纳入 contract，防止静默漂移 |
| lockfile | npm 自动清理 76 个无现存依赖引用的历史孤儿节点；反向引用检查为 `0`，`npm ls --all` 通过 |

已完成验证：

- `npm run check:dependency-security`：通过，npm / NuGet High / Critical 均为 `0`。
- `npm run validate:ci -- --report`：通过，包含 311 项 client 测试、551 项后端测试、15 项身份定向测试和全部类型检查 / contract 扫描。
- `radish.console`：type-check、27 项测试与生产构建通过。
- `radish.client`：311 项测试与生产构建通过。
- `dotnet build Radish.slnx -c Debug`：`0 warning / 0 error`；`Radish.Api.Tests`：`551 / 551` 通过。
- `git diff --check` 与 changed-only 仓库卫生通过；仍提示 `Docs/deployment/guide.md` 既有硬上限和 `validation-baseline.md` 既有软上限篇幅债务，本批不扩入 Q4 文档拆分。

本批未启动服务或执行 Gateway smoke。Q0-A 只涉及依赖与门禁，静态、构建和测试证据已经覆盖退出条件。

仓库改动只维护 workflow 与 ruleset 模板；用户已于 2026-07-11 确认将精确检查名 `Dependency Security` 配置到 GitHub 远端 `master` ruleset。首次 `PR -> master` 仍需观察该 required check 已由真实 workflow 上报，避免只凭配置项存在判定门禁有效。

## 十、Q0-B 初始实施与补充发现（2026-07-11）

Q0-B 原定两个 Controller 的退出条件已经成立；补充表面扫描发现同类生产暴露，因此 Q0-B 整体暂不关闭：

| 范围 | 结果 |
| --- | --- |
| 生产 API 表面 | `RustTestController`、`WeatherForecastController` 已从生产编译删除；正式 Rust 图片处理、文件哈希与 `Lib/radish.lib/src/benchmark/` 保留 |
| 失效资产 | 删除 7 个仅供 Weather 演示使用的 Vo、2 组 Weather 资源键和 HttpTest 中的旧演示请求 |
| 文档口径 | Scalar v2 描述与 framework、specifications、i18n 已改用真实接口或本地 Rust 测试入口，不再推荐生产性能 / Weather 端点 |
| 防回归契约 | 新增生产 Controller 反射测试，精确拒绝 `RustTestController`、`WeatherForecastController` 同名类型回归，不扩大为模糊的 Test / Demo 名称禁令 |

已完成验证：

- 目标源码、Scalar 文案、资源、HttpTest 与架构文档扫描不再命中退出端点或 Weather 契约。
- `dotnet test Radish.Api.Tests --filter FullyQualifiedName~ProductionApiSurfaceTests`：`1 / 1` 通过。
- `dotnet build Radish.slnx -c Debug`：`0 warning / 0 error`。
- `dotnet test Radish.Api.Tests`：`552 / 552` 通过。
- `npm run check:repo-hygiene:changed`：检查 40 个文件，未发现文本卫生问题；仍提示 `specifications.md`、`deployment/guide.md` 既有硬上限和 `validation-baseline.md` 既有软上限篇幅债务，本批未增加规范文档行数，也不扩入 Q4 拆分。
- `git diff --check`：通过。

### 补充暴露面发现

目标面扫描还发现以下未被原定精确删除清单覆盖的生产入口：

- `AppSettingController.GetRedisConfig`：返回 `Redis:ConnectionString` 与完整 `RedisOptions`，属于敏感配置暴露，且没有产品调用方。
- `TransactionController`：整个 Controller 为事务演示；`Get` 会尝试写入角色并故意制造除零异常，`TestTranPropagation` 仅用于事务传播测试，均没有产品调用方。
- `TenantController.AddTest`：向正式数据库写入固定测试租户，仅由演示 DTO 承载。
- `UserController.TestPushUnreadCount`：标注为 P0 测试接口，可由普通 Client policy 登录态手动触发未读数推送。

该范围超出首轮确认的精确删除清单，因此当时暂停进入 Q0-C；用户随后明确确认按 Q0-B 补充批次删除，并同步清理专用 Vo、无调用方的事务传播测试 Service 接口 / 实现、v2 示例文案与防回归契约。

首轮未启动 API / Gateway，也未执行 Scalar 运行态 smoke。原定两个 Controller 已从源码删除，编译产物反射契约与完整 API 测试覆盖其退出条件；当时在补充暴露面完成裁决前没有进入 Q0-C。

## 十一、Q0-B 最终实施结论（2026-07-11）

用户确认补充范围后，Q0-B 已完整关闭：

| 范围 | 最终结果 |
| --- | --- |
| 生产 Controller | 删除 `RustTestController`、`WeatherForecastController`、`AppSettingController`、`TransactionController`；生产 API 不再承载性能基准、敏感 Redis 配置返回或事务演示 |
| 保留 Controller 的测试 Action | 删除 `TenantController.AddTest` 与 `UserController.TestPushUnreadCount`；正常租户查询和正式通知推送服务保持不变 |
| 失效资产与服务链 | 共删除 11 个演示 / 敏感配置专用 Vo；删除只为事务传播演示存在的 `IDepartmentService`、`DepartmentService`，并从 `IUserService` / `UserService` 清理测试方法与构造依赖 |
| API 版本与文档 | 删除唯一生产 v2 Controller 及固定 v2 Scalar 描述；OpenAPI / Scalar 改为只按实际 Controller 版本生成，当前源码仅有 v1 |
| 防回归契约 | 反射契约精确拒绝 4 个已退出 Controller 和 2 个已退出 Action，不采用宽泛 Test / Demo 名称禁令 |

最终验证：

- 活动源码、OpenAPI / Scalar 文案和架构文档扫描不再命中已退出 Controller、Action、事务演示或固定 v2 入口；生产 Controller 中显式 Test / Demo / Debug / Benchmark Action 扫描无命中。
- `dotnet test Radish.Api.Tests --filter FullyQualifiedName~ProductionApiSurfaceTests`：`2 / 2` 通过。
- `dotnet build Radish.slnx -c Debug`：`0 warning / 0 error`。
- `dotnet test Radish.Api.Tests`：`553 / 553` 通过。
- `npm run check:repo-hygiene:changed`：检查 47 个文件，未发现文本卫生问题；`specifications.md` 已由 1392 行降至 1383 行，仍与 `deployment/guide.md` 一并保留既有硬上限债务，`validation-baseline.md` 保留既有软上限债务。
- `git diff --check`：通过。
...
## 十四、Q0 成组运行态补验（2026-07-11）

服务由用户在当前任务中明确说明已启动后，Q0 与 E8-B 运行态旅程成组执行，结果如下：

- Gateway、API、Auth 的 `/health` 均返回 `200`；`validate:baseline:host` 的完整基线、`DbMigrate doctor / verify` 只读自检全部通过。
- Gateway `/scalar` 跟随规范化重定向后返回 `200`；`/openapi/v1.json` 标题为 `Radish API Documentation - V1`，包含 282 条实际路径；`/openapi/v2.json` 返回 `404`，已退出 Controller 与 Action 未进入 v1 路径表。
- 经 Gateway 完成授权码签发，申请范围为 `openid profile offline_access radish-api`；Token、受保护 API、UserInfo、refresh 换新 Token，以及刷新后再次访问受保护 API均返回 `200`。
- 不申请 `radish-api` resource 的有效登录 Token 调用 API 时返回 `403`，证明缺少目标 audience / resource 的 Token 无法进入受保护 API。
- E8-B 真实旅程覆盖登录回流、评论与聊天连接；页面刷新后频道与消息恢复，通知、关系复访和 Console 权限边界连续成立。
- 公开 Docs 与论坛详情完成真实 Markdown 阅读回放；危险链接的阅读态与编辑态拒绝规则继续由共享 sanitizer 和自动化测试固定。
- `npm run check:dependency-security` 再次通过，npm / NuGet High / Critical 均为 `0`。
- 集成安全复核发现两份跨平台附件测试脚本仍沿用旧 password grant，其中 Bash 脚本还保存了完整过期 JWT。Bash / PowerShell 已统一删除 JWT、默认账号密码、client secret 和 password grant，改为只接受当前进程 `RADISH_ACCESS_TOKEN`；新增完整 JWT、硬编码 Bearer Token 与私钥 PEM 头高置信扫描，并接入 `Baseline Quick / Repo Quality`。

至此 Q0-A 至 Q0-D 的代码、静态、依赖安全、敏感字面量、身份协议、宿主健康与要求的成组运行态补验全部关闭。运行态使用本地种子账号和本地 SQLite 数据，不替代 `P3-12-F` 的生产相似环境升级演练与 Release Go 判断。
