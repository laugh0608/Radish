...
2. 在窄屏下检查商城首页，确认分类区、推荐商品区、只读标识、公开浏览提示区和顶部“工作台”入口没有重叠、截断或横向撑破，且该入口指向 `/desktop`。
3. 从首页点击分类或“查看全部”，确认能进入 `/shop/products`，并正常加载分类筛选和商品列表。
4. 在首页或 `/shop/products` 顶部检查公开浏览提示，确认都会明确表达“先看内容 / 继续进入 / 不在这里”三层信息，不把购买、订单、背包或账号动作误导成公开壳层可用能力。
5. 在 `/shop/products` 中切换分类、执行一次搜索、翻到第 `2` 页及之后任意一页，确认 `category / q / page` 会稳定回写到 URL，刷新后仍能恢复当前列表上下文。
6. 从 `/shop/products` 点击商品进入 `/shop/product/:productId`，确认详情页会继续展示“公开商品详情阅读提示”或等价的只读导览信息，明确详情重点、继续去向与工作台边界；同时页面只展示商品信息、说明和返回桌面工作台的导向，不会误弹购买确认、举报或登录提示；再点击详情页返回按钮，确认会优先回到原商品列表上下文，而不是一律退回默认列表，且返回文案应显示为“返回商品列表”。
7. 从 `/discover` 的商城推荐区点击商品进入 `/shop/product/:productId`，确认详情页返回按钮会优先回到 `/discover`，而不是丢失到商城默认列表；此时返回文案应显示为“返回社区发现”。
8. 直接刷新 `/shop/product/:productId`，确认匿名详情读取仍然可用，不会因为前端带了 `withAuth` 或把外部 `productId` 当成 `number` 而失败。
9. 在详情页点击“工作台”或同类导向入口，确认会进入 `/desktop` WebOS 保留入口；再从浏览器回退，确认还能回到当前公开详情或商品列表上下文。
10. 人工抽查一个不存在或已失效的 `productId`，确认公开壳层会给出只读的“商品不可访问”状态，而不是跳回论坛、桌面或空白页。

## 5. 预期结果

- `/shop`、`/shop/products` 与 `/shop/product/:productId` 以公开内容壳层直达，不绕回桌面入口。
- 公开商城在手机上保持“首页可扫读、列表可筛选、详情可直达”的浏览节奏。
- 首页、列表与详情当前都会明确给出只读导览信息，说明公开浏览重点、下一步去向与工作台边界，而不是只剩商品展示本身。
- 列表筛选、搜索和分页状态都能被 URL 保存，刷新后不会丢失上下文。
- 从 `/shop/products` 或 `/discover` 进入商品详情后，返回动作会优先回到原公开来源，而不是一律退回默认商品列表。
- 商品详情返回按钮会按真实来源显示为“返回商品列表”或“返回社区发现”等具体提示，而不是停留在泛化入口文案。
