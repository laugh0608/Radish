...
首轮静态审阅与 `390 x 844` Vite 直连实测均未发现必须立即抢占主线的 `P0/P1` 阻断，但发现一个与新路线强相关的高信号候选：

- 公开页顶部 `PublicShellHeader` 的 `WebOS` 链接当前指向 `/`。当根路径 `/` 切到纯 Web 默认入口后，该链接会失去“进入工作台”的语义，应在 `/` 切换前改为明确指向 `/desktop`，并把文案从 `WebOS` 收敛为“工作台”或等价表述。

因此本批建议顺序为：

1. 先按本矩阵补齐 `430 x 932` 或 Gateway 入口复核。
2. 若复核没有更高优先级阻断，第一小闭环优先处理公开页顶部工作台入口语义。
3. 再进入根路径 `/` 默认入口切换评估。
...
| 发现 | 影响 | 建议 |
| --- | --- | --- |
| `PublicShellHeader` 的工作台入口仍是 `<a href="/">WebOS</a>` | 根路径 `/` 切到纯 Web 后，公开页顶部入口会回到当前纯 Web 页面而不是 `/desktop`，且 `WebOS` 文案不符合新路线 | 作为矩阵后的第一小闭环候选：改为 `/desktop`，文案改为“工作台”或等价表述 |
...
| 根路径 `/` 目前仍落到 `RootEntry`，Tauri 环境才替换到 `/desktop` | 与新规划不冲突，因为代码切换被拆成后续批次；但矩阵需要提前识别依赖 `/` 的入口 | 根路径切换前先完成入口依赖梳理，避免公开页内部链接失效 |
...
- 公开边界：
  ...
  - 公开页顶部工作台入口仍是 `href="/"`，与根路径转纯 Web 后的路线冲突，继续作为第一小闭环候选
