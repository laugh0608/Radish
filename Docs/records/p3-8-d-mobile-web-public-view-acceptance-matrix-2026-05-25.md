...
- 范围：在不启动服务的前提下，按同一张矩阵批量复核 `/discover / forum / docs / shop / leaderboard / u/:id` 的移动端公开主路径。
- 复核重点：窄屏布局、来源返回、分享入口、公开只读边界、长文本防溢出、公开链接契约和与 `/desktop` 保留入口的边界。
- 结论：本轮未发现新的 `P0/P1` 阻断；公开 Web 移动视图矩阵可阶段收口，不再继续逐页做小循环式微调。
- 成组修复：
  - `/discover` 补显式复制链接入口，分享 URL 保留 `section` 区块状态，与 forum / docs / shop / leaderboard / profile 的公开分享入口口径对齐。
  - `/leaderboard` 补显式复制链接入口，保留榜单类型与分页分享路径。
  - `/u/:id` 移除登录态关注 / 取关写操作，公开主页保持只读资料、公开帖子和公开评论边界。
  - `/shop` 公开链路与复用商品组件补齐长商品名、长分类名、长筛选标签、价格 / 时效文本和商品详情说明的窄屏防溢出约束。
- 横向复核结果：
  - 公开页未再发现指向 `/` 的工作台入口；工作台入口保持 `/desktop` 语义。
  - 公开详情来源返回仍通过 `history.state` 保留，不污染公开分享 URL。
  - 公开个人页、公开帖子、公开文档、公开商品、公开榜单和公开分发页均具备显式分享入口或统一分享 hook。
  - 商品公开链路不直接暴露购买、订单或背包执行入口；“进入工作台继续购买”仍通过 `/desktop?app=shop&productId=...` 承接。
- 后续方向：若没有真实访问、分享预览、购买 / 订单 / 背包或权限授权的新阻断，P3-8-D 下一步应转入 Flutter 下一批高价值功能评估，而不是继续在 Web 公开页逐点打磨。
