...
`WOG-6` 聚焦已具备服务端幂等能力、但客户端接入不完整的资产写入口。首批已按确认范围补齐 Flutter 单商品购买的 `idempotencyKey` 契约，让移动原生购买与 Web 官方流程使用同一幂等语义。
...
### 2. Flutter 商城购买首批已接入 key

首批代码已完成以下调整：

- `ShopRepository.purchaseProduct` 显式要求调用方传入 `idempotencyKey`。
- `HttpShopRepository.purchaseProduct` 会校验 `idempotencyKey.trim()` 非空，并在请求体提交 `idempotencyKey`。
- `ShopProductDetailPage._submitPurchase` 在真实购买前生成并持有 `shop:{timestamp-random}`；失败后直接重试复用同一个 key，购买成功、商品 / 仓储变化、`accessToken` 变化或退出登录后清空 key。
- `shop_product_detail_page_test` 已覆盖请求体提交、空白 key 拒绝、成功购买携带 `shop:` key，以及失败后直接重试复用同一个 key。

因此 Flutter 单商品购买不再命中服务端“未传 key 的兼容路径”；重复提交会进入已实现的 `OperationIdempotencyRecord` 保护。
...
| Flutter 商品详情页 | 一次购买尝试持有一个 key，成功、商品 / 仓储变化、`accessToken` 变化或退出登录时重置 | 与 Web 弹窗口径一致 |
...
1. 商品详情页在准备发起购买确认时生成 `shop:{timestamp-random}`。
2. 同一次购买意图失败后直接重试复用同一个 key。
3. 购买成功、切换商品、仓储 / 商品变化、`accessToken` 变化或退出登录时生成新 key。
4. 仓储层不自行生成 key，只要求调用方传入非空 `idempotencyKey` 并按原字段提交。
5. 日志、错误提示和测试不输出支付口令；如未来记录 key，只记录是否存在或 key 前缀，不把 key 当作安全凭证。

Flutter 当前未引入 UUID 依赖。首批没有为单个 key 生成新增依赖，已使用 Dart 标准库生成符合服务端字符集的随机后缀。若后续已有稳定 UUID 依赖，再统一替换。
...
已在 `ShopProductDetailPage` 内维护一个购买 key：

- 新增 `_purchaseIdempotencyKey` 状态。
- `_submitPurchase` 进入实际购买前若不存在 key，则生成一个。
- 请求失败但未确认成功时保留该 key，允许用户直接重试同一购买意图。
- 购买成功后清空 key；返回详情页后下一次购买生成新 key。
- 商品 ID 变化、仓储实例变化、`accessToken` 变化或会话退出时清空 key。
...
1. WOG-6 首批只补 Flutter 单商品购买的 `idempotencyKey`，不纳入 Flutter 转账或服务端强制 key。
2. Flutter 由商品详情页生成并持有 `shop:{timestamp-random}`，仓储层只负责校验非空和提交。
3. 购买失败后保留同一个 key 供用户直接重试，购买成功或购买意图重置后再生成新 key。
4. 首批继续保持服务端 `CreateOrderDto.IdempotencyKey` 可空，等主要客户端接入后再评审强制策略。
