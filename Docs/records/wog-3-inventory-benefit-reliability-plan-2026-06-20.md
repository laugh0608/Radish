...
- 已新增部署态 SQL：`Deploy/sql/20260620_add_inventory_benefit_reliability_guards.sql`，覆盖历史权益重复清理、背包聚合项合并、新发放流水表、可确认订单流水回填和唯一索引创建。
...
3. `Deploy/sql/20260620_add_inventory_benefit_reliability_guards.sql` 版本化 SQL。
...
4. 本批保持 API / 前端契约不变，只改后端持久化、迁移 SQL、DbMigrate 和定向测试。
