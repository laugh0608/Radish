...
> **文档状态**：历史 API 方案参考。本文包含尚未落地、已经改名和与真实 Controller 不一致的接口示例；当前事实应以 `Radish.Api/Controllers/NotificationController.cs` 为准，F4-B 新契约以 [通知中心深化与通知治理](/features/notification-center-deepening) 为准。在 F4-B-C 完成消费者迁移前，现有 API 仍保留兼容，不应按本文直接新增调用。
...
- 前端打开 forum 通知时优先消费更稳定的 `postPublicId`；缺失时可从旧 `routePath` 兼容解析公开标识，再回退旧 `postId / commentId` 字符串。
...
- 数据优先从统一缓存层读取，`Redis.Enable=true` 时缓存后端为 Redis，禁用 Redis 时为内存缓存；缓存未命中时查询数据库并回填缓存
