...
> **创建日期**：2026-01-02
> **最后更新**：2026-07-18
> **状态**：历史总体规划；P0 / P1 实现事实保留，原 P2 已由 F4-B 重新审计和设计
>
> **当前入口**：通知偏好、聚合、容量、可靠目标和跨端对账统一查看 [F4-B 通知中心深化与通知治理](/features/notification-center-deepening)。本文中的占位现状、阶段估算和 P2 方案不再作为当前实施契约。

## 📊 实施进度概览

- ✅ **P0（基础可用链路）**：已完成（2026-01-07）
  - SignalR Hub 连接、未读数实时推送、React StrictMode 兼容性修复
- ✅ **P1（可用通知系统）**：已完成（2026-01-10）
  - 数据模型和独立消息数据库、NotificationService 核心业务逻辑
  - NotificationCacheService 缓存管理、NotificationDedupService 去重服务
  - API 接口完整、点赞和评论通知集成、前端通知中心和列表组件
- ⏸️ **P2（体验增强）**：待规划
  - 多端同步、通知聚合、用户偏好设置、监控告警

详细完成情况见 [开发日志 2026-01](../changelog/2026-01/week2.md)。
...
- **前端**：Dock 使用轮询获取未读数
  - 位置：`Frontend/radish.client/src/desktop/Dock.tsx:172`
...
| 阶段 | 目标 | 工作量 | 验收标准 | 状态 |
|-----|------|--------|---------|------|
| **P0（基础可用链路）** | 实现未读数实时推送，替换轮询 | 3-5 天 | SignalR 连接建立，未读数实时更新，Dock 不再轮询 | ✅ 已完成 |
| **P1（可用通知系统）** | 通知数据模型 + 列表 API + 已读状态 + 业务集成 | 5-7 天 | 评论/点赞触发通知，通知列表可查看，已读状态同步 | ✅ 已完成 (2026-01-10) |
| **P2（体验增强）** | 多端同步、通知聚合、偏好设置、容量治理 | 3-5 天 | 多端已读同步，通知聚合展示，用户可配置通知偏好 | ⏸️ 待规划 |
...
**当前实现**：
- `NotificationCacheService` 通过 `ICaching` 维护 `notification:unread_count:{userId}`，缓存 TTL 为 `30` 分钟。
- `Redis.Enable=true` 时缓存写入 Redis；禁用 Redis 时写入内存缓存。
- 增量 / 减量更新当前为普通读-改-写缓存操作；数据库中的 `UserNotification` 关系表仍是未读状态真值来源。
- `MarkAllAsRead` / 全部已读必须在数据库更新成功后同步把未读数缓存收敛为 `0`，并向用户组推送 `UnreadCountChanged(0)` 与 `AllNotificationsRead`；多端前端收到事件后应同时清理本地未读标记和 Dock 角标，避免缓存残留导致刷新后未读数回弹。

**后续增强**：
- 多实例或高并发场景再把未读数升级为 Redis 原子计数或更明确的数据库真值 + 缓存失效策略。
- 定时一致性校验任务仍属于后续治理项，不是当前已完成能力。
...
- [x] 新通知已具备全局 `Toast` 预览能力，当前会通过共享 `Toast` 展示标题 / 内容摘要。
- [x] 共享 `Toast` 已支持自动消失与剩余时间进度条，临时提示的时效反馈已有统一实现。
- [ ] 是否为通知 `Toast` 增加点击跳转或显式关闭按钮？
- [ ] 是否需要通知声音提示？
