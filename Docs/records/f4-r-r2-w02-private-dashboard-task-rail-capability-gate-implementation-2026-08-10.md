...
本批没有新增聚合端点、第二套状态源或视觉占位实现，也没有修改 Pencil、启动服务或执行浏览器 smoke。后续仍保持 R2，只在 Pencil 可用并获得当前任务授权后进入局部代表设计。
...
1. 保持 Pencil 不占用，直到用户释放设计资源并在当前任务明确授权。
2. 进入 `R2-W02` 局部代表设计时，只覆盖六页需要改变的摘要、主任务与辅助轨关系，不复制完整路由或既有 R1 壳层。
3. Mobile 继续固定主任务先于完整辅助轨；Pet 只允许最优先照料动作随当前状态提前。
4. 代表设计确认后再进入正式视觉实现与 Gateway PC / mobile 成组验收；不提前推进 R3。
