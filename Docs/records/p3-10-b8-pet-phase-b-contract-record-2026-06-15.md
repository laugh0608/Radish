...
`P3-10-B8 Phase B` 的私域主链路当前具备继续进入发布候选前批次级回归的条件。首批代码已完成领取、命名、四类照顾动作、状态变化流水、`/me` 摘要入口和本地结构同步入口；本批追加状态洞察、冷却展示、照顾反馈和后端契约测试，未引入经济消耗、商城物品、社区任务奖励、经验反向加成、Console 配置 UI、首页组件或公开个人主页默认展示。
...
| 结构同步入口 | 当前以实体和 `Radish.DbMigrate` 为准；2026-06-22 后上线前不再维护该阶段历史发布脚本 |
...
- 发布候选前再补一次批次级回归，覆盖 `/pet` 未登录回流、领取、刷新稳定、四类照顾动作、每日次数 / 冷却、`/me` 摘要和结构同步口径。
