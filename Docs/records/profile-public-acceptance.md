...
- `/u/:id` 的移动阅读布局、分页、分享入口、只读边界或回跳链路改动后
...
2. 在窄屏下检查首屏，确认头像、昵称、显示名、加入时间、只读提示、“公开主页阅读说明”和统计块没有重叠、截断或横向撑破。
...
10. 从 `/leaderboard` 点击任一用户榜单项进入 `/u/:id`，再执行返回，确认会优先回到原榜单与来源上下文，而不是异常退回桌面或 forum 默认列表；此时返回按钮文案应显示为“返回榜单”。
11. 从 `/u/:id` 顶部点击“社区发现”，确认能稳定回到 `/discover`，且不会异常跳桌面或丢失公开壳层样式。
12. 点击 `/u/:id` 顶部“复制链接”，确认复制结果为公开 canonical 链接，不包含 `/desktop`、临时来源状态或工作台窗口参数；在嵌入浏览器或权限受限上下文下应显示成功反馈，不应误报复制失败。

## 5. 预期结果

- `/u/:id` 以公开内容壳层直达，不绕回桌面入口。
- 公开个人页在手机上保持“资料优先、内容次之、回跳清晰”的阅读节奏。
- 首屏会额外给出“公开主页阅读说明”，明确当前公开页主要承载基础资料、公开帖子 / 评论阅读，以及仍留在桌面工作台里的账号治理能力。
...
- 当公开个人页带来源进入时，返回按钮文案会按真实来源显示为“返回榜单”“返回论坛”等具体提示，而不是泛化成“返回上一入口”。
- 公开个人页顶部存在统一的“社区发现”轻入口，可把用户重新收口回 `/discover`。
- 公开个人页顶部存在显式复制链接入口，复制 `/u/:id` canonical 公共链接，分享动作不改变当前来源返回上下文。
