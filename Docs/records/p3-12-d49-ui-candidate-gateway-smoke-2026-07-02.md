...
据此，`P3-12-D` UI 候选前验证可以收口，下一顺位进入 `P3-12-E` 正式版发布候选准备。进入 `P3-12-E` 不等同于已经创建 tag、恢复 PR、发布部署或进入生产稳定运营。
...
- 高频自动化全量刷新会放大当前用户接口限流风险；后续 `P3-12-E` 做同类 smoke 时应按页面组间隔执行，或优先复用 Console 内部导航。
...
进入 `P3-12-E` 正式版发布候选准备：

- 整理发布候选范围、PR 前验证入口、剩余风险和不发布项。
- 再次按风险运行必要 baseline / identity / host runtime。
- 如恢复 `dev -> master` PR，再刷新 `master..dev` 范围与回滚说明。
- 不直接创建 tag，不进入 M15 测试 / 生产部署流程。
