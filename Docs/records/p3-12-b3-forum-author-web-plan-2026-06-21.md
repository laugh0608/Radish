...
>
> 状态：方案梳理完成，尚未进入首批代码实现
>
> 结论：`P3-12-B3` 应把论坛发帖、帖子编辑、问答回答、编辑历史和作者反馈迁到正式 Web 路由，不搬迁 WebOS 三栏工作台、窗口参数、Dock 或 `openApp` 语义。首批不需要新增后端 API，重点是正式 Web 路由契约、登录回流白名单、现有论坛写操作组件的登录路径解耦，以及 `clientSubmissionId` 的持续复用。
