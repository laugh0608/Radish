...
>
> 状态：首批代码与 B3 小阶段验收已完成，后续进入 `P3-12-B4` 文档作者态归属裁决
>
...
B3 小阶段验收已于 2026-06-22 补充执行并通过：

- `git diff --check`
- `npm run check:host-runtime -- --details`：Gateway / Api / Auth health 均为 `200`。
- Gateway PC `1920x1080` 视口覆盖 `/forum`、`/forum/compose`、公开详情 canonical、`/forum/post/:id?intent=answer`、`?intent=edit`、`?intent=history`。
- Gateway 移动 `390x844` CSS 视口覆盖 `/forum`、`/forum/compose`、公开详情 canonical、`?intent=edit`、`?intent=history`；当前 Playwright CLI 未设置 DPR，本结论不写作高 DPR 物理屏完整 smoke。
- 匿名 `/forum/compose`、`?intent=edit`、`?intent=history` 均进入统一登录页，并在 `sessionStorage` 保存正式 Web return path，例如 `/forum/compose` 与 `/forum/post/pst_...?intent=edit|history`；未回退 `/desktop`。
- 公开详情 canonical 保持 `/forum/post/pst_...`，页面标题、正文、轻回应、评论区、返回论坛和复制链接均可渲染；浏览器 console 收尾检查无 error / warning。

2026-06-22 已按用户要求使用 Browser 插件和种子账号补充已登录写入成功态：

- 使用种子账号 `admin / admin123456` 登录正式 Web，`/forum/post/pst_...?intent=edit` 回流到详情后显示作者态按钮。
- 新建临时问答帖 `pst_019eef3ec8207e1e97b083ab6374d4c3`，发帖成功后进入正式 Web 详情，页面出现 `回答问题`、`编辑帖子`、`查看编辑历史`。
- 作者编辑提交成功，详情页标题与正文刷新；`Post.EditCount=1`，`PostEditHistory` 落库并可在页面“查看编辑历史”中看到编辑前后标题 / 内容。
- 回答提交成功，详情页显示 `回答 1`，`PostQuestion.AnswerCount=1`，`PostAnswer` 落库并显示回答正文。
- `ContentSubmissionRecord` 确认 `ForumPostCreate`、`ForumPostEdit`、`ForumAnswerCreate` 均为 `Succeeded`，并分别使用 `forum-post:*`、`forum-post-edit:*`、`forum-answer:*` 的 `clientSubmissionId`。
- 结束时 Browser 插件读取 error / warning 日志为空。

阶段验收或准备合并前：

- B3 不再默认重复执行同一组真实页面 smoke；准备合并到 `master` 或发布候选前，再按批次级验证入口集中复核。
