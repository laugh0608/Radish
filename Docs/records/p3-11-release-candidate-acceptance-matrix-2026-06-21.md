...
| WOG-1 至 WOG-6 写操作治理 | 后端测试、迁移 SQL、Flutter 单商品购买幂等 | 建议随 baseline / Flutter 验证重跑 | 必须重跑，并审核迁移 SQL | 多实例并发、Redis 锁和完整资产风控仍后置 |
| 论坛内容发布可靠性 | 后端论坛定向测试、Web / Flutter `clientSubmissionId` 契约 | 建议随 baseline / Flutter 验证重跑 | 必须重跑 | 不覆盖 Flutter 子评论编辑和回答编辑 |
| 数据库迁移 | P3-10 范围含多份 `Deploy/sql` 与 `DbMigrate` 补丁 | PR 前审核差异 SQL 与 DbMigrate 自检结论 | 必须在目标库前演练 | SQLite 与 PostgreSQL 行为仍需发布前分开确认 |
