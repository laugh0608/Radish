...
# 开发模式（热重载）
# 默认开发地址：http://localhost:4000/docs
npm run docs:dev
...
# 开发模式
npm run docs:dev --prefix Docs/radish.docs
...
- **源文件**：位于 `Docs/radish.docs/docs` 目录（同时也是在线文档站的内容源）
- **构建输出**：`Docs/radish.docs/dist`（默认），由独立 docs 服务或任意静态服务器托管
...
A: 所有 Markdown 与图片均位于 `Docs/radish.docs/docs` 目录，同时根目录 `docs/` 提供跳转入口，方便在 GitHub 上浏览。
