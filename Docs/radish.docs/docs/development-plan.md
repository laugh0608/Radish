...
| M9 | 商城系统 | 商品管理、库存管理、购买链路、订单系统、权益发放 | 可浏览商品列表，萝卜币购买商品并扣减库存，订单记录可追溯；用户背包、权益管理；管理后台API | ✅ 已完成 (2026-01-13~14) |
...
>
> **当前主线焦点**（2026-03-07 复核，按仓库实际进度更新）：
> - ✅ **M12-P0「身份语义收敛与 Claim 架构治理」主体已完成**：运行时统一身份视图 `CurrentUser`、`ICurrentUserAccessor`、Claim 标准化层、控制器/Hub/中间件/权限处理器迁移、兼容层冻结与回归测试已落地。
> - 🔧 **身份专项保留收尾项**：仅剩 **Phase 4 协议输出收敛**（Auth 停止继续双写历史 Claim、`UserInfo` 读取统一常量化）与 **Phase 5 防回归资产落地**（扫描规则、白名单、脚本/CI 接入）。
> - 🚀 **M12 当前功能主线已切换为 P1「内容与文档体系重构」**：优先推进 Markdown 导入/导出、Wiki/文档 App 承接与 `radish.docs` 迁移方案收口；身份专项尾项作为并行工程治理任务继续收尾。
>
> **M11 查漏补缺阶段收官进展**（2026-03-01 更新）：
> - ✅ **萝卜坑应用核心功能100%完成**（2026-01-27）
> - ✅ **前端代码规范优化**（2026-01-29）：
>   - 统一使用 viewModelMapper 进行类型映射
>   - 移除 radish.console 重复的 viewModelMapper.ts
...
> - ✅ **论坛分类/标签能力增强**（2026-02-07）：
>   - 固定标签来源由前端 `env` 切换为后端数据库驱动（`GetFixedTags`）。
>   - Console 新增“标签管理”页面，支持增删改查、启停、排序、软删除与恢复。
>   - 新增固定标签种子：`社区新闻`、`社区活动`、`精华帖`、`碎碎念`、`公告`。
>   - DbMigrate 标签种子前自动同步 `Tag` 表结构，兼容旧 SQLite 库缺列场景。
> - ✅ **API 客户端职责收口**（2026-02-07）：
>   - `radish.client` 与 `radish.console` 统一使用 `@radish/http`，`@radish/ui` 专注 UI 组件职责。
> - ✅ **论坛编辑历史能力上线**（2026-02-10）：
>   - 新增帖子/评论编辑历史记录模型与查询 API（分页）。
>   - 支持按配置控制编辑次数上限、历史写入次数与历史保留上限。
>   - 支持管理员覆盖普通用户限制（按配置开关）。
>   - 前端新增帖子/评论“历史”查看弹窗。
>   - 详见 [论坛编辑历史专题](./features/forum-edit-history.md)
> - ✅ **论坛搜索与高亮重算收口**（2026-02-23）：
>   - 论坛搜索升级为独立搜索结果页，支持关键词、排序、时间范围与分页组合检索。
>   - 帖子卡片补充作者头像、近期互动头像与紧凑神评预览，提升信息密度。
>   - 神评/沙发实时重算统一入口覆盖新增评论、删除评论、点赞/取消点赞路径。
>   - 详见 [2026年2月第3周开发日志](./changelog/2026-02/week3.md)
> - ✅ **表情包系统与聊天室系统规划文档完成**（2026-02-24）：
>   - 表情包系统完成数据模型、Console 管理方案与前端 UI 规范三篇设计文档。
>   - 聊天室系统完成后端架构设计与前端方案设计两篇文档，明确频道模型、ChatHub 事件与性能策略。
>   - 详见 [表情包系统设计](./features/emoji-sticker-system.md)、[表情包 Console 方案](./features/emoji-sticker-console.md)、[表情包 UI 规范](./features/emoji-sticker-ui-spec.md)
>   - 详见 [聊天室系统设计](./features/chat-system.md)、[聊天室前端方案](./features/chat-frontend.md)
> - ✅ **表情包系统后端 Phase 1 首版落地**（2026-02-25）：
>   - 已完成 `StickerGroup/Sticker` 实体、`StickerService`、`StickerController` 基础链路。
>   - 已完成管理端接口：分组/表情 CRUD、`BatchAddStickers`、`BatchUpdateSort`、编码预检与清洗预览。
>   - 已完成控制器单元测试补充与联调文件 `Radish.Api.Tests/HttpTest/Radish.Api.Sticker.http`。
...
> - ✅ **表情包系统 Reaction 闭环完成**（2026-03-01）：
>   - `StickerPicker` reaction 模式与 `ReactionBar` 已完成帖子/评论场景集成。
>   - Reaction 批量查询与交互链路已打通，论坛回应功能形成完整闭环。
>   - 相关文档与周报已对齐，后续细节优化转入主题系统阶段统一处理。
> - ✅ **聊天室 P0 主链路落地并完成回归收口**（2026-03-03）：
...
>   - 个人主页已新增“关系链”页签，当前已区分“关注动态”主流与“推荐/热门/最新”分发子切换，并保留“我的粉丝”“我的关注”分页视图。
...
> - ✅ **控制器 Claim 解析收口（第二批）**（2026-03-06）：
>   - 已完成 `ClientController` 的手写 `sub/jti` 解析下线，统一改为 `IHttpContextUser.UserId`。
>   - 已同步 `ClientControllerTest` 构造注入方式，避免测试仍依赖 `HttpContext` 手工塞 claim。
>   - 编译验证通过：`dotnet build Radish.Api/Radish.Api.csproj -c Debug -m:1 /nr:false`（0 Error）。
...
> - 🎯 **M11 遗留项已统一转入 M12 社区功能冲刺阶段**（见下方 M12 规划）。
>
> **M6 完成情况**：
...
>   - P4 阶段：管理功能（后端 API 完整，前端待实现）
>   - 乐观锁库存控制（防止超卖）
>   - 权益时长叠加、消耗品效果框架
>   - 通知集成（购买成功推送）
>   - 种子数据（5个分类，11个初始商品）
> - ⏳ **待完善项**（非阻塞，放到最后优化）：
>   - 前端管理界面（Console应用，移至M10实施）
>   - 定时任务（订单超时、权益过期）
>   - 单元测试和集成测试
>   - 压力测试和性能优化
> - 详见 [商城系统实施计划](./guide/shop-roadmap.md)
>
> **M10 完成情况**（2026.01.22）：
> - ✅ **Console 后台管理系统完整实现**：
>   - 基础架构（React 19 + Vite + TypeScript + OIDC 认证）
>   - 用户管理（列表、详情、个人信息、头像上传）
>   - 商品管理（列表、创建/编辑、上下架、删除）
>   - 订单管理（列表、详情、重试失败订单）
>   - 角色管理（列表、权限展示）
>   - Dashboard 仪表盘（关键指标、快速操作、最近订单）
>   - 基础设施（组件库复用、API 封装、错误处理、加载状态）
> - 详见 [Console 实施计划](./guide/console-roadmap.md)
>
> **M12 社区功能冲刺阶段规划**（2026-03-01 启动）：
>
> - ✅ **P0：身份语义收敛与 Claim 架构治理（主体完成，进入收尾阶段）**
>   - 已完成运行时唯一身份视图 `CurrentUser`，并统一通过 `ICurrentUserAccessor` 提供当前用户上下文。
>   - 已完成统一 Claim 标准化层，兼容 `sub/jti`、`tenant_id/TenantId`、`role/ClaimTypes.Role` 等历史输入，并收口到标准化组件内部。
>   - 已完成 `IHttpContextUser` 兼容层收束，`GetClaimsIdentity()`、`GetClaimValueByType(string)`、`GetUserInfoFromToken(string)` 等原始逃逸接口已标记废弃，禁止新增依赖。
>   - 已完成协议边界与运行时边界分层：Auth 保留 Claim 签发与 destinations，Api 负责 JWT/OIDC 验证与统一标准化，业务代码只消费 `CurrentUser`。
>   - 已完成控制器、Hub、中间件、权限处理器、审计链路的迁移与基础回归验证。
>   - 🔧 **剩余尾项**：
>     - **Phase 4 协议输出收敛**：`Radish.Auth` 停止继续双写 `ClaimTypes.NameIdentifier`、`ClaimTypes.Role`、`TenantId` 等历史输出，`UserInfo` 内部读取统一走常量与标准化口径。
>     - **Phase 5 防回归资产**：整理仓库扫描规则与白名单，先形成可执行口径，后续在 M13 阶段接入脚本或 CI。
>   - **当前状态**：专项主体已完成，不再阻塞 M12 新阶段推进；后续按“低风险收尾任务”持续清理。
>
> - 🚀 **P0：社区主线能力（优先保证主流程可用）**
>   - 聊天室 MVP（频道、消息收发、历史消息、在线状态、基础未读）。
>   - 社区关系链（关注/取关、粉丝列表、用户主页动态流）。
>   - 内容治理闭环（举报、审核队列、禁言/封禁、操作记录）。
>   - 分发能力补齐（推荐/热门/最新三路流与基础权重策略）。
>
> **社区主线收口快照**（2026-03-07）：
> - ✅ **已完成可验收项**
>   - 聊天室 P0：频道列表、历史消息、发送、撤回、基础未读与实时同步。
>   - 关系链：关注/取关、关注状态、粉丝/关注列表、关注动态流、个人主页入口。
>   - 内容治理：举报、审核队列、审核联动禁言/封禁、治理记录、发帖/评论拦截。
>   - 分发能力：`recommend/hot/newest` 三路流与基础权重配置化。
>   - 身份治理：运行时迁移主体完成，转入协议输出与防回归尾项收尾。
> - 🔧 **仍有缺口但可低风险补齐项**
>   - 关系链前端口径收口：明确区分“关注动态流”与“分发流”入口与文案。
>   - 联调/回归资产：补强 `.http` 验收顺序与轻量控制器回归用例。
>   - 文档对齐：统一周报、路线图与前端现状描述，避免将关系链动态与分发流混写。
> - ⏳ **留待后续阶段**
...
>
> - 📚 **P1：内容与文档体系重构（当前功能主线）**
>   - Markdown 文档导入与导出能力（前后端接口 + 前端交互）。
>   - 详见 [Wiki / Markdown 文档体系设计与实施方案](/guide/wiki-markdown-system)。
>   - 下线 `radish.docs` 独立站点，迁移到 Wiki/文档 App，并与 Markdown 导入/导出链路配套完成文档展示。
>   - 表情包管理遗留项完善：✅ Console 单个表情新增已支持直接上传文件并自动回填（URL 手填仅作兜底，2026-03-02 已完成）。
>   - ✅ 夜间遗留项已于 2026-03-04 完成收口（2026-03-03 记录项）：
>     - `FileCleanup` 已补充 `Sticker.AttachmentId` 引用保护，避免误清理仍被表情引用的附件；
>     - 表情包新增分组/表情“前端不展示”已修复（前端鉴权参数补齐 + 仓储层查询条件兼容修复）；
>     - 附件与回收站目录策略已统一到“解决方案根目录 `DataBases/`”。
>   - **当前执行顺序**：
>     1. 先完成 Markdown 导入/导出接口与最小前端交互；
>     2. 再完成 Wiki/文档 App 的展示承接；
>     3. 最后收口 `radish.docs` 迁移、导航与历史文档映射策略。
>
> - 🧩 **本周独立节点：补全租户相关逻辑**（2026-03-04 新增）
>   - 当前阶段运行口径统一为公共租户：默认用户（含种子用户与新注册用户）`TenantId = 0`。
...
>   - UI 风格规范落地（中国风视觉方向与配色规范形成可复用设计基线）。
>   - i18n 语言切换完善与规范加强（文案抽取、键值规范、缺失检测）。
>
> - 🔐 **P1：权限治理能力**
>   - 权限管理落地（角色-权限-资源映射）。
>   - 菜单/按钮级权限管理（前端可见性与后端授权双重校验）。
>
> - 🧪 **P2：新功能孵化与扩展**
>   - 第一个“文字修仙”App 立项（独立应用形态，类似 Console 的独立边界管理）。
>   - 抽奖、投票、问答三类社区互动功能规划与首版实现。
>   - 邮件通知系统（模板、触发规则、发送队列与失败重试）。
>   - 开源软件清单声明组件（优先接入 Welcome App）。
>
> **里程碑调整说明**（2026-03-01）：
> - ✅ M11（查漏补缺阶段）已收官。
> - 🎯 启动 M12（社区功能冲刺）作为当前主线里程碑。
> - 原 M12（可观测性与测试）顺延为 M13。
> - 原 M13（部署与运维）顺延为 M14。
> - 原 M14（Gateway & BFF 策略）顺延为 M15（暂缓）。
...
- Checklist:
  ...
  - [ ] 在 `Radish.DbMigrate` 中补全以下种子：默认租户、管理员用户、基础角色、系统权限与 API 模块、内置 OIDC 客户端（Frontend/radish.client/radish-scalar/Frontend/radish.console/radish-shop）
  - [ ] 确保种子流程幂等，可重复执行
...
> **详细方案**：[商城系统设计方案](/guide/shop-system) | [实施计划](/guide/shop-roadmap)

#### 核心功能
- ✅ **P1 阶段：核心购买流程**（商品 CRUD、订单创建、萝卜币支付、库存扣减）
- ✅ **P2 阶段：权益系统**（用户背包、权益发放、消耗品使用）
- ✅ **P3 阶段：前端页面**（商城首页、订单列表、背包管理）
- ✅ **P4 阶段：管理功能**（后端 API 完整，前端移至 M10 实施）

#### 技术亮点
- 乐观锁库存控制（Version 字段 + 重试机制，防止超卖）
- 权益时长叠加（同类型权益自动延长有效期）
- 消耗品效果框架（改名卡/经验卡/萝卜币红包已实现）
- 通知集成（购买成功自动推送）
- 分层架构严格遵守（Service 层通过 Repository 访问数据）

#### 待完善项（非阻塞，放到最后）
- 前端管理界面（商品管理、订单管理，移至 M10）
- 定时任务（订单超时取消、权益过期检查）
- 单元测试和集成测试
- 压力测试和性能优化
...
- ✅ 消耗品可以正确使用并产生效果
- ✅ 订单记录完整可追溯
- ✅ 管理员 API 完整（商品管理、订单管理、统计报表）

---

### 第 10 周｜后台管理Console（M10）

> **目标**：实现 radish.console 后台管理应用，提供商城管理、用户管理、系统配置等功能

#### 10.1 Console 应用基础架构

**认证与路由**：
- 使用 radish-console 客户端 ID（已在 M3 预置）
- OIDC 认证流程（与 radish.client 相同）
- 路由配置（仅 Admin/System 角色可访问）

**UI 框架选择**：
- Ant Design 或 Material-UI（待定）
- 响应式布局
- 暗色主题支持

**API 客户端**：
- 基于 axios 的统一 API 客户端
- 请求拦截器（Token、错误处理）
- TypeScript 类型定义
...
#### 验收标准
- Console 可正常登录（OIDC 认证）
- 仅 Admin/System 角色可访问 Console
- 商品管理功能完整可用（CRUD、上下架）
- 订单管理功能完整可用（列表、详情、重试）
- 统计报表数据准确
- 用户管理功能可用（列表、详情、操作）
...
### 第 11 周｜社区功能冲刺（M12）
- 社区主线：聊天室 MVP、关系链、内容治理、分发能力四条主线并行推进。
- 文档体系：落地 Markdown 导入/导出，并将 `radish.docs` 迁入 Wiki/文档 App。
- 表情包遗留项：Console 单个表情新增支持直接上传（上传后自动回填地址，URL 手填仅作兜底）。
- 体验规范：完成主题切换基础能力，落实中国风视觉与配色规范。
- 国际化：完善 i18n 切换与键值规范治理，收敛多语言维护流程。
- 权限体系：实现角色权限、菜单权限、按钮级权限控制闭环。
- 新功能孵化：文字修仙 App 立项，抽奖/投票/问答功能进入实现队列。
- 通知与合规：邮件通知系统规划落地；Welcome App 接入开源软件清单声明组件。
- 验收：
  - 社区核心链路可演示（聊天、关系链、治理、分发至少各一条端到端流程）。
  - Wiki/文档 App 可完成 Markdown 导入、展示、导出闭环。
  - Console 单个表情新增可直接上传并成功创建，无需手工填写 URL。
  - 权限、主题、i18n 在主应用具备可验证的最小可用能力。

### 第 12 周｜可观测性、测试与性能（M13）
...
- 原生扩展（Rust）：已迁移到根目录 `Lib/radish.lib/`（替代原 `Radish.Core/radish-lib`）；与其他前端项目保持一致的目录结构。建议补齐 CI/构建脚本：`cargo build --release` + 拷贝产物到 `Radish.Api/bin/<Configuration>/net10.0/`（Windows: `radish_lib.dll`；Linux: `libradish_lib.so`；macOS: `libradish_lib.dylib`）。
- 验收：
