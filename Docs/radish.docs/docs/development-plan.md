...
| **M11** | **查漏补缺阶段** | **萝卜坑应用完整实现、代码规范优化、UI美化与功能完善、WebOS美化、论坛增强** | **萝卜坑应用100%完成；代码规范符合度提升；WebOS桌面UI美化完成；论坛功能丰富** | **🔄 进行中** |
| M12 | 可观测性与测试 | 日志/Tracing、性能调优、自动化测试、CI 脚本 | `dotnet test` + `npm run test` 通过；Serilog/O11y 配置完成；性能基线达到 P95≤200ms | ⏳ 计划中 |
| M13 | 部署与运维 | Dockerfile/Compose、自监控、变更文档 | `docker compose up --build` 一键拉起 PostgreSQL + API + Auth + 前端；文档覆盖常见排障 | ⏳ 计划中 |
| M14（暂缓） | Gateway & BFF 策略 | Gateway PoC（Ocelot 路由 + 认证透传）与聚合 API 设计，仅在多服务/多入口明确后启动 | 在 `DevelopmentLog` 记录启动审批 + 回滚预案；保证现有 API 维持直连能力 | ⏸️ 暂缓 |
...
> - 🎯 **待完成**：WebOS UI美化、论坛功能丰富（表情包/聊天室进入实现阶段）、主题配置开发、i18n国际化完善、Console/Client JS 包体积优化
>
> **M6 完成情况**：
...
>   - ✅ 编译通过，类型检查通过
>   - ⏳ Phase 5增强功能待实现（用户搜索、通知系统、安全日志、UI优化、数据导出）
> - 🔄 **代码规范优化**：
>   - 后端规范符合度95%（3个待修复问题：匿名对象返回、AutoMapper配置、Db直接操作）
>   - 前端规范符合度94%（7个待修复问题：console使用、any类型）
> - 🎯 **WebOS UI美化**：
>   - 桌面壁纸系统、窗口动画效果、图标设计优化、任务栏美化
> - 🎯 **论坛功能丰富**：
>   - 帖子编辑器增强、图片/附件上传、点赞/收藏功能
>   - ✅ 表情包系统与聊天室系统设计文档已完成，后续进入接口与前端实现拆分阶段
> - 🎯 **UI整体风格设计**：
>   - 设计系统建立、色彩规范统一、组件样式优化、响应式适配
> - 🎯 **主题配置开发**：
>   - 亮色/暗色主题、主题切换动画、个性化配置、主题预设
> - 🎯 **i18n国际化完善**：
>   - 多语言支持、语言切换、文本提取工具、翻译管理
> - 🎯 **前端包体积优化**（新增）：
>   - 按路由与重型组件做懒加载收敛，减少首屏主包体积。
>   - 评估 `manualChunks` 分包策略，拆分编辑器/图标/图表等大依赖。
>   - 建立构建体积观测基线（gzip + chunk 明细），纳入每周回归检查。
...
>
> **里程碑调整说明**（2026.01.22）：
> - ✅ M10 Console 后台管理系统已于 2026-01-22 完成
> - 🎯 M11 调整为查漏补缺阶段，重点关注 UI 美化和用户体验提升
> - 原 M11（可观测性与测试）顺延为 M12
> - 原 M12（部署与运维）顺延为 M13
> - 原 M13（Gateway & BFF）顺延为 M14
