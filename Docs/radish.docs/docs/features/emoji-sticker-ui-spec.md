...
- 选择 sticker → 插入 `![name](sticker://packCode/stickerCode)` 语法，同时调用 `RecordUse`
...
- `GetGroups` 接口在**应用初始化时**（App 挂载后）一次性加载，存入全局状态（Context 或轻量状态管理）
...
### 5.3 MarkdownRenderer stickerGroups 数据传递

帖子列表页（`PostList`）渲染多个帖子卡片时，`stickerGroups` 数据从**页面级**获取一次后，通过 **React Context** 向下传递，而非每个帖子卡片各自传 prop：

```typescript
// ForumApp 层
const { data: stickerGroups } = useStickerGroups(); // 全局缓存
<StickerGroupContext.Provider value={stickerGroups}>
  <PostList />
</StickerGroupContext.Provider>

// MarkdownRenderer 内部
const stickerGroups = useContext(StickerGroupContext);
```
...
│   ├── StickerPicker/
│   │   ├── StickerPicker.tsx           # 主容器（Popover + Tab 切换）
│   │   ├── EmojiTab.tsx                # Unicode emoji 分类网格
│   │   ├── StickerTab.tsx              # 官方表情包分组网格
│   │   ├── StickerPicker.module.css
│   │   └── index.ts
│   └── ReactionBar/
│       ├── ReactionBar.tsx             # 气泡条 + 折叠逻辑
│       ├── ReactionPickerPopover.tsx   # 快速选择器（常用6个+更多）
│       ├── ReactionBar.module.css
│       └── index.ts
│
└── radish.client/src/
    ├── apps/forum/components/
    │   ├── PostDetail.tsx              # Phase 2：集成 ReactionBar
    │   └── CommentNode.tsx             # Phase 2：集成 ReactionBar
    ├── api/
    │   ├── sticker.ts                  # getStickerGroups, recordStickerUse
    │   └── reaction.ts                 # getReactionSummary, batchGetSummary, toggleReaction
    └── contexts/
