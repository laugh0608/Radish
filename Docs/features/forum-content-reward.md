...
1. 首批目标仅为 `Post / Comment`；F4-O A-D 后回答已具备独立定位、治理、正式 Web 和成组验收，但这不自动改变已关闭的 F4-N 产品与资产边界。`PostAnswer` 赞赏仍需独立候选审计和明确批准。
...
`PostAnswer` 继续留在 F4-N 范围之外。F4-O A-D 已完成回答级 PublicId / 通知定位、内容治理、生命周期和 PC / mobile 正式路径，消除了对象权威的前置缺口；但它没有授权新增资产动作，也没有定义回答赞赏在删除、采纳变化、治理失效后的公开记录与审计语义。

若未来重新评审 `PostAnswer` 赞赏，必须另立专题，至少重新固定：

- 可赞赏状态、作者自赏、屏蔽 / 账号状态，以及删除、采纳变化、治理限制与恢复后的公开记录和资产审计语义；
- 通知与目标失效、幂等键和现有唯一约束扩展，以及 PC / mobile 入口、无障碍、Gateway 代表矩阵和清理范围。
