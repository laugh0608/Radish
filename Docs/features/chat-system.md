...
- 2026-07-19 F4-F-A 已完成轻量阅读回执现状审计与权威设计：复用 `LastReadMessageId` 单调游标，Public / Announcement 不对外展示，普通 Private 仅发送者查看人数与读者分页，Accepted Direct 展示对端已读边界；旧在线头像草案失效。
...
public class ChatHub : Hub
{
    ...
    public async Task MarkChannelAsRead(long channelId)
    {
        ...
        // F4-F-B/C 将迁移为 REST 精确游标写入；Hub 只广播回执失效提示。
    }
}
```
