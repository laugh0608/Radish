...
当前没有 `src/pages/Channels` 频道管理页面，也没有 `Channel/Admin/*` CRUD 接口，旧草案中的文件与路由不得作为现状引用。Console 只在用户举报形成治理案件后，通过 `Moderation` 查看安全快照和受权当前状态；限制动作由 `ContentModerationChatRecall` 可靠任务按消息 ID 执行，不授予频道历史浏览能力。System / Admin 身份也不自动穿透成员 ACL。频道创建、分类、成员管理或普通消息浏览若要开放，必须另立权限、审计和页面专题。
