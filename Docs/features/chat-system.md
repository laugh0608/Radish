...
- 2026-07-19 F4-F-A / B 已完成轻量阅读回执设计与服务端权威契约：复用 `LastReadMessageId` 原子单调游标，Public / Announcement 不对外展示，普通 Private 仅发送者查看人数与读者分页，Accepted Direct 展示对端已读边界；旧在线头像草案失效，下一批由共用页面迁移到精确 REST 游标。
...
public class ChatHub : Hub
{
    ...
    {
        // F4-F-B 过渡兼容入口：与 REST 共用原子单调 Repository，但仍以服务端最新消息为目标
        ...
        // F4-F-C 页面迁移后删除本命令；Hub 最终只广播回执失效提示。
    }
}
```
...
| `MarkChannelAsRead` | `channelId` | F4-F-B 过渡兼容入口；F4-F-C 改用 REST 精确游标后删除 |
...
- F4-F-B 已完成 migration、原子 Repository、Service / REST、稳定错误契约与无个人数据 Hub 失效提示；F4-F-C 负责共用页面、Store 与旧 Hub 写命令退役。
