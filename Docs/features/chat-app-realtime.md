...
| `ReadReceiptsChanged` | `{ channelId }` | F4-F-C 去抖重读当前自己所发消息的 HTTP 权威回执摘要 |
...
> F4-F-B 已提供 REST `ChannelReadState/Advance(channelId, readThroughMessageId)`、发送者受限摘要 / 读者分页和 `ReadReceiptsChanged`；当前页面仍临时调用 Hub `MarkChannelAsRead(channelId)`。F4-F-C 迁移消费者并验证后删除该 Hub 写命令。
...
- F4-F-B 已完成上述服务端与 `@radish/http` 契约；F4-F-C 才新增 Store、事件消费和页面表现。
