...
当前公开 forum 以阅读为主，同时承接已经完成权威专题的受控互动：轻回应、根评论、问答回答生命周期、Post / Comment 赞赏、帖子收藏、内容举报，以及帖子 / 评论作者编辑与版本历史。`/forum/compose` 承接发帖；详情页通过受控 intent 恢复登录回流、作者现场或目标定位。

公开 forum 仍不复刻 WebOS 三栏工作台、Dock、窗口参数或 `openApp` 语义。共享组件虽然拥有更多可选能力，但当前 Public 详情不开放新建评论子回复、帖子 / 评论点赞与 reaction 写入、帖子 / 评论删除、投票提交、抽奖执行或 Console 治理动作；既有子回复继续可读、可定位。
...
- 公开 forum 与桌面 forum 可以复用 API、局部组件和展示语义；公开详情只开放本页已列出的轻回应、根评论、回答生命周期、收藏、Post / Comment 赞赏、举报及作者编辑 / 历史能力。其余写入型交互不应因为共享组件已支持就从桌面工作台直接搬入公开页。
...
- 评论树必须表达父子层级：父评论保留完整评论项和神评状态，既有子回复缩进并保留沙发与引用上下文；当前 Public 路由不据此开放新建子回复、点赞或 reaction 写入。神评 / 沙发不能降级为详情页侧栏元字段。

## 验证要点

- 匿名打开 `/forum/post/:publicId` 可阅读正文、轻回应墙和评论树。
- 匿名触发轻回应、根评论、回答、收藏或赞赏会进入 OIDC 登录，并在回调后回到同一公开详情的对应现场；收藏和赞赏仍等待用户再次确认。
- 登录后可在公开详情发布轻回应和根评论；问题帖可通过受控 `answer` intent 进入回答现场，帖子 / 评论 / 回答作者按各自专题取得编辑、历史或恢复能力，问题作者可采纳或撤销回答。
- 工作区 intent 链接支持新标签打开和复制链接；普通点击仍应保留当前页来源返回、轻回应 / 评论区聚焦和作者态现场恢复。
- 当前 Public 详情不开放新建评论子回复、帖子 / 评论点赞与 reaction 写入、投票提交、抽奖执行、帖子 / 评论删除和通知中心；内容举报只进入既有举报链路，不在 Public 页面执行治理。
