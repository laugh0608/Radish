...
- 上传接口只创建未绑定 Chat 附件，默认私有；消息发送成功后由聊天业务服务把附件绑定到 `ChannelMessage`，通用附件接口不能自行改挂 `BusinessId`。
- 已绑定附件必须同时满足租户一致、消息属于目标频道、附件引用与消息关系有效，以及当前用户拥有该频道访问权。公开频道按现有频道策略授权；一对一私聊只允许有效参与者读取。
