...
2. **Reaction 回应**：对帖子、评论进行多种表情反应；聊天消息复用表情目录与 UI，但使用 Chat 库专属持久状态
...
## 当前实现进度（2026-07-16，对齐代码）

### 已完成（后端）

- `StickerGroup` / `Sticker` 实体、DTO、Vo 与 AutoMapper 映射已落地
- `StickerController` 已完成前台与管理端基础接口：
  - `GetGroups` / `GetGroupDetail` / `RecordUse`
  - 分组 CRUD、单个表情 CRUD
  - `CheckGroupCode` / `CheckStickerCode` / `NormalizeCode`
  - `GetGroupStickers` / `GetAdminGroups`
  - `BatchAddStickers` / `BatchUpdateSort`
- `StickerService` 已接入缓存失效、批量新增、批量排序与 best-effort 缩略图生成（基于 `IFileStorage + IImageProcessor`）
- 已补控制器单元测试与 HTTP 联调示例（`Radish.Api.Tests/HttpTest/Radish.Api.Sticker.http`）
- `Reaction` 实体、服务与 `ReactionController` 已落地，控制器已启用 `[ApiErrorContract]`

### 已完成（Console 第一阶段）

- `radish.console` 已新增表情包管理菜单入口与路由
...
- 已接入 `ReactionBar`、`useReactions` 及帖子/评论汇总与 Toggle 链路

### 独立专题与后续阶段

- F4-D 聊天消息 Reaction 已完成 A-D 批并关闭：复用表情目录、`StickerPicker` 与 `ReactionBar`，数据使用 Chat 库专属 `ChatMessageReaction / ChatMessageReactionOperation`，不扩展 Main 库通用 `Reaction`
- Premium 表情商店与用户拥有关系仍属于 Phase 3
- Reaction 既有状态机与验收基线见 [Reaction Phase 2 设计冻结稿](./emoji-sticker-reaction-phase2.md)
...
| 聊天室 Reaction | F4-D（已完成） | 复用表情目录与共享 UI，Chat 库专属状态、operation 和 revision |
...
public class StickerGroup : RootEntityTKey<long>, ITenantEntity, IDeleteFilter
{
    ...
    /// <summary>分组封面附件 Id（显示时再解析封面 URL）</summary>
...
public class Sticker : RootEntityTKey<long>, IDeleteFilter
{
    ...
    /// <summary>关联附件 ID FK → Attachment（Sticker 资源真值）</summary>
...
/// 仅服务 Main 库中的 Post / Comment；ChatMessage 使用 Chat 库专属实体。
...
public class Reaction : RootEntityTKey<long>, IDeleteFilter
{
    ...
    /// <summary>sticker 类型时存储贴图附件快照 Id</summary>
...
public class ReactionSummaryVo
{
    ...
    public string? VoThumbnailUrl { get; set; }  // sticker 时有值，运行时由 StickerAttachmentId 派生
}
```
...
- Console 使用共享图片白名单：JPEG、PNG、GIF、BMP、WebP、ICO；当前图片上限统一为 5MB。
- 分组封面由 Ant `Upload.customRequest` 调用 `uploadAttachmentImage(..., { businessType: 'StickerCover' })`；表情图片使用 `Sticker`。
- `StickerService` 在附件缺少缩略图时尝试生成 96 × 96 JPEG。该步骤为 best-effort，失败记录告警但不阻断实体写入。
- `Sticker` 只保存 `AttachmentId`；原图和缩略图 URL 在返回 Vo 时由附件路径解析。
...
> 上述 `imageUrl` / `thumbnailUrl` 都是 `StickerVo` 的运行时展示字段，实际业务真值仍然是 `AttachmentId`。
...
| GET | `/api/v1/Sticker/GetAdminGroups` | System/Admin | 获取管理端分组列表（含禁用分组） |
...
| PUT | `/api/v1/Sticker/BatchUpdateSort` | System/Admin | 批量更新表情排序 |
| PUT | `/api/v1/Sticker/UpdateSticker/{id}` | System/Admin | 编辑表情（自动失效缓存） |
| DELETE | `/api/v1/Sticker/DeleteSticker/{id}` | System/Admin | 软删除表情（自动失效缓存） |
| GET | `/api/v1/Sticker/CheckGroupCode?code=xxx` | System/Admin | 分组编码预检 |
| GET | `/api/v1/Sticker/CheckStickerCode?groupId=&code=xxx` | System/Admin | 组内表情编码预检 |
| GET | `/api/v1/Sticker/NormalizeCode?filename=xxx` | System/Admin | 文件名编码清洗预览 |
| GET | `/api/v1/Sticker/GetGroupStickers/{groupId}` | System/Admin | 管理端获取分组内表情（含禁用） |
...
- `ReactionController` 已标注 `[ApiErrorContract]`，失败响应会同步为真实 HTTP 状态并补 `traceId`。控制器已写入
  `code` 时过滤器不会再推导 `messageKey`，因此现有 `InvalidArgument`、`StickerNotAvailable`、
  `ReactionLimitExceeded` 等响应仍可能缺少本地化键。
- `StickerController`（包括 `RecordUse` 与管理端写入）当前未标注该契约；领域失败通常是 HTTP 200 +
  `MessageModel.statusCode`，认证/权限拦截则仍是真实 HTTP 401/403。
- `MessageModel` 已支持 `messageKey / messageArguments / traceId`，但 Sticker 控制器通常未填充。F3-C9 需将
  Sticker 领域迁移到真实 HTTP、稳定 `code` 与结构化本地化参数，并把 Console `stickerApi.ts` 的自行解析/
  普通 `Error` 迁移到共享 `ParsedApiResponse / ApiResponseError`。
...
为避免前后端联调歧义，以下示例统一基于后端 `MessageModel<T>`（前端经 `@radish/http` 解析为 `ParsedApiResponse<T>`）。
...
interface GetStickerGroupsResponse {
  ...
  voCoverAttachmentId?: string;
  voCoverImageUrl?: string;
  voGroupType: 1 | 2; // 1=Official, 2=Premium
  voSort: number;
  voStickers: Array<{
    voId: string;
  ...
  "responseData": [
    {
      ...
      "voCode": "radish_girl",
      "voName": "萝卜娘",
      "voCoverAttachmentId": "501",
      "voCoverImageUrl": "/_assets/attachments/501",
      "voGroupType": 1,
      "voSort": 10,
      "voStickers": [
        {
          ...
          "voCode": "happy",
          "voName": "开心",
          "voImageUrl": "/_assets/attachments/9001",
          "voThumbnailUrl": "/_assets/attachments/9001/thumbnail",
          "voIsAnimated": true,
          "voAllowInline": true,
          "voUseCount": 321
        }
...
Reaction 领域失败以真实 HTTP 状态返回，并保留 `MessageModel` 壳；前端按 `code / statusCode / traceId`
处理，不匹配 `messageInfo` 文案。现有业务 `code` 未必伴随 `messageKey`，需按上文 F3-C9 边界治理。
...
### F4-D：聊天室 Reaction（已完成）

聊天消息与 Main 库帖子 / 评论的内容归属、ACL、撤回、租户和事务边界不同，因此不扩展通用 `ReactionController`。当前实现：

1. `ChannelMessageReactionController` 与 Chat Service / Repository 负责读取和 desired-state 写入。
2. `ChatMessageReaction / ChatMessageReactionOperation` 与消息同处 Chat 库，维护幂等、revision、容量和保留策略。
3. `ChatHub.MessageReactionsChanged` 广播权威完整状态，前端按 revision 合并。
4. 聊天气泡复用共享 `ReactionBar`、`StickerPicker` 和表情目录，不复制 UI 与素材协议。

长期契约、权限矩阵和验证边界以 [聊天消息 Reaction 设计](./chat-message-reaction-design.md) 为准。
...
| **F4-D** 聊天 Reaction | Chat 专属状态、幂等 operation、revision、Hub 与正式 Web / WebOS | Chat 库 2 个实体、专属 Service / Repository / Controller | 复用 ReactionBar；新增 Chat Hook、Store 与 Hub 合并 |
| **Phase 3** 扩展 | 表情商店、UseCount 排行 | 按需，不影响既有 Forum / Chat 权威边界 | 按需 |

## Phase 1 任务拆分与验收标准（补充）

| 子任务 | 主要内容 | 验收标准 |
|------|------|------|
| 后端实体与接口 | `StickerGroup/Sticker` + 管理端 API + `RecordUse` | `Radish.Api.Tests/HttpTest/Radish.Api.Sticker.http` 可完整走通 CRUD/批量上传/记录使用；软删除与唯一性规则生效 |
...
| 实体 | `Radish.Model/Sticker.cs` | 贴图实体（`AttachmentId` / `AllowInline` / `IsAnimated`） |
...
| 示例请求 | `Radish.Api.Tests/HttpTest/Radish.Api.Sticker.http` | 已新增表情包接口联调样例 |
...
7. 复核 `Reaction`（`GetSummary/BatchGetSummary/Toggle`）的真实 HTTP 与并发状态
...
- `BatchAddStickers`：成功、部分冲突、附件校验/实体构建失败在插入前返回、数据库异常事务回滚
- 缩略图：best-effort 失败只记录告警，不产生 `voFailedItems` 或整批失败
...
- 告警：关注附件构建失败分布与缩略图生成告警，二者不得混为同一失败语义
