...
- `StickerService` 已接入缓存失效、批量新增、批量排序、缩略图生成（基于 `IFileStorage + IImageProcessor`）
...
> 缩略图生成由后端在 `StickerService` 中基于 `AttachmentId` 对应的原始文件处理，
> 结果写回附件缩略图路径；`Sticker` 表本身只保存 `AttachmentId`，不再保存 URL。
