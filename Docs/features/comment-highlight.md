...
神评/沙发功能是 Radish 论坛的特色互动功能，通过定时统计 + 事件触发重算，自动标记和展示最受欢迎的评论，提升用户参与度和内容质量。
...
- Web 工作台详情页订阅全部评论事件和 typing；公开详情页订阅评论 / 点赞 / 高亮变化事件，并支持登录后两级回帖、帖子 / 回帖点赞与 reaction，但不展示 typing 输入提示，也不开放删除。论坛作者态的帖子编辑由正式 Web 详情受控 `intent=edit` 承接，不属于评论高亮专题。匿名公开页仍保持阅读边界。
...
**文件位置**：`Frontend/radish.client/src/types/forum.ts`
...
**文件位置**：`Frontend/radish.client/src/api/forum.ts`
...
**文件位置**：`Frontend/radish.client/src/apps/forum/ForumApp.tsx`
...
**文件位置**：`Frontend/radish.client/src/apps/forum/components/CommentTree.tsx`
...
**文件位置**：`Frontend/radish.client/src/apps/forum/components/CommentNode.tsx`
...
- 删除评论后会立即触发一次实时重算，减少“旧高亮”短暂残留窗口
