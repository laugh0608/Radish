...
> **状态**：F4-L-A 候选复核与权威设计已完成；下一顺位进入 F4-L-B 服务端权威契约
>
...
3. `Wiki / Document` 新上传统一为私有；Wiki 附件是否可读由实时文档状态、引用范围和现有 Wiki ACL 派生，不再长期双写 `Attachment.IsPublic`。
...
9. 旧 Wiki / Document 附件通过显式 migration 转为私有并回填引用；跨租户、错误类型或无法安全归属的冲突只报告并阻断，不猜测归属。
...
- `AttachmentService` 当前仅把 Chat 新上传设为私有；Wiki / Document 默认公开。
...
- Wiki / Document 新上传从产生时即为私有，不出现“先公开、保存后再收口”的时间窗。
...
`Attachment.IsPublic=false` 是 Wiki / Document 附件的存储安全基线，不是“当前不可公开”的第二份业务结论。Public Wiki 附件仍由 `WikiAttachmentReference + WikiDocument` 动态授权匿名读取。
...
- 只允许引用有效、未删除、同租户的 `Wiki / Document` 附件。
...
- `BusinessType=Wiki / Document` 新上传立即写 `IsPublic=false`。
...
| 未绑定 Wiki / Document | 拒绝 | 仅上传者 | 仅上传者 | 仅上传者 | 拒绝 | 不自动穿透 |
...
- `IAttachmentService`
  ...
  - Wiki / Document 调用 `IWikiAttachmentAccessService`；
  - 其他类型保持既有边界。
...
5. 为安全匹配的现有附件回填引用。
6. 把所有 `BusinessType=Wiki / Document` 附件设为 `IsPublic=false`。
7. 执行严格 verify。

doctor / verify 至少检查：

- 表、列、索引和 checksum；
- Wiki / Document 附件仍公开的数量；
- 现有附件引用缺少权威关系的数量；
