...
> `2026-06-14` 起，普通浏览器新增 `/messages` 纯 Web 复访入口。它通过虚拟窗口上下文复用 `ChatApp`，支持 `channelId/messageId` 定位和公开个人页返回“消息”；WebOS `/desktop?app=chat&channelId=...&messageId=...` 仍作为历史工作台深链保留。
...
- 已发送消息支持 `sending / failed / sent` 状态展示，失败后可直接重试或撤销。
- `/messages` 不改变聊天主组件职责，也不新增聊天协议；它只负责登录恢复、URL 参数规范化、启动 / 停止 `ChatHub`、把通知消息定位交给 `ChatApp`，以及把成员点击转为 `/u/:id` 公开个人页来源返回。
