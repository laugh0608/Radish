...
- `Radish.Api.Tests/HttpTest/Radish.Api.Forum.Poll.http`：已补“投票视图列表”请求，待本机服务启动后执行
...
- 摘要：
  - 当前仅完成自动化与专题回归入口补齐
  - 待本机服务启动后串联发帖、列表投票视图筛选、详情投票与重复投票拦截

#### 结论

- 当前改动已达到“可继续联调”的工程状态，可作为 `P3-ext` 的首个最小扩展继续推进
