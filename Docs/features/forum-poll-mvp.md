...
3. 以登录用户提交一次投票，确认首次投票成功，刷新详情后已投状态与统计结果可回显。
4. 再次提交同一投票，确认返回“重复投票”类失败语义，而不是静默成功。
5. 切换到未登录态访问帖子详情，确认可以查看投票结果但不能提交投票。
6. 使用一个已过期或即将过期的投票样例验证截止态，确认截止后提交会被稳定拦截。

#### 预期结果

- 发帖附带投票、详情展示、首次投票、重复投票拦截、截止态拦截主链路可完成。
