...
> 日期：2026-07-30；2026-08-04 更新（Asia/Shanghai）
>
> 状态：A / B、v26.7.3 破坏性基线补充批、C-0 与 C-1A 已完成；C-1B 的 `R1-P01 / 社区发现 / PC 1440` 已确认，下一步独立设计 mobile 与公开读模型
>
...
`R1-P01` 随后形成新的 Radish 项目配色裁决：`guofeng` 品牌目标由旧胭脂调整为低饱和灰玉 `#5d6c57`，悬停为 `#6e736d`，品牌柔底为 `#e2e6de`，常规操作仍使用墨蓝 `#435c74`。这是基于 `/discover` 代表页与长期视觉方向作出的项目决策，不是对上游默认值的隐式继承；当前 Client 运行时旧胭脂值将在下一实现批显式更新。
...
`2026-08-03` 的 `R1-P01` 评审证明原 Public 映射过度依赖 `22 / 23` 的帖子流语法，导致首页即使增加辅助卡片，视觉重心仍几乎全部落在帖子上。`2026-08-04` 已按 `13 / 16 / 18 / 27` 完成参考转译、公开能力审计、低保真比较和多轮视觉重校准。最终 `R1-P01 / 社区发现 / PC 1440` 不复刻参考产品的信息架构，只吸收现代无衬线排版、克制圆角阴影、小面积状态色和自然的数据反馈，并围绕 Radish 顶部公共导航、结构化问题、混合社区信息与社区洞察重组；灰玉、墨蓝与国风暖白构成家族色，“社区正在发生”并置公共频道、成员公开动态、神评、帖子和问答。该 PC 已获确认并成为 Public 内容流视觉基准；公共频道摘要、公开短动态和跨帖神评集合仍需正式读模型，mobile 仍需按主任务独立设计，详见 [R1-P01 公开社区发现结构研究](/records/f4-r-r1-p01-public-discover-structure-study-2026-08-04)。`22 / 23` 继续只负责帖子内部表达。

### 9.2 设计源

当前唯一活动设计源为 `Docs/frontend/design-sources/radish-web-family-ui-v1.pen`。文件只保留已确认的 `R1-P01 / 社区发现 / PC 1440`、`8` 个必要组件母版和主题变量；旧“社区脉搏”PC / mobile、四栏直接复刻、双区原创、固定侧栏紧凑稿及其他失败研究均已删除，由 Git 历史留存。mobile 尚未形成正式代表画板，下一轮在同一文件内按移动任务独立设计。后续 R1 / R2 继续在该文件中新增或替换必要代表设计。`v1` 在普通设计迭代中持续演进并由 Git 留存，只有不兼容的结构性升级才新建 `v2`。
...
- C-1B 已建立单一版本化活动设计源与共享组件 / 主题基座；`R1-P01 / 社区发现 / PC 1440` 已获确认并成为 Public 内容流视觉基准，活动文件只保留该 PC、`8` 个必要组件母版和主题变量，失败稿由 Git 历史留存。下一步独立设计 mobile，并为混合公共信息流补统一公开读模型；PC / mobile 共同确认后再实现命中的共享主题色、组件和 `/discover`。
