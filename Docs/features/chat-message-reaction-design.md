...
> **状态**：F4-D-A / B 设计与服务端权威契约已完成；下一批为 F4-D-C Pencil 与正式 Web
>
> **复核日期**：2026-07-19
>
...
- Hub 广播带 revision 的完整聚合快照；客户端只接受更高 revision，避免并发和乱序事件把新状态覆盖成旧状态。
...
### F4-D-C：Pencil 与正式 Web

- 先更新 PC / mobile Pencil，再实现消息气泡 Reaction、picker、Store revision 合并和重连追平。
- 完成中英文、键盘、焦点、加载、错误、长聚合、四主题 token 和 WebOS 复用。

### F4-D-D：成组验收

- 使用至少两个普通账号覆盖公开、公告、私有 / 互关私聊、待处理、拒绝、阻断、撤回和权限变化。
- 执行 `zh / en × PC / mobile`、多标签、重连、并发、幂等回放和 WebOS 矩阵。
- 清理临时 Reaction、operation、账号、消息、通知、凭据和备份，检查六库完整性。
