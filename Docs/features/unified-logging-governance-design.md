...
> 状态：项目所有者已于 2026-09-19 确认方案；作为重构基线；L1 契约及采集安全 / 故障可见性子项已实施，L2 入口及 SQL / AOP / 事务 / DbMigrate、seed / migration / Auth seed、Outbox / Rust、Hangfire / 清理任务、后台业务 Job、币 / 经验发放链及服务内清理子项已实施，生产切换门禁尚未关闭。
>
...
- L2 已完成 API / Auth / Gateway 配置加载后的共享引导与运行入口，以及 Node 静态服务适配，见[生成入口记录](../records/unified-logging-l2-producer-entry-2026-09-19.md)。`RadishLogging.Enabled` 默认 false，SQL / AOP / 事务、API 已处理异常及 DbMigrate 入口 / Runner / Doctor 已按[生成端批次](../records/unified-logging-l2-producer-governance-2026-09-23.md)推进；[seed / 具体 migration 与 Auth seed](../records/unified-logging-l2-seed-migration-2026-09-23.md)也已完成；[Outbox / Rust 调用边界](../records/unified-logging-l2-outbox-native-2026-09-23.md)已完成实现、.NET 回归与本机 Rust 返回码 / 安全输出验证；Hangfire / 清理任务、后台业务 Job、币 / 经验发放路径与服务内清理的后续进展统一见[事件契约第 10–13 节](./unified-logging-contract.md)。其余业务入口、异常安全栈帧和框架来源继续治理。
