...
> 状态：项目所有者已于 2026-09-19 确认方案；作为重构基线；L1 契约及采集安全 / 故障可见性子项已实施，L2 入口及 SQL / AOP / 事务 / DbMigrate、seed / migration / Auth seed、Outbox / Rust、Hangfire / 清理任务、后台业务 Job、币 / 经验发放链、服务内清理及币扣除 / 转账直接消费边界子项已实施，生产切换门禁尚未关闭。
>
...
下表是 2026-09-19 方案确认时的基线；已实施子项以[当前事件契约](./unified-logging-contract.md)及第 13 节实现进度为准，不作为当前代码行为描述。
...
Fluent Bit 的镜像版本、digest、目标架构、插件列表和许可证在 L1 验证后冻结，不使用浮动 `latest`。L1 已授权拉取并隔离验证 `5.1.2` 的 Linux arm64 候选；镜像身份与实验限制见[L1 记录](../records/unified-logging-l1-contract-and-transport-2026-09-19.md)，尚未冻结生产配置。
...
| SQL 诊断摘要 | 当前仅操作类别、参数数量与耗时；不输出语句结构、参数名或值 | 普通诊断关闭，独立慢链路保留安全摘要 |
| 慢操作 / 慢连接 | 独立开关，安全摘要 | 默认开启，与普通 SQL 开关无关 |
...
- 单事件首批 UTF-8 JSON 上限调整为 8 KiB（L1 已观察到 Docker 长行分片，原建议 32 KiB 不再直接采用），消息 2 KiB，属性 32 个、嵌套深度 4，栈帧最多 32；截断字段带标识，不截成无效 JSON。这些是初始预算，L1 用代表载荷校准。
...
- 使用有界 JSONL 流式读取。L1 已否决原建议的 200 条 / 2 MiB HTTP 硬上限：插件以 chunk 发送且 413 会丢弃该支路的 chunk。修订方向为 HTTP 有界流式信封与内部最多 200 条的处理批次分离；16 MiB / 32768 条仅为已通过代表样本的校准值，仍须最坏情况上界、真实事务、重放与拒收隔离验证。
...
- L2 已完成 API / Auth / Gateway 配置加载后的共享引导与运行入口，以及 Node 静态服务适配，见[生成入口记录](../records/unified-logging-l2-producer-entry-2026-09-19.md)。`RadishLogging.Enabled` 默认 false，SQL / AOP / 事务、API 已处理异常及 DbMigrate 入口 / Runner / Doctor 已按[生成端批次](../records/unified-logging-l2-producer-governance-2026-09-23.md)推进；[seed / 具体 migration 与 Auth seed](../records/unified-logging-l2-seed-migration-2026-09-23.md)也已完成；[Outbox / Rust 调用边界](../records/unified-logging-l2-outbox-native-2026-09-23.md)已完成实现、.NET 回归与本机 Rust 返回码 / 安全输出验证；Hangfire / 清理任务、后台业务 Job、币 / 经验发放路径、服务内清理及币扣除 / 转账直接消费边界的后续进展统一见[事件契约第 10–14 节](./unified-logging-contract.md)。其余业务入口、异常安全栈帧和框架来源继续治理。
