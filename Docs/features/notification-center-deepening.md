...
> **状态**：F4-B-A 现状审计与设计已完成，下一批进入 F4-B-B 数据、查询与服务端契约实现
>
> **复核日期**：2026-07-18
>
...
Radish 已具备通知持久化、列表、未读、已读、删除、SignalR 推送和若干业务生产者，但当前实现仍把通知类型、用户分类、跳转目标、缓存计数和实时送达混在一起。结果是生产者存在裸字符串和语义复用，客户端需要扫描标题与正文猜分类，偏好表没有消费者，聚合只有缓存去重而没有可见聚合，分表查询与未读缓存也缺少稳定的一致性契约。

本专题把通知中心定义为“社区事件复访与处理入口”，不是第二套私信箱。实施采用稳定通知定义、结构化目标、用户通知分组和权威摘要状态：业务事件先经可靠 Outbox 进入 Message 库，数据库事务形成事件、接收关系、分组和摘要；HTTP API 返回可本地化的权威列表，SignalR 只发送带版本号的变更提示。通知偏好、聚合、容量清理、跨端同步和失败恢复都围绕同一数据真相源实现。
...
`UserNotification` 继续表示“某个可靠通知事件属于某用户”，新增 `InboxGroupId`。唯一约束继续保持：
...
OccurrenceCount, UnreadOccurrenceCount
...
- 生产 Message 库可能存在本地样本未覆盖的非默认 delivery 字段或跨月重复键，需要 doctor 先行阻断。
