...
- **公开帖子详情首批承载范围已冻结**：公开详情页当前只承载正文、轻回应墙只读展示、评论阅读与复制 canonical 链接，不复刻桌面工作台交互，也不在本批次接入发帖、评论发布或完整通知中心。
...
- **我的轻回应回看已接入个人主页**：个人主页当前已新增“我的轻回应”页签，用户可以回看自己发出的轻回应，并直接跳回对应帖子详情，作为轻回应回流链路的首个最小闭环；回流参数优先使用 `VoPostPublicId`，旧 `VoPostId` 只作为字符串 fallback。
- **论坛通知最小回跳链路已补齐**：帖子点赞、帖子评论、评论回复、评论点赞等现有论坛通知当前会携带统一导航载荷，通知中心点击后可以稳定回到对应帖子详情；导航载荷优先消费 `postPublicId`，旧 `postId` 与 `commentId` 均保持字符串兼容；若同时携带 `commentId`，帖子详情页会在评论加载完成后自动定位到目标评论并给出一次性高亮提示。
