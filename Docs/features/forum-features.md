...
- **帖子详情头像补齐**：帖子详情页作者区已补作者头像，评论树节点也已补评论作者头像显示，详情页信息密度与身份识别更完整。
...
- **治理后台承载方式**：首版审核台已集成进 `radish.console` 的 `Moderation` 页面，不额外拆独立治理 App；当前接入对象为帖子与评论。
