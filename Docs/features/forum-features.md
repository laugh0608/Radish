...
- **公开帖子详情参与边界已更新**：公开详情页当前承载正文、轻回应墙、评论阅读、复制 canonical 链接，以及登录后发布轻回应和根评论；正式 Web 作者态由 `/forum/compose` 与详情受控 `intent=answer|edit|history` 承接，发帖 / 回答 / 编辑继续复用 `clientSubmissionId`。WebOS 三栏工作台、Dock、窗口参数、评论回复直达、点赞、投票提交、删除、治理或完整通知中心仍不进入公开详情主流程。
...
- **评论回复路线治理已立项收口**：论坛评论继续保持“父评论 + 子评论”两层结构，但新回复关系不再只靠正文中的 `@用户名` 表示；当前改为补充“回复目标评论 ID + 摘要锚点”，用于表达“回复哪条评论”与支持点击回跳定位。
...
- **神评预览主数据化**：帖子列表与公开移动列表当前改为优先消费帖子主数据自带的神评预览字段，不再把帖子卡片是否显示神评完全绑死在二次补查结果上；补查链路仅作为兜底与兼容保留。
...
- **个人公开页首批已落到公开壳层**：`/u/:id` 当前可直接进入个人公开页，首批承载公开资料、公开统计、公开帖子与公开评论阅读；公开 forum 列表、帖子详情与评论作者入口也已开始回落到该直达路由。
...
- **神评预览稳定性增强**：帖子列表当前已补“帖子主数据直出神评预览 + 二次补查失败降级”双保险，降低桌面端与移动端首屏神评偶发不展示的问题。
- **回应面板交互修复**：评论区 `ReactionBar` 快捷面板改为浮层展示，避免撑开评论卡片；Forum 场景统一为浅色主题。
- **轻回应墙 Phase 1 基础链路已落地**：帖子详情页当前已稳定为“正文 -> 轻回应墙 -> 评论区”的三段式结构；轻回应使用独立模型与接口，支持纯文本和系统默认 Unicode emoji，不支持贴纸、表情包、附件、Markdown 与 `@提及`。当前主线重心已从“基础实现”切到“治理闭环、公开详情轻参与和回流契约稳定性”。
- **我的轻回应回看已接入个人主页**：个人主页当前已新增“我的轻回应”页签，用户可以回看自己发出的轻回应，并直接跳回对应帖子详情，作为轻回应回流链路的轻量入口；回流参数优先使用 `VoPostPublicId`，旧 `VoPostId` 只作为字符串 fallback。
- **论坛通知回跳链路已补齐**：帖子点赞、帖子评论、评论回复、评论点赞等现有论坛通知当前会携带统一导航载荷，通知中心点击后可以稳定回到对应帖子详情；导航载荷优先消费 `postPublicId`，旧 `postId` 与 `commentId` 均保持字符串兼容；若同时携带 `commentId`，帖子详情页会在评论加载完成后自动定位到目标评论并给出一次性高亮提示。
- **轻回应专属通知回流已落地**：他人在你的帖子下发布轻回应时，当前会给帖子作者发送一条论坛通知；通知中心点击后可直接跳回对应帖子详情。
...
- **治理后台真实回看已补齐**：`radish.console` 的 `Moderation` 页面当前已支持帖子评论、轻回应与聊天室消息的真实回看和回跳定位；审核队列与治理动作日志不再只展示摘要文本。
- **治理证据口径已稳定**：举报目标摘要当前在创建举报时即固化到 `ContentReport`，审核台并列展示“创建时快照”和“当前状态”；若帖子评论、轻回应或聊天消息后续被编辑、删除、撤回或下线，历史治理记录不会再随目标变化而漂移。
...
- **内容治理**：已完成举报提交、审核队列、审核联动禁言/封禁、治理记录查询，以及举报目标创建时快照固化；发帖与评论入口已接入发布权限拦截。
...
- **治理回看语义**：审核队列与治理动作日志当前优先展示举报创建时快照，并并列呈现目标当前状态；其中帖子评论、轻回应与聊天室消息已支持真实回看与回跳定位，目标失效时则保留降级展示，不再让历史证据随内容变更而漂移。
...
2. **评论互动增强**:
   ...
   - ✅ 评论区 `@提及` 用户搜索下拉框（已完成）
   - ✅ 评论区粘贴图片上传（已完成）
   - 🚧 帖子正文下轻回应墙 Phase 1（基础链路已落地，当前正在补联调收口、公开详情轻参与与回流契约稳定性，详见 [论坛轻回应墙 Phase 1 设计](./forum-quick-reaction-wall.md)）
