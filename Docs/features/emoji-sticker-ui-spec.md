...
- ✅ `StickerPicker` reaction 模式已接入 `ReactionBar`，支持帖子与评论统一回应入口
- ✅ Forum 回应快捷面板改为浮层，不占据评论卡片布局高度
- ✅ Forum 回应场景统一浅色主题（外层快捷面板 + 内层 StickerPicker）
...
- 已登录：点击 `+` 展示 **ReactionPickerPopover**，再次点击或点击外部收起
...
当前实现补充（2026-03-01）：
- Forum 的 Reaction 场景已显式传入 `theme="light"`，优先保证视觉一致性。
- 暗色主题切换保留在后续主题系统统一落地时处理。
