...
`R1-P01` 当前尚未通过独立业务页面审核。首轮稿把真实 `newest` 数据误概括为热帖主流；第二轮虽分开分类、标签和结构化状态，并增加分类、社区动态与相关内容区，但帖子仍占据绝大多数视觉篇幅，全宽分类浏览笨重，整体也没有形成参考 `13 / 16 / 18 / 27` 的异构工作台节奏。两轮稿只由 Git 留档，均不得作为实现依据。

## 当前实现事实

当前 `/discover` 代码今天没有修改，实际只复用以下公开数据：
...
`getTopCategories`、`getFixedTags`、额外问答 / 投票筛选以及更多社区实体只是第二轮设计稿的候选来源，尚未进入当前页面代码，也没有因为画板出现而自动成为已批准范围。下一轮必须先审计所有可公开读取的实体、API 和路由，再决定哪些模块可以直接组合，哪些只适合导航，哪些确实需要新的公开读模型。
...
- 下一版 `PostCard(publicCompact)` 仍需明确区分这三类语义，但具体位置、换行和主次关系应随新构图复核，不在页面结构尚未通过时提前固定。
...
第二轮稿采用的“全宽分类浏览 + 最新讨论主流 + 右侧社区动态 / 热门主题 / 相关内容”已经被否决；下一轮不得只把更多小卡片继续堆在帖子主流周围。

D61 后的首屏实现要求：

- PC 首个主要视区需要体现社区的多实体、多状态和多尺度关系；候选布局至少并置三类经代码审计确认的社区实体，帖子不得继续占据绝大多数视觉面积。
- mobile 重新根据获选 PC 构图提炼单列任务顺序，不把 PC 侧栏机械堆到正文下方，也不先于 PC 结构审核制作完整高保真稿。
...
跨实体优先级尚待下一轮能力审计和低保真比较：

1. 当前帖子仍只按现有 `newest` 排序，不在客户端伪造热度或推荐顺序。
2. 当前文档和商品各只读取一项、榜单只有入口；这些事实不能直接推导它们在新首页中的最终模块尺寸。
3. 问答、投票、作者、圈子、分类和固定栏目必须先确认公开摘要数据、权限和失败边界，再决定是内容模块、活动模块、关系模块还是纯导航。
...
- `R1-P01` 的低保真构图与最终 PC / mobile 代表画板已经通过业务和视觉审核；当前两轮稿不得直接实现。
