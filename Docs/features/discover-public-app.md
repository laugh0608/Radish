...
`R1-P01` 已按当前代码完成代表设计：PC 使用“论坛热帖主流 + 相关内容 rail”，mobile 将搜索与筛选前置后改为单列顺序；关键状态复用 `WebStateSlot`，复制链接结果留在动作附近。该裁决不恢复旧稿中的混合推荐流，也不新增 feed API、权限或状态机。

## 内容流范围

当前页面复用现有公开数据，不新增推荐读模型：

- 论坛热帖：`getPostList(..., 'newest')` 读取公开帖子，页面展示前 `5` 条 `PostCard(publicCompact)`；卡片优先使用 `Post.PublicId` 路径，并输出可新标签打开的真实 `href`。
- 热门标签：`getHotTags` 读取最多 `6` 个标签，作为主列表后的紧凑标签 rail，不改变帖子排序。
...
3. 紧凑起读区：展示“发现社区动态”标题、短说明、进入论坛、搜索社区和复制链接；不扩成营销 hero。
4. 社区主区：PC 以论坛热帖为主流、相关文档 / 商品 / 榜单与参与提示为 rail；mobile 按“热帖 → 标签 → 相关内容 → 参与提示”改为单列。
5. 状态与反馈：论坛加载 / 错误 / 空态使用共享 `WebStateSlot`；复制链接成功或失败在原动作附近反馈，不用全页状态替代内容。
6. 边界说明：登录后轻互动、工作台治理、购买 / 订单等能力只在相关区域给出入口或说明，不放进公开首页主动作。

D61 后的首屏实现要求：

- PC 首屏使用产品头部 + 紧凑起读区 + 论坛热帖主流 + 右侧相关内容 rail；不再使用大型“社区发现 / 公开只读”验收型 hero、入口矩阵或混合推荐流。
...
2. 公开文档只作为相关内容首项与列表回流，不进入论坛主列表排序。
3. 公开商品只作为相关内容首项与商城回流，不在 `/discover` 内展示购买决策完整流程。
4. 榜单只作为相关入口和公开个人页线索，不在内容流里扩展成完整排行工作台。
...
- PC 下 `/discover` 能展示论坛热帖主流和相关内容 rail，mobile 下搜索 / 筛选前置且相关内容按单列顺序落位，不退回 WebOS Shell。
