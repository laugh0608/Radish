...
> 状态：`F4-G-A` 权威专题设计已完成，等待进入 `F4-G-B` 服务端数据与契约实现。
>
> 最后更新：2026-07-20
>
...
- Markdown 正文按 UTF-8 字节数限制，默认上限 `1 MiB`；上限和活跃草稿 / 协作者数量分别由 `Wiki.Authoring.MaxMarkdownUtf8Bytes / MaxActiveOwnedDrafts / MaxCollaboratorsPerDocument` 配置，不能只靠前端限制。
...
- 保留天数使用 `Wiki.Authoring.TerminalDraftRetentionDays` 配置，默认 `90`，不得散落硬编码。
