...
>
> 面向 Owner、Collaborator 与 Reviewer 的操作顺序见 [文档作者协作与审核使用说明](/guide/docs-author-collaboration)。
...
- 协作者与审核上下文只对所有者、Pending Invitee、已接受协作者和有治理查看权限的人员开放；Pending Invitee 为只读，不能保存或提交。
...
F4-G-B 使用显式 ledger migration `20260720_007_wiki_author_collaboration`，同时覆盖 SQLite 与 PostgreSQL：
...
- Author：`Frontend/radish.client/src/docs`、`apps/wiki`、`@radish/http/wiki-authoring-contract`、中英文资源和相关测试。
