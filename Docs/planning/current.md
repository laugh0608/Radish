...
1. **推进 P3-10-D Web 信息流 / UI 结构整理**
   - 首日公开页结构整理、首轮 Gateway PC / 移动运行态复核、四批同类入口语义治理和合并前验证准备已完成。
...
- `P3-10-D Web 信息流 / UI 结构整理`
  ...
  - P3-10-D 合并前验证准备已完成；下一步优先进入 PR / 合并判断，不再继续追加第五批链接语义扫尾。
...
- 第一顺位：进入 `P3-10-D` PR / 合并判断，基于 [P3-10-D 合并前验证记录](/records/p3-10-d-pre-merge-validation-record-2026-06-19) 整理变更范围、验证结论和剩余限制。
- 第二顺位：若合并前复查或真实 smoke 新命中阻断 / 清晰一致性问题，再定向回修并复跑对应验证；不再默认追加第五批链接语义扫尾。
- 第三顺位：P3-10-D 合并后，再按整体规划选择下一条产品增量主线；进入代码前仍明确不做推荐算法、联邦社交、完整 Flutter 承接、完整 WebOS 迁移、系统设置扩面、经济扩展、完整聊天、完整钱包或 P3-8-D 重复筛查。
