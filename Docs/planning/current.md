...
- 第 1 步：继续观察最小 `CI` 门禁稳定性，并把当前总回归记录作为后续判断基线
...
- 当前首版剩余重点已从“验证基线收口”进一步收束到最小 CI 门禁稳定性、总回归记录维护与最终发布判断
