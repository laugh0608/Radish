...
1. **`P3-5` 公开内容增长后续专题**
   - `P3-5-A` 已完成动态 sitemap、结构化数据和详情首包可见性评估
   - 下一批优先候选为 `P3-5-B 运行时结构化数据基线`，先在公开详情页补 JSON-LD，不改 API、Gateway、部署或 SSR 架构
   - 动态 sitemap 与详情首包 HTML 可见性继续作为单独方案评审项，不在未评估部署风险前直接启动
2. **第二阶段收口护栏**
...
- `P3-5-B` 运行时结构化数据基线可作为下一批小实现候选，范围限定在 forum detail、docs detail、shop detail 和公开个人页 JSON-LD 注入 / 清理
- `P3-4` forum / docs / shop 留存回流矩阵首轮已完成阶段性收尾判断，后续只处理真实使用中新暴露的回流断点
- `P3-3` 只保留后续观察，不继续无边界深拆 `PublicForumDetail` 内部结构
- 动态 sitemap 需先单独确认 API + Gateway 路由或构建生成器方案，不直接塞进前端构建
...
- 下一事项：若继续推进公开内容增长，优先启动 `P3-5-B 运行时结构化数据基线`；该批只补公开详情 JSON-LD helper、接入和测试，不改 SSR / SSG、Gateway 或 API 路由
