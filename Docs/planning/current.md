...
- **最近结论**：
  ...
  - 当前不直接进入新阶段，也不直接启动 `P3-10-B8` 电子宠物代码；`P3-10-B7` WebOS 功能迁移图已完成首批纯 Web 私域复访判断，后续以 Gateway smoke 补验和维护回拉为主。
...
1. **收口 P3-10-B7 / C 公开入口治理与 WebOS 功能迁移图**
   ...
   - `/notifications`、`/me`、`/messages` 已承接进入 B8 前需要的通知、个人状态、成长 / 资产只读和会话 / 消息复访前置能力；B7 可转入 Gateway smoke 补验与维护收口，不把完整聊天、完整钱包或完整个人中心作为 B8 前置门槛。
...
- `P3-10-B8 Radish 电子宠物规划`
  - B8 不直接抢跑代码；下一步先做玩法边界、经济约束、复访入口和 Web-first 页面归属设计。
- `P3-10-B7 WebOS 功能迁移图收口`
  - `/notifications`、`/me`、`/messages` 三个纯 Web 私域复访入口已完成首批代码；Gateway 恢复后补 PC / 移动 DPR smoke，覆盖未登录回流、登录态列表 / 概览、通知到公开详情 / 消息、公开个人页返回来源和基础会话定位。
...
- 第一顺位：在 Gateway / client dev server 启动后补 `/me`、`/notifications`、`/messages` PC 与移动 DPR smoke，覆盖未登录回流、登录态概览 / 列表、通知到公开详情 / 消息定位、公开个人页返回“我的状态 / 消息”和圈子复访动作。
- 第二顺位：启动 `P3-10-B8 Radish 电子宠物规划`，先做玩法边界、经济系统约束、复访入口和 Web-first 页面归属，不直接实现完整游戏系统。
- 第三顺位：保留 `P3-10-B7 WebOS 功能迁移图收口` 维护入口，只在 Gateway smoke、发布候选回归或新真实缺口暴露时回拉。
