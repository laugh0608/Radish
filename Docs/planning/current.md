...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D36-D60` 已完成 UI 专题差距口径、四类设计源差距矩阵、边界裁决、阶段验收清单、Gateway PC / mobile 阶段验收、纠偏、页面缺口核对、Docs 作者态动作补漏、Public / Private 主路径代码侧复核、Console 深层管理动作复核、移动响应式抽样、Pencil UI 实现完成度复核、UI 实现证据收口、候选前启动前验证、Gateway 真实页面复核、D49 后口径纠偏、`radish.client` 私域 / 作者态移动任务流 UI 对齐首批、Public Web 移动公开任务流 UI 对齐、Public Web Pencil 首轮真实页面对齐收口、Private / Author Pencil 页面对齐收口、Console 响应式后台 UI 差距回拉、Console Gateway 成组真实页面复核、Console 深层交互 / 真实数据态复核、UI 专题候选前集中验收准备、UI 专题候选前验证执行和 Pencil 逐页 UI 与功能缺口复盘；D60 已纠正 D59 后续口径，并把后续工作拆为 Public、Private / Author、Console 三个小专题，每个专题分别承接设计稿 UI 差距、现有接口内功能缺口和专题内后置产品 / API 缺口。
  - `P3-12-D61` 已开始 Public Web Pencil 逐页实现，首批完成 `/discover` 对齐 `P01-P02` 的公开首页结构：产品级 `Radish / 公开入口` 头部、`发现` 命名、右侧登录 / 登录状态入口、紧凑公开脉搏、真实帖子讨论流和右侧公共内容亮点已落地；D61 仍未完成，后续继续覆盖论坛、文档、商城、榜单、公开主页和移动公开任务流。
2. **保持 P3-10 可恢复合并状态**
...
- 继续推进 `P3-12-D61 Public Web Pencil 逐页 UI 与功能缺口实现`：`/discover` 已完成 `P01-P02` 首批对齐，下一步继续按 `public-web-unified-experience.pen` 的 `P03-P14` 页面族补 UI 设计差距和现有 API 内可承接的前端功能状态；`P15-P16` 公开聊天室作为 Public 小专题内的后置产品 / API 缺口记录，不再放入全局功能缺口总专题。
