...
- `F1-A / F1-B` 已完成本地实现与静态回归；下一步进入 `F1-C 权益唯一选择与失效`，完成前不开放任何新商品类型。
...
1. 按[商城商品效力与权益履约专题](/features/shop-product-effect-entitlement-fulfillment)进入 `F1-C 权益唯一选择与失效`，先建立 `UserActiveBenefit` 唯一指针、实时 UTC 有效状态与迁移边界。
