...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
   ...
   - 第一顺位是 `P3-12-A` 功能资产盘点与迁移矩阵，按“正式版必需 / 发布前建议 / WebOS 保留 / 后置评审”分类 WebOS 与纯 Web 能力。
   - 第二顺位是根据矩阵推进首批 Web 正式版必需功能迁移，并同步清理直接冲突的 WebOS 残留入口。
...
- `P3-12 Web 完全化与 WebOS 收束`
  ...
  - 首批推进 `P3-12-A` 功能资产盘点与迁移矩阵；先判定哪些 WebOS / 纯 Web 能力属于正式版必需，再进入代码迁移。
  - 功能迁移只迁移正式版产品能力，不迁移 WebOS Dock、窗口系统、桌面背景、窗口几何记忆或桌面 app 外壳。
...
- 第一顺位：推进 `P3-12-A` 功能资产盘点与迁移矩阵，只读梳理 WebOS 能力、纯 Web 现状和正式版缺口。
- 第二顺位：按矩阵选择第一组正式版必需功能迁移，并同步列出需要清理的 WebOS 残留入口。
- 第三顺位：若进入 UI 设计与美化专题，必须先使用 Pencil 做设计稿，再更新设计 / 说明文档，最后进入实现。
