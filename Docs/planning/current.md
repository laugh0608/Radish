...
- **最近结论**：
  ...
  - `P3-10-B7` 已启动首轮 Chrome / Gateway PC 视图审计，确认公共 Web 入口与 WebOS 工作台分流、公共头部和详情参与路径的迁移决策点；首批只推进无运行时行为变化的公共壳层一致性清理。
...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - `P3-10-B7 Web UI 改造与去 WebOS 化迁移图` 已启动首轮审计和公共壳层一致性清理；下一步评审公共头部动作策略和论坛详情参与路径迁移。
...
- 第一顺位：继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，评审公共头部动作策略和论坛详情“参与讨论 / 轻回应”从 WebOS 回迁纯 Web 的条件。
