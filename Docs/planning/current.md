...
- **最近结论**：
  ...
  - `P3-10-B7` 已完成公共壳层一致性清理、论坛公开详情轻互动首批回迁、Gateway / Chrome PC + 移动视图补验和公共头部动作策略首批代码：公共页头部固定为公开发现、登录态圈子和历史工作台三层动作，`/desktop` 保留但不作为新增功能主动作；公开发现 / 论坛阅读边界文案已对齐轻回应与根评论回迁，本轮 Gateway 页面复核因本地服务未监听待补。
...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - `P3-10-B7 Web UI 改造与去 WebOS 化迁移图` 已完成公共壳层一致性清理、论坛详情轻互动首批回迁、Gateway PC / 移动视图补验和公共头部动作策略首批代码；下一步补 Gateway 页面复核，并继续选择同一组高价值入口推进。
...
- `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`
  ...
  - 已完成公共头部动作策略首批代码和公开边界文案同步，公共页头部统一露出社区发现、我的圈子和工作台入口；服务恢复后先补 Gateway PC / 移动视图复核，再继续选择能提升纯 Web 默认入口、公开详情阅读、个人主页 / 圈子复访和 Console 高频治理效率的一组入口推进代码。
...
- 第一顺位：继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，服务恢复后补公共头部 Gateway PC / 移动视图复核，再选择下一组能改善纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访或 Console 高频治理效率的代码入口。
