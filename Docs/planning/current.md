...
- 第一顺位：围绕 `P3-10-B4 / B5` 做双用户人工联调与小范围缺口修正，优先覆盖 Gateway 下的 Web 详情页和公开详情页评论实时链路。
- 第二顺位：若评论实时链路稳定，推进 `P3-10-B2` 个人圈子产品边界，先定 URL、对象类型、PublicId 和隐私边界，不直接实现完整关注流。
- 第三顺位：进入 `P3-10-B6` Token 不活跃过期方案前，先整理前端活跃记录、refresh 校验和退出登录 UX 的边界。
