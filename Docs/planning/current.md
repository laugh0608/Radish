...
12. **第十三批首个小闭环**
   - discover 只读分发上下文补强已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮只补强发现页来源 `/discover`、公开只读边界、forum / docs / profile 阅读入口上下文和摘要长文本窄屏显示，不新增 API、不扩完整通知中心、系统通知栏推送、发帖、完整评论提交、点赞、投票、编辑治理、购买、订单或背包能力
13. **第十四批首个小闭环**
   - discover 聚合摘要局部失败降级已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮只让 forum / docs / shop 三路摘要在原生聚合层独立降级，单一区块失败不再拖垮整页发现页；不新增 API、不扩 Flutter 专属 BFF、独立重试队列或跨端同步治理
14. **第十五批首个小闭环**
   - discover 刷新体验与局部 issue 清理复核已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮只补齐已有摘要刷新时的旧内容保留、刷新失败局部提示、成功刷新清理旧 issue，以及三路全降级时的 issue 展示；不新增 API、不扩 Flutter 专属 BFF、独立重试队列、下拉刷新体系或跨端同步治理
15. **第十六批首个小闭环**
   - forum / docs 主列表刷新体验一致性已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮只补齐 forum feed 与 docs 列表已有内容刷新时的旧列表保留、刷新失败局部提示与成功刷新清理旧提示；不改变分页、搜索、排序切换语义，不新增 API、不扩 Flutter 专属 BFF、下拉刷新体系或跨端同步治理
16. **第十七批首个小闭环**
   - profile 主资料刷新体验一致性已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮只补齐已有公开资料刷新时的旧内容保留、刷新失败局部提示与成功刷新清理旧提示；不改变加载更多局部失败逻辑，不新增 API、不扩 Flutter 专属 BFF、完整浏览历史、资料编辑、关注或治理能力
17. **第十八批文档收口**
   - Flutter 刷新体验收口复核 / 模拟器验证清单整理已完成，当前只补开发阶段人工检查口径，不扩业务功能
18. **第十九批首个小闭环**
   - profile 空态人称与文档口径复核已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮只调整我的主页公开帖子 / 评论为空时的人称文案，并修正文档条目错位；不新增 API、不扩资料编辑、关注、完整浏览历史或治理能力
19. **第二十批首个小闭环**
   - profile 公开主页长文本窄屏显示复核已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮只收紧公开资料、最近文档、最近阅读、公开帖子与公开评论预览长文本显示；不新增 API、不扩资料编辑、关注、完整浏览历史或治理能力
20. **第二十一批文档收口**
   - Android MVP RC 补验评估已完成，可作为第二十批后的 release 前置材料梳理收口
   - 本轮只区分已具备能力、开发阶段可先补项与必须等 testing Gateway / release APK / 真机安装后补齐的 release 前验收；不新增业务功能，不扩大 Android MVP RC 阻断范围
21. **第二十二批文档收口**
   - 第八至第二十批验证索引整理已完成，可作为 release 前批次级回归留痕的索引入口
   - 本轮只整理各批次已记录验证入口、记录来源与 release 前缺口；不新增业务功能，不替代真实 testing Gateway / release APK / 真机验收
22. **第二十三批文档收口**
   - 第十九批与第二十批命令级回归记录补洞已完成，可作为 release 前追溯的独立记录入口
   - 本轮补跑 profile 定向测试、壳层 smoke、Flutter analyzer 与 diff 格式检查；不新增业务功能，不替代真实 testing Gateway / release APK / 真机验收
...
- `Phase 2-3` 第三批后续判断
  ...
  - 第五批首个候选方向 `profile` 最近阅读轻量多条列表已完成收口；第六批 `forum detail` 轻回应发布后局部体验补强、第七批首个小闭环 `docs` 搜索体验增强、第八批首个小闭环 `profile` 最近文档轻量多条列表、第九批首个小闭环 `profile` 复访区块体验整理、第十批首个小闭环 `profile` 区块顺序与信息密度微调、第十一批首个小闭环 `docs` 详情只读上下文补强、第十二批首个小闭环 `forum detail` 来源上下文与错误态补强、第十三批首个小闭环 `discover` 只读分发上下文补强、第十四批首个小闭环 `discover` 聚合摘要局部失败降级、第十五批首个小闭环 `discover` 刷新体验与局部 issue 清理复核、第十六批首个小闭环 `forum / docs` 主列表刷新体验一致性、第十七批首个小闭环 `profile` 主资料刷新体验一致性、第十八批 `Flutter` 刷新体验收口复核 / 模拟器验证清单整理、第十九批 `profile` 空态人称与文档口径复核、第二十批 `profile` 公开主页长文本窄屏显示复核、第二十一批 `Android MVP RC` 补验评估、第二十二批第八至第二十批验证索引整理，以及第二十三批第十九 / 第二十批命令级回归记录补洞均已完成代码或文档口径收口；其中第八批已补自动化收口复核，真机安装仍留到正式 release 包发布前补；后续若继续做功能，仍应选择窄范围复访或只读体验补强
