...
1. 按 [F3 i18n 完成度治理实施说明](/frontend/i18n-completion-governance)进入 `F3-C`：继续按剩余正式 Web 业务域推进 client Docs 作者态、圈子、宠物、经验、萝卜坑与低频设置，以及 Console 商品、文档、权限、分类标签、表情、系统设置、经验和胡萝卜管理；不重新混入已收口的主题运行时。
...
1. 进入 `F3-C` 前先按剩余业务域复核专题边界和真实用户可见缺口，优先选择一组能形成完整页面与错误契约闭环的 client 或 Console 业务面。
