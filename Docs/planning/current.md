...
- **当前阶段**：`截至 2026-04-18，第一开发阶段已于 2026-04-06 通过 v26.3.2-release 完成真实发布收口；第二开发阶段现已正式启动。`Phase 2-1 社区深化第一批` 已完成论坛轻回应墙 Phase 1 与最小回流链路收口；`Phase 2-2 移动 Web 形态` 已完成 forum / docs / `u/:id` / leaderboard / shop / discover 公开内容壳层首批收口，并转入稳定维护。当前产品主线正式切到 `Phase 2-3 Flutter 客户端 MVP`，第一批只做 Android 起步的范围定义、真相源文档切换与仓库工程骨架，不继续沿公开壳层扩页面细节，也不回头把桌面工作台搬进原生端。`
- **复核日期**：`2026-04-18`
...
- `Phase 2-3 Flutter 客户端 MVP` 当前已正式进入主线，但第一批只承载真相源切换、范围定义与工程骨架，不同时进入完整业务页铺设

## 当前批次目标

1. **Flutter MVP 第一批范围定义**
   - 当前固定以 Android 起步，不同时铺 Windows / Linux
   - 当前只定义原生客户端壳层职责，不继续扩公开内容壳层页面细节
2. **Flutter 工程骨架落地**
   - 在 `Clients/radish.flutter` 建立最小工程结构
   - 补齐导航壳层、环境配置、认证存储占位与主题基线
3. **复用约束先于业务页**
   - Flutter 优先复用现有 API、认证契约与品牌语义
   - 当前不为 Flutter 单独设计新的 BFF 或复刻 WebOS 窗口交互
4. **范围明确收紧**
...
- `Phase 2-3` 第二批业务链路
  - 目标是补登录 / 会话恢复，以及 `discover / forum / docs / profile` 的首批真实页面接线
