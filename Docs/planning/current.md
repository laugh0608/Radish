...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - `P3-10-C` 首批公开内容卡片链接契约已完成代码推进和 Gateway PC / 移动视图复核；第二批圈子到公开详情 / 公开个人页来源返回已完成代码推进、定向测试、client 构建和 Gateway PC / 移动视图复核。
...
- 第一顺位：进入 `P3-10-C` 第三批，围绕 `/discover`、`/forum/post/:id`、`/u/usr_...`、`/circle` 做公开参与 / 登录恢复 / 来源返回矩阵复核；发现同类问题时按公开路由契约、登录回流和来源状态成组修复。
- 第二顺位：若公开参与与来源返回矩阵未暴露新增代码缺口，转入 Console 高频治理入口复核与改造，只覆盖 `/console/`、`/console/users`、内容治理和订单 / 用户排障等高频工作流，不回到低频页面逐页筛查。
- 第三顺位：继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图` 的小范围壳层一致性治理；涉及公共页主动作、公开详情交互边界或 Console 工作流行为变化时先说明页面策略。
