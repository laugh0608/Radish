...
- **最近结论**：
  ...
  - `P3-8-D` 切为当前主线：先从移动 Web 公开视图验收矩阵起步，再评估 Flutter 下一批高价值功能或 WebOS / PC 工作台主路径补强
...
1. **推进 P3-8-D 多端主路径缺口补齐与验收矩阵**
   ...
   - 再按验收结果评估 Flutter 下一批高价值功能或 WebOS / PC 工作台主路径补强，不默认继续做 Console 剩余页面微调
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 第二顺位：根据矩阵结论选择一个小闭环修复，或评估 Flutter 下一批高价值功能；不直接启动完整移动商城、完整通知中心或完整创作器
  - 第三顺位：WebOS / PC 工作台只处理成片流程断点，例如复访、通知、订单、背包、窗口参数和主路径回流，不扫低收益边角
