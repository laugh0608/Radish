...
- `F1-A 订单履约安全` 已完成本地实现与静态回归；下一步进入 `F1-B 消耗品幂等与使用流水`，完成前不开放任何新商品类型。
...
1. 按[商城商品效力与权益履约专题](/features/shop-product-effect-entitlement-fulfillment)进入 `F1-B 消耗品幂等与使用流水`，先固化改名卡、经验卡和萝卜币红包的原子使用、请求回放与审计契约。
