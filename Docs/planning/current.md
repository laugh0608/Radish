...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`F4-B-B 通知数据、查询与服务端契约：定义注册表、结构化目标、偏好、分组 / 摘要、迁移和 revision 实时提示`
- **产品下一顺位**：`F4-B-C 正式 Web 通知工作区；完成后进入 F4-B-D 双账号 PC / mobile 成组验收`
- **复核日期**：`2026-07-18`
- **当前判断**：
...
2. [F4-B 通知中心深化与通知治理](/features/notification-center-deepening) 的 A 批审计与设计已经完成；下一批固定为 F4-B-B，先完成定义注册表、结构化目标、偏好、分组 / 摘要、SQLite / PostgreSQL migration、专属仓储事务、新 HTTP API 和 revision SignalR 事件。
3. F4-B-B 通过后进入 F4-B-C 正式 Web 通知工作区，再执行 F4-B-D 双账号 `zh / en × PC / mobile` 成组验收；不在服务端契约未稳定时先堆页面兜底。
4. 聊天搜索、聊天 Reaction、消息置顶与移动系统通知继续作为后续候选，不与通知专题并行展开；主动生产证据采集保持最终收尾冻结。
