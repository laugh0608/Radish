...
1. **推进 P3-10-B10 系统设置治理**
   ...
   - 当前已补低风险 `Site.Branding.FaviconUrl` 覆盖值编辑、恢复默认、变更审计、修改原因 / 确认参数基础和历史查看入口；未注册历史 JSON 记录不作为运营设置展示。
...
- `P3-10-B10 系统设置治理`
  - 已推进设置定义注册表、默认值、覆盖值、风险等级、低风险编辑、恢复默认、修改原因 / 确认参数基础和变更审计历史；Console 不再把未注册 JSON 记录作为运营设置展示。
  - 当前仅开放 `Site.Branding.FaviconUrl` 低风险覆盖值；Medium 及以上设置仍等待逐项影响范围、二次确认策略和权限边界确认后再开放。
...
- 第一顺位：`P3-10-B10 系统设置治理` 补齐 Gateway 页面联调和下一批设置开放评审。重点看注册定义列表、默认值 / 当前值、覆盖状态、风险等级、生效方式、favicon 上传 / 恢复默认、修改原因和历史查看。
