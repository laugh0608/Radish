...
- **当前子阶段**：`F4 既有功能持续完成`
- **工程第一顺位**：`F4-R C-1B 已完成 R1-F01；R1-P01 两轮评审均未通过，下一步先重做社区发现结构校准`
- **产品下一顺位**：`复核参考图与现有公开能力，先产出多元社区首页的低保真布局备选；通过结构审核前不进入 /discover 实现`
- **复核日期**：`2026-08-03`
...
8. 已按当前代码重新审计 `/discover`，确认真实结构为按 `newest` 读取的公开帖子、热门标签、相关文档 / 商品 / 榜单 rail 与参与提示；不存在 `PublicDiscoverFeed.tsx`，专题文档中的旧混合流口径已修正。
9. 已在同一活动设计源形成 `R1-P01` PC、390px mobile、loading / error / empty 与分享反馈，并把原复合说明板拆为四个可独立定位的顶层画板。
10. R1-F01 与 R1-P01 初稿已改用灰玉品牌、墨蓝操作的共享契约；独立画板布局检查和节点截图通过，设计源索引已把 `R1-P01` 调整为“待审核”，未启动服务或执行真实 smoke。
11. R1-P01 第二轮评审稿分开了帖子分类、标签和结构化状态，并尝试增加分类、社区动态与热门主题，但最终仍因帖子占比过高、全宽分类浏览笨重、整体偏离参考图而未通过；该分区结构不得直接进入代码。
12. 日终代码反查确认 `/discover` 今天未修改，仍只加载 `newest` 帖子、热门标签、单个公开文档 / 商品和榜单入口；`publicSeoStatic.test.ts` 仍固定“单一社区内容主流”，下一轮若确认多元首页方向，必须在实现批同步重新裁决该测试，不能让旧断言反向绑架设计。

## 明日事项（2026-08-04）

1. 新会话先读取本页、[日终回顾](/records/f4-day-end-doc-review-2026-08-03)、[F4-R 专题](/features/family-ui-convergence-design)、[公开发现专题](/features/discover-public-app)和 [C-1A 代码事实审计](/frontend/f4-r-representative-page-audit)，不默认展开历史验收流水。
2. 重新查看 family-ui 参考 `13 / 16 / 18 / 27`，先写一张结构转译表：只提炼非对称主次、异构模块尺度、连续工作面、紧凑工具区和人员 / 任务 / 状态并置节奏；不复制后台左侧栏、外部品牌或业务字段。
3. 只读审计当前可公开读取的实体、API 与路由，区分“现成可组合”“只有入口没有摘要数据”“必须新增公开读模型”三类；不得再只从 `PostCard` 和论坛筛选反推整个社区首页。
4. 在同一 `radish-web-family-ui-v1.pen` 内先做轻量 `R1-P01 / Layout Studies`，用 `2~3` 个低保真 PC 构图比较主次与多元性，不新建 `.pen`、不先画完整 mobile、不继续精修当前失败稿。候选构图不得使用全宽分类浏览大块，且首个主要视区应并置至少三类经代码审计确认的社区实体，帖子不能继续占据绝大多数视觉面积。
5. 结构方向经用户选择后，再改写唯一 PC / mobile 代表画板与关键状态；保留已经成立的分类、`#标签`、结构化状态分层和禁止眉题 / 描述性副标题约束。
6. 只有 `R1-P01` 通过业务与视觉审核后，才规划 `/discover`、`publicSeoStatic.test.ts`、locale 文案和灰玉运行时 token 的成组实现；设计轨不提前扩张 `R1-P02`。
7. 默认不启动服务或执行真实 smoke；Web 优先、Flutter 条件式承接、WebOS 只兼容、Tauri 冻结与生产证据采集停止线不变。
