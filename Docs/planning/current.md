...
- `P3-10-B7 WebOS 功能迁移图收口`
  ...
  - 按“已有纯 Web 替代 / 应迁移到纯 Web / 更适合 Flutter / 保留在 `/desktop` / 放弃或重做”归类，选择首批高价值迁移，不把全量 WebOS 搬迁设为前置门禁。
  - 首批优先看通知 / 消息复访、个人状态 / 经验 / 资产这类登录后高价值能力；论坛、公开详情、发现、圈子、榜单、文档阅读和商城公开浏览已进入维护补漏。
...
- 第一顺位：推进 `P3-10-B7 WebOS 功能迁移图收口`，基于 `Frontend/radish.client/src/desktop/AppRegistry.tsx` 与现有纯 Web 入口产出迁移决策表，明确首批高价值迁移能力。
- 第二顺位：若迁移图确认首批应迁能力，先选择一组用户复访价值高、边界清晰的纯 Web 入口推进方案；涉及新路由、API 契约或运行时行为变化时先说明策略。
