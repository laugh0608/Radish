...
2. **选择 P3-10 下一批产品 / 治理增量**
   - 下一步优先处理边界清楚、能降低真实风险、不会扩成大专题的候选项。
   - 当前建议从 [密码传输与请求签名临时评审](/guide/password-transport-and-request-signature) 中回拉“前端敏感日志脱敏”：只处理 `paymentPassword / password / currentPassword / newPassword / confirmPassword` 等敏感字段日志，不做浏览器通用 `sign`、支付口令慢哈希升级、幂等体系、安全会话或资产风控扩面。
...
- `P3-10 后置治理候选：前端敏感日志脱敏`
  - 依据 [密码传输与请求签名临时评审](/guide/password-transport-and-request-signature)，优先排查和收敛前端日志中可能携带 `paymentPassword`、`password`、`currentPassword`、`newPassword`、`confirmPassword` 的路径。
  - 只做日志参数移除 / 脱敏和必要测试；不启动浏览器通用签名、字段级加密、支付口令慢哈希迁移、幂等体系或安全会话治理。
- `P3-10 阶段整理与自动化验证维护线`
...
- 第一顺位：跳过本轮 PR 创建步骤，保留 [P3-10-D PR 准备记录](/records/p3-10-d-web-feed-pr-prep-record-2026-06-19) 作为后续恢复合并动作的依据。
- 第二顺位：说明并推进“前端敏感日志脱敏”窄范围治理，先查 `radish.client` / `@radish/http` / `radish.console` 是否存在敏感字段对象日志，再按命中结果修复和补测试。
