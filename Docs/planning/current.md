...
- 先读取本页、[P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement)、[P3-12-D23 radish.console 角色权限外层语义迁移记录](/records/p3-12-d23-radish-console-role-permission-visual-migration-2026-06-30) 和 [Console 治理工作台设计端点](/frontend/console-governance-workbench-design)，确认内容治理 / 经验治理工作台下一批迁移口径。
- 第一顺位：评估并推进 `P02` 内容治理工作台外层语义收口，优先处理页头、指标、状态 chip、硬编码色和 inline 样式；保留举报审核、手动治理、治理日志、审核弹窗和 URL 状态契约。
- 第二顺位：继续 `P03` 经验治理工作台外层语义收口，保留用户查询、观察摘要、流水、复核、调经验、冻结 / 解冻和等级配置 API。
