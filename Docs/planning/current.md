...
1. 按 [F3 i18n 完成度治理实施说明](/frontend/i18n-completion-governance)继续 `F3-C`：推进 client 宠物、经验、萝卜坑、低频设置与公开承诺，以及 Console 权限、分类标签、表情、经验和胡萝卜管理；不重新混入已收口的主题运行时、Docs / 商品 / 圈子域或后置在线翻译平台。
...
7. 已提交 `F3-C2 / F3-C3` 为 `d0e72b2b`；随后完成 `F3-C4` client 圈子域的 locale 展示、英文数量规则、共享 UserFollow 结构化错误和后端稳定错误契约。
