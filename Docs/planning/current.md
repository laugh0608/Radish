...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - 下一步进入 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，先做页面层级、设计边界和验证矩阵审计，再选择同一组高价值入口推进代码。
...
- `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`
  - 先审 `/discover`、`/forum` 详情、`/u/usr_...`、`/circle` 和 Console 高频入口的页面层级、设计边界与验证矩阵。
  - 优先选择能提升纯 Web 默认入口、公开详情阅读、个人主页 / 圈子复访和 Console 高频治理效率的一组入口推进代码。
...
- 第一顺位：进入 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，先审 `/discover`、`/forum` 详情、`/u/usr_...`、`/circle` 和 Console 高频入口。
- 第二顺位：输出 B7 页面层级、设计边界、验证矩阵和首批代码入口；不把 WebOS `/desktop` 作为新增功能承载方向。
- 第三顺位：优先选择同一组高价值入口推进代码，保证纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访和 Console 高频治理体验一致。
