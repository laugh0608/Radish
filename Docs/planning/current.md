...
- **最近结论**：
  ...
  - `P3-7-A` 已启动 WebOS / PC 工作台复访与继续使用链路复核；首个高信号小闭环收口为订单通知回流：订单类通知携带合法业务 ID 时复用 / 打开商城订单详情，缺失合法 ID 时仍回落商城入口；同批次补齐订单详情回商品详情入口，并将购买成功后的去向收敛到订单详情，减少商城购买 / 通知 / 订单复访的上下文断点
