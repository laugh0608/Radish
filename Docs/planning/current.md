...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D28` 已完成 `radish.console` D14-D27 阶段静态收口：路由认证中、无 Console 权限和懒加载状态的旧 inline 样式已迁入 `routerComponents.css`，`routerComponents` 与 `SystemTools` 不再命中目标样式残留；阶段记录已归档深层表单、详情抽屉和批量上传弹窗的剩余静态风险，记录见 [P3-12-D28 radish.console 阶段静态收口记录](/records/p3-12-d28-radish-console-stage-static-closure-2026-06-30)。下一步若继续静态治理，优先进入深层表单 / 详情静态收口；若准备阶段验收，则在用户确认前后端已启动后执行 Gateway PC / mobile 复核。
2. **保持 P3-10 可恢复合并状态**
...
- 先读取本页、[P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement)、[P3-12-D28 radish.console 阶段静态收口记录](/records/p3-12-d28-radish-console-stage-static-closure-2026-06-30) 和 [Console 治理工作台设计端点](/frontend/console-governance-workbench-design)，确认下一步是继续深层表单 / 详情静态收口，还是准备阶段真实验收。
- 第一顺位：若继续静态治理，按 `ProductForm / CategoryForm / StickerForm / StickerGroupForm` 的上传预览、隐藏输入和宽度规则先拆清重复点，再决定是否抽共享样式或组件。
- 第二顺位：若准备阶段验收，先告知用户需要启动前后端并等待确认，再执行 Gateway PC / mobile 页面复核。
