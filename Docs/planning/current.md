...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D63` Console 已完成内容治理 / 经验治理、商业运营、文档治理、用户管理和权限矩阵页面族首批实现、成组静态收口和 Gateway 成组真实页面复核；`P3-12-D64-D66` 已完成 UI 候选前集中验收准备、启动无关候选前验证和运行态补验；随后 `P3-12-E1-E6` 已完成成熟度矩阵与差距审计、信任治理与公开承诺、Workbench 社区活动中心、通知行动队列、隐私安全边界、治理证据链、用户可恢复错误反馈、无业务写入 / 受控写入旅程验证和发布候选进入判断。当前未发现进入 `P3-12-F` 前必须先修复的阻断项，下一步进入 `P3-12-F 正式版发布候选整备`。
2. **保持 P3-10 可恢复合并状态**
...
- `P3-12-E6` 发布候选进入判断已完成：E1-E5 证据支持进入 `P3-12-F 正式版发布候选整备`，当前未发现进入 F 前必须先修复的阻断项；记录见 [P3-12-E6 发布候选进入判断记录](/records/p3-12-e6-release-candidate-entry-decision-2026-07-06)。
- 下一步进入 `P3-12-F`：先建立候选整备记录或清单，刷新候选范围、验证矩阵、可接受限制和后置专题清单；不直接创建 tag，不进入 M15 测试或生产部署。
- F 内重点补齐或明确边界：注册提交、聊天发送、轻回应 / 问答回答、最终购买扣费、Console 高风险治理动作、真实登录过期、断网 / 弱网、慢接口、运营默认值和可访问性抽样。
- 如果 F 命中接口、错误模型、后端日志、权限、审计或运行时契约缺口，先补小方案并确认边界；仅文档记录、规划口径、低风险文案或状态说明可直接推进。
...
## 后续事项（2026-07-05 之后）
- 先读取本页、[P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement) 与 [P3-12-E 正式产品成熟度与质量硬化](/planning/p3-12-product-maturity-quality-hardening)，确认 `P3-12-E1-E6` 已完成，下一步进入 `P3-12-F 正式版发布候选整备`。
- 继续推进 `P3-12-F`：先建立候选整备记录或清单，承接 [P3-12-E6 发布候选进入判断记录](/records/p3-12-e6-release-candidate-entry-decision-2026-07-06) 的发布前建议项，刷新候选范围、验证矩阵、可接受限制和后置专题清单。
- 若需要新增真实 Gateway smoke，仍必须先由用户当日明确说明前后端已启动；普通开发优先使用相关前端 workspace 类型检查 / 构建、repo hygiene 和 `git diff --check`。
- E6 判断已确认：当前无进入 `P3-12-F` 前必须先修复的阻断项；后置专题必须保留替代路径、首发边界和不阻断理由。
...
- 不把维护观察、等待真实反馈、恢复 P3-11 PR、创建发布 tag、进入 M15 或生产稳定运营当作当前主线。
- 不继续把 P3-8-D 购买 / 订单 / 背包、权限授权、ID 守护、移动 Web 逐页打磨或 Console 历史 CSS 筛查作为默认主线。
- 不在 P3-12 UI 专题中绕过 Pencil 设计稿直接实现页面级或跨页面视觉改造。
- 不把 Tauri、移动版 WebOS 或 WebOS 全量迁移当作当前默认分发 / 前置门槛；新功能默认不进入 WebOS。
- 不让 PC 客户端或 Flutter 抢占当前纯 Web 默认入口、信息流、ID 契约和社区互动治理主线。
- 不一次性启动首页瀑布流、个人圈子、推荐算法、联邦社交、完整移动商城、完整通知中心、完整资产中心、完整创作器、完整浏览历史治理、完整 `PublicId` 主键迁移、完整 E2E 平台或大而全可观测性平台。
