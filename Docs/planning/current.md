...
- **最近结论**：
  ...
  - `论坛内容发布可靠性与编辑历史治理` 创建链路、首批编辑重试幂等和 Flutter 当前论坛写入口承接已按确认方案完成：新增 `ContentSubmissionRecord` 内容提交意图记录，Web 发帖 / 评论 / 回答 / 帖子编辑 / 评论编辑生成并复用 `clientSubmissionId`，Flutter 纯文本发帖、根评论和评论回复生成并复用同一字段，服务端按请求摘要和短窗口内容指纹处理重复提交；帖子 / 评论无变化保存不写编辑历史、不递增编辑次数，评论重放 / 无变化不重复推送实时事件。独立频率限制、Flutter 问答回答 / 编辑入口是否新增产品能力承接后续再评审。
...
2. **选择 P3-10 下一批产品 / 治理增量**
   ...
   - 下一批推荐候选已切到 [论坛内容发布可靠性与编辑历史治理](/guide/forum-content-write-reliability-governance)，创建链路、首批帖子 / 评论编辑重试幂等和 Flutter 纯文本发帖 / 评论写入口承接已按确认方案完成；独立频率限制和 Flutter 问答回答 / 编辑入口是否新增产品能力承接后续再评审。
...
- `P3-10 后续治理专题：论坛内容发布可靠性与编辑历史治理`
  ...
  - 已按确认方案推进 `PublishPostDto`、`CreateCommentDto`、`CreateAnswerDto`、`UpdatePostDto` 和 `UpdateCommentDto`：Web 端生成并复用 `clientSubmissionId`，Flutter 当前已有纯文本发帖、根评论和评论回复生成并复用同一字段，服务端用 `ContentSubmissionRecord`、请求摘要和短窗口内容指纹防重复提交。
  - 编辑历史继续以既有 `PostEditHistory` / `CommentEditHistory` 为真值，不新增通用编辑历史表；帖子 / 评论编辑无变化不写历史、不递增编辑次数，同 key 成功重放不重复执行更新。
  - 本批已确认新增 `ContentSubmissionRecord`、先覆盖创建链路和首批编辑重试、采用默认短窗口、`Pending` 重试直接返回“正在提交”，Flutter 已承接当前存在的纯文本发帖、根评论和评论回复入口。
...
- 先读取本页、[论坛内容发布可靠性与编辑历史治理](/guide/forum-content-write-reliability-governance)、[写操作可靠性与并发保护治理](/guide/write-operation-reliability-governance) 和 [论坛帖子/评论编辑历史设计与实现](/features/forum-edit-history)，确认当前第一顺位已完成论坛内容发布可靠性创建链路和首批编辑重试幂等。
- 第一顺位：从独立频率限制、Flutter 问答回答 / 编辑入口是否新增产品能力承接和下一批 P3-10 产品 / 治理增量中重新评审下一步，不顺带扩大 Flutter 转账、完整移动商城或服务端强制资产写入口 key。
