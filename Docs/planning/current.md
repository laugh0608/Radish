...
- `F1-A / F1-B / F1-C / F1-D` 已完成本地实现与静态回归；下一步进入 `F1-E 专题验收`。Badge / Title 仅开放服务端能力，不自动恢复任何历史商品销售；其余权益类型继续关闭。
...
1. 按[商城商品效力与权益履约专题](/features/shop-product-effect-entitlement-fulfillment)进入 `F1-E 专题验收`，成组复核订单、背包、公开身份与 Console 排障链路；运行态 PC / mobile 验收仍须在当前任务明确确认服务已启动或另行授权启动。
...
1. 已完成 `F1-D` 共享公开身份装饰契约、批量读取、UTC 有效性、失效降级与 DTO 隐私边界。
2. 已完成公开主页、帖子、问答回答、评论及回复的共享组件接入和移动端长称号布局。
3. 已完成服务端权威商品能力元数据；Badge / Title 能力开放，历史商品保持下架，Theme 及其余权益继续关闭。
4. 已完成后端、Client、Console 成组静态回归；未启动服务、未执行 Gateway smoke、未创建 PR、tag 或发布。

## 明日事项（2026-07-15）

1. 进入 `F1-E 专题验收`，先做变更范围、公开隐私、历史商品状态和订单 / 背包 / Console 排障静态复核。
2. 如需执行真实 Gateway PC / mobile 验收，必须在当轮确认 API、Auth、Gateway、Client、Console 已启动，或说明启动命令、端口、运行影响并取得明确授权。
3. F1-E 完成后回写专题结论，并判断当前 `dev` 批次是否达到统一创建 `dev -> master` PR 的完整交付边界；不自动创建 tag 或发布。
