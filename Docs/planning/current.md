...
1. 按[商城商品效力与权益履约专题](/features/shop-product-effect-entitlement-fulfillment)进入 `F1-D 徽章与称号真实生效`，建立共享公开身份装饰契约并接入公开主页、帖子和评论作者面；真实效果、失效回退和回归闭环前不开放类型。
...
4. 继续观察生产登录、聊天、通知、Console、证书复用和 DbMigrate 重入信号；没有新 P0/P1 时，按确认后的设计进入 `F1-A 订单履约安全`。
