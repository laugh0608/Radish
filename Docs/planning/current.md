...
- **最近结论**：
  ...
  - `P3-8-D` 移动 Web 公开视图验收矩阵已建立，并完成 `390 x 844` Vite 直连实测；暂未发现新的 `P0/P1`，优先修正公开页顶部工作台入口从 `/` 指向 `/desktop`
...
1. **推进 P3-8-D 多端主路径缺口补齐与验收矩阵**
   ...
   - 再按验收结果评估根路径 `/` 切换、纯 Web 主路径补强或 Flutter 下一批高价值功能，不默认继续做 Console 剩余页面微调
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 第二顺位：根据矩阵结论选择一个纯 Web 小闭环修复，优先评估根路径 `/` 默认入口切换、移动阅读、来源返回、分享入口、购买 / 订单 / 背包等浏览器主路径；不直接启动完整移动商城、完整通知中心或完整创作器
