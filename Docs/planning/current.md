...
- **最近结论**：
  ...
  - P3-9 合并后本地最小验证已完成：`validate:ci` 通过；因规划入口命中默认执行面 / 门禁资产，已补 `validate:identity` 并通过。
  - 当前进入 P3-9 发布候选部署前材料收口：整理 tag / 镜像 / 部署 / smoke / 回滚目标待执行项，再评估是否创建 tag 与进入 M15 发布 / 部署流程。
...
1. **推进 P3-9 发布候选落地准备**
   - 从“PR 前合并准备”转向“合并后发布候选落地准备”。
   - 首批围绕 tag、镜像、部署、smoke、回滚目标和发布留痕收口待执行项。
   - 只回拉阻断主路径、造成跨端回流丢失、破坏 LongId / PublicId 契约、影响登录恢复或影响 Console 排障效率的问题。
...
- `P3-9-F 合并后发布候选落地准备`
  - 执行合并后最小验证，确认 `master` 合并结果、`Repo Quality` 与本地验证入口一致。
  - 整理发布候选部署前材料，明确 tag / 镜像 / 部署 / smoke / 回滚目标的待执行项。
  - 若合并后验证暴露 `P0/P1`，只回拉主路径阻断、状态恢复、身份契约、公开链接、跨端回流或 Console 排障效率问题。

## 今日事项

- 第一顺位：整理 tag / 镜像 / 部署 / smoke / 回滚目标待执行项，准备进入 M15 发布候选流程。
- 第二顺位：若决定进入真实部署准备，执行 `validate:baseline:host` 并生成启动前报告。
- 第三顺位：创建规范 tag 后等待 `Docker Images` workflow 产出对应镜像，再进入测试部署复核。
- 第四顺位：若合并后验证命中 Flutter 商城 / 钱包、登录恢复、公开链接、LongId / PublicId 或 Console 排障阻断，按同类问题成组修复并补定向测试；不要恢复 P3-8-D 旧路径反复深挖。
- 第五顺位：将 Flutter `发现 / 消息 / 更多 / 我的` 信息架构作为后续产品设计评估项，只先评估跨端任务归属和入口命名，不抢占发布候选落地准备。
