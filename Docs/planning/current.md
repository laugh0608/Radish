...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D29` 已完成 `radish.console` 深层表单静态收口首批：新增 `adminForm.css`，商品、分类、贴纸和贴纸分组表单的上传预览、隐藏输入、控件宽度、弱提示文本和弹窗 footer 样式已迁出 inline / 硬编码色；上传 API、附件字段、校验和保存载荷保持不变，记录见 [P3-12-D29 radish.console 深层表单静态收口记录](/records/p3-12-d29-radish-console-deep-form-static-closure-2026-06-30)。下一步若继续静态治理，优先进入详情 / 抽屉静态收口；若准备阶段验收，则在用户确认前后端已启动后执行 Gateway PC / mobile 复核。
2. **保持 P3-10 可恢复合并状态**
...
- 先读取本页、[P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement)、[P3-12-D29 radish.console 深层表单静态收口记录](/records/p3-12-d29-radish-console-deep-form-static-closure-2026-06-30) 和 [Console 治理工作台设计端点](/frontend/console-governance-workbench-design)，确认下一步是继续详情 / 抽屉静态收口，还是准备阶段真实验收。
- 第一顺位：若继续静态治理，按 `OrderDetail / ProductDetail / DocumentGovernancePage` 的详情区、抽屉、图片展示、隐藏输入和危险色规则集中收口；批量上传弹窗可作为同批或下一批低风险扫尾。
