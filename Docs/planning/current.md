...
- **最近结论**：
  ...
  - `P3-10-D` 阶段级 Gateway PC / 移动收口复核已开始：公开壳层主要入口、公开论坛流、公开文档详情、公开个人页、榜单、商城、圈子和我的状态入口均已覆盖首轮运行态检查；本轮修复公开文档详情重复 H1 与正文内链公开 URL 口径，并完成公开商城首页 / 列表 / 详情标题层级收口，后续继续按同类问题成组治理。
...
1. **推进 P3-10-D Web 信息流 / UI 结构整理**
   - 首日公开页结构整理与首轮 Gateway PC / 移动运行态复核已完成，下一步继续收口复核命中的同类问题。
   - 继续围绕公开壳层的进入动作、分享 URL、来源返回、登录回流、公开 / 私域边界、标题层级和真实内容密度，覆盖 `/discover`、公开论坛列表 / 搜索 / 标签 / 类型流、公开帖子详情、公开文档详情、公开个人页、`/leaderboard`、`/shop`、`/circle` 和 `/me`。
   - 若只命中文案、状态、按钮或行为等价问题，按同类问题成组修复；若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新设计源文件 / Pencil 或对应专题说明。
...
- `P3-10-D Web 信息流 / UI 结构整理`
  ...
  - 首轮阶段级 Gateway PC / 移动收口复核已开始，并已修复公开文档详情重复 H1、正文内链公开 URL 口径，以及公开商城首页 / 列表 / 详情标题层级。
  - 下一步优先继续收口复核命中的同类问题：进入动作、分享 URL、登录回流、来源返回、公开 / 私域边界、标题层级、内容密度和入口动作一致性。
...
- 第一顺位：继续推进 `P3-10-D` 阶段级收口复核后的同类问题治理，围绕公开壳层进入动作、分享 URL、来源返回、登录回流、公开 / 私域边界、标题层级、真实内容密度和入口动作一致性，静态复核与必要运行态复核 `/discover`、公开论坛列表 / 搜索 / 标签 / 类型流、公开帖子详情、公开文档详情、公开个人页、`/leaderboard`、`/shop`、`/circle` 和 `/me`。
- 第二顺位：若只命中文案、状态、按钮或行为等价问题，按同类问题成组修复；若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新 Pencil / 设计源文件或对应功能说明。
- 第三顺位：整理 P3-10-D 合并前验证准备，开发中优先跑构建、测试、类型检查、`git diff --check` 和仓库卫生检查；准备合并或用户确认前后端已启动后，再集中执行 baseline / identity / host runtime 与 Gateway PC + 移动真实页面 smoke。
- 第四顺位：进入代码前明确不做范围：不一次性启动推荐算法、联邦社交、完整 Flutter 承接、完整 WebOS 迁移、系统设置扩面、经济扩展、完整聊天、完整钱包或 P3-8-D 购买 / 订单 / 背包重复筛查。
