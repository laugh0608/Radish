...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D36-D47` 已完成 UI 专题差距口径、四类设计源差距矩阵、边界裁决、阶段验收清单、Gateway PC / mobile 阶段验收、纠偏、页面缺口核对、Docs 作者态动作补漏、Public / Private 主路径代码侧复核、Console 深层管理动作复核、移动响应式抽样、Pencil UI 实现完成度复核和 UI 实现证据收口；下一步进入 `P3-12-D48 UI 候选前验证执行准备`。
2. **保持 P3-10 可恢复合并状态**
...
## 后续事项
- 先读取本页、[P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement)、[P3-12-D36 UI 专题差距与退出标准整理记录](/records/p3-12-d36-ui-topic-gap-and-exit-criteria-2026-06-30)、[P3-12-D47 UI 实现证据收口与候选前验证清单](/records/p3-12-d47-ui-evidence-and-candidate-validation-checklist-2026-07-02)，确认下一步仍在 `P3-12-D` UI 候选前验证执行准备内。
- 第一顺位进入 `P3-12-D48 UI 候选前验证执行准备`：按 D47 清单先执行启动前自动化验证，记录结果、阻断和是否具备进入真实页面复核的条件。
- 如 D48 需要补 PC、移动或 DPR 物理高分屏真实 Gateway 结论，仍需先告知用户启动前后端并等待明确确认，不沿用历史会话启动状态。
- 发布候选准备必须后置到 UI 候选前验证完成并确认具备工程条件之后；当前不整理 PR 发布材料，不创建 tag，不进入 M15 测试 / 生产部署流程。
