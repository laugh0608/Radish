...
- [x] 已补发布留痕模板：当前已新增 [M15 发布记录模板](/guide/m15-release-record-template)，用于固定记录一次发布的 tag、镜像 tag、test/prod 结论与回滚目标
...
- 截至 `2026-04-06`，`M15` 第三批也已补齐发布记录模板：当前已不只定义“怎么发、怎么部署、怎么回滚”，也开始固定“每次发布最少要留下哪些事实”
