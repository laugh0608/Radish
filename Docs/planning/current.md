...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`F4-A 生产六链路证据补采与反馈归因`
- **产品下一顺位**：`证据达到门槛后确认一个完整功能专题`
- **复核日期**：`2026-07-18`
- **当前判断**：
  ...
  - 2026-07-18 F4-A 首轮证据归因已完成：仓库内唯一明确的发布后生产 UX 证据是首次管理员入口门禁不一致，该 `P2` 已由 `54852c73` 在 `dev` 修复并等待正常批次发布复核；其余六链路数据均来自本地种子、受控 smoke、自动化或发布前设计反馈，不能证明真实使用价值。当前无未处置 `P0/P1`，证据不足以确定功能专题，下一顺位继续补采生产六链路聚合与可追溯反馈。详见 [F4-A 首批真实使用证据整理与反馈归因记录](/records/f4-a-first-real-usage-evidence-attribution-2026-07-18)。
...
- F4-A 首轮已完成仓库证据、失败记录、审计能力和观测盲区归因；本地 smoke 与自动化不计为真实使用价值。
- F4-A 第二批已校准生产五库、宿主日志、时间分表和六链路采集合同，并建立只读脱敏采集说明与批次模板；当前仍未取得生产脱敏导出或可追溯反馈原文。
- 当前不启动新的功能完成专题；先补生产六链路聚合与可追溯反馈，达到选题门槛后再只推进一个完整专题。

## 下一顺位

1. 继续 `F4-A`，按[真实使用证据采集说明](/guide/real-usage-evidence-collection)和[生产证据采集模板](/records/f4-a-production-evidence-capture-template)，从 2026-07-12 成功部署日起补采连续 7 至 14 天的生产登录、首次参与、回应回流、聊天、通知和 Console 脱敏聚合，并排除种子、开发者 smoke 与维护操作。
2. 用户反馈按来源、原始时间、页面路径、用户目标、期望 / 实际、复现条件和诊断材料人工脱敏归档；没有原文或可追溯来源时只记为线索。
3. 单个已验证 `P0/P1` 可直接进入维护线；`P2/P3` 需按共同成因成组。只有两个独立真实使用信号相互印证且能明确数据、接口、页面、停止线和验证口径时，才建立一个完整功能专题。
...
4. 已建立 F4-A 专题记录，明确生产证据获取入口、脱敏字段和选题门槛；当前证据不足，不推荐产品功能专题。
