...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`通知中心深化专题：先完成现状审计与设计边界，不直接进入代码`
- **产品下一顺位**：`围绕通知偏好、聚合 / 容量与可靠目标回流形成完整专题；聊天搜索 / Reaction / 置顶继续候选后置`
- **复核日期**：`2026-07-18`
- **当前判断**：
...
2. 下一专题固定先做通知中心深化的现状审计与设计，明确通知偏好、聚合 / 容量治理、可靠目标回流、权限、失败恢复、停止线与验证口径，再决定实现批次；不直接从页面增量开始编码。
3. 聊天搜索、聊天 Reaction、消息置顶与移动系统通知继续作为后续候选，不与通知专题设计并行展开；主动生产证据采集保持最终收尾冻结。
