...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`一对一私聊与会话管理批次 D：定向回归与双账号 PC / mobile 成组验收`
- **产品下一顺位**：`批次 D 通过后关闭私聊专题，再按完整功能边界选择下一专题`
- **复核日期**：`2026-07-18`
- **当前判断**：
...
1. 按 [一对一私聊与会话管理设计](/features/chat-direct-conversation-design) 执行批次 D，先完成 Chat Service / Controller / Hub / migration 与 `radish.client` 的定向回归和候选构建。
2. 经当轮明确授权后启动服务，使用两个普通账号和一个陌生关系账号覆盖 `zh / en × PC / mobile` 的互关、陌生请求、接受 / 拒绝、阻断 / 解除、归档 / 恢复、未读与消息深链。
3. 记录临时账号与会话数据、清理方式、结果和未覆盖风险；全部完成标准满足后关闭私聊专题，再选择下一项完整功能。
