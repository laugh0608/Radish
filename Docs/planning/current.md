...
- `Phase 2-3` 第三批后续判断
  ...
  - 第五批首个候选方向 `profile` 最近阅读轻量多条列表已完成收口；第六批 `forum detail` 轻回应发布后局部体验补强、第七批首个小闭环 `docs` 搜索体验增强、第八批首个小闭环 `profile` 最近文档轻量多条列表、第九批首个小闭环 `profile` 复访区块体验整理、第十批首个小闭环 `profile` 区块顺序与信息密度微调、第十一批首个小闭环 `docs` 详情只读上下文补强、第十二批首个小闭环 `forum detail` 来源上下文与错误态补强，以及第十三批首个小闭环 `discover` 只读分发上下文补强均已完成代码与自动化验证；其中第八批已补自动化收口复核，真机安装仍留到正式 release 包发布前补；后续若继续做功能，仍应选择窄范围复访或只读体验补强
