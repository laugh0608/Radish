...
- **最近结论**：
  ...
  - `P3-9-E` 已完成发布候选路径自动化总回归；`validate:baseline`、`validate:identity`、client / console / Flutter 定向验证均通过。当前未启动本地服务，人工复核仍需按记录执行。
...
- `P3-9-E 人工复核与 PR 前扩大验证准备`
  - 按 [P3-9-E 发布候选路径总回归与收口结论（2026-06-07）](/records/p3-9-e-release-candidate-total-regression-record-2026-06-07) 执行访客公开访问、Flutter 登录移动用户和 Console 管理员路径人工复核。
  - 人工复核无新增阻断时，进入 `PR -> master` 前扩大验证和合并准备。
  - 人工复核若暴露 `P0/P1`，只回拉主路径阻断、状态恢复、身份契约、公开链接、跨端回流或 Console 排障效率问题。

## 明日事项

- 第一顺位：由用户按 P3-9-E 记录启动本地服务和测试账号，完成人工复核。
- 第二顺位：人工复核无新增阻断时，准备 `PR -> master` 前扩大验证与合并材料。
- 第三顺位：若人工复核命中 `P0/P1`，按同类问题成组修复并补定向测试；不要恢复 P3-8-D 旧路径反复深挖。
