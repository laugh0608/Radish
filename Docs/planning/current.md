...
- **最近结论**：
  ...
  - `P3-10-B6` 首批代码已完成：Auth refresh idle 校验、前端活动记录、退出登录 UX、Console refresh 前置等待、通知 / 聊天 Hub 停连和评论 Hub 匿名恢复已落地；下一步按页面真实联调规则覆盖 Gateway 下 PC 与移动视图。
...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - Token 不活跃过期已完成首批代码，后续优先做 Gateway 下 PC + 移动视图页面真实联调。
   - Web UI 改造和 Radish 电子宠物继续作为同阶段候选，不抢占 B6 运行时行为联调。
...
- `P3-10-B6 Token 不活跃过期`
  - 使用 Gateway + Web / Console 覆盖 PC 与移动端视图，验证登录后活动记录、refresh 参数、idle 过期退出、页面恢复和 Hub 重连。
  - 关注通知 / 聊天授权 Hub 停连、评论匿名订阅恢复、受保护页面重定向和 Console `/console/login?auto=1&reason=idle`。
...
- 第一顺位：围绕 `P3-10-B6` 做页面真实联调，按 [页面真实联调](/guide/browser-smoke) 覆盖 Gateway 下 PC 与移动端视图。
- 第二顺位：重点验证登录成功活动记录、refresh 请求参数、idle 过期退出、页面前台恢复、Console 登录回跳、通知 / 聊天 Hub 停连和评论 Hub 匿名恢复。
- 第三顺位：联调暴露问题时，按 Auth refresh 判定、前端活动记录、退出登录 UX、Hub token factory / 重连和页面恢复入口成组修复，并执行对应精准验证。
...
- 第六顺位：若 B6 联调通过且没有新的阻断，进入 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，先审 `/discover`、`/forum` 详情、`/u/usr_...`、`/circle` 和 Console 高频入口的页面层级、设计边界与验证矩阵，再选择同一组高价值入口推进代码；不把 WebOS `/desktop` 作为新增功能承载方向。
