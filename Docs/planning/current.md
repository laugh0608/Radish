...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`从既有业务域选择并设计下一项完整功能专题`
- **产品下一顺位**：`按确认后的专题边界推进完整实现与成组验收`
- **复核日期**：`2026-07-18`
- **当前判断**：
...
1. 盘点现有产品边界和专题文档，从 Docs、聊天、通知、治理、宠物或其他既有业务域中确认一项长期价值明确、能够完整交付的功能专题。
2. 进入代码前先完成专题设计，明确用户路径、数据模型、接口、页面、权限、失败恢复、停止线和验证口径；确认后按完整功能与成组验收推进。
3. 已被动收到的明确生产故障或可追溯反馈继续进入维护线；主动生产证据采集保持冻结，不因旧 F4-A 记录、观察周期或新会话自动重启。
