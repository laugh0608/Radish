...
1. **`P3-4` 用户留存轻闭环**
   - 当前采用主动验收矩阵推进，不再默认等待开发者从真实使用中逐个发现断点
   - 优先串起已有能力：公开分享进入正确壳层、通知 / 最近阅读 / 我的轻回应回到上下文、桌面工作台与 Flutter 移动端复访边界一致
   - 每批选择同一高价值链路内的一组成片问题，批量复核、成组修复、补充定向测试后一次性交付结论
2. **第二阶段收口护栏**
...
- `P3-4-A` 首批 forum 回流小闭环已覆盖通知、我的轻回应、最近阅读 / 浏览历史补齐策略和窄范围真实链路复核；下一步进入 `P3-4-B` 留存回流主动验收批次，不继续扩全量 `PublicId`
...
- 下一事项：`P3-4-B` 留存回流主动验收批次；优先由 AI 按矩阵覆盖公开分享、通知、最近阅读、我的轻回应到 WebOS / Flutter 详情的一致性，并成组修复发现的问题
