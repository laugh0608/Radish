...
1. **推进 P3-10 Web-first 信息架构与下一批开发任务选择**
   - 从“发布候选落地准备”转向“继续开发前的 Web 默认入口、信息流、UI / 功能差距和历史功能规划回拉治理”。
   - 首批围绕 Web 首页信息流、个人圈子边界、PublicId 分批、评论实时 / 神评稳定性、Token 不活跃过期、Radish 电子宠物规划和去 WebOS 化迁移形成可执行开发批次。
   - 只把能改善真实使用路径、跨端一致性、登录恢复、对象标识契约、评论互动或治理效率的问题纳入下一批主线。
...
- `P3-10-A Web-first 任务归属与历史功能规划回拉评估`
  - 先梳理 Web 默认入口、首页信息流、个人圈子、PublicId、评论互动、Token 不活跃过期、UI 改造和 Radish 电子宠物的阶段归属。
  - 输出下一批开发候选矩阵，按真实使用价值、实现成本、验证成本和长期路线排序。
  - 暂不直接启动 Flutter 导航重组、完整推荐系统、完整联邦社交、完整移动商城、完整通知中心、完整资产中心或 PC/Tauri 分发增强。

## 今日事项

- 第一顺位：更新规划状态，明确 P3-9 已合并但本轮跳过发布，当前继续开发而不是进入稳定运营。
- 第二顺位：启动 `P3-10-A`，整理 Web 首页信息流 / 个人圈子、PublicId、评论实时 / 神评稳定性、Token 不活跃过期、UI 改造和历史功能规划回拉候选。
- 第三顺位：形成下一批开发候选矩阵，优先选择能提升真实使用路径、Web 默认入口质量和长期契约一致性的任务。
