...
1. **`P3-2` `PublicId` 最小试点方案**
   - 只做方案和最小试点边界，不启动数据库主键迁移或全量外部契约切换
   - 优先审计公开路由、通知 `extData`、窗口参数、分享链接和 API 返回中的外部对象 ID
   - 选出最小对象集、迁移顺序、兼容策略、验证入口和回滚边界
2. **第二阶段收口护栏**
...
- `P3-2`：读取外部 ID 契约现状，形成 `PublicId` 最小试点对象、边界、兼容策略与验证口径
