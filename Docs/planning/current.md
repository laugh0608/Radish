...
1. 按[前端主题与 i18n 落地记录](/frontend/theme-i18n-implementation)进入 `F2 主题系统完成度治理`，先复核现有专题说明、主题切换入口、语义 token、持久化和页面覆盖范围；说明明显过期时先收紧设计边界，再进入代码实现。
...
1. 进入 `F2 主题系统完成度治理`，先核对主题专题说明与当前实现，明确正式入口、语义 token、持久化、系统偏好和页面覆盖停止线。
2. 若现有主题说明明显过期，先更新专题边界和验收口径，再拆分首个实现切片；不把 i18n 顺带混入同一实现批次。
3. 评估当前 `dev` 的 F1 完整专题成果是否达到统一创建 `dev -> master` PR 的交付边界；不自动创建 PR、tag 或发布。
