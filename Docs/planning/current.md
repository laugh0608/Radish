...
- 第 9 步：继续对“标签页后台闲置后恢复 / 再次登录明显变慢 / Dock 显示与真实会话漂移”补手工回归，重点确认旧 SQLite 库执行 `DbMigrate apply` 后的索引自愈、登录页重复提交抑制，以及慢连接日志是否收敛
- 第 10 步：继续观察论坛首页分类 / 标签 / 帖子列表首屏耗时与日志噪音，重点确认旧库执行 `DbMigrate apply` 后的索引自愈，以及 `Logs/{ProjectName}` 与 `SkipTables` 行为在开发态保持稳定
- 第 11 步：本轮社区体验问题已达到当前批次收口标准，后续先保持文档、验证与发布口径冻结，不在今天继续扩张到下一阶段实现
...
- 认证基础入口虽已于 `2026-03-23` 完成一轮真实首版 Smoke，但 `2026-03-30` 又针对后台闲置恢复、慢登录与重复提交场景补了一轮治根治理；当前重点已从“补功能”切到“看回归是否真正收敛”
...
- 对于今天这轮社区体验修复，当前判断已达到“可转维护、可结束本轮主线”的状态；下一阶段应另起议题和计划，而不是继续在本页内无边界膨胀
