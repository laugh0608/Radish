...
- **里程碑**：`M15 最小交付与部署基线`
- **当前主线**：`M15 最小交付与部署基线`
- **当前阶段**：`截至 2026-04-06，M14 的“启动前 validate:baseline:host -> 启动后 check:host-runtime -> 部署后最小复核”三层主路径已完成首轮真实闭环：本地默认主路径、Docker 测试部署与生产部署均已确认无阻塞问题，宿主维护样例与部署复核记录也已补齐。基于此，当前主线已正式切换到 M15 第一批；本轮只收口“发版 / 部署 / 发布后最小复核 / 回滚”的单一入口与默认 tag 口径，不扩张到 workflow 改造、自动回滚或 Gateway & BFF 专题。详见：[M15 最小交付与部署基线](/guide/m15-delivery-baseline)。`
- **复核日期**：`2026-04-06`
...
- 把当前真实可用的发版、部署、发布后最小复核与回滚动作收束成单一入口
- 明确测试部署与生产部署默认都应固定明确版本 tag，而不是继续依赖漂移别名
- 把部署指南、环境示例与规划口径统一到同一套最小顺序，减少“文档建议”和“示例默认值”打架
- 维持 `M14` 的运行、自检与部署后复核作为 `M15` 的默认复核骨架，而不是重开一套并行检查清单
- 明确 `M15` 第一批不扩张到 workflow 改造、自动回滚、蓝绿 / 金丝雀或 `Gateway & BFF` 深化专题

### 当前输出

- [x] 已确认 `M14` 首轮真实闭环完成：本地默认主路径、Docker 测试部署与生产部署均已确认无阻塞问题
- [x] 已新增 [M15 最小交付与部署基线](/guide/m15-delivery-baseline) 作为当前单一入口，统一收口“发版 / 部署 / 发布后最小复核 / 回滚”顺序
- [x] 已将 `Deploy/.env.test.example` 与 `Deploy/.env.prod.example` 的默认镜像值改为明确版本占位，并保留手动切回 `test-latest` / `latest` 的说明
- [x] 已将 [部署与容器指南](/deployment/guide) 补齐 `M15` 入口与最小回滚说明，避免部署步骤与回滚步骤继续散落在不同页面
- [x] 已将 [开发路线图](/development-plan) 与 [未来规划 / Backlog](/planning/backlog) 最小同步到 `M15` 第一批已启动的口径
- [x] 已完成首轮测试环境真实回滚演练：在 `v26.3.1-test` 已知不可用的前提下，当前先补 `v26.3.2-r1-test` 作为第二个已知可用锚点，并已完成 `v26.3.2-r1-test -> v26.3.2-test` 的真实回滚闭环；详见 [M15 测试环境最小回滚演练记录（2026-04-06）](/guide/m15-test-rollback-rehearsal-2026-04-06)
- [x] 已补生产环境最小回滚预案：当前不为了补记录去做真实生产回滚，而是先把触发条件、目标选择规则、回滚前确认项与回滚后最小复核固化为 [M15 生产环境最小回滚预案（2026-04-06）](/guide/m15-prod-rollback-playbook-2026-04-06)
...
- 今天不再拉回新的业务功能主线，优先用 `M15` 第一批把交付与回滚基线收口
- `M14` 已完成首轮真实闭环并转入维护；当前主线已正式切换为 [M15 最小交付与部署基线](/guide/m15-delivery-baseline)
- `M13` 继续保留在候选池中，`M15` 不再视作未启动候选

### 当前结论

- 社区主功能收口只是首版 `dev` 的一部分，不再代表整产品已经达到首版边界
- 当前主线已从“身份语义 Phase 4 协议输出收敛”进一步切换为“`M14` 宿主运行与最小可观测性基线”，并在其首轮真实闭环完成后进入“`M15` 最小交付与部署基线”第一批
- 截至 `2026-04-06`，`M15` 第一批也已完成首轮测试环境真实回滚演练：当前不再只有文档口径，`v26.3.2-r1-test -> v26.3.2-test` 的最小回滚流程已被真实验证
- 截至 `2026-04-06`，`M15` 第二批也已补齐生产环境最小回滚预案：当前口径明确为“测试环境已实操、生产环境先预案化”，避免误把未做过的生产动作写成既成事实
...
> 当前主线已切换到 `M15 最小交付与部署基线`，以下只保留后续候选池。
