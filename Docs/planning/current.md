...
- **最近结论**：
  ...
  - `P3-12-B1` 账户资产与商城交易 Web 化方案、路由 / 登录回流契约、商城私域正式 Web 入口和资产正式入口已完成：新增正式 Web 资产与商城交易 return path，接入 `/me/assets`、`/me/assets/transactions`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory`，并把 `/me` 完整钱包转向 `/me/assets/transactions`；下一步继续复用既有组件和主题接入公开购买回流和公开商城 `/desktop` 回跳替换，统一 UI 设计后置到页面迁移完成后的 `P3-12-D`，记录见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)。
...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
   ...
   - `P3-12-B1` 方案、路由 / 登录回流契约、商城私域正式 Web 入口和资产正式入口已完成；下一步复用既有组件和主题接入公开购买回流，并替换公开商城购买中的 `/desktop` 回跳。
...
- `P3-12 Web 完全化与 WebOS 收束`
  ...
  - `P3-12-B1` 方案、路由 / 登录回流契约、商城私域正式 Web 入口和资产正式入口见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)；下一步复用既有组件和主题接入公开购买动作和交易回流替换。
...
- 第一顺位：接通公开商品购买登录回流和购买成功回流，再替换公开商城购买中的 `/desktop` 回跳。
- 第二顺位：补 B1 定向测试、构建和必要后端交易测试；阶段完成后，在用户确认服务已启动后再做 PC / mobile Gateway 复核。
