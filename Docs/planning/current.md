...
2. **选择 P3-10 下一批产品 / 治理增量**
   ...
   - 下一条更适合作为评审候选的是 [密码传输与请求签名临时评审](/guide/password-transport-and-request-signature) 中的“支付 / 转账幂等与重放边界”：只评审重复点击、网络重试和短窗口重放保护，不直接进入完整钱包、经济扩展、资产风控、安全会话、浏览器通用 `sign` 或开放 API 签名。
...
- `P3-10 后置安全治理评审：支付 / 转账幂等与重放边界`
  ...
  - 下一候选建议先评审商城购买、萝卜币转账等写操作的 `idempotencyKey`、业务上下文绑定和短窗口重复提交保护。
  - 该候选涉及写操作运行时行为和资产一致性边界；进入代码前必须先确认方案，不顺带启动浏览器通用 `sign`、字段级加密、安全会话、完整钱包、经济扩展或资产风控平台。
- `P3-10 阶段整理与自动化验证维护线`
...
- 第一顺位：若继续 P3-10 后置安全治理，先确认“支付 / 转账幂等与重放边界”方案边界，再决定是否进入代码。
