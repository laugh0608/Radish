...
- **最近结论**：
  ...
  - `P3-8-D` Flutter 下一批主路径候选矩阵已完成：下一批推荐推进 `Flutter 轻量 forum 通知列表`，仅扩展最近少量可跳 forum 通知和现有帖子 / 评论 handoff，不启动完整通知中心、系统推送、完整移动商城或创作器
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 第二顺位：Flutter 下一批高价值功能已落地公开商品只读详情；后续优先推进轻量 forum 通知列表，只做少量可跳通知到帖子 / 评论的复访闭环，不复刻 WebOS
