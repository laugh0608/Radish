...
- **当前子阶段**：`F4 既有功能持续完成`
- **工程第一顺位**：`F4-R C-1B 已完成 R1-F01；R1-P01 正按分类 / 标签分层与多区块社区首页修订独立画板`
- **产品下一顺位**：`先完成并审核 /discover R1-P01 独立业务页面；通过后再实现灰玉共享主题契约与代表页`
- **复核日期**：`2026-08-03`
...
- `2026-08-03` 已形成并继续修订 `R1-P01`：原复合说明板已拆为独立 PC、mobile、关键状态和辅助说明四个顶层画板；设计复核进一步确认“热帖主流”口径与真实 `newest` 数据不符，分类、标签和其他社区内容层级不足，当前调整为“分类浏览 + 最新讨论 + 社区正在发生 + 热门主题 + 相关内容”，尚不计作完成。
...
11. R1-P01 设计复核确认帖子分类与标签表达不足，且首页不能只以“热帖”概括社区内容；公开发现专题已改为复用现有分类、固定标签、热门标签、问答和投票筛选的分区式社区首页，未新增推荐算法或 API。
...
2. 先完成并审核 `R1-P01 / PC 1250`、`R1-P01 / Mobile 390` 与独立关键状态画板：分类、`#标签` 与结构化状态必须分层，页面必须覆盖分类浏览、最新讨论、社区正在发生、热门主题和次级相关内容；未通过前不把 `R1-P01` 标记为完成。
3. 审核通过后，在 `theme-tokens.css` 及对应契约测试中实现 `guofeng` 灰玉品牌、悬停、柔底和品牌前景；墨蓝操作、状态色与其他三主题保持独立，不机械全仓替换硬编码颜色。
4. 按通过审核的 `R1-P01` 收敛 `/discover` 及直接命中的共享组件：保留真实数据、路由、href、locale、加载 / 错误 / 空态和分享状态，不新增 feed API 或功能。
5. 开发中先执行定向测试、Client build、`git diff --check` 与 changed-only 仓库卫生检查；代表实现完成后再评估阶段性 PC / mobile 真实截图复核。
6. 设计轨下一顺位为 `R1-P02` Public 详情与互动，但不抢在 R1-P01 审核和实现闭环前扩张新画板。
7. 默认不启动服务或执行真实 smoke；只有专题准备验收且获得当前任务授权后再启动。Web 优先、Flutter 条件式承接、WebOS 只兼容、Tauri 冻结与生产证据采集停止线不变。
