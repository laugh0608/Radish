...
- **最近结论**：
  ...
  - `P3-10-B7` 已完成公共壳层一致性清理与论坛公开详情轻互动首批回迁：公开详情登录后可直接发布轻回应和根评论，登录回流只放开带 `intent=comment|quickReply` 的 `/forum/post/:id`；本轮定向契约测试和 `radish.client` 构建通过，Gateway / Chrome 页面复核因本地服务未监听待补。
...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - `P3-10-B7 Web UI 改造与去 WebOS 化迁移图` 已完成公共壳层一致性清理和论坛详情轻互动首批回迁；下一步优先补 Gateway 下 PC / 移动视图联调，再评审公共头部动作策略。
...
- `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`
  - 先补 Gateway 下论坛公开详情 PC / 移动视图联调，覆盖未登录点击参与回流、登录后轻回应 / 根评论发布、评论 Hub 匿名恢复和页面前台恢复。
  - 再评审公共头部动作策略，继续选择能提升纯 Web 默认入口、公开详情阅读、个人主页 / 圈子复访和 Console 高频治理效率的一组入口推进代码。
...
- 第一顺位：继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，先在开发者手动启动服务后补论坛公开详情 Gateway PC / 移动视图联调，再评审公共头部动作策略。
