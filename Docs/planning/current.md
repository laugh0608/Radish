...
- **里程碑**：`M12 社区功能冲刺`
- **当前主线**：`下一主线确认与启动准备（优先候选：P4-ext 论坛问答增强）`
- **当前阶段**：`截至 2026-03-15，Console-ext 一期与 P2-ext 内容扩展版已完成收口，当前进入下一主线确认与排期阶段`
- **复核日期**：`2026-03-15`
...
- 确认 `Console-ext` 一期与 `P2-ext` 当前范围已经达到可转出状态，不再误挂为当前主线
- 为下一条真正要进入开发的主线建立统一口径，优先候选为 `P4-ext`
- 保持 `Console`、OIDC 与开源软件清单进入“稳定维护 + 有需要再扩展”的状态
- 避免规划文档继续沿用已过时的“仍在联调 / 即将开始”描述

### 当前输出

- [x] 已确认 `Console-ext` 一期完成本轮收口：角色授权页、资源模型、接口权限过滤、`console.access` 与 OIDC 联调边界均已闭环
- [x] 已确认 `P2-ext` 的内容扩展版完成本轮收口：欢迎页开源软件说明已扩展为更完整的静态清单
- [x] 已将 `Console-ext Phase 2+` 与“开源软件清单自动生成版”回收到 backlog，不再继续占用当前主线
- [ ] 待确定下一条正式启动主线，并同步到路线图、backlog 与相关专题文档
