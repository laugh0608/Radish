...
- Android MVP 内测分发前置整理当前已完成文档与本机预检收口：内测 RC 候选链路成立，真实签名材料已由用户本机补齐并通过 `:app:checkReleaseSigningConfig`；实际外部分发仍需 testing Gateway、测试账号 / 测试数据、分发对象、反馈回收方式与真机安装复核；对应记录见 [Flutter Android MVP 内测分发前置整理记录（2026-05-01）](/guide/flutter-android-internal-rc-prep-record-2026-05-01)
- Flutter 第八批首个小闭环“profile 最近文档轻量多条列表”已完成代码与自动化验证：我的 `profile` 中最近文档从单条入口扩展为最多 5 条的轻量列表，按 `slug` 去重并保留最新打开优先；Android 本地持久化兼容旧单条 `docs_recent_document_target` 并新增列表存储；壳层状态条仍只展示最新一条“继续阅读文档”；本轮按用户要求跳过真机安装复核
...
6. **Android MVP 内测分发前置整理**
   - 第七批首个小闭环后，下一步优先整理内测 RC 前置材料、验证入口与留痕口径，不启动第八批功能扩张
   - 当前已明确实际外部分发仍依赖真实签名材料、测试 Gateway、测试账号 / 测试数据、分发对象与反馈回收方式
   - 缺少上述前置材料时，保持“等待分发材料”的判断，不回头扩系统通知栏推送、完整通知中心、发帖、完整评论提交、点赞、投票或编辑治理
7. **第八批首个小闭环**
   - profile 最近文档轻量多条列表已完成代码与自动化验证，可作为一个窄范围小批次收口
   - 本轮仍不扩展完整浏览历史中心、删除、清空、跨端同步、搜索历史、目录树、编辑、发布、版本历史、后端搜索改造或 Flutter 专属 BFF
   - Android 真机安装与 testing Gateway 验收按用户要求暂缓，后续准备实际内测分发前再补
...
- `Phase 2-3` 第三批后续判断
  ...
  - 第五批首个候选方向 `profile` 最近阅读轻量多条列表已完成收口；第六批 `forum detail` 轻回应发布后局部体验补强、第七批首个小闭环 `docs` 搜索体验增强与第八批首个小闭环 `profile` 最近文档轻量多条列表均已完成代码与自动化验证；后续若继续做功能，仍应选择窄范围复访或只读体验补强
  - Android RC 外部分发继续等待 testing Gateway、测试账号 / 测试数据、分发对象、反馈回收方式与真机安装复核，不把发布线冻结误判为业务功能阻塞
