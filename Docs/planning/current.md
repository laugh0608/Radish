...
- **最近结论**：
  ...
  - `P3-9-E` 已完成发布候选路径自动化总回归；`validate:baseline`、`validate:identity`、client / console / Flutter 定向验证均通过。人工复核已开始，剩余路径和 Flutter 商城 / 钱包修复复测待执行。
  - 人工复核已暴露 Flutter 登录态商城 / 钱包路径问题：商品购买检查失败态被显示为接口格式异常，商品详情购买面板缺少余额可见性。当前已修复 API 失败态识别、登录态商品详情余额展示和购买后余额刷新，待明日人工复测。
...
- `P3-9-E 人工复核与 PR 前扩大验证准备`
  - 执行访客公开访问、Flutter 登录移动用户和 Console 管理员路径人工复核。
  - 人工复核无新增阻断时，进入 `PR -> master` 前扩大验证和合并准备。
  - 人工复核若暴露 `P0/P1`，只回拉主路径阻断、状态恢复、身份契约、公开链接、跨端回流或 Console 排障效率问题。

## 明日事项

- 第一顺位：由用户启动本地服务、Android 模拟器和测试账号，复测 Flutter 登录态商品详情余额展示、购买资格检查、单商品购买、订单详情、背包发放和胡萝卜资产 / 流水入口；同时按矩阵抽查访客公开访问与 Console 管理员排障路径。
- 第二顺位：人工复核无新增 `P0/P1` 时，准备 `PR -> master` 前扩大验证与合并材料，优先整理 `validate:baseline`、`validate:identity`、client / console / Flutter 验证结果和人工复核结论。
- 第三顺位：若人工复核仍命中 Flutter 商城 / 钱包、登录恢复、公开链接、LongId / PublicId 或 Console 排障阻断，按同类问题成组修复并补定向测试；不要恢复 P3-8-D 旧路径反复深挖。
- 第四顺位：将 Flutter `发现 / 消息 / 更多 / 我的` 信息架构作为后续产品设计评估项，只先评估跨端任务归属和入口命名，不在发布候选人工复核前启动导航重组。
