...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D36 / D37` 已完成 UI 专题退出口径与四类设计源差距矩阵：public / private / foundation / console 画板均已对齐真实路由、代码状态、验证证据和剩余动作；当前明确公开聊天室 `P15 / P16`、公开首页 `P01`、Console 内部调度 / 运维平台 `P04 / P13 / P18` 和移动 Console 画板需要先做边界裁决，记录见 [P3-12-D37 UI 设计源差距矩阵记录](/records/p3-12-d37-ui-design-source-gap-matrix-2026-07-01)。
2. **保持 P3-10 可恢复合并状态**
...
## 明日事项
- 先读取本页、[P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement)、[P3-12-D36 UI 专题差距与退出标准整理记录](/records/p3-12-d36-ui-topic-gap-and-exit-criteria-2026-06-30)、[P3-12-D37 UI 设计源差距矩阵记录](/records/p3-12-d37-ui-design-source-gap-matrix-2026-07-01) 和 public / private / foundation / console 四份设计说明，确认下一步仍在 `P3-12-D` UI 专题内。
- 第一顺位：推进 `P3-12-D38` 边界裁决与阶段验收清单，先确认公开聊天室 `P15 / P16`、公开首页 `P01`、Console 内部调度 / 运维平台 `P04 / P13 / P18` 和移动 Console 画板是否进入发布前范围。
- 若 D38 裁决为后置，再组织一次 Gateway PC / mobile 阶段验收；真实联调前必须等待用户明确说明前后端已启动。`P3-12-E` 发布候选准备必须后置到 UI 专题退出条件确认后。
