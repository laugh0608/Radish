...
- **最近结论**：
  ...
  - `P3-10-B7` 已完成公共壳层一致性清理、论坛公开详情轻互动首批回迁、Gateway / Chrome PC + 移动视图补验和公共头部动作策略首批代码：公共页头部固定为公开发现、登录态圈子和历史工作台三层动作，`/desktop` 保留但不作为新增功能主动作；公开发现 / 论坛阅读边界文案已对齐轻回应与根评论回迁，下一步进入同组高价值入口代码推进与真实联调复核。
  - `P3-10-C` 首批代码与 Gateway PC / 移动视图复核已完成：公开发现、公开论坛列表 / 搜索 / 标签 / 类型流和公开个人页内容项已补齐真实链接契约，普通点击继续保留公开壳层来源返回，新开标签 / 复制链接使用公开 URL。
  - `P3-10-C` 第二批代码已完成本地验证：`/circle` 关注动态和关系链用户跳转公开详情 / 公开个人页时，可通过一次性来源转交返回“我的圈子”；Gateway PC / 移动视图真实复核待开发服务就绪后继续。
...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - `P3-10-B7 Web UI 改造与去 WebOS 化迁移图` 已完成公共壳层一致性清理、论坛详情轻互动首批回迁、Gateway PC / 移动视图补验和公共头部动作策略首批代码；下一步继续选择同一组高价值入口推进。
   - `P3-10-C` 首批公开内容卡片链接契约已完成代码推进和 Gateway PC / 移动视图复核；第二批圈子到公开详情 / 公开个人页来源返回已完成代码推进和本地验证，待 Gateway PC / 移动视图补验。
...
- `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`
  - 已完成 Gateway 下论坛公开详情 PC / 移动视图补验，覆盖未登录点击参与回流、登录后轻回应 / 根评论发布、移动视图无横向溢出和计数一致性。
  - 已完成公共头部动作策略首批代码、公开边界文案同步和 Gateway PC / 移动视图复核，公共页头部统一露出社区发现、我的圈子和工作台入口；继续选择能提升纯 Web 默认入口、公开详情阅读、个人主页 / 圈子复访和 Console 高频治理效率的一组入口推进代码。
...
- 第一顺位：继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，选择下一组能改善纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访或 Console 高频治理效率的代码入口。
- 第二顺位：补验 `/circle` 到 `/forum/post/:id`、`/u/usr_...` 的 Gateway PC / 移动视图真实链路；通过后继续围绕 `/discover`、`/forum` 详情、公开个人页 / 圈子复访和 Console 高频入口推进同一组高价值入口代码。
- 第三顺位：B7 行为级调整前先说明页面策略；无行为变化的样式残留、i18n 和共享壳层一致性问题可直接成组清理。
