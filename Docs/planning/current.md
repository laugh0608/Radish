...
- **最近结论**：
  ...
  - `P3-10-B1 / B3` 首批代码已完成：`/discover` 已调整为 Web-first 公开信息流，User PublicId 公开主页 / 榜单契约已落地并通过 PC 视角浏览器复核；下一步进入 `P3-10-B4 / B5` 评论实时与神评稳定性治理。
...
- 第一顺位：推进 `P3-10-B4 / B5` 评论实时与神评稳定性治理，先复核现有 `CommentService / CommentHighlight`、`NotificationHub / ChatHub` 和 Web 评论区，再给出代码范围并开始实现。
- 第二顺位：评论实时优先按 Web-first 公开帖子详情链路推进，明确 post group、事件类型、typing 节流、前端去重和双用户验证方式；Flutter 后续复用稳定 API，不抢占本批主线。
- 第三顺位：神评 / 沙发优先收敛稳定窗口、并列展示、替换阈值和奖励幂等，避免同赞数或实时重算造成展示抖动。
- 第四顺位：保留 `P3-10-B1 / B3` 回归入口，必要时复核 `/discover`、`/leaderboard`、`/u/usr_...`、公开 head 与 `DbMigrate apply` 后的旧库补列结果。
- 第五顺位：若评论链路暴露需要先补的历史契约问题，按根因治理；不要回到 P3-8-D 购买 / 订单 / 背包或 Console 低频页面筛查作为默认主线。
