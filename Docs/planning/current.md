...
- **最近结论**：
  ...
  - `P3-10-B7` 已完成公共壳层一致性清理、论坛公开详情轻互动首批回迁和 Gateway / Chrome PC + 移动视图补验：公开详情登录后可直接发布轻回应和根评论，登录回流只放开带 `intent=comment|quickReply` 的 `/forum/post/:id`；联调中发现并修复轻回应只读动作与评论计数同步问题。
...
1. **收口 P3-10-B / C Web-first 首批代码**
   ...
   - `P3-10-B7 Web UI 改造与去 WebOS 化迁移图` 已完成公共壳层一致性清理、论坛详情轻互动首批回迁和 Gateway PC / 移动视图补验；下一步评审公共头部动作策略，并继续选择同一组高价值入口推进。
...
- `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`
  ...
  - 下一步评审公共头部动作策略，继续选择能提升纯 Web 默认入口、公开详情阅读、个人主页 / 圈子复访和 Console 高频治理效率的一组入口推进代码。
...
- 第一顺位：继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，评审公共头部动作策略，并选择下一组能改善纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访或 Console 高频治理效率的代码入口。
