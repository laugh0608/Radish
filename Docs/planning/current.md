...
- **最近结论**：
  ...
  - 当前不直接进入新阶段，也不直接启动 `P3-10-B8` 电子宠物代码；下一步仍在 `P3-10-B7` 内收口 WebOS 功能迁移图，先判断哪些 `/desktop` 能力应进入纯 Web / Flutter / 保留 / 后置，再选择首批高价值迁移。
  - `P3-10-B7` 首批迁移代码已推进纯 Web 通知复访入口：`/notifications` 作为登录态私域入口接入主路由、登录恢复和通知目标分流；本地契约测试、`radish.client` type-check / build 已通过，Gateway PC / 移动 smoke 已覆盖未登录回流边界，登录态完整列表与点击仍待可用登录态补验。
...
1. **收口 P3-10-B7 / C 公开入口治理，启动 WebOS 功能迁移图**
   ...
   - 下一步以 `/desktop` 应用注册表和现有纯 Web / Flutter 覆盖为依据，产出 WebOS 能力迁移决策表，不把“去 WebOS 化”误解为全量搬迁。
...
- `P3-10-B7 WebOS 功能迁移图收口`
  - 第一顺位不再继续打磨公开入口矩阵，而是盘点 `/desktop` 现有应用：论坛、文档、聊天、通知、个人主页、等级、萝卜坑、商城、榜单、Console / Scalar 和开发工具。
  - 按“已有纯 Web 替代 / 应迁移到纯 Web / 更适合 Flutter / 保留在 `/desktop` / 放弃或重做”归类，逐步选择高价值迁移，不把全量 WebOS 搬迁设为前置门禁。
  - 纯 Web 通知复访入口已完成首批代码；纯 Web 我的状态入口已进入本地验证，下一步在 Gateway 恢复后补 `/me` PC / 移动 smoke，并继续观察个人状态 / 经验 / 资产是否足以支撑后续 B8 前置能力。
  - 论坛、公开详情、发现、圈子、榜单、文档阅读和商城公开浏览已进入维护补漏。
- `P3-10-B8 Radish 电子宠物规划`
  - B8 不直接抢跑代码；等 WebOS 迁移图确认没有必须先迁的用户复访 / 经济系统前置能力后，再进入玩法边界、经济约束和 Web-first 入口设计。
...
- 第一顺位：在 Gateway / client dev server 启动后补 `/me` PC 与移动 DPR smoke，覆盖未登录回流、登录态概览、打开公开主页后的“返回我的状态”、`/circle` 和 `/notifications` 复访动作。
- 第二顺位：继续 `P3-10-B7 WebOS 功能迁移图收口`，评估 `/me` 只读状态页与 `/notifications` 是否已覆盖当前用户复访前置能力；如果足够，再准备进入 B8 电子宠物边界设计，不直接搬完整钱包或完整个人中心。
- 第三顺位：迁移图收口且无必须前置迁移后，再启动 `P3-10-B8 Radish 电子宠物规划`，先做玩法边界、经济系统约束和入口归属，不直接实现完整游戏系统。
