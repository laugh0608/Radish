...
- **最近结论**：
  ...
  - `P3-12-D6` Console 视觉代码实现前盘点已完成：当前应先在 `radish.console` 内推进公共壳层、路由分组、顶栏、工具条和状态 chip，不把 Console 专用壳层上提到 `@radish/ui`，记录见 [P3-12-D6 Console 视觉代码实现前盘点](/records/p3-12-d6-console-visual-code-prep-2026-06-27)。
...
- 第一顺位：确认 Console 公共壳层实现方案，优先落 `ConsoleShell`、浅色图标侧栏、顶栏、页面工具条和状态 chip。
- 第二顺位：按 [P3-12-D6 Console 视觉代码实现前盘点](/records/p3-12-d6-console-visual-code-prep-2026-06-27) 进入首批代码实现；先改公共壳层和一个低风险代表页，不混入治理动作语义。
