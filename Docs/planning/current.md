...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`F4-C 聊天历史搜索与消息定位：现状审计和专题设计`
- **产品下一顺位**：`设计确认后按服务端检索契约、正式 Web 页面、成组验收推进；消息 Reaction / 置顶继续后置`
- **复核日期**：`2026-07-18`
- **当前判断**：
...
3. 下一专题固定为 F4-C 聊天历史搜索与消息定位；先交叉审计现有 Chat 数据、ACL、分页 / 索引、删除 / 阻断语义、Hub 与 PC / mobile 消费面，并形成专题设计，再进入代码实现。
4. F4-C 后续应依次完成服务端权威检索与消息定位契约、正式 Web 搜索工作区、`zh / en × PC / mobile` 双账号成组验收；消息 Reaction、置顶和移动系统通知不并行展开，主动生产证据采集保持最终收尾冻结。
