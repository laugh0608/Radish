...
- 第一顺位：启动 `P3-10-B8 Radish 电子宠物规划` 的边界设计，先确认玩法循环、经济系统约束、复访入口、Web-first 页面归属、与 `/me` 成长 / 资产摘要的关系，以及哪些内容需要 Console 配置或流水审计。
- 第二顺位：B8 只输出方案和实现范围判断，不直接实现完整游戏系统；如进入代码前发现需要调整经济、经验、通知或公开展示契约，先同步专题文档和必要设计说明。
