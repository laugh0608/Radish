...
- 先读取本页、[P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement)、[P3-12-D27 radish.console 系统工具与运维外壳收口记录](/records/p3-12-d27-radish-console-system-ops-shell-2026-06-30) 和 [Console 治理工作台设计端点](/frontend/console-governance-workbench-design)，确认 D14-D27 是否可以进入更大范围静态阶段收口。
- 第一顺位：整理 `radish.console` D14-D27 的阶段静态收口结论、已迁移页面清单、剩余内联样式 / 旧壳层风险和后续是否需要真实阶段验收。
- 第二顺位：如果要继续做项目内运维任务平台，先补任务队列、失败重试、运行审计的数据来源、API 契约、权限动作和留痕策略设计，再进入代码。
