...
- **当前阶段**：`截至 2026-03-18，社区主要功能已基本具备最小闭环：论坛主链、投票、问答、抽奖、聊天室 P1、个人中心浏览记录、Console 一期与首轮验证入口均已完成本轮收口。当前策略已明确切换为“先主线、后细节”：不再继续深挖社区小切片，而是优先完成主功能总回归、问题清单收口与首个 dev 版本准备。`
- **复核日期**：`2026-03-18`
...
- 先主线，后细节：优先完成真正影响首个 dev 版本可用性的主功能与主链路，不再把非阻塞体验优化持续拉成当前主线。
- 先闭环，后打磨：只要功能已形成稳定闭环、没有明显断链，就先进入总回归与收口，不在当前阶段反复抠局部交互细节。
- 不留坑，不留技术债：允许保持克制实现，但不允许把明显缺口、脆弱契约、长期已知风险或结构性脏债留给后续。
- 敏捷推进：继续采用小步快走，但小步的目标要服务于“尽快完成主线”，而不是无限拆分局部增强项。
- 非阻塞增强统一后置：`P3-ext`、`P4-ext`、`P5-ext`、`Console-ext Phase 2+` 等增强项统一后置到首个 dev 版本之后再评估。

### 本阶段目标

- 把社区主功能的真实完成状态统一回写到规划、专题文档与日志，避免“代码已完成、规划仍显示仍在细节迭代”的口径偏差
- 对社区主链路做一轮总回归视角的盘点，确认哪些问题必须在首个 dev 版本前处理，哪些可以安全后置
- 基于当前仓库真实资产，明确首个 dev 版本之前的真正主线：功能收官、验证基线、最小发布准备，而不是继续扩写局部体验增强
- 对 `M13`、`M14`、`M15` 的进入条件做现实化排序，确保后续阶段服务于“能尽快形成可发布的 dev 版本”
...
- [x] 已确认社区主功能当前已具备最小闭环：论坛主链、投票 MVP、问答 MVP、抽奖 MVP、聊天室 P1、浏览记录、Console 一期均已完成本轮主功能收口
- [x] 已明确从 `2026-03-18` 起停止继续把社区非阻塞小切片当作当前主线，后续细节增强统一后置到首个 dev 版本之后再评估

### 下一步拆分

- 第 1 步：冻结社区非阻塞增强项，把 `P3-ext / P4-ext / P5-ext / Console-ext Phase 2+` 明确转入 backlog（进行中）
- 第 2 步：以“社区主功能总回归”为视角整理首个 dev 版本前必须处理的问题清单（待执行）
- 第 3 步：按 `M13 -> M14 -> M15` 的现实顺序推进验证、运行与最小交付基线（待执行）
- 第 4 步：在主线功能、验证基线与最小发布条件满足后，再决定首个 dev 版本之后的增强优先级（待执行）

### 当前结论

- `P5` 论坛抽奖 MVP 与个人中心浏览记录优化已达到“本轮可收口、可转维护”的状态
- 旧版里程碑口径与基础说明的偏差已完成本轮收口，`M13` 也已拥有第一版统一验证入口
- 社区主要功能当前已达到“可演示、可联调、可回归”的首轮收口标准，可以从“持续补细节”切换到“主线收官与 dev 首版准备”
- 从 `2026-03-18` 起，社区方向不再以投票 / 问答 / 抽奖的小切片增强作为当前主线，只有阻塞首个 dev 版本的问题才继续前置处理

### 当前不做

- [ ] 直接按旧版 `M13` 口径启动“可观测性与测试”整包工程
- [ ] 继续把 `P3-ext`、`P4-ext`、`P5-ext` 的非阻塞细节增强当作当前主线持续推进
- [ ] 在社区主功能已基本完成后，再并行启动多个社区候选增强项
...
> 当前主线已切换到 `M12 社区主功能收官与 dev 首版准备`，以下只保留后续候选池。
