...
- 明日事项（`2026-05-21`）：优先做 `P3-7-A` 商城复访链路收尾复核，确认购买 / 订单 / 背包来源回流是否还存在高信号断点；若继续推进，只评审 `UserInventory.SourceProductId` 是否需要正式暴露到背包消耗品来源入口，不扩大到商城重构、运营平台或全量 `PublicId`
- 当前事项：`P3-7-A` WebOS / PC 工作台复访与继续使用链路复核；只从真实使用高信号断点中选择小闭环，不扩大成低收益入口扫描
