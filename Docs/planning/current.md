...
- **最近结论**：
  ...
  - `P3-10-B7` 首批迁移代码已推进纯 Web 通知复访入口：`/notifications` 作为登录态私域入口接入主路由、登录恢复和通知目标分流；本地契约测试、`radish.client` type-check / build 已通过，Gateway PC / 移动 smoke 因本地服务未启动待补。
...
- `P3-10-B7 WebOS 功能迁移图收口`
  ...
  - 纯 Web 通知复访入口已完成首批代码；下一批优先评估个人状态 / 经验 / 资产入口，或在 Gateway 恢复后先补 `/notifications` PC / 移动 smoke。
...
- 第一顺位：在 Gateway / client dev server 启动后补 `/notifications` PC 与移动 DPR smoke，覆盖登录态或未登录回流边界、头部动作和通知目标分流。
- 第二顺位：继续 `P3-10-B7 WebOS 功能迁移图收口`，优先评估个人状态 / 经验 / 资产入口是否应作为下一批纯 Web 迁移能力；涉及新路由、API 契约或运行时行为变化时先说明策略。
