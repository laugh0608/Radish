...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D63` Console 已完成内容治理 / 经验治理、商业运营、文档治理、用户管理和权限矩阵页面族首批实现、成组静态收口和 Gateway 成组真实页面复核；`P3-12-D64-D66` 已完成 UI 候选前集中验收准备、启动无关候选前验证和运行态补验；随后 `P3-12-E2/E3-A` 已完成信任治理与公开承诺首批实现，`P3-12-E3-B` 已完成 Workbench 社区活动中心和通知行动 scope 首批补强，`P3-12-E3-C` 已完成通知行动队列深化，`P3-12-E4-A` 已完成隐私边界与反骚扰首批硬化，`P3-12-E4-B` 已完成 `/legal` 与 `/me` Gateway PC / mobile 运行态复核、可见边界矩阵、通知目标缺失提示和 Console 治理证据链说明收口。下一步进入错误恢复、可观测性、运营默认值、可访问性和旅程级验证。
2. **保持 P3-10 可恢复合并状态**
...
- 下一步继续错误恢复收口：在首轮无业务写入验证通过后，优先选择受控写入旅程验证或进入 `E6` 发布候选进入判断；若验证注册 / 登录、发帖 / 评论、商城购买到账、Docs 保存或 Console 治理处理需要新增服务端诊断编号、错误上报或 Console 支持入口，先补小方案确认边界。
- `P3-12-E4-B` 已收口，不再围绕同一组可见边界、举报说明、通知目标缺失或 Console 证据链反复深挖；只在真实旅程验证、新扫描命中或用户反馈暴露缺口时回拉。
- 如果 `E5-A` 需要改动接口、错误模型、后端日志或跨端运行时行为，先补小方案并确认边界；仅文案、状态说明、错误反馈组件或文档矩阵可按低风险批次推进。
- 验证按风险分层执行：文档轮次使用 repo hygiene 与 `git diff --check`；前端代码轮次补对应 workspace test / build；真实 Gateway smoke 只在用户当日确认服务已启动后执行。
