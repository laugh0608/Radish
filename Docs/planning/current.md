...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`F4-B-D 通知生产链路回归与双账号 zh / en × PC / mobile 成组验收`
- **产品下一顺位**：`F4-B-D 通过后关闭 F4-B，再从总体规划选择下一完整专题`
- **复核日期**：`2026-07-18`
- **当前判断**：
...
2. [F4-B 通知中心深化与通知治理](/features/notification-center-deepening) 的 A / B / C 批已经完成；下一批固定为 F4-B-D 双账号 `zh / en × PC / mobile` 成组验收。
3. F4-B-D 必须覆盖真实生产链路、偏好抑制、聚合竞态、已读 / 删除、断线 / 多标签、cursor 恢复、目标失效和数据清理矩阵；完整通过后才能关闭 F4-B。
