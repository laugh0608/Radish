...
- **里程碑**：`M12 社区功能冲刺`
- **当前主线**：`社区主链回归收口完成，转入下一里程碑入口重审`
- **当前阶段**：`截至 2026-04-02，社区主链多轮回归已完成，论坛、聊天室、通知中心、个人中心资料展示、认证登录稳定性，以及附件访问与论坛评论分页等本轮触达范围当前均无新的明显阻塞问题；首版 dev 范围内的业务主线、体验主线与发布链路也已完成本轮统一回归确认。当前口径不再是“继续等待总回归确认”，而是“社区主线转入稳定维护，首版 dev 文档与事实完成收口”。在此基础上，当前工作的重心从继续补社区体验尾项，切换为下一里程碑入口重审：先冻结现有文档、验证与部署口径，避免事实再次漂移；再优先确认身份语义 Phase 4 的启动前提，明确外部客户端兼容边界，并据此决定是进入协议输出收敛，还是先重定义宿主运行与最小可观测性基线。详见：[首版 dev 边界](/planning/dev-first-scope)。`
- **复核日期**：`2026-04-02`
...
- 冻结当前首版 `dev` 的文档、验证与部署口径，避免“事实已收口、文档仍漂移”
- 把社区主链正式转入稳定维护，不再继续把非阻塞体验尾项膨胀成当前主线
- 优先确认身份语义 Phase 4 的启动前提，明确外部客户端兼容边界与进入条件
- 若身份语义 Phase 4 暂不具备启动条件，则先按 [M14 宿主运行与最小可观测性基线（重定义）](/guide/m14-host-runtime-observability-baseline) 收束范围，作为下一条明确主线
...
- 第 5 步：若短期内仍无法确认仓库外兼容边界，则先按 [M14 宿主运行与最小可观测性基线（重定义）](/guide/m14-host-runtime-observability-baseline) 收束目标、范围与验收标准，避免继续沿用过早形成的旧阶段名直接开工
- 第 6 步：旧 `GetCommentTree` 兼容入口继续保留在观察池中，仅在确认仓库外也已无依赖后，再安排正式删除窗口

### 当前结论

- 社区主功能收口只是首版 `dev` 的一部分，不再代表整产品已经达到首版边界
- 当前主线已从“社区体验与运营能力优化”进一步收束为“社区主链回归收口完成，转入下一里程碑入口重审”
...
- 本轮社区主链回归已达到“可转维护、可结束本轮主线”的状态；下一阶段优先进入身份语义 Phase 4 启动前提确认与后续阶段重定义，而不是继续在本页内无边界膨胀社区增强项
