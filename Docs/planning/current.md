...
- `F1-A / F1-B / F1-C` 已完成本地实现与静态回归；下一步进入 `F1-D 徽章与称号真实生效`，F1-D 完成能力消费和失效回退前仍不开放任何新商品类型。
...
1. 按[商城商品效力与权益履约专题](/features/shop-product-effect-entitlement-fulfillment)进入 `F1-D 徽章与称号真实生效`，建立共享公开身份装饰契约并接入公开主页、帖子和评论作者面；真实效果、失效回退和完整回归完成前不开放类型。
...
3. 已建立[论坛内容赞赏（胡萝卜 +1）专题](/features/forum-content-reward)，明确复用现有萝卜币账本且不新增积分；专题作为后续完整功能储备，不改变当前 F1 顺位。
4. 生产登录、聊天、通知、Console、证书复用和 DbMigrate 重入继续保留为并行维护线；今日没有新 P0/P1 进入本地开发批次，F1 顺位未被打断。
5. 若需要真实 Gateway 浏览器复核，先确认本任务中的服务启动状态或由用户明确说明生产环境可供复核；不沿用历史运行状态授权。
6. 已完成 F1 商城商品效力与权益履约的首轮代码 / 文档交叉复核并建立专题；确认订单失败阶段、快照发放、履约资源字段和未支付订单重试门禁为 `F1-A`，本日不直接开放新商品。
7. 已完成 `F1-A` 本地实现与静态回归；下一开发切片确认进入 `F1-B`，先补设计与数据契约，不开放新商品类型。
8. 已完成 `F1-B` 本地实现与静态回归：三类现有消耗品具备原子使用、请求回放、业务流水、正式 Web 幂等提交和 Console 只读排障；下一切片进入 `F1-C`，本批仍不创建 PR 或 tag。
9. 已完成 `F1-C` 本地实现与静态回归：唯一当前权益指针、UTC 实时状态、过期物化、管理员撤销、通用流水和 ledger 迁移已收口；下一切片进入 `F1-D`，所有权益类型继续保持关闭，本批仍不创建 PR 或 tag。

## 明日事项（2026-07-14）

1. 推进 `F1-D 徽章与称号真实生效`：先固定 `UserAdornmentVo` 共享只读契约、Badge / Title 配置规则、公开字段和失效降级口径；不向公开响应泄露权益、订单、价格或内部流水信息。
2. 建立服务端批量身份装饰读取能力，以 `UserActiveBenefit` 为当前选择真相并按 `TimeProvider` UTC 实时校验拥有记录；一次装配多个用户，避免帖子与评论列表产生 N+1 查询。
3. 让公开用户主页、帖子及问答作者、评论及回复消费同一身份装饰对象；Client 建立共享展示组件，完整处理仅徽章、仅称号、同时存在、均不存在、资源失效和移动端长称号布局。
4. 将商品类型支持收口为服务端权威能力矩阵，并把能力元数据提供给 Client / Console；本切片只允许 Badge / Title 在配置、发放、启用、真实展示、失效和排障均完成后开放，Theme 及其余权益继续关闭。
5. 保持历史徽章 / 称号商品下架，不因能力策略调整自动恢复销售；管理员必须检查资源、价格、库存和有效期后显式上架。
6. 成组补齐后端批量装配、UTC 边界、跨租户、失效降级、公开 DTO 隐私、能力矩阵和历史商品状态测试，以及 Client / Console 契约、组件和响应式回归；开发中继续只做静态与代码侧验证，F1-E 前不执行真实 Gateway smoke。
7. F1-D 达到完整功能边界后再进入 F1-E 专题验收；本批继续保留在 `dev`，不创建 PR、tag，不主动发版。
