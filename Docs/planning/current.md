...
- `P3-10-D Web 信息流 / UI 结构整理`
  ...
  - P3-10-D 合并前验证准备和 PR 合并判断已完成；下一步优先发起 `dev -> master` PR，不再继续追加第五批链接语义扫尾。
...
- 第一顺位：发起 `P3-10-D` `dev -> master` PR，基于 [P3-10-D PR 准备记录](/records/p3-10-d-web-feed-pr-prep-record-2026-06-19) 说明变更范围、验证结论和剩余限制。
- 第二顺位：若 PR 检查、合并前复查或真实 smoke 新命中阻断 / 清晰一致性问题，再定向回修并复跑对应验证；不再默认追加第五批链接语义扫尾。
