...
- Q0-C 为当前工程第一顺位：收紧 JWT audience、Claims 日志和 Auth transport security。
- Q0-D 统一 Markdown 编辑态与阅读态危险链接协议防护。
- Q0-C、Q0-D 继续保持独立代码批次，分别确认后实施，不与 E8-B 页面调整混合。
...
1. Q0-A、Q0-B 已完成并形成验证记录。
2. 单独确认并实施 Q0-C，收紧 audience、敏感 Claims 日志和 Auth transport security。
3. 随后按独立方案推进 Q0-D。
4. 执行 E8-B 有限产品矩阵；若需要页面级结构调整，先更新对应设计说明与 Pencil，再进入代码。
5. Q0 与 E8-B 收口后，刷新 `master...dev` 并准备集成 PR；不创建 tag，不部署。
6. 进入 `P3-12-F`，完成发布 Go 门禁与小规模受控试用，再根据真实使用证据判断是否结束第三阶段。
