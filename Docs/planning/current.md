...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
  ...
  - `P3-12-D62` Private / Author 已完成 `/workbench` 首批实现和 `/me` 内容历史复访组首批实现：通过 Pencil MCP 读取 `private-web-workflows.pen` 的 `P01 / P21` 与 `P03-P06 / P23`，`/workbench` 补继续处理队列、私域状态 rail 和 private 移动底栏承接；`/me/content`、`/me/history`、`/me/attachments`、`/me/experience` 已补私域子页任务面、指标、当前页预览 rail、公开详情来源返回、附件归属和经验流水上下文；记录见 [P3-12-D62 Private / Author Workbench 首批实现记录](/records/p3-12-d62-private-author-workbench-first-implementation-2026-07-04) 与 [P3-12-D62 `/me` 内容历史复访组首批实现记录](/records/p3-12-d62-me-content-history-first-implementation-2026-07-05)。不新增 API、权限、数据库、路由语义或提交载荷。
2. **保持 P3-10 可恢复合并状态**
...
- 继续推进 `P3-12-D62 Private / Author Pencil 逐页 UI 与功能缺口实现`：`/workbench` 与 `/me` 内容历史复访组已完成首批对齐，下一步建议进入资产 / 订单 / 背包 `P07-P11 / P24-P25`，覆盖 `/me/assets`、`/me/assets/transactions`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory` 的余额摘要、流水筛选、订单状态分组、订单详情时间线、权益状态和背包回流；随后处理通知 / 消息、圈子 / 宠物、论坛作者态和 Docs 作者态；只使用现有 API、现有路由语义和现有前端状态，完整钱包、退款 / 售后、支付口令、资产风控、完整聊天平台和复杂资料 / 安全设置继续作为 D62 内后置产品 / API 缺口记录。
