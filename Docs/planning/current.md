...
- **最近结论**：
  ...
  - `P3-12-B1` 账户资产与商城交易 Web 化方案与路由 / 登录回流契约首批代码已完成：新增正式 Web 资产与商城交易 return path，收窄 `/shop/*` 公开路由识别，并以 `/shop/product/:productId?intent=purchase` 承接公开商品购买登录回流；下一步继续复用既有组件和主题接入正式 Web 资产 / 交易页面，统一 UI 设计后置到页面迁移完成后的 `P3-12-D`，记录见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)。
...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
   ...
   - `P3-12-B1` 方案与路由 / 登录回流契约首批代码已完成；下一步复用既有组件和主题接入正式 Web 资产 / 交易页面，并在替代路径可用后替换直接冲突的 `/desktop` 回跳。
...
- `P3-12 Web 完全化与 WebOS 收束`
  ...
  - `P3-12-B1` 方案和路由 / 登录回流契约首批代码见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)；下一步复用既有组件和主题接入余额 / 流水、购买、订单、库存 / 权益和交易回流的正式 Web 页面。
...
- 第一顺位：复用既有组件和主题接入 `/me/assets`、`/me/assets/transactions`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory` 等正式 Web 资产 / 交易页面。
- 第二顺位：接通公开商品购买登录回流和购买成功回流，再替换 `/me` 完整钱包和公开商城购买中的 `/desktop` 回跳。
