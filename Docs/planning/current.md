...
- **当前子阶段**：`发布后长期维护与功能完成`
- **工程第一顺位**：`一对一私聊与会话管理批次 A：数据、成员 ACL、消息幂等与 Chat 附件访问`
- **产品下一顺位**：`会话请求生命周期、正式 Web 页面与成组验收`
- **复核日期**：`2026-07-18`
- **当前判断**：
...
1. 按 [一对一私聊与会话管理设计](/features/chat-direct-conversation-design) 实现批次 A：新增 `DirectConversation` 与 Chat ledger migration，补齐 REST / Hub / 附件共用的成员 ACL，并持久化 `clientRequestId` 发送幂等。
2. 批次 A 通过 SQLite / PostgreSQL、并发建会话和未授权访问测试后，推进会话请求生命周期、服务端权威分区与可靠附件绑定。
3. 后端边界稳定后先更新 `private-web-workflows.pen`，再实现公开个人页“发消息”和 `/messages` 的 PC / mobile 请求、归档与阻断交互。
