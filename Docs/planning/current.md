...
2. **选择 P3-10 下一批产品 / 治理增量**
   - 前端敏感日志脱敏已完成，不再作为下一步开发项。
   - 下一条更适合作为评审候选的是 [密码传输与请求签名临时评审](/guide/password-transport-and-request-signature) 中的“支付口令哈希升级”：只评审支付口令存储 / 验证兼容升级，不直接进入完整钱包、经济扩展、资产风控、安全会话、浏览器通用 `sign` 或开放 API 签名。
...
- `P3-10 后置安全治理评审：支付口令哈希升级`
  - 前端敏感日志脱敏已在 `a3d7df4f` 完成并转入维护线。
  - 下一候选建议先评审支付口令从当前 `SHA256(salt + password)` 迁移到 Argon2id 或等价慢哈希的兼容升级方案，重点确认版本字段、旧口令验证后写回、迁移风险、测试入口和发布回滚口径。
  - 该候选涉及密码存储、验证运行时行为和资产相关安全边界；进入代码前必须先确认方案，不顺带启动浏览器通用 `sign`、字段级加密、幂等体系、安全会话、完整钱包或经济扩展。
- `P3-10 阶段整理与自动化验证维护线`
...
- 第一顺位：若继续 P3-10 后置安全治理，先确认“支付口令哈希升级”方案边界，再决定是否进入代码。
