...
### 1. 进入 F1 商城商品效力与权益履约设计复核

- 先复核商城路线、支付与幂等说明、商城实体 / Service、正式 Web 商城和 Console 现有实现。
- 明确消耗品、权益商品和普通商品的状态模型，以及发放、激活、使用、过期、撤销与审计语义。
- 若现有专题说明不足，先补商品效力与权益履约设计，不直接进入代码。
...
1. 启动 `F1 商城商品效力与权益履约` 专题，代码前先复核并补齐对应设计边界。
...
4. 继续观察生产登录、聊天、通知、Console、证书复用和 DbMigrate 重入信号；没有新 P0/P1 时，进入 `F1 商城商品效力与权益履约` 的专题文档复核。
