...
- **当前阶段**：`截至 2026-04-16，第一开发阶段已于 2026-04-06 通过 v26.3.2-release 完成真实发布收口；第二开发阶段现已正式启动。`Phase 2-1 社区深化第一批` 当前已完成论坛轻回应墙 Phase 1 与最小回流链路收口，主线不再继续围绕论坛联调尾项扩张，而是正式切到 `Phase 2-2 移动 Web 形态`。forum 公开列表与帖子详情首批、公开 docs 目录与正文阅读首批、个人公开页首批、公开榜单首批、公开商城浏览首批与公开社区分发页首批当前都已达到可收口状态，并已转入稳定维护；forum 公开分类首批当前已接入 `/forum/category/:categoryId`，forum 公开标签首批当前已接入 `/forum/tag/:tagSlug`，forum 公开搜索首批当前已接入 `/forum/search`，docs 公开搜索首批当前也已接入 `/docs/search`。本轮已把公开壳层总基线、专题验收清单与回归索引收口到统一口径，默认经验榜单的规范路径当前为 `/leaderboard`，兼容直链 `/leaderboard/experience` 会自动收口回规范地址，同时保留“经验体系公开展示”说明区；`/discover` 首屏当前也已继续补到“摘要卡先预览本页区块、再决定是否直接进入公开页”的高价值只读阅读增强，并开始以 `section` 级来源回记稳定 forum / docs / leaderboard / shop 与“社区发现”之间的回流；若继续沿 `Phase 2-2` 前进，下一条更合适的增量任务应优先落在公开壳层的真实联调复核、来源返回稳定性观察与高价值只读阅读增强，而不是回头把桌面工作台里的个人经验明细或萝卜坑资产中心直接搬进公开壳层。`
- **复核日期**：`2026-04-16`
...
1. **公开内容壳层按入口逐批落地**
  ...
  - forum 当前已完成“公开列表 + 分类直达 + 标签直达 + 搜索直达 + 帖子详情”五条最小移动访问链路
   - 文档阅读首批已完成“公开目录 + 正文阅读 + 公开搜索”三条最小移动访问链路
   - 个人公开页首批已完成“公开资料 + 公开帖子 / 评论阅读”链路
   - 公开榜单、公开商城、公开社区分发页与 forum 公开搜索首批当前也已接入，继续沿公开内容壳层推进，而不是回头扩桌面治理能力
2. **移动阅读形态建立**
