...
- **当前阶段**：`截至 2026-04-15，第一开发阶段已于 2026-04-06 通过 v26.3.2-release 完成真实发布收口；第二开发阶段现已正式启动。`Phase 2-1 社区深化第一批` 当前已完成论坛轻回应墙 Phase 1 与最小回流链路收口，主线不再继续围绕论坛联调尾项扩张，而是正式切到 `Phase 2-2 移动 Web 形态`。forum 公开列表与帖子详情首批、公开 docs 目录与正文阅读首批、个人公开页首批、公开榜单首批、公开商城浏览首批与公开社区分发页首批当前都已达到可收口状态，并已转入稳定维护；forum 公开分类首批当前已接入 `/forum/category/:categoryId`，forum 公开标签首批当前已接入 `/forum/tag/:tagSlug`，forum 公开搜索首批当前已接入 `/forum/search`，docs 公开搜索首批当前也已接入 `/docs/search`。本轮已把公开壳层总基线、专题验收清单与回归索引收口到统一口径；若继续沿 `Phase 2-2` 前进，下一条更合适的增量任务是先在公开榜单内补“经验体系公开展示”说明，而不是回头把桌面工作台里的个人经验明细或萝卜坑资产中心直接搬进公开壳层。`
- **复核日期**：`2026-04-14`
...
- 公开榜单当前若继续扩展，下一条更合适的增量方向是“经验体系公开展示”说明，而不是直接开放“我的等级 / 经验交易记录”这类强登录个人明细
