...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
   ...
   - 第一开发顺位切到 `P3-12-B1 账户资产与商城交易 Web 化`，重点补正式 Web 资产 / 交易路径，并同步清理直接冲突的 `/desktop` 回跳。
...
- `P3-12 Web 完全化与 WebOS 收束`
  ...
  - 下一步推进 `P3-12-B1 账户资产与商城交易 Web 化`：优先补齐余额 / 流水、购买、订单、库存 / 权益和交易回流的正式 Web 路径。
...
- 第一顺位：推进 `P3-12-B1 账户资产与商城交易 Web 化` 的功能说明、路由边界和实现方案，先确认资产 / 商城正式 Web 入口如何承接余额、流水、购买、订单、库存和权益。
- 第二顺位：准备 B1 直接相关的 `/desktop` 回跳替换清单，避免在无 Web 替代路径时清理历史能力。
