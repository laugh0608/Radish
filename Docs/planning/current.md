...
1. **推进 P3-10-D Web 信息流 / UI 结构整理**
   - 首日公开页结构整理、首轮 Gateway PC / 移动运行态复核和四批同类入口语义治理已完成，当前转入阶段收束与合并前验证准备。
   ...
   - 不再默认追加第五批链接扫尾；若真实复核或验证命中明确缺口，再按同类问题成组修复。若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新设计源文件 / Pencil 或对应专题说明。
...
- `P3-10-D Web 信息流 / UI 结构整理`
  ...
  - 下一步优先进入 P3-10-D 收束与合并前验证准备：补齐批次结论、运行非运行态验证；前后端启动状态重新确认后，再集中执行 Gateway PC + 移动真实页面补验。
...
- 第一顺位：推进 `P3-10-D` 阶段收束与合并前验证准备，不再默认追加第五批链接语义扫尾。
- 第二顺位：开发中先跑非运行态验证、仓库卫生和必要构建；准备合并或用户重新确认前后端已启动后，再集中执行 baseline / identity / host runtime 与 Gateway PC + 移动真实页面 smoke。
- 第三顺位：真实 smoke、自动化验证或明确缺口命中后，只修阻断或清晰的一致性问题；若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新 Pencil / 设计源文件或对应功能说明。
