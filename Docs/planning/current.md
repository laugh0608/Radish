...
- **最近结论**：
  ...
  - `P3-12-B1` 账户资产与商城交易 Web 化方案已完成：建议新增 `/me/assets`、`/me/assets/transactions`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory` 等正式 Web 私域路由，并以 `/shop/product/:productId?intent=purchase` 承接公开商品购买登录回流；方案见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)。
...
1. **启动 P3-12 Web 完全化与 WebOS 收束**
   ...
   - `P3-12-B1` 方案已完成；下一步进入设计 / 实现确认，重点补正式 Web 资产 / 交易路径，并在替代路径可用后替换直接冲突的 `/desktop` 回跳。
...
- `P3-12 Web 完全化与 WebOS 收束`
  ...
  - `P3-12-B1` 方案见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)；下一步先确认是否需要 Pencil 设计稿，再补齐余额 / 流水、购买、订单、库存 / 权益和交易回流的正式 Web 路径。
  - 功能迁移只迁移正式版产品能力，不迁移 WebOS Dock、窗口系统、桌面背景、窗口几何记忆或桌面 app 外壳；B1 完成替代路径后再清理对应 `/desktop` 回跳。
  - UI 设计与美化专题统一走 Pencil 设计稿 -> 设计 / 说明文档 -> 代码实现 -> PC / mobile 复核。
...
- 第一顺位：基于 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)，先确认 `/me/assets` 与商城私域交易壳是否需要 Pencil 设计稿。
- 第二顺位：进入 B1 实现时，先补正式 Web 路由与登录回流契约，再替换 `/me` 完整钱包和公开商城购买中的 `/desktop` 回跳。
- 第三顺位：若 B1 涉及页面级 UI 设计或跨页面视觉重塑，必须先使用 Pencil 做设计稿，再更新设计 / 说明文档，最后进入实现。
