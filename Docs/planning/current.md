...
- **最近结论**：
  ...
  - `P3-11-A / B` 已完成发布候选验收矩阵与轻量复访缺口只读审计；当前 `dev` HEAD 为 `0e0cccd8`，`master..dev` 为 50 个提交，`46b9e8b2` 之后仅新增 P3-11 文档启动影响面。只读审计未命中需要立即进入代码回修的 `P0/P1` 或高信号阻断，不执行真实页面 smoke，不沿用历史服务启动状态。
...
1. **启动 P3-11 发布候选整备与轻量复访补齐**
   ...
   - `P3-11-A` 发布候选验收矩阵和 `P3-11-B` 轻量复访缺口只读审计已完成；当前未发现需要立即回修的阻断。
   - 下一顺位是 `P3-11-D` 恢复 PR / 发布候选决策：刷新 Git 范围，决定是否恢复 `dev -> master` PR 材料，或继续保持 PR 暂缓。
2. **保持 P3-10 可恢复合并状态**
...
- `P3-11 发布候选整备与轻量复访补齐`
  ...
  - `P3-11-A` 发布候选验收矩阵和 `P3-11-B` 轻量复访缺口只读盘点已完成；不直接启动 PR、发布、完整 Flutter 套件、电子宠物经济或第五批链接扫尾。
  - 当前不启动 `P3-11-C` 定向回修；下一步进入 `P3-11-D` 决策，若后续真实验收命中明确阻断，再回到定向修复。
- `P3-10 阶段收束准备维护线`
...
- 第一顺位：推进 `P3-11-D` 恢复 PR / 发布候选决策，刷新 Git 范围并复用 `P3-11-A / B` 记录给出范围、验证结论、剩余风险和是否建议创建 PR。
- 第二顺位：若新验证或用户真实复核命中明确缺口，再进入 `P3-11-C` 定向回修；否则继续保持 PR 暂缓和 P3-10 可恢复合并状态。
- 第三顺位：如需真实页面复核，必须先要求用户明确确认前后端已启动。
