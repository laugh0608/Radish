...
- **工程第一顺位**：`P3-12-E8-Q0-D Markdown 危险链接协议防护`
...
- Q0-D 为当前工程第一顺位：统一 Markdown 编辑态与阅读态危险链接协议防护。
- Q0-D 继续保持独立代码批次，单独确认后实施，不与 E8-B 页面调整混合。
...
1. Q0-A、Q0-B、Q0-C 已完成并形成验证记录。
2. 单独确认并实施 Q0-D，统一 Markdown 阅读态与编辑态危险链接协议防护。
3. 执行 E8-B 有限产品矩阵；若需要页面级结构调整，先更新对应设计说明与 Pencil，再进入代码。
4. Q0 与 E8-B 收口后，刷新 `master...dev` 并准备集成 PR；不创建 tag，不部署。
5. 进入 `P3-12-F`，完成发布 Go 门禁与小规模受控试用，再根据真实使用证据判断是否结束第三阶段。
