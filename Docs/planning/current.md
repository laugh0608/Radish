...
2. **选择 P3-10 下一批产品 / 治理增量**
   ...
   - 下一条候选已拆出 [支付与转账幂等治理](/guide/payment-idempotency-governance)：只治理商城购买和萝卜币转账的重复点击、网络重试和短窗口重放保护，不直接进入完整钱包、经济扩展、资产风控、安全会话、浏览器通用 `sign` 或开放 API 签名。
...
- `P3-10 后置安全治理评审：支付 / 转账幂等与重放边界`
  ...
  - [支付与转账幂等治理](/guide/payment-idempotency-governance) 已固定首批范围：商城购买和萝卜币转账接入 `idempotencyKey`、请求摘要绑定和成功结果重放。
  - 该候选涉及写操作运行时行为和资产一致性边界；进入代码前按专题文档确认实现顺序，不顺带启动浏览器通用 `sign`、字段级加密、安全会话、完整钱包、经济扩展或资产风控平台。
- `P3-10 阶段整理与自动化验证维护线`
...
- 第一顺位：按 [支付与转账幂等治理](/guide/payment-idempotency-governance) 确认实现顺序，再进入首批代码。
