...
> 状态：`进行中（B1-B8 已完成首批代码与维护收口；B9 用户身份语义首批代码完成，Gateway 运行态补验待宿主恢复）`
>
...
当前 `P3-10-B1`、`P3-10-B2`、`P3-10-B3`、`P3-10-B4 / B5`、`P3-10-B6`、`P3-10-B7 / C` 和 `P3-10-B8` 已完成首批代码推进与维护收口；公开返回文案、Console 高频入口、WebOS 历史应用迁移和电子宠物 Phase B 不再作为当前默认深挖方向。本轮不合并 B8，已继续完成 `P3-10-B9 用户身份语义与公开索引` 首批实现，并进入 `P3-10-B10 系统设置治理`。

`P3-10-B9` 首批代码已落地，覆盖 `User.PublicIndex`、公开显示句柄、注册 / 登录名与邮箱规范化、登录名或邮箱登录、公开资料 / 榜单 / 关系链 / 艾特搜索 / Console 用户排障展示和迁移入口。`P3-10-B10` 已完成设置定义注册表、默认值、覆盖值、风险等级、低 / 中风险编辑、修改原因 / 确认参数、变更审计历史、统一读取入口和首批帖子 / 评论长度设置接入；High / Critical 设置继续后置。
...
下一步从 B7 / B8 维护线转入 B9 运行态补验与后续缺口判断：
...
4. B9 后续只处理真实身份语义缺口：注册 / 登录文案、公开展示句柄、艾特搜索、榜单 / 关系链、Console 排障入口、迁移入口和 Gateway 页面补验。
...
3. `P3-10-C 下一批代码开发`（进行中）
   - B7 / B8 / B9 已进入维护收口；当前推进 `P3-10-B10 系统设置治理` 运行态联调、校验规则可视化和下一批设置开放评审。
   - 明确完成条件、验证入口和不做范围。
   - 不把评估项直接扩成全套能力建设。
