...
| `notification` | WebOS 通知中心完整管理，通知 Hub / API 已存在 | 首批高价值候选：纯 Web 通知复访入口，服务登录恢复、评论 / 神评 / 系统消息回流 | 优先进入下一组方案 |
...
下一批代码建议先从 `notification` 或“个人状态 + 经验 + 资产”二选一：

- 如果目标是提升日常复访和互动回流，优先做纯 Web 通知入口，覆盖系统通知、帖子互动、评论定位和登录恢复。
- 如果目标是为 `P3-10-B8` 电子宠物铺路，优先做登录后个人状态 / 成长页，聚合经验、胡萝卜资产和近期互动，但不直接实现完整宠物玩法。

两者都涉及新路由、登录态行为和可能的 API 组合，进入代码前需先给出页面策略、路由契约、验证范围和不做范围。
...
3. 迁移图首批代码优先从通知复访或个人状态 / 经验 / 资产入口中选择一组，避免同时启动聊天、钱包、个人中心和宠物玩法。
