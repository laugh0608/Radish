...
> 状态：`P3-10 后续产品 / 治理增量选择（P3-10-D 已收束，前端敏感日志脱敏已完成）`
>
...
3. 下一条后置安全治理候选建议先评审“支付口令哈希升级”：只评审支付口令从当前 `SHA256(salt + password)` 迁移到 Argon2id 或等价慢哈希的兼容升级，不顺带启动浏览器通用 `sign`、字段级加密、幂等体系、安全会话、完整钱包、经济扩展或资产风控扩面。
4. 若真实 smoke、自动化验证或人工复核新命中 P3-10-D 阻断 / 清晰一致性问题，按影响面定向回修；若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新 Pencil / 设计源文件或对应专题说明。
5. B10 进入维护观察线，不继续把候选配置默认排成新增开发批次。
6. 阶段验证进入维护线，后续按合并 / 发布候选需要集中执行自动化验证和真实页面补验；开发中优先跑构建、测试、类型检查、`git diff --check` 和仓库卫生检查。
7. 公开参与 / 登录恢复 / 来源返回矩阵、Console 高频治理入口、B9 用户身份语义和 B8 电子宠物进入维护线；只在真实缺口、发布候选回归或跨端承接需要时回拉。
8. `P3-10-B6` 保留补验入口：工具条件满足时补移动 DPR 视图；发布候选前再做真实 idle、通知 / 聊天 Hub 停连、评论匿名恢复和 Console 回跳批次级回归。
