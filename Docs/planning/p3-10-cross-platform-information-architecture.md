...
1. 明日优先推进 P3-10-D 阶段级收口复核，围绕公开壳层进入动作、分享 URL、登录回流、来源返回、公开 / 私域边界、标题层级、内容密度和入口动作一致性做静态矩阵。
2. 若只命中文案、状态、按钮或行为等价问题，按同类问题成组修复；若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新 Pencil / 设计源文件或对应专题说明。
