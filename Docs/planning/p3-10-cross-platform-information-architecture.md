...
6. B7 视觉参考以 [视觉主题规范](/frontend/visual-theme-spec) 与 [UI 设计灵感参考](/frontend/ui-design-inspiration) 中的淡雅古风 UI 口径为准，只借鉴视觉气质，不直接复制外部品牌、插画、命令框、页面结构或文案。
7. 不启动完整 E2E 平台，不把 Flutter 完整会话治理、完整通知偏好或设备管理并入本批；不把 WebOS `/desktop` 作为新增功能承载方向。
