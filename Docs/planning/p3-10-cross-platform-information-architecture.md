...
> 状态：`进行中（P3-10-B1 / B2 / B3 / B4 / B5 / B6 首批代码完成，B7 论坛详情轻互动首批回迁与 Gateway 补验完成）`
>
...
当前 `P3-10-B1`、`P3-10-B2`、`P3-10-B3`、`P3-10-B4 / B5` 和 `P3-10-B6` 已完成首批代码推进；`P3-10-B7` 已完成公共壳层一致性、论坛公开详情轻互动和 Gateway 页面补验。下一步继续 B7 公共头部动作策略与下一组高价值入口，不恢复 WebOS 作为新增功能默认承载方向。
...
第三批代码已完成公共头部动作策略收口：`PublicShellHeader` 统一支持社区发现、我的圈子和工作台三层动作；`/discover` 默认展示“我的圈子”作为登录态复访入口，论坛 / 文档 / 榜单 / 商城 / 公开个人页展示“社区发现 + 我的圈子 + 工作台”，`/circle` 自身只保留“社区发现 + 工作台”。移动端保留主动作文本，圈子和工作台次级动作转为图标，避免窄屏头部挤压；公开发现和论坛阅读边界文案同步改为“公开详情开放轻回应与根评论，评论回复 / 点赞 / 投票 / 治理留在工作台”。本轮完成 `radish.client` 构建和公开路由 / 登录回流契约测试；Gateway 页面复核因 `https://localhost:5000`、`http://localhost:3000`、`http://localhost:3100` 均未监听而待补。

## 后续开发建议

下一步继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，建议按以下顺序推进：

1. 服务恢复后先补公共头部 Gateway PC / 移动视图复核，覆盖 `/discover`、公开帖子详情、公开个人页和 `/circle` 的头部动作层级、无横向溢出、圈子登录回流与 `/desktop` 可达性。
...
3. 下一批代码继续选择同一组高价值入口推进，目标是让纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访和 Console 高频治理体验更一致；涉及运行时行为变化时继续通过 Gateway 覆盖 PC 与移动视图。
4. B7 不删除 `/desktop`，不把 WebOS `/desktop` 作为新增功能承载方向，不启动完整 E2E 平台，也不把 Flutter 完整会话治理、完整通知偏好或设备管理并入本批。
5. `P3-10-B6` 保留补验入口：工具条件满足时补移动 DPR 视图；发布候选前再做真实 idle、通知 / 聊天 Hub 停连、评论匿名恢复和 Console 回跳批次级回归。
6. 若 B7 审计发现需要跨页面视觉体系、页面结构或设计口径调整，先更新 Pencil / 设计稿；小范围状态、文案、按钮或等价行为修正不被该要求阻塞。
