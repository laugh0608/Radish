...
> 状态：`进行中（B1-B9 已完成首批代码与维护收口；B10 系统设置治理推进中）`
>
...
`P3-10-B9` 首批代码已落地，覆盖 `User.PublicIndex`、公开显示句柄、注册 / 登录名与邮箱规范化、登录名或邮箱登录、公开资料 / 榜单 / 关系链 / 艾特搜索 / Console 用户排障展示和迁移入口。`P3-10-B10` 已完成设置定义注册表、默认值、覆盖值、风险等级、低 / 中风险编辑、修改原因 / 确认参数、变更审计历史、统一读取入口、首批帖子 / 评论长度设置接入，以及校验规则元数据 / 数字编辑控件约束；High / Critical 设置继续后置。
