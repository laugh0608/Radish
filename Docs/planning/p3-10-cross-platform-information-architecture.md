...
> 状态：`进行中（B1-B10 已完成首批代码与阶段收束；等待下一主线选择）`
>
...
下一步从 B10 阶段收束转入 `P3-10` 后续主线选择：

1. B10 进入维护观察线，不继续把候选配置默认排成新增开发批次。
2. 优先做 P3-10 阶段整理，判断下一主线是 Web 信息流 / UI 结构整理、Flutter 承接准备，还是发布候选前自动化总验证。
3. 公开参与 / 登录恢复 / 来源返回矩阵、Console 高频治理入口、B9 用户身份语义和 B8 电子宠物进入维护线；只在真实缺口、发布候选回归或跨端承接需要时回拉。
4. B7 视觉参考以 [视觉主题规范](/frontend/visual-theme-spec) 与 [UI 设计灵感参考](/frontend/ui-design-inspiration) 中的淡雅古风 UI 口径为准；涉及跨页面视觉体系、页面结构或设计口径调整时先更新 Pencil / 设计稿，小范围状态、文案、按钮或行为等价修正不被该要求阻塞。
...
3. `P3-10-C 下一批代码开发`（进行中）
   - B7 / B8 / B9 / B10 已进入维护收口；当前等待 P3-10 后续主线选择。
   - 明确完成条件、验证入口和不做范围。
   - 不把评估项直接扩成全套能力建设。
