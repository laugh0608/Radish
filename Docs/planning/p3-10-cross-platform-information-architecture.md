...
4. [支付与转账幂等治理](/guide/payment-idempotency-governance) 首批代码已完成：商城购买、萝卜币转账已接入 `idempotencyKey`、请求摘要绑定和终态响应重放；后续转入验证收束与发布候选前回归线，不顺带启动浏览器通用 `sign`、字段级加密、安全会话、完整钱包、经济扩展或资产风控平台。
