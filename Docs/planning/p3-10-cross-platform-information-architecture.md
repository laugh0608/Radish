...
当前 `P3-10-B1`、`P3-10-B2`、`P3-10-B3`、`P3-10-B4 / B5`、`P3-10-B6`、`P3-10-B7 / C` 和 `P3-10-B8` 已完成首批代码推进与维护收口；公开返回文案、Console 高频入口、WebOS 历史应用迁移和电子宠物 Phase B 不再作为当前默认深挖方向。本轮不合并 B8，直接进入 `P3-10-B9 用户身份语义与公开索引` 首批实现。

`P3-10-B9` 首批代码已落地，覆盖 `User.PublicIndex`、公开显示句柄、注册 / 登录名与邮箱规范化、登录名或邮箱登录、公开资料 / 榜单 / 关系链 / 艾特搜索 / Console 用户排障展示和迁移入口；`P3-10-B10` 当前仍保持专题方案与后续排期状态。
...
3. `P3-10-C 下一批代码开发`（进行中）
   - B7 / B8 已进入维护收口；当前推进 `P3-10-B9 用户身份语义与公开索引` 首批代码和运行态补验。
   - 明确完成条件、验证入口和不做范围。
   - 不把评估项直接扩成全套能力建设。
