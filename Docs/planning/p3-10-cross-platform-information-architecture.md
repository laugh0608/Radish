...
> 状态：`进行中（P3-10-B1 / B2 / B3 / B4 / B5 / B6 首批代码完成；B7 / C 公开入口治理与 WebOS 功能迁移图进入收口，准备 B8 边界设计）`
>
...
| `notification` | WebOS 通知中心完整管理，通知 Hub / API 已存在 | 首批高价值迁移已推进：`/notifications` 纯 Web 登录态入口接入登录恢复、通知列表和目标分流；聊天通知转入 `/messages` | Gateway PC / 移动 smoke 待服务启动后补验，完整通知偏好仍后置 |
...
- Gateway 恢复后需要补 PC / 移动 DPR smoke：覆盖未登录回流、登录态频道列表、通知到 `/messages?channelId=...&messageId=...`、消息定位、公开个人页返回 `/messages` 和窄屏布局。

## 后续开发建议

下一步从 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图` 转入收口和 B8 边界设计：
...
4. 目标是让纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访、通知 / 消息 / 个人状态复访和 Console 高频治理体验更一致；涉及运行时行为变化时继续通过 Gateway 覆盖 PC 与移动视图。
