...
> 状态：`P3-10-D 进行中（Web 信息流 / UI 结构整理）`
>
...
当前已转入 `P3-10-D Web 信息流 / UI 结构整理`：

1. B10 进入维护观察线，不继续把候选配置默认排成新增开发批次。
2. Web 信息流 / UI 结构整理先确认页面结构、信息密度、Pencil / 设计边界和验证入口，再进入代码；大页面结构调整不绕过设计源文件。
3. 阶段验证进入维护线，后续按合并 / 发布候选需要集中执行自动化验证和真实页面补验。
4. 公开参与 / 登录恢复 / 来源返回矩阵、Console 高频治理入口、B9 用户身份语义和 B8 电子宠物进入维护线；只在真实缺口、发布候选回归或跨端承接需要时回拉。
5. `P3-10-B6` 保留补验入口：工具条件满足时补移动 DPR 视图；发布候选前再做真实 idle、通知 / 聊天 Hub 停连、评论匿名恢复和 Console 回跳批次级回归。
6. 密码传输、支付口令和请求签名加强暂不抢占 `P3-10-D` 主线；当前先按 [密码传输与请求签名临时评审](/guide/password-transport-and-request-signature) 进入后置治理，只有前端敏感日志脱敏这类低风险维护项可单独回拉处理。
...
3. `P3-10-C 下一批代码开发`（进行中）
   ...
   - 明确完成条件、验证入口和不做范围。
   - 不把评估项直接扩成全套能力建设。
4. `P3-10-D Web 信息流 / UI 结构整理`（当前主线）
   - 先确认 `/discover`、公开论坛详情、公开个人页、`/circle`、`/me` 和轻互动入口的信息结构与视觉层级。
