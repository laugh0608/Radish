...
> 状态：`进行中（P3-10-B1 / B2 / B3 / B4 / B5 / B6 首批代码完成，B7 论坛详情轻互动首批回迁完成）`
>
...
`P3-10-B9 / B10` 当前只完成专题方案落档，用于后续排期。它们不抢占 B6 页面真实联调，也不要求立即进入代码实现。
...
- 公共 Web 入口已共享 `PublicShellHeader`，但所有公共页头部仍显式展示 `/desktop` 工作台入口；论坛详情“发送轻回应 / 参与讨论”已首批从 `/desktop?app=forum...` 回迁到纯 Web 详情页，登录回流只放开带 `intent=comment|quickReply` 的 `/forum/post/:id`。
...
- 涉及 `/desktop` 入口露出策略、公开详情继续扩展评论回复 / 点赞 / 投票等更重交互，或公共页头部主动作调整时，先输出页面策略再改代码；不在当前批次里直接删除 `/desktop` 或扩大工作台级动作。
...
第二批代码已完成论坛公开详情轻互动回迁：公开详情页登录后可直接发布轻回应和根评论，评论 typing 继续复用评论 Hub；评论回复、点赞、投票、编辑和治理动作仍留在工作台边界。本轮完成定向契约测试与 `radish.client` 构建；Gateway / Chrome 页面复核因本地 `https://localhost:5000`、`http://localhost:5173`、`http://localhost:3000` 未监听而未执行，待服务由开发者手动启动后补验。

## 后续开发建议

下一步继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，建议按以下顺序推进：

1. 先在 Gateway 下补验论坛公开详情 PC / 移动视图，覆盖未登录点击参与回流、登录后轻回应 / 根评论发布、评论 Hub 匿名恢复和页面前台恢复。
...
3. 下一批代码继续选择同一组高价值入口推进，目标是让纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访和 Console 高频治理体验更一致。
