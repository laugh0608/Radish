...
> 状态：`进行中（P3-10-B1 / B2 / B3 / B4 / B5 / B6 首批代码完成；B7 / C 公开入口治理收口，进入 WebOS 功能迁移图收口）`
>
...
当前 `P3-10-B1`、`P3-10-B2`、`P3-10-B3`、`P3-10-B4 / B5` 和 `P3-10-B6` 已完成首批代码推进；`P3-10-B7 / C` 已完成公共壳层一致性、论坛公开详情轻互动、公共头部动作策略、公开内容链接契约、圈子来源返回、公开参与 / 登录恢复 / 来源返回矩阵和 Console 高频治理入口。下一步不再继续围绕同一批公开返回文案和 Console 高频入口反复深挖，转入 B7 的 WebOS 功能迁移图收口，不恢复 WebOS 作为新增功能默认承载方向。
...
| `chat` | 当前主要在 WebOS 窗口和 Hub 中工作 | 候选迁移：登录后消息 / 会话复访入口，需先定路由和通知关系 | 可作为首批候选，但不做完整聊天平台重构 |
| `profile` | `/u/usr_...` 覆盖公开主页，Flutter 已承接部分“我的”路径 | 候选迁移：登录后个人状态页，聚合资料、经验、资产、最近访问和关系链 | 可与经验 / 资产一起评估 |
| `radish-pit` | WebOS 承接钱包、转移、流水、统计、安全；Flutter 已补资产 / 流水主路径 | 候选迁移：经济系统轻量状态和流水直达；转移 / 安全设置保持审慎 | 等电子宠物或经济玩法需要时前置 |
| `notification` | WebOS 通知中心完整管理，通知 Hub / API 已存在 | 首批高价值迁移已推进：`/notifications` 纯 Web 登录态入口接入登录恢复、通知列表和目标分流 | Gateway PC / 移动 smoke 待服务启动后补验，完整通知偏好仍后置 |
| `leaderboard` | `/leaderboard` 公开排行已覆盖 | 已迁移，保留维护 | 不进入首批 |
| `experience-detail` | WebOS 承接登录态经验详情，公开榜单只展示排行 | 候选迁移：并入登录后个人状态 / 成长页 | 与 B8 电子宠物前置关系一起评估 |
...
- 通知目标优先分流到纯 Web 可承接的公开帖子详情、公开个人页和论坛列表；订单、聊天等私域或工作台能力继续回到 `/desktop` 深链。
- 本批不做完整通知偏好、完整聊天平台、移动系统通知或通知聚合规则重构。

下一批代码建议转向“个人状态 + 经验 + 资产”，或在 Gateway 恢复后先补 `/notifications` PC / 移动 smoke：

- 如果目标是为 `P3-10-B8` 电子宠物铺路，优先做登录后个人状态 / 成长页，聚合经验、胡萝卜资产和近期互动，但不直接实现完整宠物玩法。

个人状态 / 成长页涉及新路由、登录态行为和可能的 API 组合，进入代码前需先给出页面策略、路由契约、验证范围和不做范围。

## 后续开发建议

下一步继续 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，但推进重心从公开入口细节治理切换为 WebOS 功能迁移图收口：
...
3. 迁移图首批代码已从通知复访开始；下一组优先评估个人状态 / 经验 / 资产入口，避免同时启动聊天、钱包、个人中心和宠物玩法。
4. 目标是让纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访、登录后复访和 Console 高频治理体验更一致；涉及运行时行为变化时继续通过 Gateway 覆盖 PC 与移动视图。
...
7. 若 B7 迁移发现需要跨页面视觉体系、页面结构或设计口径调整，先更新 Pencil / 设计稿；小范围状态、文案、按钮或等价行为修正不被该要求阻塞。
