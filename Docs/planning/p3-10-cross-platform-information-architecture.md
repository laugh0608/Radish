...
> 状态：`P3-10 阶段收束准备（论坛内容发布可靠性与后续增量评审完成）`
>
...
1. **明确 Web-first 产品入口**
   ...
   - Tauri：现已正式弃用，只保留历史实验资产；长期桌面安装包由 Flutter Native 分阶段承接。
...
2026-06-22 后按 P3-12-B6 数据库结构口径处理：上线前不再为 `User.PublicId` 维护历史发布脚本；当前以开发库 `DbMigrate` 路径和本地验证为准，存在正式数据库后再生成发布 SQL。
...
- `DbMigrate` 当时补齐旧库 `PublicIndex` 列、种子保留索引纠偏、旧用户回填和唯一索引创建；2026-06-22 后由 `P3-12-B6` 接管身份语义收口，上线前不再维护该阶段历史发布脚本，本地 SQLite 开发库继续按当前实体和 `Radish.DbMigrate init/apply` 处理。
...
4. [支付与转账幂等治理](/guide/payment-idempotency-governance) 首批代码、分层验证和本地提交已完成：商城购买、萝卜币转账已接入 `idempotencyKey`、请求摘要绑定和终态响应重放；后续转入发布候选前回归线。
5. 新增 [写操作可靠性与并发保护治理](/guide/write-operation-reliability-governance)，作为支付 / 转账幂等之后的写操作分级入口。下一步建议先做 `WOG-1 写操作分级盘点`，只读代码和文档梳理写入口，再选择首批代码候选。
6. 若真实 smoke、自动化验证或人工复核新命中 P3-10-D 阻断 / 清晰一致性问题，按影响面定向回修；若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新 Pencil / 设计源文件或对应专题说明。
7. B10 进入维护观察线，不继续把候选配置默认排成新增开发批次。
8. 阶段验证进入维护线，后续按合并 / 发布候选需要集中执行自动化验证和真实页面补验；开发中优先跑构建、测试、类型检查、`git diff --check` 和仓库卫生检查。
9. 公开参与 / 登录恢复 / 来源返回矩阵、Console 高频治理入口、B9 用户身份语义和 B8 电子宠物进入维护线；只在真实缺口、发布候选回归或跨端承接需要时回拉。
10. `P3-10-B6` 保留补验入口：工具条件满足时补移动 DPR 视图；发布候选前再做真实 idle、通知 / 聊天 Hub 停连、评论匿名恢复和 Console 回跳批次级回归。
11. [论坛内容发布可靠性与编辑历史治理](/guide/forum-content-write-reliability-governance) 已按确认方案完成创建链路、首批帖子 / 评论编辑重试幂等、创建类频率限制，以及 Flutter 纯文本发帖 / 根评论 / 回复 / 问答回答 / 作者帖子正文编辑 / 作者根评论编辑写入口承接；[P3-10 后续产品 / 治理增量评审记录](/records/p3-10-next-product-governance-review-2026-06-21) 已确认 Flutter 子评论编辑和回答编辑不进入下一批默认代码，[P3-10 阶段收束准备记录](/records/p3-10-stage-closure-prep-record-2026-06-21) 已补恢复 PR 前的完整范围和验证顺序。
...
6. 写操作重复提交、并发冲突、终态响应或多端重试语义不清的问题。
7. 明显影响可读性、可操作性或主路径信任感的 UI 断裂。

上一批候选 [论坛内容发布可靠性与编辑历史治理](/guide/forum-content-write-reliability-governance) 已完成当前确认范围。`2026-06-21` 后续产品 / 治理增量评审已确认不把 Flutter 子评论编辑和回答编辑列为下一批默认代码；P3-10 下一步转向阶段收束准备和入口文档同步。恢复 `dev -> master` 前必须刷新 `master..dev` 完整提交范围，继续优先于扩展 Flutter 转账、完整移动商城、B10 第十批设置和 P3-10-D 第五批链接扫尾。
