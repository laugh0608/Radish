...
> 状态：`进行中（P3-10-B1 / B2 / B3 / B4 / B5 / B6 首批代码完成，B6 Gateway 首轮真实联调完成，准备进入 B7）`
>
...
- 公共 Web 入口已共享 `PublicShellHeader`，但所有公共页头部仍显式展示 `/desktop` 工作台入口；论坛详情还把“发送轻回应 / 参与讨论”引导到 `/desktop?app=forum...`。这符合当前只读边界，但与 B7 后续“公开页轻互动尽量留在纯 Web”的方向存在迁移决策点。
...
- 涉及 `/desktop` 入口露出策略、公开详情参与动作是否从 WebOS 回迁到纯 Web、或公共页头部主动作调整时，先输出页面策略再改代码；不在清理批次里直接删除 `/desktop` 或改变登录 / 参与流程。
...
下一步进入 `P3-10-B7 Web UI 改造与去 WebOS 化迁移图`，建议按以下顺序推进：

1. 先审 `/discover`、`/forum` 详情、`/u/usr_...`、`/circle` 和 Console 高频入口，输出页面层级、设计边界、验证矩阵和首批代码入口。
...
3. 首批代码优先选择同一组高价值入口推进，目标是让纯 Web 默认入口、公开详情阅读、个人页 / 圈子复访和 Console 高频治理体验更一致。
