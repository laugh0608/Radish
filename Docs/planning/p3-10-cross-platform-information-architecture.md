...
> 状态：`进行中（P3-10-B1 / B3 / B4 / B5 首批代码完成，准备双用户联调与下一候选收口）`
>
...
明日第一顺位围绕 `P3-10-B4 / B5` 做联调收口，建议按以下顺序推进：

1. 使用 Gateway 下的 Web 详情页做双用户人工验证，覆盖评论创建 / 更新 / 删除 / 点赞 / typing / 神评变化和公开详情页匿名订阅。
2. 若联调只暴露小缺口，按评论实时链路就地修正；若暴露 Hub 鉴权、断线恢复或历史高亮数据兼容问题，优先按根因治理。
3. 评论链路稳定后，进入 `P3-10-B2` 个人圈子边界和 `P3-10-B6` Token 不活跃过期方案，先写清 URL、对象类型、PublicId、隐私与 session 判定口径。
4. 不启动完整 E2E 平台，不把 Flutter 评论实时、完整个人圈子或完整通知偏好治理并入本批。
