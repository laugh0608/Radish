...
> 状态：`进行中（P3-10-B1 / B3 首批完成，准备推进 P3-10-B4 / B5）`
>
...
当前 `P3-10-B1`、`P3-10-B3` 已完成首批代码推进；下一批优先进入 `P3-10-B4 / B5`，把评论实时、编辑状态提示和神评 / 沙发稳定性从方案收口推进到 Web-first 代码实现。
...
明日第一顺位进入 `P3-10-B4 / B5`，建议按以下顺序推进：

1. 先复核 `CommentController / CommentService / CommentHighlight`、现有 `NotificationHub / ChatHub` 和 Web 帖子详情评论区，确认评论实时事件与神评稳定性应落在哪些边界。
2. 评论实时先服务 Web-first 公开帖子详情链路：明确 hub 路径、post group 命名、评论创建 / 更新 / 删除 / typing 事件、前端去重和断线恢复后的刷新策略。
3. 神评 / 沙发治理先收敛后端 current 语义和前端展示：稳定窗口、并列展示、替换阈值、奖励幂等和旧数据兼容要一起定义。
4. 验证入口优先覆盖双用户 Web 链路、评论写入后刷新 / 推送一致性、同赞数展示和既有神评奖励路径；不启动完整 E2E 平台，不把 Flutter 或完整个人圈子并入本批。
