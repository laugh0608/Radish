...
2. 下一步优先从后置治理候选中推进前端敏感日志脱敏：只收敛 `paymentPassword / password / currentPassword / newPassword / confirmPassword` 等敏感字段日志，不扩展为浏览器通用 `sign`、字段级加密、支付口令慢哈希迁移、幂等体系或安全会话治理。
3. 若真实 smoke、自动化验证或人工复核新命中 P3-10-D 阻断 / 清晰一致性问题，按影响面定向回修；若触达大页面结构、跨页面视觉体系或端点级视觉治理，先更新 Pencil / 设计源文件或对应专题说明。
4. B10 进入维护观察线，不继续把候选配置默认排成新增开发批次。
5. 阶段验证进入维护线，后续按合并 / 发布候选需要集中执行自动化验证和真实页面补验；开发中优先跑构建、测试、类型检查、`git diff --check` 和仓库卫生检查。
6. 公开参与 / 登录恢复 / 来源返回矩阵、Console 高频治理入口、B9 用户身份语义和 B8 电子宠物进入维护线；只在真实缺口、发布候选回归或跨端承接需要时回拉。
7. `P3-10-B6` 保留补验入口：工具条件满足时补移动 DPR 视图；发布候选前再做真实 idle、通知 / 聊天 Hub 停连、评论匿名恢复和 Console 回跳批次级回归。
