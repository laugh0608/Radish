...
该评估不要求 Web 或 Flutter desktop 照搬移动端底部栏。跨端一致性优先体现在任务归属、入口命名、登录恢复、返回语义和错误状态上；Web 端继续保持纯 Web 公开访问与 WebOS `/desktop` 保留入口，Flutter desktop 后续按 expanded 导航、键鼠、焦点和窗口模型独立设计，不复刻工作台 / Dock 心智。
...
- 不让 WebOS、Tauri 或脱离明确移动价值的 Flutter 工作抢占 Web 优先主线。
