...
> 状态：`进行中（自动化总回归已通过，Flutter 商城 / 钱包人工复核缺口已修复，待复测与剩余人工复核）`
>
...
本口径属于后续产品信息架构规划，不进入当前 `P3-9-B` 修复范围；当前只处理人工复核暴露的登录态商城、余额和失败态阻断。

## 当前人工复核状态

截至 `2026-06-07` 晚间，自动化总回归已通过，人工复核显示发现 / 论坛 / 文档 / 我的等路径基本正常；Flutter 登录态商城 / 钱包路径暴露真实缺口：商品详情购买检查遇到非预期响应时只显示“接口返回格式异常”，且购买面板没有展示当前余额。

本轮已修复 Flutter API 失败态识别、登录态商品详情余额读取和购买后余额刷新，并补充定向测试。下一步由用户启动本地服务和 Android 模拟器复测商品详情余额、购买资格检查、单商品购买、订单详情、背包发放、钱包余额与流水；若无新增 `P0/P1`，转入 `PR -> master` 前扩大验证与合并材料准备。
...
5. `P3-9-E 发布候选路径总回归与收口结论`
   ...
   - 自动化总回归已通过；Flutter 商城 / 钱包人工复核缺口已修复，剩余人工复核和该缺口复测待执行。
