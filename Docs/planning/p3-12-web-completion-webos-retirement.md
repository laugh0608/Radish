...
`P3-12-B1` 已完成账户资产与商城交易 Web 化方案梳理，结论见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)。首批建议新增 `/me/assets`、`/me/assets/transactions`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory` 等正式 Web 私域路由，并让 `/shop/product/:productId?intent=purchase` 承接公开商品购买登录回流；`/me` 完整钱包和公开商城购买的 `/desktop` 回跳应在替代路径实现后替换。
...
2. `P3-12-B1`：推进账户资产与商城交易 Web 化，方案已完成，见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)；下一步先确认是否需要 Pencil 设计稿，再补正式 Web 资产 / 交易路径并替换 `/desktop` 回跳。
3. `P3-12-C1`：清理与 B1 直接冲突的 WebOS 残留入口。
4. `P3-12-D1`：B1 若涉及页面级 UI 设计或跨页面视觉重塑，先做 Pencil 设计稿，再补说明文档并实现。
