...
`P3-12-D36-D38` 已完成 UI 专题退出口径、四类设计源差距矩阵、边界裁决与阶段验收清单：public / private / foundation / console 四类设计源均已对齐真实路由、当前代码状态、验证证据和剩余动作。D38 明确公开首页 `P01` 发布前由 `/discover` 承接，公开聊天室 `P15 / P16`、Console 内部调度中心、内部 Jobs 平台和独立移动 Console 应用后置；下一步进入 `P3-12-D39` Gateway PC / mobile 阶段验收准备，发布候选准备继续后置。
...
25. `P3-12-D39+`：准备并执行 Gateway PC / mobile 阶段验收；真实联调只在用户确认前后端已启动后集中执行。
26. `P3-12-E`：P3-12-D UI 专题退出条件确认后再进入正式版发布候选，不提前创建发布 tag。
