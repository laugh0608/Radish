...
> 状态：`P3-12-B1 代码侧主路径已完成；P3-12-C1 首轮 B1 直接残留清理已完成；P3-12-B2 完整个人中心 Web 化首批代码和正式链接语义补口已完成；P3-12-B3 论坛作者态 Web 化首批代码与小阶段验收已完成；P3-12-B4-1 正式 Web 文档作者入口首批代码已完成；P3-12-B4-2 Console 文档治理首批代码已完成；B4 / D1 阶段运行态 smoke 已完成；P3-12-B5 Web 功能总入口首批代码与 Gateway smoke 已完成；P3-12-B6-1 身份基础与注册登录首批代码已完成；P3-12-D1 统一 UI 设计准备已启动`
>
...
`P3-12-D1` 统一 UI 设计准备已启动，结论见 [P3-12-D1 统一 UI 设计准备记录](/records/p3-12-d1-unified-ui-design-prep-2026-06-22)。当前只完成页面矩阵、设计源拆分、停止线和后续执行顺序；Pencil app 暂未连接，因此本轮不创建 `.pen`、不修改设计源、不进入视觉代码。用户确认前后端已启动后，B4 / D1 阶段 Gateway PC / mobile smoke 已补齐，结论见 [P3-12 B4 / D1 阶段运行态 Smoke 记录](/records/p3-12-b4-d1-stage-smoke-record-2026-06-22)。

`P3-12-B5` 已确认改为 Web 功能总入口专题，结论见 [P3-12-B5 Web 功能总入口设计](/records/p3-12-b5-web-workbench-entry-design-2026-06-22)。`/messages` 已作为正式 Web 消息 / 聊天入口存在，本批不重做聊天室；当前缺口是公共壳层缺少清晰的功能地图，导致用户难以发现已迁移功能。首批建议新增 `/workbench`，把公共壳层“工作台”指向正式 Web 功能总览，并把 `/desktop` 降级为“桌面版 / WebOS 历史入口”功能项。

`P3-12-B6` 身份语义二次收口设计已确认，`B6-1 身份基础与注册登录` 首批代码已完成，结论见 [P3-12-B6 身份语义二次收口设计](/records/p3-12-b6-identity-contract-convergence-design-2026-06-22)。本专题从 B5 拆出独立推进：登录凭证改为邮箱 + 密码，注册必须填写 `DisplayName`；`DisplayName` 只允许中文、英文字母和数字，`DisplayHandle = DisplayName#PublicIndex` 作为用户可见唯一身份、搜索和艾特主键；`PublicId` 只用于 URL、分享和前后端传递；`LoginName`、`UserRealName` 公开回退和 `usr_...` 普通资料页展示必须退场。当前项目尚未上线且无正式数据库，B6 按破坏性 schema 收口处理，不为旧本地 SQLite 编写兼容迁移；后续继续推进 `B6-2` 公开展示与前端状态收敛、邮箱白名单 Console 设置、展示名修改频率限制、关注备注、种子数据和上线前数据库结构口径收束。
...
7. `P3-12-B5`：Web 功能总入口设计已确认，见 [P3-12-B5 Web 功能总入口设计](/records/p3-12-b5-web-workbench-entry-design-2026-06-22)。下一步先实现 `/workbench` 和公共壳层入口调整，再集中验收正式 Web 功能发现路径。
8. `P3-12-B6`：身份语义二次收口设计已确认，`B6-1` 注册登录与 CurrentUser / OIDC 首批收敛已完成，见 [P3-12-B6 身份语义二次收口设计](/records/p3-12-b6-identity-contract-convergence-design-2026-06-22)。B6 影响 Auth、注册登录、种子数据、Console 设置、公开展示、搜索 / 艾特和数据迁移，后续继续按 `B6-2` 至 `B6-6` 分批实现与验证。
9. `P3-12-D1`：统一 UI 设计准备已启动，见 [P3-12-D1 统一 UI 设计准备记录](/records/p3-12-d1-unified-ui-design-prep-2026-06-22)；待 B5 / B6 收口后，再在 Pencil 可用时创建 / 更新设计源，补说明文档并实现视觉专题。
