...
`P3-12-D15-D19` `radish.console` 表格与治理页面迁移已连续推进：订单、用户、商品三类高频表格代表页已迁入 D14 语义组件；文档治理页先固定首屏区块边界，再迁移页头、指标和筛选区；标签 / 分类普通列表已完成同口径迁移并补 Gateway PC / 移动 CSS 视图真实联调。上述批次均保持 API、权限、业务动作、筛选参数、分页、表格列、批量操作和后端契约不变。下一步优先评估贴纸分组 / 贴纸列表是否可作为同类普通列表继续迁移；若素材、分组详情或上传预览会改变工作流，应先拆清边界再进入代码。
...
17. `P3-12-D15-D19`：`radish.console` 订单、用户、商品、文档治理首屏、标签和分类页面已完成视觉语义迁移，D19 已补 Gateway PC / 移动 CSS 视图真实联调；下一步评估贴纸类列表迁移或 Console 表格视觉成组静态收口。
