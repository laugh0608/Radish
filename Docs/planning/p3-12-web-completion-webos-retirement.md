...
> 状态：`P3-12-B1 路由 / 登录回流契约与商城私域正式 Web 入口已完成，继续推进资产页、公开购买回流和 /desktop 回跳替换；统一 UI 设计后置到页面迁移完成后的 P3-12-D`
>
...
`P3-12-B1` 已完成账户资产与商城交易 Web 化方案梳理，结论见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)。首批代码已完成路由 / 登录回流契约和商城私域正式 Web 入口：新增正式 Web 资产与商城交易 return path，收窄 `/shop/*` 公开路由识别，并接入 `/shop/orders`、`/shop/order/:orderId`、`/shop/inventory`；下一步继续复用既有组件和主题接入资产页、公开购买回流和 `/desktop` 回跳替换，统一 UI 设计放到页面迁移齐后的 `P3-12-D`。
...
2. `P3-12-B1`：推进账户资产与商城交易 Web 化，方案、路由 / 登录回流契约和商城私域正式 Web 入口已完成，见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)；下一步复用既有组件和主题接入资产页、公开购买回流，并在替代路径可用后替换 `/desktop` 回跳。
