...
`P3-12-D36-D43` 已完成 UI 专题退出口径、四类设计源差距矩阵、边界裁决、阶段验收清单、Gateway PC / mobile 阶段验收、退出判断修正、页面开发缺口源码核对、Docs 作者态真实动作代码侧补漏和 Public / Private 主路径代码侧复核：public / private / author / console 主要页面族均有真实路由和组件，公开论坛详情工作区动作已补真实 intent `href`，D38 后置项不重新拉回当前批次。下一步按 D41 排序推进 Console 深层管理动作复核，再进入移动响应式抽样。
...
30. `P3-12-D44`：按 D41 排序推进 Console 深层管理动作复核，再进入移动响应式抽样。
31. `P3-12-E`：P3-12-D UI 专题真实退出后再进入正式版发布候选，不提前创建发布 tag。
