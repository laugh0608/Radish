...
10. `P3-12-D8`：`radish.client` 视觉实现首批已完成，共享壳层、状态槽、公开内容宽度 token 和移动底部留白已进入代码；下一步继续 client 第二批私域 / 作者态视觉实现，Console 代码实现后移。
11. `P3-12-E`：阶段完成后再进入正式版发布候选，不提前创建发布 tag。
