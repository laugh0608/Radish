...
`P3-12-D36 / D37` 已完成 UI 专题退出口径与差距矩阵：public / private / foundation / console 四类设计源均已对齐真实路由、当前代码状态、验证证据和剩余动作。当前明确差距包括公开聊天室 `P15 / P16` 尚无正式 `/chat` 路由、公开首页 `P01` 由 `/discover` 概念承接、Console `P04 / P13 / P18` 若要成为内部调度 / 运维平台必须先补数据来源与 API 契约，以及移动 Console 画板当前主要作为响应式验收参考。下一步进入 `P3-12-D38` 边界裁决与阶段验收清单整理，发布候选准备继续后置。
...
24. `P3-12-D38+`：先裁决公开聊天室、公开首页语义、Console 内部调度 / 运维平台和移动 Console 画板是否进入发布前范围，再成组推进代码治理、Pencil / 说明回拉或 Gateway PC / mobile 阶段验收；真实联调只在用户确认前后端已启动后集中执行。
25. `P3-12-E`：P3-12-D UI 专题退出条件确认后再进入正式版发布候选，不提前创建发布 tag。
