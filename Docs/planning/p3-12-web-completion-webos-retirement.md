...
`P3-12-D15-D20` `radish.console` 表格与治理页面迁移已连续推进：订单、用户、商品三类高频表格代表页已迁入 D14 语义组件；文档治理页先固定首屏区块边界，再迁移页头、指标和筛选区；标签 / 分类普通列表已完成同口径迁移并补 Gateway PC / 移动 CSS 视图真实联调；贴纸分组 / 分组表情列表已确认按普通 CRUD 外层迁移，不拆媒体资产工作台。上述批次均保持 API、权限、业务动作、筛选参数、分页、表格列、批量操作、上传流程、排序保存和后端契约不变。下一步做 Console 表格视觉成组静态收口检查。
...
17. `P3-12-D15-D20`：`radish.console` 订单、用户、商品、文档治理首屏、标签 / 分类和贴纸类页面已完成视觉语义迁移，D19 已补 Gateway PC / 移动 CSS 视图真实联调；下一步做 Console 表格视觉成组静态收口。
