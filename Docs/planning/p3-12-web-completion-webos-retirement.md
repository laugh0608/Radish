...
17. `P3-12-D15-D29`：`radish.console` 订单、用户、商品、文档治理首屏、标签 / 分类、贴纸类页面、角色权限页面、内容 / 经验治理工作台外层语义、点名内部区块样式迁移、治理页面成组静态收口、系统工具 / 运维外壳收口、阶段静态收口和深层表单静态收口首批已完成，D19 已补 Gateway PC / 移动 CSS 视图真实联调，D22 已完成复杂 Console 页面类型边界评估，D27 已确认 `/hangfire` 仍为外部 Dashboard 承载。
18. `P3-12-D30`：按收益选择详情 / 抽屉静态收口，或在用户确认前后端已启动后进入阶段真实验收。
