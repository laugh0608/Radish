...
23. `P3-12-D37+`：按 D36 矩阵继续成组推进代码治理、Pencil / 说明回拉和 Gateway PC / mobile 阶段验收；真实联调只在用户确认前后端已启动后集中执行。
24. `P3-12-E`：P3-12-D UI 专题退出条件确认后再进入正式版发布候选，不提前创建发布 tag。
