...
`P3-12-D36-D39` 已完成 UI 专题退出口径、四类设计源差距矩阵、边界裁决、阶段验收清单和 Gateway PC / mobile 阶段验收：public / private / foundation / console 四类设计源均已对齐真实路由、当前代码状态、验证证据和剩余动作。D38 明确公开首页 `P01` 发布前由 `/discover` 承接，公开聊天室 `P15 / P16`、Console 内部调度中心、内部 Jobs 平台和独立移动 Console 应用后置；D39 未发现阻断级真实页面问题，下一步进入 `P3-12-D40` UI 专题退出判断，发布候选准备继续后置到退出结论之后。
...
26. `P3-12-D40+`：整理 UI 专题退出判断、剩余风险和发布候选前置清单。
27. `P3-12-E`：P3-12-D UI 专题退出条件确认后再进入正式版发布候选，不提前创建发布 tag。
