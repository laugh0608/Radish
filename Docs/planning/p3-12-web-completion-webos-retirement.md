...
`P3-12-D36-D40` 已完成 UI 专题退出口径、四类设计源差距矩阵、边界裁决、阶段验收清单、Gateway PC / mobile 阶段验收和退出判断：public / private / foundation / console 四类设计源均已对齐真实路由、当前代码状态、验证证据和剩余动作。D38 明确公开首页 `P01` 发布前由 `/discover` 承接，公开聊天室 `P15 / P16`、Console 内部调度中心、内部 Jobs 平台和独立移动 Console 应用后置；D39 未发现阻断级真实页面问题，D40 已确认 UI 专题具备退出条件，下一步进入 `P3-12-E1` 发布候选准备。
...
26. `P3-12-D40`：完成 UI 专题退出判断、剩余风险和发布候选前置清单，记录见 [P3-12-D40 UI 专题退出判断](/records/p3-12-d40-ui-topic-exit-decision-2026-07-01)。
27. `P3-12-E1`：进入正式版发布候选准备，先整理验证矩阵、PR 范围、剩余风险和回滚口径，不提前创建发布 tag。
