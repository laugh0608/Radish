...
> 状态：`P3-12-B1 路由 / 登录回流契约、商城私域正式 Web 入口和资产正式入口已完成，继续推进公开购买回流和公开商城 /desktop 回跳替换；统一 UI 设计后置到页面迁移完成后的 P3-12-D`
>
...
- P3-12-A 盘点时 `/me` 完整钱包入口仍指向 `/desktop?app=radish-pit`，公开商城购买仍回桌面商城窗口；当前 B1 已先把 `/me` 完整钱包替换到 `/me/assets/transactions`，公开商城购买仍待替换。
...
`P3-12-B1` 已完成账户资产与商城交易 Web 化方案梳理，结论见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)。首批代码已完成路由 / 登录回流契约、商城私域正式 Web 入口和资产正式入口：新增正式 Web 资产与商城交易 return path，收窄 `/shop/*` 公开路由识别，并接入 `/me/assets`、`/me/assets/transactions`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory`；下一步继续复用既有组件和主题接入公开购买回流和公开商城 `/desktop` 回跳替换，统一 UI 设计放到页面迁移齐后的 `P3-12-D`。
...
2. `P3-12-B1`：推进账户资产与商城交易 Web 化，方案、路由 / 登录回流契约、商城私域正式 Web 入口和资产正式入口已完成，见 [P3-12-B1 账户资产与商城交易 Web 化方案](/records/p3-12-b1-account-shop-web-plan-2026-06-21)；下一步复用既有组件和主题接入公开购买回流，并替换公开商城购买中的 `/desktop` 回跳。
