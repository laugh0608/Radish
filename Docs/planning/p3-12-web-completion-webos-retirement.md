...
18. `P3-12-D32`：若继续补 Console 运行态覆盖，先准备或定位安全测试数据，再复核订单详情、分组表情列表和批量上传弹窗；若进入发布候选，转入 `P3-12-E` 验证矩阵和 PR 材料准备。
