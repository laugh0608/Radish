...
`P3-12-D36-D51` 已完成 UI 专题差距口径、四类设计源差距矩阵、边界裁决、阶段验收清单、Gateway PC / mobile 阶段验收、纠偏、页面开发缺口源码核对、Docs 作者态真实动作代码侧补漏、Public / Private 主路径代码侧复核、Console 深层管理动作复核、移动响应式抽样、Pencil UI 实现完成度复核、UI 实现证据收口、候选前启动前验证、Gateway 真实页面复核、D49 后口径纠偏和 `radish.client` 私域 / 作者态移动任务流 UI 对齐首批：D49 只能证明当前已实现页面无阻断，不能证明整个 app 已按 Pencil 设计稿完成 UI 实现，也不能直接进入 `P3-12-E`。D51 已按 `P21-P30` 口径收紧资产、订单 / 背包、通知、消息、圈子、宠物和 Docs 作者态的移动摘要与任务列表密度；下一步进入 `P3-12-D52 Public Web 移动公开任务流 UI 对齐`，D38 后置项不重新拉回当前批次。
...
38. `P3-12-D52`：下一顺位进入 `Public Web 移动公开任务流 UI 对齐`，优先处理 `/discover`、`/forum`、论坛详情、`/docs`、Docs 详情、`/shop`、商品详情、`/leaderboard` 和公开主页。
39. `P3-12-E`：后置到 P3-12-D UI 实现与退出条件真实完成之后，不提前创建发布 tag。
