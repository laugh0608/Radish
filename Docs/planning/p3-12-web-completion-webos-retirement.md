...
`P3-12-D36-D40` 已完成 UI 专题退出口径、四类设计源差距矩阵、边界裁决、阶段验收清单、Gateway PC / mobile 阶段验收和退出判断修正：D39 未发现已有代表页面的阻断级运行态问题，但这不能证明设计源页面全部开发完成。D40 已修正为“不满足退出条件”，下一步继续留在 `P3-12-D41`，回到源码和真实路由逐页确认页面开发缺口。
...
27. `P3-12-D41`：页面开发缺口排序与成组实现准备，按 public / private / author / console 逐页核对设计源、源码路由、真实数据态和关键动作。
28. `P3-12-E`：P3-12-D UI 专题真实退出后再进入正式版发布候选，不提前创建发布 tag。
