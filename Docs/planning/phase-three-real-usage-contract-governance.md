...
> 状态：`P3-12-E8 Pre-RC`；当前按 `Q0 工程硬门禁 + E8-B 有限产品收口 → F` 推进，`Q1 / Q2 / Q3` 在 `F` 内完成 Release Go，`Q4` 转持续维护
>
> 启动日期：2026-05-13（Asia/Shanghai）
>
> 本页承载第三开发阶段的目标定义、边界和阶段记录。当前执行入口以 [当前进行中](/planning/current) 为准，发布工程分层见 [P3-12-E8-Q](/planning/p3-12-e8-release-engineering-maturity-security-closure)，产品收口边界见 [P3-12-E](/planning/p3-12-product-maturity-quality-hardening)；第二阶段事实以 [第二阶段收口评审](/planning/phase-two-closure-review) 与 [已完成摘要](/planning/archive) 为准。
...
因此第三阶段主题暂定为：**真实使用增长与长期契约治理**。当前 `P3-12-E8 Pre-RC` 仍属于“把既有产品做到可控试用”的候选整备期，不把代码量、页面数量或本地通过当作增长证据。真正的增长验证从 `F` 的候选验收与受控真实试用开始，以真实内容、参与转化、复访、治理反馈和运行数据决定后续投入。
...
`P3-8` 至 `P3-11` 已完成多端补全、Web-first 信息架构、公开与私域主路径、管理员治理路径以及阶段性回归；当时的“暂缓 PR / 跳过发布”属于对应批次的历史约束，不再作为当前冻结门禁。当前进入 [P3-12 Web 完全化与 WebOS 收束](/planning/p3-12-web-completion-webos-retirement) 的 `E8 Pre-RC`：先完成 `Q0` 工程硬门禁，再按 `E8-B` 有限矩阵收口发现页、四条核心用户旅程、公开文档服务端契约和 Console 低风险响应式边界；满足进入条件后转入 `F` 候选验收与受控真实试用。`Q1 / Q2 / Q3` 是 `F` 内的 Release Go 任务，`Q4` 为持续维护，不再把所有成熟度治理前置成无限期冻结。
