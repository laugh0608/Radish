...
> 状态：`Q0、Q1、Q2、Q3 与 Release Go 已完成；v26.7.1.1204-release 已生产部署；Q4 转入持续维护线`
>
> 复核日期：`2026-07-12`
>
...
状态：`2026-07-12 已完成；目标库 apply / verify 与 PostgreSQL 双 Worker 用例均通过`。
...
状态：`2026-07-12 已完成 Release Go 必要子集与运行态 smoke；真实使用观察调整到发布后维护线`。
...
- Q1-A、Q1-B、Q1-C、Q2-A/B/C、Q3、候选运行态与回滚材料均已完成，工程 Release Go 已通过。
- PostgreSQL / OpenIddict 升级闭环、版本单一真值、高风险时间语义、client / console / HTTP / 后端 / 依赖候选基线，以及迁移、并发和身份高风险集成测试均已进入正式发布证据。
- `v26.7.1.1204-release` 已完成固定 tag 生产部署；真实使用观察进入维护 / 功能 backlog，不再回流为本专题发布门禁。
...
7. 本次正式发布矩阵明确为 Gateway、API、Auth、DbMigrate、client 和 Console；纯 Web 是唯一正式前端主线，Flutter 为条件维护线，WebOS 为兼容线，Tauri 冻结为实验资产，Rust 保持可选实验线。
...
发布后的早期真实使用观察继续记录激活、首次参与、收到回应后的回流、核心任务失败和用户反馈；它用于维护和功能排序，不再阻挡正式 tag。若部署后发现 `P0/P1`，立即停止扩大发布范围并进入修复 / 回滚。
...
- 记录发布后真实使用观察；发布记录只保留汇总结论与证据链接，命令级流水写入 `Docs/records/`。
