...
> 状态：`已立项，待按 Q0 → Q4 分批实施；Q0 为当前立即优先项，完成前不能进入 P3-12-F`
>
> 复核日期：`2026-07-10`
>
...
因此新增 `P3-12-E8-Q 正式发布工程成熟度与安全收口`：

- `E8-B` 继续负责正式 Web 深层链路、公开文档契约和移动返回体验。
- `E8-Q` 负责正式发布前的安全、可靠性、数据演进和工程门禁。
- `Q0` 是立即优先的发布阻断批次；未完成前不继续扩大生产暴露面。
- `Q1 / Q2` 涉及架构、接口、事务或运行时契约时，先提交子方案并等待确认。
- `Q4` 不要求一次清空全部历史债务，只要求处理当前发布相关高风险项，并建立可持续下降基线。
...
`E8-B` 和 `E8-Q` 不是互相替代关系。产品体验通过不能替代安全与数据可靠性，工程门禁通过也不能替代产品负责人对正式产品形态的判断。
...
- 审计分支为 `dev`，工作区干净。
- `master...dev` 为 `8 / 79`，双方均有独有提交；恢复合并前必须刷新差异并确认主线归属。
- `master..dev` 当前约 `253` 个文件、`24,873` 行新增、`4,491` 行删除，属于大批次集成风险。
...
- 不直接执行包安装或项目启动；依赖调整和运行态验证继续遵循仓库协作边界。
...
退出条件：生产 OpenAPI 不再暴露 Rust 性能测试和 WeatherForecast 演示接口。
...
按 `Q0-A → Q0-B → Q0-C → Q0-D` 推进。依赖、安全配置和生产端点改动应形成一个独立安全批次，不与 E8-B 页面改造混为同一提交。

### 第二批：Q1 可靠性

先做 fire-and-forget 分类和错误契约设计，再实施不可丢失写迁移与文件令牌治理。不得在没有事务和幂等设计的情况下逐处替换为新的后台调用。

### 第三批：Q2 数据演进

时间语义、schema ledger、OpenIddict 迁移和版本单一真值源可以分子专题推进，但正式发布前必须完成最小生产升级闭环。

### 第四批：Q3 门禁升级

Q3 可与 Q1 / Q2 的定向测试同步推进。先纳入现有 console / HTTP 测试和安全审计，再新增高风险集成测试。

### 第五批：Q4 持续治理

只先处理影响当前发布的乱码、原生发布分类和超大热区；其余债务进入可量化基线，按后续真实开发触达逐步下降。

## 六、P3-12-F 进入条件

进入 `P3-12-F 正式版发布候选整备` 前必须同时满足：
...
5. 不可丢失奖励、资产、权益和通知写不再依赖裸 fire-and-forget。
6. 文件访问令牌通过并发上限测试，数据库不保存可直接使用的明文 token。
7. 未知服务端异常不再以原始 `ex.Message` 返回客户端。
8. 完成一次生产相似 PostgreSQL 与 OpenIddict 升级演练，并具备验证和恢复说明。
9. 版本号、tag、镜像和发布记录规则一致且可自动校验。
10. 根 lint 无 error；client、console、HTTP 和后端测试进入候选前基线。
11. Tauri / Rust / Flutter 已明确属于“本次发布、维护或实验”中的哪一类；属于本次发布的资产必须构建通过。
12. 活动源码不存在已知 `U+FFFD`；其余历史卫生和大文件债务已有基线、负责人和后续入口。
13. `E8-B` 产品链路、公开 Docs 契约和移动返回体验达到产品负责人确认的正式版标准。
14. 用户当轮确认前后端已启动后，完成 Gateway PC / mobile 候选级 smoke。

任何一项未满足，都不能用“已有大量测试”“之前 smoke 通过”或“页面看起来正常”替代退出证据。
...
- npm / NuGet 依赖审计
- 全量 lint、全量 repo hygiene 或明确的历史债务基线对比
- PostgreSQL / OpenIddict 迁移演练结果
- `master...dev` 范围与双向独有提交复核
...
- 发布记录只保留汇总结论与证据链接；命令级流水写入 `Docs/records/`。

### 4. 文档维护

- 本专题只维护范围、顺序、停止线和退出条件。
- 每个 Q 子批次的方案、实现事实和验证结果写入对应专题或 `Docs/records/`。
- `current.md` 只记录正在执行的 Q 批次和下一顺位，不复制本专题全部清单。
