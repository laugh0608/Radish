...
当前项目仍处于开发阶段，没有稳定用户反馈和专职测试。公开页面、移动端 Web 视图、Flutter 移动客户端、Console 等仍存在未适配、未开发完或体验割裂的能力。`P3-8` 当时按纯 Web + Flutter 补齐多端缺口；现行路线已进一步收敛为纯 Web 唯一正式主线，Flutter 条件维护，WebOS 只保留 `/desktop` 历史入口，Tauri 冻结且不再绑定 WebOS。
