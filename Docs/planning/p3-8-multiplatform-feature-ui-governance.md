...
- 根路径 `/` 默认入口切换已完成：普通浏览器访问 `/` 时进入 `/discover` 公开分发页，Vite 直连与 Gateway 入口均已复核；Tauri 当前仍保留 `/desktop`，`?demo` 与 `/oidc/callback` 继续保留既有逻辑。
- 后续只选择一个高价值小闭环继续实现，候选方向为移动 Web 修复、纯 Web 登录后轻量链路补强或 Flutter 下一批功能。
