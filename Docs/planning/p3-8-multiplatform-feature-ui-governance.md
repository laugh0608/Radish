...
- 下一步应从已收口矩阵中重新选择高信号缺口，优先单一真实用户动作或单一治理排障动作；购买 / 订单 / 背包、权限授权、公开访问和 `ID Phase A` 转为维护线，只在真实阻断、排障定位缺口、权限页面问题、扫描命中或编译暴露新问题时回拉。不要继续重复 Flutter 个人主页来源返回、公开主页链接展示、轻量通知列表、公开商城列表、论坛详情评论发布 / 回复、评论登录回流、原生公开详情分享入口、公开文档阅读链路、订单 / 背包只读入口、订单详情 / 背包来源承接、订单 / 背包 / 钱包失败态与来源回流、购买后背包发放确认、最近访问公开路由守护、登录态通知只读列表、通知已读状态、通知打开后的已读回流、胡萝卜资产、经验记录、最近访问只读入口、纯文本发帖、单商品购买、购买 / 资产排障复核、权限授权 ID 契约、登录态个人资料编辑或逐页打磨 Web 公开页；不直接启动完整浏览历史治理、清空 / 删除、推荐系统、完整移动商城、完整通知中心、完整创作器、完整账号设置、完整 `PublicId` 全量迁移、数据库主键迁移或 WebOS 复刻。
