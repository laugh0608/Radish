...
- 矩阵已补充 `390 x 844` Vite 直连实测，暂未发现新的 `P0/P1`；第一小闭环候选为修正公开页顶部工作台入口：从指向 `/` 的 `WebOS` 链接收敛为指向 `/desktop` 的工作台保留入口。
- 矩阵完成后只选择一个高价值小闭环进入实现，候选方向为根路径 `/` 默认入口切换、移动 Web 修复、纯 Web 登录后轻量链路补强或 Flutter 下一批功能。
