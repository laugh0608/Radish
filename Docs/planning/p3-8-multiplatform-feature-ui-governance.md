...
- 下一步优先回到购买 / 资产链路做跨端排障复核收尾，确认 Flutter 订单、Console 订单、胡萝卜流水和用户详情可定位同一笔购买；`ID Phase A` 转为自动化守护维护线，只在扫描或编译暴露新命中时回拉。不要继续重复 Flutter 个人主页来源返回、公开主页链接展示、轻量通知列表、公开商城列表、论坛详情评论发布 / 回复、评论登录回流、原生公开详情分享入口、公开文档阅读链路、订单 / 背包只读入口、订单详情 / 背包来源承接、登录态通知只读列表、通知已读状态、胡萝卜资产、经验记录、最近访问只读入口、纯文本发帖或逐页打磨 Web 公开页；不直接启动完整浏览历史治理、清空 / 删除、推荐系统、完整移动商城、完整通知中心、完整创作器、完整 `PublicId` 全量迁移、数据库主键迁移或 WebOS 复刻。
...
- 受控写入已完成通知已读状态、纯文本发帖和单商品购买路径，且单商品购买真实数据人工复核已通过；`ID Phase A` 第三轮小写参数审计已收口，后续回到购买 / 资产跨端排障复核，购物车、退款、权益使用、富文本、推送、通知设置、资产转账和完整财务后台仍不纳入本批。
- 建立 UI 设计稿与实现同步机制。
