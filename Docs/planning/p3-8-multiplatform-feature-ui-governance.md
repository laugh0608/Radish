...
> 状态：`P3-8-D` 已作为默认主线阶段收口，后续仅在发布候选验收、新扫描命中、真实编译错误或高信号缺口暴露时回拉
>
...
当前项目仍处于开发阶段，没有稳定用户反馈和专职测试。公开页面、移动端 Web 视图、Flutter 移动客户端、Console 等仍存在未适配、未开发完或体验割裂的能力。`P3-8` 当时按纯 Web + Flutter 补齐多端缺口；现行路线已收束为 Web / Flutter Native 两条产品线，WebOS 只保留 `/desktop` 历史入口，Tauri 正式弃用且不再绑定 WebOS。
...
`P3-7-C` 已完成工作台与后端 Service 首批热区治理，继续硬拆低风险候选的收益下降。`P3-8-A` 已完成多端功能缺口与 UI 设计入口审计，首批实现任务 `P3-8-B1 Flutter 公开榜单只读入口` 已完成。`P3-8-B2` 已建立 Console 治理工作台设计端点，`P3-8-C1` 已完成内容治理与经验治理首批结构基座，`P3-8-C2` 已把 `P01 / P04 / P05 / P06` 页面类型试点落到首批高频历史页面。继续默认筛 Console 剩余 CSS / 历史页面会进入低收益打磨，因此 `P3-8-D` 当时切到纯 Web + Flutter 缺口补齐、验收矩阵与受控写入评估；该批次名不代表现行双主线。

截至 `2026-06-07`，`P3-8-D` 已完成公开访问、移动 Web、纯 Web `/desktop` 登录回流、Flutter 登录态主路径、购买 / 订单 / 背包 / 胡萝卜流水排障、Console returnTo / 权限授权与 LongId 字符串契约多轮治理。该方向继续作为默认主线的收益已经下降，后续只在 [P3-9 真实使用主路径产品化与发布候选整备](/planning/p3-9-real-usage-release-candidate) 暴露真实缺口时回拉。
...
阶段收口主线（历史批次名）：`P3-8-D 纯 Web + Flutter 主线缺口补齐、验收矩阵与受控写入评估`
...
- 根路径 `/` 默认入口切换已完成：普通浏览器访问 `/` 时进入 `/discover` 公开分发页，Vite 直连与 Gateway 入口均已复核；暂时弃用的 Tauri 历史资产仍保留 `/desktop` 兼容行为；历史 `?demo` 认证测试页已移除，`/oidc/callback` 保留为独立正式回调入口。
...
- Flutter 单商品购买真实数据人工复核已通过，Web 工作台购买回流同步补资格检查，避免 `CheckCanBuy` 不通过时继续打开支付口令弹窗；后续购买链路不继续扩展完整移动商城，优先保留订单、背包、胡萝卜流水和 Console 排障定位能力。
...
- `ID Phase A` 第三轮小写参数审计已收口：附件 / 分片上传、内容治理、排行榜、论坛分类、公开路由、工作台回跳和聊天 / 论坛导航中的 `businessId / targetContentId / attachmentId / categoryId / postId / channelId / messageId` 等外部 ID 契约继续收敛为字符串；LongId 守护补充小写 ID 声明扫描，避免函数参数层继续绕过检查。
...
- Flutter 纯文本发帖后的公开详情链路已完成复核：发帖接口仍返回内部帖子 ID，Flutter 成功后打开原生详情并由详情接口取得 `VoPublicId`；新帖详情页公开链接展示为 `/forum/post/:publicId`，评论和轻回应等私有操作继续使用详情解析后的内部 `VoId`。本轮不扩展富文本、附件、投票、抽奖、草稿箱、编辑、点赞、完整创作器或 WebOS 新功能。
- Flutter 纯文本发帖失败边界已补定向守护：已登录用户提交失败时保留标题、标签和正文草稿，展示服务端错误，不触发详情跳转或成功态；该守护与成功发帖、公开详情 PublicId 链路同文件覆盖。
- Flutter 订单详情到扣款流水排障入口已补强：订单详情只要存在内部订单 ID，即可进入钱包流水页并按 `transactionType=CONSUME`、`BusinessType=Order`、`BusinessId=OrderId` 定位同一笔购买的扣款记录；`VoCoinTransactionId` 只作为展示元信息，不再作为排障入口门禁。
- Flutter 订单 / 背包 / 钱包失败态与来源回流审计已补定向守护：订单详情刷新失败会保留当前订单上下文并提示刷新问题；背包来源订单或来源商品不存在 / 加载失败时进入对应错误态，仍可返回背包；钱包按 `Order #orderId` 筛选无结果或刷新失败时继续展示筛选上下文、空态和上次可用余额。本轮不扩展完整移动商城、退款、权益使用、完整资产中心、完整通知中心或 WebOS 新功能。
- Flutter 通知打开后的已读回流受控写入已完成：未读 forum 通知从通知 sheet 打开原生帖子 / 评论详情前，复用既有 `Notification/MarkAsRead` 标记单条已读；标记失败时显示轻提示并继续打开详情，保留通知来源返回。本轮不扩展批量已读、删除、推送、通知设置或完整通知中心。
- Flutter 购买后背包发放确认小闭环已完成：订单详情新增“查看背包发放”只读入口，可直接进入背包确认权益 / 道具发放结果；背包从订单详情进入时保留“返回订单详情”上下文，加载失败时展示明确错误。本轮不扩展权益激活、道具使用、退款、购物车或完整移动商城。
- Flutter 最近访问公开路由守护已完成：浏览记录模型中 forum / docs 类型优先使用 `targetSlug` / `PublicId` 作为打开目标和可见路径，旧 `routePath` 只作为缺少公开标识时的回退，避免内部 LongId 回潮到公开详情入口。本轮不启动完整 `PublicId` 全量迁移或数据库主键迁移。
- Flutter 纯文本发帖登录回流与草稿恢复已完成：匿名态从发帖表单提交会保留页面存活期间的标题、分类、标签和正文草稿，登录回调后回到发帖表单继续发布；提交失败继续保留草稿且不误打开详情。本轮不扩展富文本、附件、投票、草稿箱、编辑或点赞。
- 公开 forum 详情分享预览一致性已补守护：旧 LongId 公开详情加载到 `VoPublicId` 后，运行时 canonical、Open Graph 与 JSON-LD 会刷新到同一个 PublicId 路径；保留旧路径打开兼容，但不把 LongId 作为面向用户的分享预览主口径。
- Flutter 背包来源回流 LongId 口径已回拉：来源订单 / 商品入口统一按正则规范化的字符串 LongId 承接，不再通过 `int.tryParse` 进入数值域，并补非规范来源 ID 守护。本轮不扩展完整移动商城、权益使用、退款或 PublicId 全量迁移。
- Console 商品 / 订单排障回流已补守护：订单页和商品页 URL 状态构造抽出为可测 helper，商品详情进入相关订单会携带当前商品详情作为 `returnTo`，订单详情关闭、分页、筛选和重置保留合法排障来源；LongId 查询参数和返回来源继续保持字符串与同源相对路径约束。
- Console 角色授权并发保存守护已完成：授权保存中禁用保存按钮并阻止重复提交，避免旧 `expectedModifyTime` 被重复请求触发伪并发冲突；资源 / API 模块 ID、祖先补选、接口预览去重和保存 payload 排序继续按字符串契约执行。
- 阶段约束已复盘：移动 Web 公开视图逐页打磨、Console 剩余页面默认迁移、Flutter “只能只读”的总门禁、购买 / 订单 / 背包重复复核和 ID Phase A 广泛扫描可以结束为默认主线。
- 后续进入 `P3-9` 后，只有发布候选路径验收、新增外部 ID 边界、新扫描命中、真实编译错误或高信号缺口暴露时，才回拉本专题中的同类治理。
- Flutter 后续可按同一用户工作流推进一组成熟 API 支撑的受控写入动作，但仍必须覆盖登录回流、错误态、返回状态和定向测试。
- 不要继续重复 Flutter 个人主页来源返回、公开主页链接展示、轻量通知列表、公开商城列表、论坛详情评论发布 / 回复、评论登录回流、原生公开详情分享入口、公开文档阅读链路、订单 / 背包只读入口、订单详情 / 背包来源承接、订单 / 背包 / 钱包失败态与来源回流、购买后背包发放确认、最近访问公开路由守护、登录态通知只读列表、通知已读状态、通知打开后的已读回流、胡萝卜资产、经验记录、最近访问只读入口、纯文本发帖成功路径、纯文本发帖登录回流、单商品购买、购买 / 资产排障复核、权限授权 ID 契约、登录态个人资料编辑、逐页打磨 Web 公开页或完整 PublicId 全量迁移。
- WebOS 只保留 `/desktop` 历史入口，不再作为新增功能候选；Tauri 正式弃用，原生 PC 安装包由 Flutter Native 在阶段门禁后承接。
...
- 受控写入已完成通知已读状态、纯文本发帖、单商品购买路径和 Flutter 个人资料编辑，且单商品购买真实数据人工复核已通过；`ID Phase A` 第三轮小写参数审计、购买 / 资产跨端排障复核和 Console 角色授权 ID 契约复核已收口，购物车、退款、权益使用、富文本、推送、通知设置、资产转账、完整账号设置和完整财务后台仍不纳入本批。
- 建立 UI 设计稿与实现同步机制。
...
- 不把新功能默认加入 WebOS。
- 不让 PC 客户端或 Flutter 抢占纯 Web 正式主线。
...
1. 购买 / 订单 / 背包跨端回流与排障链路中的真实阻断缺口。
2. 验收矩阵暴露的高信号公开 / 移动 / 回流问题。
3. 权限授权、公开访问或 `ID Phase A` 守护中的新增命中。
