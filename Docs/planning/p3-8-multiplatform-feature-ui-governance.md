...
- 下一步先做 Flutter 单商品购买真实数据复核；若购买 / 资产链路仍有阻断，优先修正该链路，若通过则推进 `ID Phase A` 外部 LongId 字符串安全自动化守护。不要继续重复 Flutter 个人主页来源返回、公开主页链接展示、轻量通知列表、公开商城列表、论坛详情评论发布 / 回复、评论登录回流、原生公开详情分享入口、公开文档阅读链路、订单 / 背包只读入口、订单详情 / 背包来源承接、登录态通知只读列表、通知已读状态、胡萝卜资产、经验记录、最近访问只读入口、纯文本发帖或逐页打磨 Web 公开页；不直接启动完整浏览历史治理、清空 / 删除、推荐系统、完整移动商城、完整通知中心、完整创作器、完整 `PublicId` 全量迁移、数据库主键迁移或 WebOS 复刻。
...
- 受控写入已完成通知已读状态、纯文本发帖和单商品购买路径；后续先复核购买真实数据链路，再根据结果推进购买 / 资产修复或 `ID Phase A` 外部 LongId 字符串安全守护，购物车、退款、权益使用、富文本、推送、通知设置、资产转账和管理员治理仍不纳入本批。
- 建立 UI 设计稿与实现同步机制。
