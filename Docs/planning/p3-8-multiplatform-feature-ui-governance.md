...
- 下一步按购买 / 订单 / 背包跨端回流做一天级可验收批次：围绕同一笔购买复核 Flutter 订单详情 / 背包来源、纯 Web / `/desktop` 商品购买入口、Console 商品 / 订单 / 胡萝卜流水排障之间的返回、确认、筛选上下文和 LongId 字符串契约。若未暴露新缺口，再回拉权限授权、公开访问或 `ID Phase A` 中的高信号问题。不要继续重复 Flutter 个人主页来源返回、公开主页链接展示、轻量通知列表、公开商城列表、论坛详情评论发布 / 回复、评论登录回流、原生公开详情分享入口、公开文档阅读链路、订单 / 背包只读入口、订单详情 / 背包来源承接、订单 / 背包 / 钱包失败态与来源回流、购买后背包发放确认、最近访问公开路由守护、登录态通知只读列表、通知已读状态、通知打开后的已读回流、胡萝卜资产、经验记录、最近访问只读入口、纯文本发帖成功路径、纯文本发帖登录回流、单商品购买、购买 / 资产排障复核、权限授权 ID 契约、登录态个人资料编辑、逐页打磨 Web 公开页或完整 PublicId 全量迁移。
