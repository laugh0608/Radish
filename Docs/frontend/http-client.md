...
- 上传进度等必须使用 `XMLHttpRequest` 的场景，也要通过 `getApiClientConfig()` 读取统一 base URL、token 和语言配置。
...
| `translateMessage` | 根据服务端 `MessageKey` 获取宿主本地翻译 |
...
- `message` 是用户展示候选：宿主存在 `MessageKey` 翻译时使用本地文案，否则回退安全 `MessageInfo`。
...
业务 helper 需要抛错时使用 `ApiResponseError`：
...
- 抛出普通 `Error(response.message)` 后丢失 `Code / status / MessageKey / TraceId`；
...
- `getApiClientConfig`：供上传等特殊传输读取统一配置，不用于页面修改全局状态。
