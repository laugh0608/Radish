...
>
> 状态：旧设计源 `P01-P16` 与专题状态画板只读留档；当前活动源的 `R1-P01` 已关闭，`R1-P02` 已完成代码事实与设计边界审计、尚待代表画板确认；`P15 / P16` 仍仅作为后续公开聊天室参考
...
- 帖子详情必须表达帖子级轻回应、既有评论父子关系、表情 reaction 摘要、引用上下文和登录评论回流。当前 Public 路由只创建根评论，不开放新建子回复、帖子 / 评论点赞与 reaction 写入；这些能力不得根据共享组件的可选回调自行补齐。
...
- 评论树视觉应优先表达父子层级：父评论是完整评论项，既有子回复缩进并保留引用上下文；当前 Public 动作只包含根评论创建、评论作者编辑 / 历史、Comment 赞赏与举报，不展示未接入的回复、点赞、reaction 或删除入口。
