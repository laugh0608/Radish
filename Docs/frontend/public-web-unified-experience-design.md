...
>
> 更新：2026-07-08（Asia/Shanghai）：E8 首日已把正式 Web public / private 主导航统一为 PC `发现 / 论坛 / 聊天 / 更多` + 右侧 `通知 / 账号`，移动底栏统一为 `发现 / 论坛 / 聊天 / 更多 / 我的`；公开 Docs 列表、目录、搜索和详情只展示公开已发布内容，登录可看、受限和草稿内容进入作者态或权限路径。
>
> 状态：设计源 `P01-P16` 已补齐；公开 Web 当前发布前范围 `P01-P14` 已完成首轮前端实现、Gateway PC / mobile CSS 视口真实 smoke、E7-B / E7-C 用户语言和信息密度收口，以及 E8 首日正式 Web 主导航和公开 Docs 口径回拉；`P15 / P16` 作为后续公开聊天室参考，不阻断当前 UI 专题退出判断
...
- 公开 Docs 列表、目录、搜索和详情只展示 `Published + Public` 文档；登录可看、受限、草稿和已删除内容不在公开阅读页显式列出。
...
- 公开 Docs 的列表、目录、搜索结果和详情内容只承载公开已发布文档；作者草稿、登录可看文档、受限文档和回收站状态进入 Docs 作者态或 Console 治理，不在公开阅读页混排。
