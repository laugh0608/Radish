...
>
> 更新：2026-07-11（Asia/Shanghai）：`E8-B` 将 `/discover` 固定为“社区内容优先”的正式 Web 首页：帖子、问答和真实互动承担首屏主体，Docs、商城、榜单只做相关内容或“更多”承接；普通状态不再重复展示汇总指标、混合 Feed、路由说明和四类完整领域列表。
>
> 状态：设计源 `P01-P16` 已补齐；当前进入 `E8-B` 有限产品收口，先完成 Discover 内容优先裁决、四条核心用户旅程和 Public Docs 服务端公开契约，再进入 `F` 候选验收；`P15 / P16` 作为后续公开聊天室参考，不阻断当前退出判断
