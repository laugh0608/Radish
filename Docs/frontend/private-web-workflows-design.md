...
>
> 状态：设计源 `P01-P30` 已补齐；`radish.client` 私域 / 作者态视觉实现首轮完成，后续只在真实缺口或阶段验收中定向回拉
...
5. 后续改动只在新增真实缺口、阶段验收发现问题或进入发布候选前复核时回拉；回拉仍需保持业务契约不变，并执行对应 `radish.client` 类型检查 / 构建与必要页面 smoke。
