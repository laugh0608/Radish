...
>
> 更新：2026-06-28（Asia/Shanghai）：`P3-12-D8` 已开始将共享壳层和状态槽落到 `radish.client`，私域复访页与作者态页已进入首批共享结构对齐；下一批继续处理私域 / 作者态真实数据面和移动单列任务流。
>
> 状态：设计源 `P01-P30` 已补齐；`radish.client` 私域 / 作者态视觉实现已进入代码阶段
...
3. 下一批继续推进资产 / 订单 / 背包数据面、通知 / 消息复访、圈子 / 宠物、论坛作者态、Docs 作者态和移动单列任务流。
