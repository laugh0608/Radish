...
>
> 状态：设计源 `P01-P30` 已补齐；`radish.client` 私域 / 作者态视觉实现首轮完成，后续按 D36+ 差距矩阵和阶段验收结果定向回拉
...
5. D36+ 先确认 `P01-P30` 与真实路由族、移动单任务页和共享壳层的对齐缺口；后续回拉仍需保持业务契约不变，并执行对应 `radish.client` 类型检查 / 构建与必要页面 smoke。
