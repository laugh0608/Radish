...
>
> 更新：2026-06-28（Asia/Shanghai）：设计源已按 `F02` 公共壳层契约二次重构为 `P01-P30`，旧模板化页面已替换为真实路由驱动的 PC / mobile 页面族；统一 private header 与移动 tab，并补齐移动端单任务页面。private 移动底栏作为跨源基准样式，固定为 5 项浮动胶囊栏。
>
