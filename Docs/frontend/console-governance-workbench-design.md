...
> 状态：`P3-12-D5` 已完成 Console 治理设计源 `P00-P18` 设计收口；`P3-12-D14-D21` 已完成 `radish.console` 壳层首批、系统设置、订单、用户、商品、文档治理首屏、标签 / 分类和贴纸类页面的语义组件迁移与成组静态收口
>
...
`P3-12-D14-D21` 已完成第二轮 Console 语义迁移与成组静态收口：
...
- 进入代码前先判断目标页面属于 `P12` 权限矩阵、`P02 / P03` 治理工作台还是 `P13` 运维任务流，并明确首屏区块、动作区、留痕和移动任务流边界。
...
2. D21 后优先评估角色权限、内容治理、经验治理和运维任务页的页面类型边界。
3. 复杂页面进入代码前先确定首屏区块、证据 / 动作 / 留痕结构和移动任务流，不把普通表格迁移口径直接套用到治理工作台或权限矩阵。
