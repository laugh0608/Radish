...
> 状态：`P3-12-D5` 已完成 Console 治理设计源 `P00-P18` 设计收口；`P3-12-D14-D22` 已完成 `radish.console` 壳层首批、系统设置、订单、用户、商品、文档治理首屏、标签 / 分类和贴纸类页面的语义组件迁移、成组静态收口与复杂页面类型边界评估
>
...
- `SystemConfigList`、`OrderList`、`UserList`、`ProductList`、`DocumentGovernancePage`、`TagList` 和 `CategoryList` 已迁移到 D14 语义页头、指标和工具条组件。
...
`P3-12-D14-D22` 已完成第二轮 Console 语义迁移、成组静态收口与复杂页面类型边界评估：
...
- `P3-12-D23` 优先按 `P12` 推进角色权限外层语义迁移，覆盖 `RoleList` 与 `RolePermissionPage`；保留角色 API、授权资源树、勾选继承、保存载荷、权限键预览和路由守卫。
- 内容治理和经验治理继续按 `P02 / P03` 治理工作台推进，不把举报审核、手动治理、经验复核、冻结 / 解冻和管理员调整压进普通表格工具条。
...
2. D23 优先推进 `P12` 角色权限外层语义迁移，覆盖 `RoleList` 与 `RolePermissionPage`。
3. 内容治理和经验治理迁移前继续按 `P02 / P03` 确认首屏区块、证据 / 动作 / 留痕结构和移动任务流，不把普通表格迁移口径直接套用到治理工作台。
