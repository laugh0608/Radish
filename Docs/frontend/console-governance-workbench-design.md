...
> 状态：`P00-P18` 设计与 Console 首轮视觉 / 交互治理已收口；文档治理 `P11 / P16` 已承接待审队列，内容治理 `P02 / P07` 将按 F4-I 在服务端权威契约完成后升级为案件工作台；移动 Console 继续只作为响应式管理后台参考
>
...
>
> `2026-07-21` 更新：F4-I-A 已完成[内容治理案件、证据与动作一致性](/features/content-moderation-case-evidence-action-design)权威设计。`P02 / P07` 后续从孤立举报单升级为“案件 → 证据 → 决定 → 动作 → 留痕”，但必须等待 F4-I-B 的 Case / Evidence / State 与权限契约落地后再修改 Pencil；当前代码和画板仍代表既有举报审核工作台。
