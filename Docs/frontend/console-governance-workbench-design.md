...
> 状态：`P3-12-D5` 已完成 Console 治理设计源 `P00-P18` 设计收口；`P3-12-D14-D31` 已完成 `radish.console` 壳层首批、系统设置、订单、用户、商品、文档治理首屏、标签 / 分类、贴纸类页面、角色权限页面、内容 / 经验治理工作台外层语义迁移、点名内部区块样式收口、治理页面成组静态收口、复杂页面类型边界评估、系统工具 / 运维外壳收口、阶段静态收口、深层表单静态收口、详情 / 抽屉静态收口和阶段运行态复核
>
...
`P3-12-D14-D31` 已完成第二轮 Console 语义迁移、成组静态收口、复杂页面类型边界评估和阶段运行态复核：
...
- 阶段运行态复核：Gateway 下 Console 登录回流、商品详情、文档详情 / 版本治理、订单空态和表情分组空态已覆盖 PC `1920x1080` 与 mobile `390x844` CSS 视图；本地数据缺少订单和表情包分组，因此 `OrderDetail` 与批量上传弹窗仍需数据条件补验。

后续评估：

- 若继续补运行态覆盖，先准备或定位安全测试数据，再复核 `OrderDetail`、分组表情列表和批量上传弹窗。
- 若继续静态治理，先按中宽 PC 固定列交互和移动窄屏表格可读性做目标扫描评估收益。
...
8. 下一批若进入阶段真实验收，先等待用户确认前后端已启动；若继续静态治理，先按目标扫描评估收益。
9. 移动端实现不把 PC 表格硬缩，按 `P14-P18` 的任务卡、分组详情和底部动作重排。
