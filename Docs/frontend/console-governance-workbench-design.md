...
> 状态：`P3-12-D5` 已完成 Console 治理设计源 `P00-P18` 设计收口；`P3-12-D14-D19` 已完成 `radish.console` 壳层首批、系统设置、订单、用户、商品、文档治理首屏、标签和分类页面的语义组件迁移
>
> 首次日期：2026-05-24（Asia/Shanghai）
>
> 最近更新：2026-06-29（Asia/Shanghai）
>
...
`P3-12-D14-D19` 已完成第二轮 Console 语义迁移：
...
- `StickerGroupList` 和 `StickerList` 已有权限、上传、排序和表格 CRUD 功能闭环，但还未迁入 D14 语义组件。贴纸页包含封面 / 预览图、分组详情跳转、单个新增、批量上传和批量排序，下一批应先判断它属于普通表格收尾，还是需要拆成媒体资产列表和分组详情两层。
...
2. 贴纸分组 / 贴纸列表进入下一批前先判断媒体资产管理边界：
   - 如果仅迁移页头、指标和筛选工具条，可按普通表格 CRUD 页处理。
   - 如果需要调整图片预览、分组详情、批量上传确认表、批量排序或素材状态流，应拆为媒体资产列表与分组详情批次。
3. 角色权限、内容治理、经验治理和运维任务页继续按页面类型推进，不把复杂治理动作压进普通表格工具条。
