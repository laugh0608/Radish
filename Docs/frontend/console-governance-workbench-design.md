...
> 状态：`P3-12-D5` 已完成 Console 治理设计源 `P00-P18` 设计收口；`P3-12-D14-D28` 已完成 `radish.console` 壳层首批、系统设置、订单、用户、商品、文档治理首屏、标签 / 分类、贴纸类页面、角色权限页面、内容 / 经验治理工作台外层语义迁移、点名内部区块样式收口、治理页面成组静态收口、复杂页面类型边界评估、系统工具 / 运维外壳收口和阶段静态收口
>
...
`P3-12-D14-D28` 已完成第二轮 Console 语义迁移、成组静态收口与复杂页面类型边界评估：
...
- 阶段静态收口：`routerComponents` 的认证中、无 Console 权限和懒加载状态已迁出 inline 样式；深层表单、详情抽屉和批量上传弹窗残留作为后续专题处理。

后续评估：

- `P3-12-D29` 优先按收益选择深层表单 / 详情静态收口，或在用户确认前后端已启动后进入阶段真实验收。
...
7. 下一批优先按收益选择深层表单 / 详情静态收口，或准备阶段真实验收。
