...
> 状态：`P3-12-D5` 已完成 Console 治理设计源 `P00-P18` 设计收口；`P3-12-D14-D24` 已完成 `radish.console` 壳层首批、系统设置、订单、用户、商品、文档治理首屏、标签 / 分类、贴纸类页面、角色权限页面和内容 / 经验治理工作台外层语义迁移、成组静态收口与复杂页面类型边界评估
>
...
- 治理工作台外层首批：`ModerationPage` 与 `ExperienceAdminPage` 已迁入语义页头、状态 chip 和工作台指标，内容治理局部硬编码颜色已转为 Console token，举报审核、手动治理、治理日志、经验复核、调经验、冻结 / 解冻和等级配置 API 保持不变。

后续评估：

- `P3-12-D25` 优先按 `P02 / P03` 继续推进内容治理和经验治理工作台内部区块样式收口，不把举报审核、手动治理、经验复核、冻结 / 解冻和管理员调整压进普通表格工具条。
...
3. D25 继续处理治理工作台内部区块样式和 token 收口，不把普通表格迁移口径直接套用到治理工作台。
