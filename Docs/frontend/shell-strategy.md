...
1. **纯 Web 壳层**
   ...
   - 已完成 `/notifications`、`/me`、`/messages`、`/pet` 四个登录态私域复访入口，分别承接通知列表 / 目标分流、个人状态 / 成长 / 资产只读概览、会话 / 消息定位、电子宠物领取与照顾
...
- 登录态私域复访固定使用 `/notifications`、`/me`、`/messages`、`/pet` 承接轻量通知、个人状态、会话定位和电子宠物照顾；这些路由都不进入公开 sitemap，也不替代 `/desktop` 的完整工作台能力
...
| 已完成首批纯 Web 私域迁移 | 登录后复访、通知回流、个人状态、成长 / 资产只读概览、会话 / 消息定位和电子宠物照顾 | `/notifications`、`/me`、`/messages`、`/pet` |
...
首批迁移决策已完成 `/notifications`、`/me`、`/messages`、`/pet` 四个纯 Web 私域复访入口；后续不把聊天、通知、个人中心、钱包、商城、创作器和宠物经济系统同时搬迁，也不把完整聊天平台、完整钱包、完整个人中心、宠物公开名片或 Console 数值配置作为同一批前置门槛。
