...
Radish 的当前前端投入顺位为 **Web 优先、Flutter 次级**。

Web 承担浏览器默认入口、公开访问、登录后使用、SEO、分享以及 PC / 移动浏览器体验，是功能、视觉和验收的正式优先主线。Flutter Native 是次级原生安装包产品线，目标覆盖 Android、iOS、Windows、macOS 与 Linux；移动优先，桌面达到阶段门禁后再产品化，不机械追平 Web。Tauri 正式弃用并只保留历史资产。WebOS 不再作为未来功能扩展方向，只保留在 `/desktop` 作为历史桌面工作台入口。

当前前端策略固定为 **Web 优先 + Flutter Native 次级 + WebOS 历史兼容 + Tauri 弃用隔离**：

| 壳层 | 主要用户 | 主要设备 | 主要任务 | 交互形态 |
| --- | --- | --- | --- | --- |
| 纯 Web 壳层 | 游客、轻登录用户、PC / 移动浏览器用户 | 手机 / 平板 / 桌面浏览器 | 浏览帖子、查看详情、访问公开主页、阅读文档、轻互动、逐步承接登录后高价值功能 | 单任务、直达内容、响应式 Web |
| Flutter Native 次级产品 | 原生安装包用户 | Android / iOS / Windows / macOS / Linux；Android AVD 与 iOS Simulator 开发门禁已关闭，desktop runner 尚未生成 | Web 边界成立后承接高价值阅读、参与与复访；mobile-first、desktop stage-gated | 移动触控与生命周期；桌面键鼠、焦点和窗口自适应 |
...
1. **纯 Web 壳层**
   ...
   - 已完成 `/notifications`、`/circle`、`/me`、`/shop/*` 私域、`/messages`、`/pet` 等登录态私域入口，分别承接权威通知收件箱 / 偏好 / 结构化目标、关系链复访、完整个人中心子路径、商城购买 / 订单 / 背包、一对一会话生命周期 / 消息定位、电子宠物领取与照顾
...
3. **Flutter Native 原生安装包**
   ...
   - Windows / macOS / Linux 是长期正式目标；P8-A 已确认 shared UI 有 expanded 与基础快捷键证据，但 desktop runtime 当前落入内存 fallback，hover、滚轮 / scrollbar、完整键盘焦点和真实窗口门禁未关闭。后续按 macOS、Windows、Linux 分平台建立工程与 runtime，再独立处理签名、更新与分发
   - Flutter Web 不进入路线；Console、SEO 与完整 Author 默认继续由 Web 承担
4. **Tauri 历史资产**
   - Tauri 正式弃用，不进入日常开发、UI、CI、构建、签名、分发或验收流程
   - 既有窗口、loopback 登录、deep link、安装器等结果只作为历史验证事实保留，不构成恢复入口
   - Flutter Native 承接长期桌面安装包目标；不得因桌面原生需求恢复 Tauri 或重新绑定 WebOS
...
### Flutter Native 已有资产与新增条件

以下条目描述已经落地的 Android MVP 资产；后续新增仍需证明原生价值，并服从 Web 优先顺位：
...
Flutter 不复刻 WebOS 工作台，也不因次级产品线定位自动扩完整通知中心、完整商城工作台、完整账号设置或桌面治理能力。compact、medium、expanded 可以采用不同导航和编排；跨端一致性优先体现在任务归属、入口命名、登录恢复、返回语义、状态语义和错误状态上。具体 UI 与阶段门禁见 [Flutter Native 产品化与 UI 重构](/features/flutter-native-product-ui-design)。
...
原则不是“所有能力都双端同步”，而是“所有新能力先在 Web 中成立；具有明确原生价值时再进入 Flutter”。WebOS 不再作为新功能候选，Tauri 正式弃用。
...
`radish.client` 的浏览器入口由 `BrowserAppRouter` 统一解析并在最外层挂载 `BootstrapGate`。公开页、Workbench、登录态私域、WebOS Root 和 `/oidc/callback` 都先完成首次管理员状态检查，再挂载实际页面；业务入口组件不再各自重复包裹门禁。该边界保证初始化未完成时不会提前进入 OIDC 或局部应用，同时避免不同入口出现不同的首次部署体验。

- 当前保持增量迁移，不立即删除 WebOS 路由结构
- 普通浏览器根路径 `/` 已切向 Web 公开分发页 `/discover`；Tauri 历史资产仍保留 `/desktop` 兼容入口但不继续开发；历史 `?demo` 认证测试页已移除，`/oidc/callback` 保留为独立正式回调入口
...
- 登录态私域复访固定使用 `/notifications`、`/circle`、`/me`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory`、`/messages`、`/pet` 承接权威通知收件箱、关系链复访、个人状态与个人中心子路径、商城订单 / 背包、一对一会话和电子宠物照顾；这些路由都不进入公开 sitemap，也不替代 `/desktop` 的窗口工作台能力
...
- Tauri 历史客户端不再恢复；Flutter desktop 也不把 `/desktop` 或 WebOS 作为默认 UI
...
| 已完成首批纯 Web 私域迁移 | 登录后复访、权威通知收件箱、个人状态、完整个人中心子路径、商城交易、一对一会话 / 消息定位、电子宠物照顾和正式 Web 功能发现 | `/notifications`、`/circle`、`/me`、`/shop/orders`、`/shop/order/:orderId`、`/shop/inventory`、`/messages`、`/pet`、`/workbench` |
...
| Flutter Native 次级承接 | Web 产品边界已成立，且路径具有明确原生价值 | 高频移动复访；桌面宽屏阅读、键鼠效率与安装体验 |
...
- 不做“移动版 WebOS”
- 不要求所有页面同时进入移动和桌面首批；按页面族与平台门禁推进
- 不把 WebOS 全部功能一次性迁移到纯 Web
- 不把公开内容强行塞回窗口系统
- 不把新功能默认加入 WebOS
- 不机械扩展 Flutter、追平 Web 全部功能或在共享 UI 未稳定时启动平台分发工程
- 不把 Capacitor 作为移动端产品化路线
- 不恢复 Tauri，不再处理其签名、自动更新、SmartScreen、生产 Auth 或公开分发链路
- 不把 WebOS 作为 Flutter desktop 的默认体验
- 不引入 Flutter Web 或独立 Flutter Console
...
- 如果能力依赖移动安装包、系统推送、后台任务或原生生命周期，且 Web 产品边界和移动价值已经明确，再由 Flutter 次级产品线承接
- 如果能力在桌面安装包中能通过宽屏编排、键鼠输入、窗口生命周期或本地集成形成独立价值，并通过共享 UI 门禁，再由 Flutter desktop 分平台承接
- Tauri 保持正式弃用，不作为任何功能归属候选
