...
1. **纯 Web 壳层**
   ...
   - 已完成 forum / docs / `u/:id` / leaderboard / shop / discover 首批公开入口收口；`/discover` 当前是公开内容流，`/u/:id` 当前优先使用 `User.PublicId`，`/circle` 作为登录态关系链复访入口接入公开详情来源返回
   - 已完成 `/notifications`、`/me`、`/messages` 三个登录态私域复访入口，分别承接通知列表 / 目标分流、个人状态 / 成长 / 资产只读概览、会话 / 消息定位
...
- 文档阅读与搜索
- 公开榜单、公开商城浏览、社区发现分发
- 登录后轻量个人链路、通知复访、会话 / 消息定位、个人状态 / 成长 / 资产只读概览、购买 / 订单 / 背包等适合浏览器直达的功能
...
- 公开内容直达路由已经覆盖 `/discover`、`/forum`、`/forum/post/:id`、`/docs`、`/docs/:slug`、`/u/:id`、`/leaderboard`、`/shop` 等首批路径；forum detail 优先使用 `Post.PublicId`，公开个人页优先使用 `User.PublicId`，旧 LongId 字符串只保留兼容读取
...
- 登录态私域复访固定使用 `/notifications`、`/me`、`/messages` 承接轻量通知、个人状态和会话定位；这三类路由都不进入公开 sitemap，也不替代 `/desktop` 的完整工作台能力
- 从 `/me` 或 `/messages` 普通点击进入公开个人页时，使用当前标签页一次性来源转交补足返回“我的状态”或“消息”；分享链接和公开 canonical 仍只使用 `/u/usr_...`
- 公共页头部动作固定为“社区发现 / 我的圈子 / 工作台”三层；`/circle` 自身只保留“社区发现 + 工作台”，`/desktop` 保留为历史工作台入口但不作为新增功能主动作
...
| 已完成首批纯 Web 私域迁移 | 登录后复访、通知回流、个人状态、成长 / 资产只读概览和会话 / 消息定位 | `/notifications`、`/me`、`/messages` |
| 应优先评估纯 Web | 后续仍有真实浏览器直达价值、但不适合直接复刻 WebOS 的轻量能力 | 宠物状态入口、轻任务复访、受控的资产 / 经验联动展示 |
...
首批迁移决策已完成 `/notifications`、`/me`、`/messages` 三个纯 Web 私域复访入口；后续不把聊天、通知、个人中心、钱包、商城和创作器同时搬迁，也不把完整聊天平台、完整钱包或完整个人中心作为 B8 前置门槛。
