...
1. **纯 Web 壳层**
   ...
   - 已完成 `/notifications`、`/circle`、`/me`、`/messages`、`/pet` 五个登录态私域复访入口，分别承接通知列表 / 目标分流、关系链复访、个人状态 / 成长 / 资产只读概览、会话 / 消息定位、电子宠物领取与照顾
...
- 登录态私域复访固定使用 `/notifications`、`/circle`、`/me`、`/messages`、`/pet` 承接轻量通知、关系链复访、个人状态、会话定位和电子宠物照顾；这些路由都不进入公开 sitemap，也不替代 `/desktop` 的完整工作台能力
