...
Web 承担浏览器默认入口、公开访问、登录后使用、SEO、分享以及 PC / 移动浏览器体验，是功能、视觉和验收的正式优先主线。Flutter 是次级移动原生产品线，按明确移动价值承接能力，不机械追平 Web。Tauri 暂时弃用并保留历史验证资产，未来等待重新评估。WebOS 不再作为未来功能扩展方向，只保留在 `/desktop` 作为历史桌面工作台入口。

当前前端策略固定为 **Web 优先 + Flutter 次级 + 历史兼容 / 暂时弃用隔离**：

| 壳层 | 主要用户 | 主要设备 | 主要任务 | 交互形态 |
| --- | --- | --- | --- | --- |
| 纯 Web 壳层 | 游客、轻登录用户、PC / 移动浏览器用户 | 手机 / 平板 / 桌面浏览器 | 浏览帖子、查看详情、访问公开主页、阅读文档、轻互动、逐步承接登录后高价值功能 | 单任务、直达内容、响应式 Web |
| Flutter 次级原生壳层 | 移动原生用户 | Android 已完成 MVP，iOS 仍需独立评估 | Web 边界成立后承接明确移动价值，不机械追平 Web | 原生导航、原生手势、原生生命周期 |
| WebOS 保留入口 | 历史桌面工作台用户 | 桌面浏览器 | 继续访问既有桌面工作台能力，承接迁移过渡期 | Dock、窗口系统、工作台 |
| Tauri 暂时弃用资产 | 暂无正式目标用户 | Windows / macOS / Linux | 仅保留既有验证资产；未来满足重新评估条件后再决定是否立项 | 历史 Tauri 系统壳 + Web UI |
...
以下条目描述已经落地的移动 MVP 资产；后续新增仍需证明移动价值，并服从 Web 优先顺位：
...
Flutter 不复刻 WebOS 工作台，也不因次级产品线定位自动扩完整通知中心、完整商城工作台、系统分享 SDK、发帖创作器、完整账号设置或桌面治理能力。移动端信息架构不要求 Web / PC / Tauri 照搬；跨端一致性优先体现在任务归属、入口命名、登录恢复、返回语义、状态语义和错误状态上。
...
原则不是“所有能力都双端同步”，而是“所有新能力先在 Web 中成立；具有明确移动原生价值时再进入 Flutter”。WebOS 不再作为新功能候选，Tauri 暂时弃用。
...
| Flutter 次级承接 | Web 产品边界已成立，且路径具有明确的移动原生价值 | 高价值移动消息、通知复访、后台任务或原生生命周期能力 |
...
- 不把公开内容强行塞回窗口系统
- 不把新功能默认加入 WebOS
- 不机械扩展 Flutter、追平 Web 全部功能或在缺少独立价值判断时启动 iOS 套件
...
- Tauri 当前暂时弃用；只有桌面原生价值、目标用户和维护预算同时明确时才重新评估，若重启 UI 默认复用正式 Web
