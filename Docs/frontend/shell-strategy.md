...
- `/workbench` 固定作为正式 Web 功能地图，并作为产品级主导航中的一个普通页面；`/desktop` 只作为功能地图中的 WebOS 历史入口和旧深链承接点
...
- 公共页与登录态页面共享同一套产品级主导航；不再把公开页、私域页和工作台页拆成三套全局入口。“工作台”进入 `/workbench`，`/desktop` 保留为历史工作台入口但不作为新增功能主动作
...
- Public / Private 页面默认共用一套产品级主导航：PC 导航固定为 `发现 / 论坛 / 文档 / 商城 / 工作台`，右侧固定为 `消息 / 我的或登录`；移动底栏固定为 `发现 / 论坛 / 工作台 / 消息 / 我的`。
- `/me`、`/circle`、`/pet` 和 `/me/*` 归属“我的”；`/messages` 与 `/notifications` 归属“消息”；Docs 作者态、论坛作者态、订单 / 背包、Console、规则页、榜单和 `/desktop` 兼容入口归属“工作台”。
- 页面可以传入 `navItems / actionItems / mobileNavItems` 覆盖具体动作，但这只能用于同一产品级导航内的定向收敛；不能绕过共享组件另写平行壳层，也不能把工作台做成突出于主导航之外的大 CTA。
