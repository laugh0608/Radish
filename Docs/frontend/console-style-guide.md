...
- 桌面侧栏 `224px`、折叠侧栏 `64px`、顶栏 `56px`；主体边距 `14–16px`，公共面板内边距 `12px`、区块间距 `12px`，通过 `--console-*` 尺寸 token 承接。
- 通用标题和指标字号 `20px`，列表优先展示数据；`ConsoleResourceList` 将说明与权限上下文放到列表后的原生折叠区，避免常驻侧栏压缩表格。
- 筛选工具条按面板实际可用宽度自动排列，动作组可换行、按钮不压缩成竖排文字。公共桌面按钮以 `32px` 最小高度为基准，表格单元格使用 `10px 12px` 内边距。
- 定时任务页面直接展示紧凑页头、认证状态与看板，不再用静态入口指标卡占据首屏。
- 以上规则限定 Console；不使用 CSS zoom 或缩放整个页面，不修改 Client 主题。移动端继续保留独立导航、触控控件和列表卡片，PC 密度规则不压缩移动触控目标。
...
| 表格 CRUD 页 | 指标条 + 筛选工具栏 + 表格 + 可展开说明 | 用户、订单、商品、角色 |
