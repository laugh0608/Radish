...
- 已迁移页面：`SystemConfigList`、`OrderList`、`UserList`、`ProductList`、`DocumentGovernancePage`、`TagList`、`CategoryList`。
- 迁移边界：只迁移页头、指标、筛选工具条和首屏区块承载，不改变 API、权限、业务动作、表格列、分页、表单字段或后端契约。
