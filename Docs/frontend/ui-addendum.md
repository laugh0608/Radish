...
>
> 状态：`2026-08-03` 完成 v26.7.3 破坏性基线升级补充批与 `R1-F01`；`R1-P01` 两轮页面评审未通过，下一步先重做多元社区结构校准，不进入共享主题色与 `/discover` 实现。
...
Radish 全部产品表面的标题只表达真实页面、对象或任务名称。标题上方禁止出现装饰性眉题，包括英文大写栏目名、中文分类词、路由 / 阶段 / 编号标签和主标题的同义复述；标题下方禁止出现仅用于解释标题、概括内容或营造氛围的描述性副标题。标题旁只保留真实状态、数量、时间、来源、权限和可执行动作，必要说明进入对应正文、控件提示、内联反馈或状态区域。
...
`R1-P01` 颜色校准已把 `guofeng` 品牌目标从旧胭脂调整为低饱和灰玉：品牌 `#5d6c57`、悬停 `#6e736d`、设计柔底 `#e2e6de`、实底前景 `#fffdf8`；常规操作仍使用墨蓝 `#435c74`。该颜色目标不因页面构图评审失败而撤销，但当前 `theme-tokens.css` 仍是旧胭脂运行时值；必须等 `R1-P01` 结构通过后再与代表页成组实现，不能把设计目标误写成已上线事实。
...
| --- | --- | --- | --- |
| 通用参考默认与 `R1-P01` 颜色目标均使用灰玉品牌 | 当前 `guofeng` 运行时仍是旧胭脂值 | 页面结构两轮评审未通过，不能先改运行时再反向约束设计 | 代表页通过后显式更新品牌、悬停、柔底和品牌前景映射；不机械替换状态色或其他主题 |
...
- `v26.7.3` 的破坏性窗口已于 `2026-08-03` 完成：新增 `text-on-brand`，当时显式固定 Radish 四主题品牌前景并保留 `guofeng` 胭脂身份；随后 `R1-P01` 颜色校准把国风品牌目标改为灰玉，但页面结构尚未通过审核，运行时实现必须后置并单独提交。
