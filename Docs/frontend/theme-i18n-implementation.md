...
- 不重构正式 Web 全局导航信息架构；
- 本批不推进 Flutter 主题发布能力；Tauri 正式弃用，不进入主题开发、构建或验收门禁；
...
现有 i18n 已覆盖桌面壳层及商城、论坛、聊天、通知、个人中心、文档等高频链路。F2 只为新增主题入口补必要翻译键，不扩展为全站文案治理。

F3 已完成三端现状审计，并已实现首批语言状态、资源 registry、共享 locale 与错误消费契约；后续目标、边界、实施状态和验证矩阵统一见 [F3 i18n 完成度治理实施说明](/frontend/i18n-completion-governance)。本页继续只维护已完成的 F2 主题运行时，不再承载 F3 实施细节。
