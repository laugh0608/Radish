...
- 公开内容壳层当前已完成 forum、docs、个人公开页与公开榜单四个首批入口：`/forum`、`/forum/post/:postId`、`/docs`、`/docs/:slug`、`/u/:id`、`/leaderboard` 与 `/leaderboard/:type` 都已可直接进入公开阅读壳层
...
> 截至 `2026-04-09`，仓库中的 `radish.client` 仍然是桌面 / WebOS 优先架构，当前并没有真正落地的完整 `MobileShell` 实现；但 forum 公开内容壳层首批已经落到 `/forum` 与 `/forum/post/:postId`。本节描述的是“已落地事实 + 后续移动 Web 规划方向”的组合口径。

### 6.1 当前现实

- 论坛等个别页面已有窗口内响应式处理，但这不等于真正的移动端产品形态
- 当前主入口仍然是桌面 Shell、Dock 与窗口系统
- 公开内容壳层当前已完成 forum、docs、个人公开页与公开榜单四个首批入口落地，帖子列表、帖子详情、公开文档目录、个人公开页与公开榜单都可以绕开桌面 Shell 直接进入公开阅读形态
- 公开 forum 当前只冻结“列表 + 详情 + 轻回应墙展示 + 评论阅读”，并明确保持只读阅读边界
...
- 第一批已先从公开内容浏览起步：forum 列表、帖子详情、轻回应墙展示与评论阅读当前已进入公开内容壳层
- 个人公开页与公开榜单首批当前已先接入公开内容壳层，更深的轻互动能力与商城浏览链路仍按价值逐步接入，不一次性照搬桌面 App
...
- 桌面 Shell 与应用路由关系：
  - 桌面仍然挂在 `/` 路径；
  - 对于搜索引擎访问 `/forum`、`/forum/post/*`、`/docs`、`/docs/*` 等路径时，可以直接渲染公开内容壳层而不是完整桌面壳，以减少噪音并提升首屏内容密度。
