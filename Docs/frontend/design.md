...
> Radish 第一开发阶段以前端 **WebOS / 超级应用** 为主入口完成首版交付，第二开发阶段完成公开 Web、Flutter Android MVP 与 Tauri 历史验证。`2026-08-15` 起正式只保留 Web / Flutter Native 两条产品线：Web 优先覆盖 PC / mobile 浏览器、公开分发、Author、Private 与 Console；Flutter 次级覆盖原生 PC / mobile 安装包，mobile-first、desktop stage-gated；`/desktop` 仅保留 WebOS 历史入口，Tauri 正式弃用。本文档描述当前前端事实、演进方向与相关实现约束。
...
### 1.2 当前定位：Web 优先、Flutter Native 次级

截至 `2026-08-15`，当前官方定位已经从“所有能力统一走桌面入口”和“三端并行分工”进一步收敛为：

- **纯 Web 壳层**
  - 面向公开浏览、分享传播、搜索流量、PC / 移动浏览器与登录后轻量链路
- **Flutter Native 次级原生产品线**
  - 保留 Android MVP 已有路径，长期覆盖 Android / iOS / Windows / macOS / Linux；mobile-first、desktop stage-gated，不提供 Flutter Web
- **WebOS `/desktop` 保留入口**
  - 面向已有桌面工作台能力和迁移过渡，不再作为新增功能默认承载层
- **Tauri 正式弃用资产**
  - 只保留历史代码与验证事实，不进入当前开发、UI、CI、构建、发布或验收门禁
...
5. **受控扩展性**：Web 先覆盖 PC 与移动浏览器；Flutter 在明确原生价值成立时复用数据、认证和主题语义，移动与桌面按阶段门禁推进；Tauri 不进入扩展范围

### 1.4 当前边界

- 当前代码事实仍然保留 `Desktop Shell + WindowManager`，但该能力后续仅作为 `/desktop` 历史入口维护和迁移来源
- `Clients/radish.flutter` 已完成 Android MVP 第一轮 RC 验收并落地登录、forum / docs / discover / profile、轻量通知回流、商城与个人复访等路径；现有业务 owner 继续保留，页面视觉按 [Flutter Native 产品化与 UI 重构](/features/flutter-native-product-ui-design)系统重建，不机械追平 Web
- Android 先完成产品级 UI 重构；iOS 与 Windows / macOS / Linux 在共享 UI、宽屏和输入门禁成立后分别产品化。系统推送、后台任务、签名、更新与分发仍需独立价值判断，Capacitor Android spike 只保留历史参考
- Windows / macOS / Linux 的 Tauri 壳验证只作为历史事实保留；Tauri 正式弃用，长期桌面安装包由 Flutter Native 承接
...
- `2026-08-03` 起 Web / Console 唯一活动设计源为 `Docs/frontend/design-sources/radish-web-family-ui-v1.pen`；`2026-08-19` 起 Flutter Native 使用独立 `radish-flutter-native-ui-v1.pen`。两者共享家族语义与主题身份，但各自维护组件、壳层和响应式代表设计，不跨文件复制完整页面
...
- Console 当前已形成 `Case Desk` 设计方向：低饱和暖灰 / 纸色背景、轻侧栏、克制边框、明确按钮层级和可扫描的后台信息密度；旧设计稿在 `console-governance-workbench.pen` 留档，当前代表设计进入 `radish-web-family-ui-v1.pen` 的 `R1-C01 / R1-C02`，但不直接复刻 Console 的管理后台信息结构
...
- `/notifications`、`/me`、`/messages`、`/pet` 进入公开详情或公开个人页时同样使用一次性来源转交，返回文案分别保持“通知中心 / 我的状态 / 聊天 / 电子宠物”；新开标签、复制链接、canonical、OpenGraph 和 sitemap 仍只保留公开 URL
- forum 公开分类、公开标签、公开结构化类型与公开搜索承载分类 / 标签 / 类型上下文、关键词检索、帖子列表阅读、排序分页与详情回跳；[F4-Q](/features/forum-tag-public-discovery-seo-design)已完成标签相关主题、统一公开可见性、Gateway 首包 head、runtime canonical、不可用态 `noindex` 与 tags sitemap，并保持标签关注、推荐和独立标签首页后置
...
- 公开文档详情当前已补访问属性、文档属性和时间线三组元信息：访问属性展示可见性 / 发布状态，文档属性展示 slug / 来源类型，时间线展示更新时间 / 创建时间；公开 `/docs` 列表、目录、搜索和详情只展示公开已发布文档，登录可看、受限、草稿和已删除内容进入作者态或 Console 治理，不在公开阅读页混排
- 公开榜单首批当前承载榜单切换、分页、登录用户“我的排名”增强，以及用户榜单跳转个人公开页、热门商品榜单跳转公开商品详情；默认经验榜单页 `/leaderboard`（兼容 `/leaderboard/experience` 收口）会额外展示“经验体系公开展示”说明。榜单自身继续保持只读，不直接发起购买，也不把经验明细、订单、背包、资产或其他账号工作流搬进公开壳层
...
- 公开文档搜索当前也已开始补结构化只读导览：`/docs/search` 会强调“先看结果 / 继续进入 / 不在这里”，把关键词检索、结果回跳与作者 / Console 治理边界拆清楚
...
- Flutter 原生文档详情当前复用 Web 公开文档路由口径：公开文档正文里的 `/docs/:slug`、完整公开 URL、`docs/:slug`、`./:slug` 与普通相对 slug 链接会继续打开原生文档详情；页内锚点、附件路径和非文档链接不在 Flutter 内扩成外部跳转或附件治理
...
- 后续不立即删除现有 WebOS 路由，而是把 `/desktop` 作为保留入口；有长期价值的能力优先进入 Web，具有明确原生价值时再由 Flutter Native 次级承接

## 2. 系统架构

> 说明：本节的大部分代码与结构图仍然描述 **当前 WebOS `/desktop` 保留入口的真实实现**。Web 优先、Flutter Native 次级、Tauri 正式弃用的职责分工，请优先参考 [前端多壳层策略](/frontend/shell-strategy)。
...
## 6. Web 与 Flutter Native 适配（执行中：Web 优先，Flutter 次级）

> 截至 `2026-08-15`，`radish.client` 已形成公开内容直达路径和 WebOS `/desktop` 保留入口，普通浏览器根路径 `/` 已切向 `/discover` 公开分发页；Web 继续承担 PC / mobile 浏览器主路径，Flutter Native 作为次级原生产品线按 mobile-first、desktop stage-gated 推进。
...
- Android MVP 第一轮已完成后，前端多端形态不按“React WebView 统一所有端”推进；当前分工固定为 Web 优先、Flutter Native 次级、WebOS `/desktop` 历史兼容和 Tauri 正式弃用
- Flutter 长期覆盖 Android / iOS / Windows / macOS / Linux，但必须先完成共享 UI 和 compact / medium / expanded 门禁，再分平台生成工程和处理分发；不提供 Flutter Web
...
- 登录后的轻量链路优先接入 Web；具有明确原生价值时，再在 Flutter 中增加对应能力
...
- Web 是正式优先主线；Flutter Native 是次级原生安装包产品线，WebOS `/desktop` 保留历史入口，Tauri 正式弃用，Console 继续作为 Web 治理后台
...
示例：
/ - Web 默认入口（普通浏览器切向 /discover；Tauri 历史资产不再作为正式入口）
/desktop - WebOS 保留入口
/forum - 论坛首页
