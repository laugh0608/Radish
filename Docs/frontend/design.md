...
> 截至 `2026-04-07`，仓库中的 `radish.client` 仍然是桌面 / WebOS 优先架构，当前并没有真正落地的 `MobileShell` 实现。本节描述的是 **第二开发阶段的移动 Web 规划方向**，不是当前代码事实。
...
- 第一批优先支持公开内容浏览与高频轻互动，例如论坛列表、帖子详情、文档阅读、个人公开页
