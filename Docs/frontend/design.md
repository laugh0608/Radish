...
截至 `2026-03-20`，`radish.client` 的首轮主题实现已经从概念层进入代码落地，当前结构为：
...
- 论坛帖子列表、帖子详情讨论区、评论树、评论表单、评论卡片与帖子卡片
...
截至 `2026-03-20`，`radish.client` 的 `i18n` 已从“仅有基础设施”进入“高频主链路首轮接入”阶段：
...
- 论坛剩余详情组件与发帖相关次级交互；
- 文档、通知中心、个人中心的剩余业务页；
- 与主题联动的深层样式 token 清理，避免继续出现“文案已国际化但样式仍绑定单语长度或硬编码颜色”的问题。
