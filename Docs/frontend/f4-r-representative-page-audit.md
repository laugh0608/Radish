...
>
> 状态：C-1A 与 `R1-F01` 已完成；C-1B 的 `R1-P01` 独立业务页面正在按社区内容层级修订
>
...
| `R1-P01` | Public 内容流；锚点 `/discover` | `2/2/1/2/2/1` | `10` | 建立家族化内容流和 Public 响应式语法 | `radish-web-family-ui-v1.pen`（独立画板修订中） |
...
`R1-P01` 的代码事实与设计裁决为：现有首页读取 `newest` 而非真正热帖，帖子模型同时具备单一分类、多个标签及问答 / 投票 / 抽奖等结构化状态；因此 PC / mobile 代表页必须明确区分“分类 → `#标签` → 结构化状态”，并按“分类浏览 + 最新讨论 + 社区正在发生 + 热门主题 + 相关内容”组织。所有区块复用现有 `getPostList` 类型筛选、`getTopCategories`、`getFixedTags` 与 `getHotTags`，不引入混合推荐流、额外 feed API、权限或状态机。当前设计源保留 PC 1250、mobile 390、关键状态和辅助说明四个顶层画板并继续修订；`guofeng` 品牌目标仍为灰玉 `#5d6c57`，常规操作继续使用墨蓝 `#435c74`。
...
1. `R1-F01` 已在 `radish-web-family-ui-v1.pen` 完成，固定共享契约；`R1-P01` 已形成独立业务页面画板但尚待审核。
2. 当前先审核 `R1-P01` 的 PC 1250、mobile 390 与关键状态；只有独立业务页面通过审核后才标记完成。
3. 审核通过后实现 `R1-P01` 命中的共享主题色、组件与 `/discover`，完成代码侧验证后再进入阶段性 PC / mobile 真实截图复核。
4. 设计轨随后按 `R1-P02 → R1-A01 → R1-W01 → R1-C01 → R1-C02` 更新或复核代表画板；R2 只补必要差异，不扩为路由镜像。
5. 每完成一个 R1，先对照当前代码确认功能与文案，再进入对应共享组件和代表页实现。
6. 代表页代码落地并完成截图复核后，以复核结果驱动 R3 成组实现。
