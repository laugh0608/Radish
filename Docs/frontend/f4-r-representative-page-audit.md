...
>
> 状态：C-1A 与 `R1-F01` 已完成；C-1B 的 `R1-P01` 两轮独立业务页面评审均未通过，下一步先做参考重校准与低保真结构比较
>
...
| `R1-P01` | Public 内容流；锚点 `/discover` | `2/2/1/2/2/1` | `10` | 建立家族化内容流和 Public 响应式语法 | `radish-web-family-ui-v1.pen`（两轮评审未通过） |
...
`R1-P01` 的稳定代码事实是：现有首页读取 `newest` 帖子与热门标签，并各取一个公开文档、商品和榜单入口；帖子模型同时具备单一分类、多个标签及问答 / 投票 / 抽奖等结构化状态。分类、`#标签` 与结构化状态必须继续分层，但第二轮“全宽分类浏览 + 最新讨论主流 + 辅助 rail”已经被否决：帖子仍占据绝大多数篇幅，首页没有形成多元社区感，构图也偏离参考 `13 / 16 / 18 / 27`。下一轮先审计更多公开实体与读模型，再用低保真构图比较异构模块和非对称主次；`guofeng` 灰玉品牌目标 `#5d6c57` 与墨蓝操作色 `#435c74` 继续保留，但不先于页面结构审核进入运行时。
...
1. `R1-F01` 已在 `radish-web-family-ui-v1.pen` 完成，固定共享契约；`R1-P01` 两轮业务页面评审均未通过，现有稿只由 Git 留档。
2. 当前重新查看参考 `13 / 16 / 18 / 27`，并审计帖子之外的公开实体、API、路由、权限与失败边界，不再只从 `PostCard` 反推首页。
3. 在同一活动设计源中先用轻量布局研究比较 `2~3` 个 PC 低保真构图；用户选择结构方向后，再更新唯一 PC / mobile 代表画板和关键状态。
4. 代表画板审核通过后，才实现 `R1-P01` 命中的共享主题色、组件、`/discover` 和对应静态测试；完成代码侧验证后再进入阶段性 PC / mobile 真实截图复核。
