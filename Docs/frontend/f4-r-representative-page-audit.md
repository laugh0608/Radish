...
>
> 状态：C-1A 与共享组件 / 主题基座已完成；C-1B 的 `R1-P01 / R1-P02 / R1-A01 / R1-W01` 已关闭，`R1-C01` 代码与静态验证已完成、运行态验收待授权
>
...
| `R1-C01` | Console 表格 / 明细；锚点 `/console/orders` | `2/1/2/2/2/1` | `10` | 高密度表格与 mobile 收敛规则影响多个资源页 | `radish-web-family-ui-v1.pen`（PC / Mobile / 必要关键状态、正式代码与静态验证已完成；运行态待验收） |
...
`R1-C01` 已于 `2026-08-08` 完成[Console 订单表格—明细设计前代码事实与能力覆盖门禁](/records/f4-r-r1-c01-console-orders-readiness-audit-2026-08-08)，并完成 PC `1440 × 900`、Mobile `390 × 844` 与必要关键状态正式代表设计。随后已完成[正式代码与静态验证](/records/f4-r-r1-c01-console-orders-implementation-2026-08-08)：PC 使用连续单行薄表格与按需详情 inspector；Mobile 使用连续三列订单行、Client 风格五项胶囊导航和隐藏全局导航的全屏详情任务，中列承载商品类型 / 单价与支付证据。现有 API、权限、结构化错误、LongId、幂等与事务边界保持不变；Gateway PC / mobile 运行态验收仍待后续明确授权，验收前不进入 `R1-C02`。
...
8. `R1-W01` 已完成并关闭；`R1-C01` readiness、PC / Mobile / 必要关键状态设计、正式 Console 代码与静态验证已完成。Gateway 运行态验收待后续授权，验收前不切换 `R1-C02`。
