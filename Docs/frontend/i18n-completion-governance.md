...
- **阶段**：发布后长期维护与功能完成。
- **顺位**：F1 商城与 F2 主题均已完成，F3-D Console 管理态补验是当前工程第一顺位。
- **审计基线**：`dev` 提交 `0d8e63be`；`F3-C8` 实施提交为 `d5341095 / 762e32ac`。
- **当前进度**：三端与共享边界审计、`F3-A / F3-B / F3-C1-C9` 已完成；`F3-D` 的 Public / Private / Auth 和 Console 认证 / 无权限边界已通过 `zh / en × PC / mobile` 运行态矩阵，当前只剩管理员会话下的 Console 受权内页补验。该证据补齐前 F3 不宣告完成，也不提前转入 F4。
...
`F3-D2（待补验）` 只保留管理员会话下的 Console 受权内页矩阵：Dashboard、用户、内容治理、订单、应用、个人资料与 Hangfire 在 `zh / en × PC / mobile` 下的页面壳、长文本、表格 / 抽屉、日期数字、无权限和 not-found。当前本机浏览器没有可复用的本地管理员会话，验收没有通过改角色或数据库绕过权限；取得管理员会话后只做只读补验。
...
- Console 受权内页尚未在管理员会话下完成 PC / mobile 双语运行态矩阵；其余正式 Web 代表面、OIDC 往返、认证与无权限边界已经验收。
...
| `Frontend/radish.console` | 已完成 i18n 初始化、管理壳、route meta、Dashboard、用户 / 治理 / 订单、设置、商品、文档、权限、分类标签、表情、经验、萝卜、应用、个人资料与 Hangfire 资源；认证和无权限边界已验收，受权内页仍待管理员会话补验 |
...
| `Docs/` | 已建立专题说明、F3-D 验收记录并同步规划入口；管理员会话补验后更新最终结论 |
...
2026-07-17 已在当轮授权下启动完整本地宿主并执行 F3-D1 运行态矩阵；详细页面、验证命令、数据影响和未覆盖边界见 [F3-D i18n 专题验收记录](/records/f3-d-i18n-stage-acceptance-2026-07-17)。F3-D2 仍须在新的管理员会话与当轮启动授权下执行。
