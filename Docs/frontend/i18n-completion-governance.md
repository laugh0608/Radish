...
- **审计基线**：`dev` 提交 `0d8e63be`；`F3-C8` 首个实施检查点为 `d5341095`。
- **当前进度**：三端与共享边界审计、`F3-A` 核心契约及 `F3-B1 / F3-B2` 首批正式 Web 链路均已完成；`F3-C1` 至 `F3-C8` 已按设置、商品、Docs、圈子、宠物、经验、萝卜资产、低频页面与共享反馈 / 上传边界完成本地实现和静态回归，下一顺位进入 Console 剩余管理域。
...
- 动态参数统一使用 `MessageArguments` 短数组，只允许字符串、字符、布尔与有限数值等安全标量；服务端统一规范化长度和控制字符，未知对象、非有限数值等降级为安全占位，前端只把参数交给 `MessageKey` 翻译器，不用于控制流或任意对象透传。
...
下一顺位 `F3-C9` 成组治理 Console 角色权限、分类标签、表情、经验和萝卜管理；先核对正式路由、真实消费者与既有专题边界，再统一宿主词元、locale formatter、英文数量规则和结构化错误，不建设第二套上传 / HTTP 契约，也不把在线词元编辑混入 `SystemConfig`。
...
- `F3-C` 所列 Console 权限、分类标签、表情、经验和胡萝卜管理尚未按同一契约完成迁移；高频领域错误继续随真实消费面推进，不做 Controller 全仓机械资源化；
...
| 模块 | 已完成改动与后续职责 |
| --- | --- |
| `Frontend/radish.client` | 已完成 registry、language runtime / switcher、Web Header、B1 页面、B2 Messages / Me、C3 Docs 作者态、C4 圈子、C5 宠物、C6 经验、C7 萝卜资产与 C8 低频页面 / 公开承诺 / head / 上传消费者；后续只处理真实回归并准备 F3-D 验收 |
