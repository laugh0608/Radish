...
- **F3-B2（下一组）**：Console 用户 / 内容治理 / 订单完整业务文案，client Messages / Me 与共享弹层残余反馈，以及这些链路实际消费的高频领域 `Code / MessageKey`。
...
| 模块 | 已完成改动与后续职责 |
| --- | --- |
| `Frontend/radish.client` | 已完成 registry、language runtime / switcher、Web Header、B1 页面格式化与结构化错误消费；B2 继续 Messages / Me 和共享弹层 |
| `Frontend/radish.console` | 已完成依赖、i18n 初始化、管理壳、route meta、Dashboard 与 Settings；B2 继续用户 / 治理 / 订单业务域 |
...
| `Radish.Api / Radish.Shared / Radish.Model` | B2 补首批高频领域错误键和稳定错误码；不重做 Q1-B 已有错误管线 |
