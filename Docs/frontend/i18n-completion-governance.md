...
- **审计基线**：`dev` 提交 `0d8e63be`；当前实施基线为 `cf76c647`。
- **当前进度**：三端与共享边界审计、`F3-A` 核心契约及 `F3-B1 / F3-B2` 首批正式 Web 链路均已完成；`F3-C1` 至 `F3-C7` 已按设置、商品、Docs、圈子、宠物、经验与萝卜资产业务域完成本地实现和静态回归，下一顺位继续 client 剩余低频页面与 Console 管理域。
...
- 正式 Web 仍存在用户可见硬编码，当前剩余代表面包括公开承诺页、页面 head、低频设置和共享上传等组件；已完成业务域不因历史审计命中重复返工。
...
下一顺位 `F3-C8` 成组治理 client 剩余低频设置、公开承诺长文本、页面 head 与正式页面实际消费的共享反馈 / 上传组件；先完成真实消费矩阵和内容边界核对，再统一共享 labels、长文本布局、locale formatter 与实际消费 API 错误，不把共享组件改成宿主状态持有者。
...
- `F3-C` 所列 client 低频设置与公开承诺，以及 Console 权限、分类标签、表情、经验和胡萝卜管理尚未按同一契约完成迁移；高频领域错误继续随真实消费面推进，不做 Controller 全仓机械资源化；
...
| 模块 | 已完成改动与后续职责 |
| --- | --- |
| `Frontend/radish.client` | 已完成 registry、language runtime / switcher、Web Header、B1 页面、B2 Messages / Me、C3 Docs 作者态、C4 圈子、C5 宠物、C6 经验与 C7 萝卜资产；F3-C 继续低频设置与公开承诺 |
| `Frontend/radish.console` | 已完成 i18n 初始化、管理壳、route meta、Dashboard、B2 用户 / 治理 / 订单，以及 C1 设置、C2 商品配置与 C3 文档治理域；F3-C 继续其余低频管理域 |
| `Frontend/radish.ui` | 已完成 `ThemeProvider` locale、共享 formatter、首批 labels 契约与 `ExperienceBar` 宿主 presentation；后续按正式宿主消费面扩展 |
...
| `Radish.Api / Radish.Shared / Radish.Model` | 已补 B2 订单 / 权益 / 治理、C2 商品、C3 Wiki、C4 UserFollow、C5 Pet、C6 Experience 与 C7 Coin / PaymentPassword 错误键和稳定错误码，并提供订单有效期结构字段和商品能力说明 key；后续随 F3-C 真实消费面扩展，不重做 Q1-B 已有错误管线 |
