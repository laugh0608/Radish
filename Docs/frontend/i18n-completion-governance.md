...
- **审计基线**：`dev` 提交 `0d8e63be`；实施基线已推进至 `d0e72b2b`。
- **当前进度**：三端与共享边界审计、`F3-A` 核心契约及 `F3-B1 / F3-B2` 首批正式 Web 链路均已完成；`F3-C1` 设置域、`F3-C2` 商品配置域、`F3-C3` Docs 业务域与 `F3-C4` 圈子域已完成本地实现与静态回归，下一顺位继续 `F3-C` 剩余业务域。
...
- `F3-C` 所列 client 宠物、经验、萝卜坑、低频设置与公开承诺，以及 Console 权限、分类标签、表情、经验和胡萝卜管理尚未按同一契约完成迁移；高频领域错误继续随真实消费面推进，不做 Controller 全仓机械资源化；
...
| 模块 | 已完成改动与后续职责 |
| --- | --- |
| `Frontend/radish.client` | 已完成 registry、language runtime / switcher、Web Header、B1 页面、B2 Messages / Me、C3 Docs 作者态与 C4 圈子；F3-C 继续剩余业务域 |
...
| `Radish.Api / Radish.Shared / Radish.Model` | 已补 B2 订单 / 权益 / 治理、C2 商品、C3 Wiki 与 C4 UserFollow 错误键和稳定错误码，并提供订单有效期结构字段和商品能力说明 key；后续随 F3-C 真实消费面扩展，不重做 Q1-B 已有错误管线 |
