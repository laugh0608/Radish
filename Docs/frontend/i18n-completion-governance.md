...
- **审计基线**：`dev` 提交 `0d8e63be`，工作区无未提交变更。
- **当前进度**：三端与共享边界审计已完成；`F3-A` 核心契约和 `F3-B` 第一组正式 Web 链路已实现，专题仍处于开发中。
...
- Console 用户、内容治理和订单页面仍有大量业务文案待按域迁移，当前仅保证壳层、Dashboard、文档标题和关键格式化契约；
- client Messages、Me 及 Forum / Discover 的既有资源虽已纳入 registry，但仍需按首批矩阵复核长文本、共享弹层和残余硬编码；
- API 高频领域错误仍需继续补稳定 `Code / MessageKey`，当前已先闭环通用错误消费和结构化控制流；
...
| 模块 | 预计改动 |
| --- | --- |
| `Frontend/radish.client` | i18n registry、language runtime / switcher、Web Header、首批页面资源、格式化调用、错误状态判断与测试 |
| `Frontend/radish.console` | 依赖声明、i18n 初始化、Console 资源、AdminLayout / route meta / Settings、首批管理页面与测试 |
| `Frontend/radish.ui` | `ThemeProvider` locale 输入、正式链路使用的 labels / formatter 契约、定向测试 |
| `Frontend/radish.http` | language / translator 配置、`Accept-Language`、响应解析字段与本地化测试 |
| `Radish.Auth` | `.resx`、三张 Razor View、文化持久化入口、Controller 反馈与测试 |
| `Radish.Api / Radish.Shared / Radish.Model` | 首批错误键、必要的稳定错误码与契约测试；不重做 Q1-B 已有错误管线 |
| `Docs/` | F3 状态、长期 i18n 规范、HTTP / 错误职责和最终验证口径同步 |

Console 新增 i18next 依赖会修改 `Frontend/radish.console/package.json` 与根 `package-lock.json`。实施前必须按协作规则单独说明版本和命令，并获得当前任务授权。
...
| 新依赖扩大 lockfile | 只为 Console 添加与 client 对齐的既有依赖版本，执行前单独授权并复核 lockfile |
