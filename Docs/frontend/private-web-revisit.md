...
| `/messages` | 复访聊天频道与一对一会话、处理陌生人消息请求、定位通知中的消息、从成员进入公开主页后返回聊天 | 频道列表、会话分区、私聊请求与归档、消息历史、`channelId/messageId` 定位、Chat Hub | 公开聊天室、私有群组、消息搜索、Reaction、置顶、逐条阅读回执、移动系统通知 |
...
私域复访入口统一接入 `radish.client` 共享 Web shell：
...
通知未读角标由共享 `notificationStore` 承接，但数据真值始终来自 `GetInboxSummary`。正式 Web header、Workbench 与 WebOS Dock 不自行增减未读数；`NotificationInboxChanged` 只携带变更提示和单调 `revision`，重复、乱序、重连、重新获得页面可见性或 cursor 过期时统一刷新权威 summary，必要时重载列表。跨标签页只广播“需要对账”的信号，不复制某个标签页的本地计数。
...
纯 Web `/notifications` 只使用服务端返回的 `NotificationTargetVo.voKind` 分流，不读取标题、正文、关键字、`extData` 或 `businessType` 猜测目标：

| `voKind` | 目标 |
| --- | --- |
| `ForumPost` | 优先使用 `voPostPublicId` 进入 `/forum/post/:id`，可携带合法 `voCommentId` 定位，并记录来源为通知 |
| `ChatConversation` | 使用 `voChannelId` 进入 `/messages`，可携带合法 `voMessageId` 定位 |
| `UserProfile` | 优先使用 `voUserPublicId` 进入 `/u/:identifier`，并记录来源为通知 |
| `ShopOrder` | 使用 `voOrderId` 进入 `/shop/order/:orderId` |
| `Inventory` | 进入 `/shop/inventory` |
| `Experience` | 进入 `/me/experience` |
| `DocsDocument` | 使用 `voDocumentSlug` 进入公开文档详情 |
| `GovernanceCase` | 使用 `voGovernanceCaseId` 进入 Console 治理入口，并保留返回通知中心参数 |
| `None` | 不提供跳转动作；目标已失效时根据服务端 `voUnavailableReason` 展示本地化原因 |
...
- 目标字段缺失、非法或服务端判定资源不可用时保持通知可读，但不回退到首页、论坛或相似业务页面；是否仍可访问由目标页继续执行自身 ACL。
...
- 消息：频道列表、`互相关注 / 陌生人私信 / 好友群组 / 公共频道` 服务端权威分区、通知消息定位、请求处理、归档、成员 ACL、消息幂等和附件访问；一对一私聊的完整边界见 [正式 Web 一对一私聊与会话管理设计](/features/chat-direct-conversation-design)，前端不根据频道名称或类型回退猜测会话关系。
- 宠物：领取、命名、状态展示、四类照顾动作、每日次数 / 冷却展示和最近流水。

后续若要扩展公开聊天室、私有群组、消息搜索、Reaction / 置顶 / 阅读回执、转账、支付口令、安全设置、资料编辑深水区、Web Push / 系统通知栏、宠物经济系统或公开宠物名片，必须重新评审产品边界和验证范围。
...
- 通知页面、Workbench、正式 Web header 与 WebOS Dock 复用 `Frontend/radish.client/src/apps/notification/NotificationCenter.tsx`、`notificationStore` 和 `notificationInboxSync`，不各自维护未读真值。
