...
- 消息：频道列表、`互相关注 / 陌生人私信 / 好友群组 / 公共频道` 会话分区、通知消息定位和基础发送；一对一私聊按 [正式 Web 一对一私聊与会话管理设计](/features/chat-direct-conversation-design) 补齐服务端权威分区、消息请求、归档、成员 ACL、消息幂等和附件访问，前端不再长期保留频道类型回退猜测。
- 宠物：领取、命名、状态展示、四类照顾动作、每日次数 / 冷却展示和最近流水。

后续若要扩展公开聊天室、私有群组、消息搜索、Reaction / 置顶 / 阅读回执、转账、支付口令、安全设置、资料编辑深水区、通知偏好、宠物经济系统或公开宠物名片，必须重新评审产品边界和验证范围。
