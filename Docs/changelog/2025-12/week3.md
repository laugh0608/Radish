...
- **docs(client)**: 重构文档结构，文档归档到 radish.docs
  - 将 `COMPONENTS.md` 移至 `Docs/radish.docs/docs/ComponentLibrary.md`
  - 将 `QUICKSTART.md` 移至 `Docs/radish.docs/docs/WebOSQuickStart.md`
  - 更新 `Frontend/radish.client/README.md` 为简洁版本，指向详细文档
...
- **refactor(docs/folder-rename)**: 重命名根目录 docs 文件夹为 Docs
  - 将 `docs/` 重命名为 `Docs/`，与 `Docs/radish.docs/` 区分
  - 更新 `README.md`、`CLAUDE.md`、`AGENTS.md` 中的相关引用
  - `Docs/` 作为文档入口，提供跳转链接到 `Docs/radish.docs/docs/` 的实际文档内容
...
- **feat(gateway+client/oidc-through-gateway)**: 通过 Gateway 打通 Auth + Api + 前端的完整 OIDC 授权码链路
  ...
  - `Frontend/radish.client/src/App.tsx` 新增最小 OIDC Demo：
...
- **docs(auth+gateway+frontend)**: 同步认证与 Gateway 文档，标记当前实现状态
  - `AuthenticationGuide.md` 中说明当前仓库已在 `Frontend/radish.client/src/App.tsx` 中实现一套通过 Gateway 的极简 OIDC 流程，本节其余 `oidc-client-ts/react-oidc-context` 内容为后续正式接入方案。
...
- **refactor(forum/components)**: 优化前端组件架构，将通用组件迁移到共享库
  - 将 `MarkdownRenderer` 组件从 `Frontend/radish.client/src/shared/` 迁移到 `Frontend/ui/src/components/`
    - 基于 react-markdown v9 + remark-gfm + rehype-highlight
    - 支持 GitHub Flavored Markdown (GFM)
    - 外链自动添加 target="_blank" 和安全属性
    - 表格自动添加横向滚动容器
    - 代码块语法高亮（rehype-highlight）
  - 将 `GlassPanel` 毛玻璃面板组件迁移到 `Frontend/ui/src/components/`
    - 可配置透明度和模糊度
    - GPU 加速优化
  - 更新所有组件导入路径：`import { MarkdownRenderer, GlassPanel } from '@radish/ui'`
  - 删除 `Frontend/radish.client/src/shared/` 目录（已完全迁移）
...
- **docs(forum/features)**: 创建论坛功能详细文档
  - 新增 `Docs/radish.docs/docs/features/forum-features.md`
  - 详细说明已实现的 5 大核心功能：
...
- **docs(logging)**: 创建日志系统专项文档并整理现有文档
  - **新增文档**：`Docs/radish.docs/docs/guide/logging.md`
  ...
  - **更新 CLAUDE.md**：
    ...
    - 添加指向详细文档的链接
  - **更新文档导航**：
    - 修改：`Docs/radish.docs/.vitepress/config.mts`
    - 在"开发指南"章节添加"日志系统"链接
  - **文档特点**：
    - 完整覆盖三种日志类型
    - 包含实际代码示例和配置示例
...
- **feat(forum/comment-edit)**: 实现评论编辑功能
  ...
  - **样式设计**：
    ...
    - 取消按钮：灰色主题
  - **文件变更**：
    - 修改：`Frontend/radish.client/src/apps/forum/components/CommentNode.tsx` (+100 行)
    - 修改：`Frontend/radish.client/src/apps/forum/components/CommentNode.module.css` (+120 行)
    - 修改：`Frontend/radish.client/src/apps/forum/components/CommentTree.tsx` (+2 行)
    - 修改：`Frontend/radish.client/src/apps/forum/ForumApp.tsx` (+30 行)
    - 修改：`Frontend/radish.client/src/api/forum.ts` (+25 行)
...
- **fix(forum/text-align)**: 修复论坛帖子和评论的文本对齐问题
  ...
  - **文件变更**：
    - 修改：`Frontend/radish.client/src/apps/forum/components/PostDetail.module.css` (+2 行)
    - 修改：`Frontend/radish.client/src/apps/forum/components/CommentNode.module.css` (+1 行)
...
- **feat(ui/markdown-editor)**: 实现 MarkdownEditor 富文本编辑器组件
  ...
  - **文件变更**：
    - 新增：`Frontend/ui/src/components/MarkdownEditor/MarkdownEditor.tsx` (450 行)
    - 新增：`Frontend/ui/src/components/MarkdownEditor/MarkdownEditor.module.css` (175 行)
    - 新增：`Frontend/ui/src/components/MarkdownEditor/index.ts` (2 行)
    - 修改：`Frontend/ui/src/components/index.ts` (+3 行)
    - 修改：`Frontend/radish.client/src/apps/forum/components/PublishPostForm.tsx` (+8 行)
  - **提交**: 
...
- **docs(ui-library)**: 更新 UI 组件库文档
  ...
  - 更新导入示例
  - **文件变更**：
    - 修改：`Docs/radish.docs/docs/frontend/ui-library.md` (+60 行)
