...
- `P3-12-D40` 完成 UI 专题退出判断：D36 退出条件均有 D37-D39 证据支撑，公开首页由 `/discover` 承接，公开聊天室、内部调度 / Jobs 平台和独立移动 Console 应用后置，D39 剩余的 DPR、写动作和 Docs 编辑详情限制进入发布候选准备风险清单。
- 下一步进入 `P3-12-E1` 发布候选准备：先整理验证矩阵、`master..dev` / PR 范围、剩余风险、回滚口径和不进入本次发布的后置项；不提前创建 tag 或进入部署流程。
