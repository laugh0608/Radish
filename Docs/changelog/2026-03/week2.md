...
- **当日最终口径**：`P4-ext` 收口后先保持空档，优先完成文档、测试资产与必要补丁对齐，不在当天提前启动下一主线。
...
- **当日后续更新**：晚些时候随着 `P4-ext` 首轮闭环与回归完成，规划已进一步改为“暂不提前指定新的当前主线”。
...
- **附件误清理已修复**：`FileCleanupJob` 已补识别聊天室图片引用与 Markdown 上传引用；`ChatService / PostService / CommentService` 也已在业务成功后回绑附件业务归属，避免聊天图片、问答回答图片再被误判为孤立附件。
- **日志文件锁问题已缓解**：`Serilog` 文件输出已补 `shared: true`，用于缓解 `Logs/Radish.Api/SerilogDebug` 中暴露出的日志文件占用问题。
- **受影响附件已恢复**：`DataBases/Recycle/orphan/20260315` 中被误移走的聊天图片与问答回答图片，已在当日移回 `DataBases/Uploads/...`，并同步恢复 `Attachment` 记录的 `IsDeleted / BusinessId` 状态。
- **当日结束口径**：本轮以“P4-ext 首轮最小闭环完成 + 收口后补丁完成 + 受影响数据恢复”作为实际结束状态，不提前展开下一阶段任务。
