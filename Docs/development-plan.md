...
- **最近结论**：
  ...
  - 当前切到 `P3-8-D`：移动 Web 公开视图验收矩阵、根路径 `/` 默认入口切换、历史 `?demo` 入口移除、公开个人页分享入口、Flutter 公开商品只读详情、轻量 forum 通知列表、公开商城列表、论坛详情评论发布 / 回复、原生公开详情分享入口、公开文档阅读链路和公开主页链接 / 来源返回复核已完成；后续继续从纯 Web / Flutter 真实主路径缺口中选择下一批可验证任务
...
- `20%`：纯 Web 维护线，只回拉公开访问、分享预览、购买 / 订单 / 背包、权限授权或运行日志暴露的高信号问题
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 根路径 `/` 默认入口切换、`?demo` 清理、`/u/:id` 显式分享入口、移动 Web 公开视图矩阵阶段收口、Flutter 公开商品只读详情、轻量 forum 通知列表、公开商城列表、论坛详情评论发布 / 回复、公开详情分享入口、docs 文档内链阅读和原生公开主页链接 / 来源返回复核已完成；下一批建议回看纯 Web 登录后轻量链路或跨端回流缺口，购买 / 订单 / 背包只在维护线暴露阻断时回拉
