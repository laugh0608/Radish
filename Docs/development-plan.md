...
- **当前主线**：`P3-10 Web-first 入口收口与 B8 Radish 电子宠物边界设计`
- **最近结论**：
  ...
  - `/notifications`、`/me`、`/messages` 已承接进入 B8 前需要的通知、个人状态、成长 / 资产只读和会话 / 消息复访能力；下一顺位转向 `P3-10-B8 Radish 电子宠物规划` 的玩法边界、经济约束、复访入口和 Web-first 页面归属设计。
...
- `35%`：`P3-10-B8 Radish 电子宠物规划`，先做玩法边界、经济系统约束、复访入口、Web-first 页面归属、Console 配置和流水审计需求。
- `20%`：WebOS 功能迁移图维护收口；`/notifications`、`/me`、`/messages` 已完成首批纯 Web 私域复访入口，后续只在新真实缺口或发布候选回归暴露问题时回拉。
...
- `P3-10-B8 Radish 电子宠物规划`
  - B7 已确认没有必须先迁的用户复访或经济系统前置能力；下一步先定义玩法边界、经济系统约束、Web-first 入口、与 `/me` 成长 / 资产摘要的关系、Console 配置和流水审计需求。
  - 不直接实现完整游戏系统，也不把宠物系统塞进 WebOS `/desktop` 作为新增主线。
