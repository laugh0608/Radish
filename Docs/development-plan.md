...
- **当前结论**：
  ...
  - F4-L-A 已完成候选审计与权威设计：相较匿名公开聊天、论坛作者版本恢复和内容赞赏，Wiki 附件隐私与生命周期更直接修复现有正式 Author / Console 链路的隐私与回收缺口；专题固定 Main `WikiAttachmentReference`、私有默认、动态 ACL、事务同步、令牌先鉴权后消费、清理引用权威、迁移、页面与 A-D 批次。下一顺位进入 F4-L-B。
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-K 均已关闭，F4-L-A 权威设计完成。下一顺位执行 F4-L-B 服务端权威契约。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-K 已关闭，功能完成线当前进入 F4-L-B。
...
12. [F4-L Wiki 附件隐私与生命周期权威闭环](/features/wiki-attachment-privacy-lifecycle-design) 已完成候选审计和权威设计；下一顺位进入 F4-L-B，落地 Main 权威引用、私有默认、动态 ACL、事务同步、迁移与清理链。
