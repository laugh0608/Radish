...
- **产品下一顺位**：`先冻结账号删除与 iOS 数字权益购买策略，再实施 distribution Gateway fail-closed、Flutter 举报 / 屏蔽和合规门禁；真机与签名后置`
...
- **当前结论**：
  ...
  - [P6-B Android AVD 第一轮运行态验收](/records/f4-flutter-native-p6b-android-avd-runtime-acceptance-2026-08-29)已修正会话续签、公开资料 Public API、Forum revision、轻回应 `10` 字上限与地址清空契约；[第二轮关闭记录](/records/f4-flutter-native-p6b-android-avd-runtime-acceptance-closure-2026-08-30)进一步确认 medium 系统浏览器 OIDC / 冷启动、四主题代表面、compact 真实输入法与根评论连续 CAS。项目所有者随后通过 [P6 AVD 门禁关闭裁决](/records/f4-flutter-native-p6-android-avd-gate-owner-closure-2026-08-30)给出 `Android UI AVD RC Go`，P6-C 真机转为分发前后置门禁；[P7-A iOS readiness](/records/f4-flutter-native-p7a-ios-platform-readiness-2026-08-30)已确认工具链与实施边界。[P7-B 平台基座](/records/f4-flutter-native-p7b-ios-platform-foundation-implementation-2026-08-30)已完成 iOS 工程、安全认证存储、非敏感偏好、Android 幂等迁移及 UIScene OIDC callback，Flutter `435 / 435`、analyze、Android JVM `7 / 7`、新 Debug APK、iOS 无签名 build 与 RunnerTests build-for-testing 均通过；[P7-C Simulator 关闭记录](/records/f4-flutter-native-p7c-ios-simulator-runtime-acceptance-closure-2026-08-31)进一步以同一 ad-hoc 候选完成 iPhone compact 与 iPad medium / expanded 的 Safari OIDC、UIScene 冷 callback、Keychain / preferences、TLS、旋转、真实输入、四主题和 fail-closed 验收，结论为 `Simulator Go`。[P7-D readiness](/records/f4-flutter-native-p7d-ios-device-signing-distribution-readiness-2026-09-01)已确认实际真机 / 分发为 `No-Go`，下一顺位先关闭 distribution Gateway、账号删除、Flutter UGC 与 iOS commerce 策略门禁。
...
- 当前已进入发布后常态开发与 F4；一对一私聊、F4-B 至 F4-Q、F4-S 及 F4-R Web 页面族 / 主题退出门禁均已关闭，`v26.8.1-release` 正式 tag 与五镜像发布完成。Flutter Native P1–P5、P6 Android 双 AVD 开发门禁与 P7-C iOS Simulator 开发门禁均已关闭；P6-C 真机验收按项目所有者裁决转为 Android 分发前门禁，P7-D readiness 已完成并把 P7-D1 distribution compliance capability gate 设为当前第一顺位。D1 方案确认前不修改账号 / 交易规则，不读取 / 写入签名材料，不连接或安装真机，不创建 archive 或外部分发状态。生产部署与长期运维由项目所有者独立负责，不进入当前开发顺位。
...
2. **Flutter**
   - 作为次级原生安装包产品线，长期覆盖 Android / iOS / Windows / macOS / Linux；移动优先，桌面达到 UI 与输入门禁后分平台产品化。
   - 不机械追平 Web，不提供 Flutter Web；Console、SEO 和完整 Author 默认留在 Web，系统推送、后台任务、签名与分发分别立项。
3. **WebOS `/desktop`**
   - 只作为历史兼容入口，不承接新增功能。
   - 只处理阻断级问题和迁移所需缺口。
4. **Tauri**
   - 正式弃用，保留历史代码与验证资产，不进入日常开发、UI、CI、构建、签名、分发或验收门禁。
   - Flutter Native 承接长期桌面安装包目标；不得以“桌面需要壳”为由恢复 Tauri。
5. **Console**
...
- 阶段结论已归档；WebOS 与 Tauri 不再作为新增功能主线，长期桌面安装包目标转由 Flutter Native 分阶段承接。
...
17. [F4-R 家族 UI 统一接入与产品视觉重构](/features/family-ui-convergence-design)的 Web 基线已关闭；[Flutter Native 产品化与 UI 重构](/features/flutter-native-product-ui-design)已完成 P0–P5，[P5-E 成组静态门禁](/records/f4-flutter-native-p5e-grouped-static-gate-implementation-2026-08-27)以 `396 / 396` 成组、`419 / 419` 全量、Shell `51 / 51` 与 analyze 零问题关闭首轮静态门禁。P6-A / B 已完成 Android 候选装配和双 AVD 运行态，项目所有者通过 [P6 AVD 门禁关闭裁决](/records/f4-flutter-native-p6-android-avd-gate-owner-closure-2026-08-30)允许以 `Android UI AVD RC Go` 进入下一阶段，P6-C 真机验收改为 Android 分发前后置门禁。[P7-A iOS readiness](/records/f4-flutter-native-p7a-ios-platform-readiness-2026-08-30)与 [P7-B 平台基座](/records/f4-flutter-native-p7b-ios-platform-foundation-implementation-2026-08-30)已完成工具链、`ios/`、安全认证存储、非敏感偏好、Android 迁移、iOS OIDC callback owner 和原生 build 门禁；[P7-C Simulator 关闭记录](/records/f4-flutter-native-p7c-ios-simulator-runtime-acceptance-closure-2026-08-31)以 iPhone compact 与 iPad medium / expanded 真实运行给出 `Simulator Go`。[P7-D readiness](/records/f4-flutter-native-p7d-ios-device-signing-distribution-readiness-2026-09-01)进一步冻结 D1 合规能力、D2 真机、D3 Internal TestFlight、D4 external / App Store 分层；下一步只进入 D1 方案，不自动扩大为 Apple 账号、真机或外部分发。
...
- WebOS `/desktop` 阻断级兼容。
- Flutter analyze / test、已落地主路径回归和经裁决的 Native PC / mobile 页面族；桌面平台构建与发布在阶段门禁后分别进入。
- Q4 大文件、共享前端边界、全量卫生和文档归档按触达范围持续下降。

## 明确后置

- 在 `dev -> master` PR、正式发布记录、required checks 或镜像供应链门禁未通过时创建 tag 或生产部署。
- WebOS 新功能、Tauri 恢复 / 分发、Flutter Web、Flutter 机械追平 Web 和独立 Native Console。
