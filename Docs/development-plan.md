...
- **产品下一顺位**：`在既有公开个人主页消费权威 VoPet；先设计、后实现`
...
- **当前结论**：
  ...
  - F4-H-A / B 已完成：电子宠物公开名片的权威设计、独立公开 VO、服务端用户 / 租户 / 公开 / 软删除查询、`UserPublicProfileVo.VoPet` 聚合和 client 类型已落地；下一顺位进入 F4-H-C Pencil 与正式 Web。
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-G 均已关闭，F4-H-A / B 已完成。下一顺位执行 F4-H-C，不并行实施其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B / C / D / E / F / G 已关闭，功能完成线当前进入 F4-H-C Pencil 与正式 Web。
...
8. [F4-H 电子宠物公开名片与隐私闭环](/features/radish-pet-roadmap) 已完成 A / B 批设计与服务端契约；下一顺位进入 F4-H-C，先更新公开个人主页 PC / mobile Pencil，再实现正式 Web，移动系统通知继续后置。
