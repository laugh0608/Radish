...
- **产品下一顺位**：`按服务端检索契约、正式 Web 页面和成组验收推进；消息 Reaction / 置顶继续后置`
...
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊与 F4-B 通知中心深化已关闭，功能完成线当前进入 F4-C 现状审计和专题设计。
...
3. 下一专题固定为 F4-C 聊天历史搜索与消息定位；先完成 Chat 数据、ACL、索引 / 分页、结果定位、删除 / 阻断语义、PC / mobile 入口和恢复边界的现状审计与专题设计。
4. 设计确认后依次推进服务端权威检索契约、正式 Web 搜索工作区和双账号成组验收；消息 Reaction、置顶及移动系统通知继续后置。
