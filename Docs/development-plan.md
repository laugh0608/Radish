...
- **当前子阶段**：`F4 既有功能持续完成`
- **工程第一顺位**：`F4-B-B 通知数据、查询与服务端契约`
- **产品下一顺位**：`F4-B-C 正式 Web 通知工作区与 F4-B-D 成组验收；聊天搜索 / Reaction / 置顶继续后置`
...
- **当前结论**：
  ...
  - F4-B-A 已完成通知生产者、数据 / 服务 / Hub、正式 Web 消费面和历史文档审计，并固定稳定定义、结构化目标、偏好、分组 / 摘要 revision、容量和验收边界。下一顺位进入 F4-B-B 服务端实现。
...
- 当前已进入发布后常态开发与 F4；一对一私聊已关闭，当前只推进 F4-B 通知中心深化，不并行展开其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n 与一对一私聊已关闭，功能完成线当前进入通知中心深化服务端实现。
...
2. [F4-B 通知中心深化与通知治理](/features/notification-center-deepening) 的审计与设计已经完成；下一批先实现定义注册表、结构化目标、偏好、分组 / 摘要、迁移、专属仓储事务、新 HTTP API 和 revision SignalR 事件。
3. 服务端契约通过后推进正式 Web 通知工作区，再以双账号、双语和 PC / mobile 完成专题验收。
4. 聊天搜索 / Reaction / 置顶及移动系统通知继续后置，不与通知专题并行扩展。
