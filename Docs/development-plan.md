...
- `P3-10 后置安全治理评审：支付 / 转账幂等与重放边界`
  - 前端敏感日志脱敏和支付口令哈希升级已完成；支付 / 转账幂等边界已拆到 [支付与转账幂等治理](/guide/payment-idempotency-governance)。
  - 只治理商城购买、萝卜币转账等写操作的重复点击、网络重试和短窗口重放保护；不直接启动浏览器通用签名、字段级加密、安全会话、完整钱包、经济扩展或资产风控扩面。
