...
- 旧 `GetCommentTree` 当前已进入兼容观察态：仓库内主链、验收脚本与功能文档已切到新分页口径，后续只需再确认仓库外是否仍有依赖，再决定正式删除旧入口
...
- 下一轮社区技术债回归焦点已切换到分页链路稳定性与旧入口退役评估：优先补论坛评论“默认排序 + 加载更多 + 评论发布后局部更新”的边界回归，并结合旧 `GetCommentTree` 命中情况决定正式删除窗口
