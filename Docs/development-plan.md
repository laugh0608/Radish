...
- **最近结论**：
  ...
  - 当前进入 `P3-9`：把已打通能力组织成访客、登录移动用户和管理员三类真实使用路径，形成可体验、可演示、可验收的发布候选整备批次。
...
- `45%`：P3-9 登录移动用户主路径产品化，重点是 Flutter 中浏览、发帖、评论、单商品购买、订单、背包、胡萝卜流水、通知、资料编辑的连续体验与状态恢复。
- `25%`：P3-9 访客公开访问与分享整备，重点是纯 Web 公开内容发现、阅读、分享、来源返回和 SEO 预览的发布候选验收。
- `20%`：P3-9 Console 排障与治理入口整备，重点是用户、订单、商品、胡萝卜流水、权限授权和治理动作的管理员路径。
- `5%`：WebOS `/desktop` 保留迁移线，只处理阻断级问题和迁移所需缺口。
- `5%`：P3-8-D / ID Phase A / Console UI 维护回拉，只处理新增命中、真实编译错误、发布候选验收缺口或明显视觉断裂。
...
- `P3-9-A 真实使用路径验收入口`
  - 建立首批主路径验收矩阵，覆盖访客公开访问、登录移动用户和 Console 管理员。
  - 优先沉淀可复用的路径 helper、路由状态守护、手动验收脚本和定向测试。
  - 不引入完整 E2E 平台，不启动本地服务作为文档或 helper 开发的前置。
- `P3-9-B 登录移动用户主路径整备`
  - 复核 Flutter 从发现、论坛、文档、商城和我的入口进入常用任务的连续体验。
  - 只补阻断体验、登录恢复、失败态、刷新态、返回态和 LongId / PublicId 契约缺口。
- `P3-9-C 访客公开访问与分享整备`
  - 复核纯 Web 公开路径的可访问、可分享、可预览、可返回和窄屏可读。
  - 不恢复移动 Web 逐页打磨节奏。
- `P3-9-D Console 排障与治理入口整备`
  - 复核同一用户、同一订单和同一商品在 Console 商品、订单、胡萝卜流水、用户详情与权限授权之间的排障路径。
  - 只在真实使用、权限页面、明显视觉断裂或数据契约问题暴露时回拉。
