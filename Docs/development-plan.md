...
- **最近结论**：
  ...
  - 当前切到 `P3-8-D`：移动 Web 公开视图验收矩阵、根路径 `/` 默认入口切换、历史 `?demo` 入口移除、公开个人页分享入口、Flutter 公开商品只读详情与轻量 forum 通知列表小闭环已完成；后续继续从纯 Web / Flutter 真实主路径缺口中选择下一批小闭环
...
- `15%`：Flutter 真实移动主路径小闭环，已补公开商品只读详情，后续仍只选择一个高价值小批次
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 根路径 `/` 默认入口切换、`?demo` 清理、`/u/:id` 显式分享入口、移动 Web 公开视图矩阵阶段收口、Flutter 公开商品只读详情和轻量 forum 通知列表已完成；下一批仍只选择一个高价值小闭环进入实现，纯 Web 登录后轻量链路与购买 / 订单 / 背包只在维护线暴露阻断时回拉
