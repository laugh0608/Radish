...
- **工程第一顺位**：`Q3 候选质量门禁与测试升级`
- **产品下一顺位**：`Release Go 后小规模受控试用`
...
- **当前结论**：
  ...
  - Q1、Q2-A 时间语义、Q2-B 数据库演进和 Q2-C 版本单一真值的 Release Go 必要子集均已关闭；当前进入 Q3 候选质量门禁。
...
- `50%`：Q3 候选 lint、编译 warning、现有测试基线与高风险集成测试。
- `20%`：Gateway smoke、候选交付与回滚材料。
- `15%`：Q4 时间存量、strict、仓库卫生和触达范围治理。
- `10%`：版本、数据库迁移和依赖安全门禁维护。
...
- 当前 P3-12-F 负责候选期可靠性、数据库演进、版本、候选验证与受控试用。
...
1. Q1 与 Q2 Release Go 必要子集已完成。
2. 先审计并形成 Q3 候选质量门禁有限方案，再分批收口 lint、warning、baseline 与高风险集成测试。
3. 完成 Gateway smoke、交付 / 回滚材料和小规模受控试用。
4. 根据真实使用证据判断第三阶段是否收束，不用继续内部 UI 扫描替代用户反馈。
