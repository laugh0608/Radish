...
- **产品下一顺位**：`从社区核心与支撑能力中审计现有断点，选定一个长期价值和权威边界清楚的完整专题`
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-H 均已关闭。下一顺位执行 F4-I-A 候选复核与权威设计，不并行实施多个候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-H 已关闭，功能完成线当前进入 F4-I-A 下一专题候选复核与权威设计。
...
9. 下一顺位进入 `F4-I-A`，只读交叉审计社区核心与支撑域的真实断点，选定一个完整专题并先建立权威设计、批次边界、停止线和验收口径；不重启主动生产证据采集。
