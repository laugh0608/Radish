...
- **当前结论**：
  ...
  - F4-M 已完成 A-D 批并关闭：Post / Comment Revision、CAS、完整快照、旧历史兼容、安全恢复、正式 Web、多身份 PC / mobile、临时数据清理及 SQLite / PostgreSQL 验证形成闭环。F4-N-A 权威设计已完成，首批固定 `Post / Comment`、预设理由、Main 原子资产事务、Log 幂等审计投影和逐笔屏蔽抑制通知；等待批准进入 F4-N-B。
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-M 均已关闭。F4-N-A 权威设计已完成，下一顺位等待批准进入 F4-N-B 服务端与 migration。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-M 已关闭，功能完成线当前停在 F4-N-A 完成后的批准门。
...
13. [F4-M 论坛内容版本完整性与作者恢复](/features/forum-content-version-recovery-design) 已完成 A-D 批并关闭；[F4-N 论坛内容赞赏](/features/forum-content-reward)已完成 A 批权威设计，进入服务端、migration、Pencil 或页面前等待明确批准。
