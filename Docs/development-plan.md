...
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B / C / D 已关闭，功能完成线当前进入 F4-E-A 消息置顶现状审计与专题设计。
...
5. F4-E 进入聊天消息置顶。首批只执行现状审计与专题设计，固定治理权限、单条 / 多条状态、频道顶部入口、消息定位、Hub 同步、撤回 / 删除 / 失权恢复和成组验收口径；逐条已读和移动系统通知继续后置。
