...
1. 进入 F4-A，按登录、首次内容参与、回应后回流、聊天、通知与 Console 管理链路盘点可用的真实使用证据、用户反馈和失败记录，记录影响面、复现条件与责任模块。
2. 将 `P0/P1` 直接纳入维护线，把 `P2/P3` 按同类成因成组排期；不把技术告警、产品缺口和新增需求混成一份无边界清单。
3. 从证据支持的候选中确认一个具备明确用户路径、长期价值、数据 / 接口 / 页面边界、停止线和验证口径的完整功能专题，先更新或建立专题设计，再进入代码实现。
