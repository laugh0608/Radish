...
- **产品下一顺位**：`从现有明确缺口中裁决一个边界完整、长期有价值的单一专题`
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-L 均已关闭。下一顺位执行 F4-M-A，只读复核明确缺口并裁决一个唯一专题。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-L 已关闭，功能完成线当前进入 F4-M-A。
...
13. 下一顺位进入 `F4-M-A`：从现有明确功能缺口中只读复核并裁决一个唯一专题，先完成权威设计，不并行启动多个实现，不重启主动生产证据采集。
