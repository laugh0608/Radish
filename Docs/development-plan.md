...
- **最近结论**：
  ...
  - 当前切到 `P3-8-D`：移动 Web 公开视图验收矩阵、根路径 `/` 默认入口切换、历史 `?demo` 入口移除和公开个人页分享入口小闭环已完成，后续继续从纯 Web / Flutter 真实主路径缺口中选择下一批小闭环
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 根路径 `/` 默认入口切换、`?demo` 清理和 `/u/:id` 显式分享入口已完成；下一批只选择一个高价值小闭环进入实现，候选为移动阅读 / 来源返回复核、购买 / 订单 / 背包浏览器轻链路、纯 Web 登录后轻量链路补强或 Flutter 下一批功能
