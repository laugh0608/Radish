...
- **当前结论**：
  ...
  - F1 商城、F2 主题与 F3 i18n 已完成；F3-D 的 Public / Private / Auth、Console 认证 / 无权限和管理员受权内页均已通过双语 PC / mobile 代表矩阵。当前进入 F4，从既有产品边界和长期价值中选择下一项完整功能专题。
...
- 当前已进入发布后常态开发与 F4；从现有产品定位、功能缺口、长期系统价值和稳定边界中确认一个完整功能专题，不等待主动生产数据采集。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题与 F3 i18n 已关闭，功能完成线当前进入下一专题的选择与设计。
...
1. 从 Docs、聊天、通知、治理、宠物或其他既有业务域中确认一个长期价值明确且能够完整交付的功能专题。
2. 进入代码前先完成专题设计，明确用户路径、数据模型、接口、页面、权限、失败恢复、停止线和验证口径。
3. 已被动收到的明确生产故障或可追溯反馈继续进入维护线；F4-A 主动采集保持冻结，不因缺少生产证据阻断当前专题。
