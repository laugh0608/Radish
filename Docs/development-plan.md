...
- **最近结论**：
  ...
  - `P3-6-A` 已完成本地 Gateway 公开增长观察首轮收口，sitemap index / 分片 `<loc>` origin 检查已纳入 smoke 自动验证，未发现新的公开访问 `P0/P1`
...
- `45%`：`P3-6` 真实使用运营观察与反馈分流，优先做公开增长 smoke 失败诊断增强和真实反馈分流
...
- `P3-6`：真实使用运营观察与反馈分流；下一步优先做公开增长 smoke 失败诊断增强，帮助部署后定位 Gateway / API / Frontend 配置问题
