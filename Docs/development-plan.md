...
- **最近结论**：
  ...
  - `P3-10-B9` 已完成用户身份语义首批代码和自动化验证：`PublicIndex`、公开句柄、注册登录名 / 邮箱规范化、登录名或邮箱登录、公开资料 / 榜单 / 关系链 / 艾特搜索 / Console 用户展示和迁移入口已落地；运行态 Gateway 页面补验待宿主恢复。
...
- `30%`：`P3-10-B9 用户身份语义与公开索引` 运行态补验，宿主恢复后覆盖 Gateway PC / 移动视图，发现真实缺口后按契约、服务端规则、前端展示和页面路径成组修复。
- `20%`：`P3-10-B10 系统设置治理` 方案和首批低风险能力评审，先确认设置定义、默认值、覆盖值、风险等级、审计和安全边界。
- `15%`：B7 / C 公开入口、WebOS 功能迁移图和 `/notifications`、`/me`、`/messages` 维护收口，只在真实缺口或发布候选回归暴露问题时回拉。
- `10%`：B8 电子宠物维护回归，不继续扩展经济消耗、商城物品、社区任务奖励、经验反向加成或 Console 配置 UI。
- `10%`：用户 PublicId、评论实时 / 神评稳定性、Token 不活跃过期等长期契约发布候选前回归。
- `10%`：文档、验证留痕和发布候选前批次级回归准备，保持规划入口、专题说明、日志与真实代码状态一致。
...
- `P3-10-B9 用户身份语义与公开索引`
  - 首批代码和自动化验证已完成；下一步补 Gateway PC / 移动页面验收，重点覆盖注册、登录名或邮箱登录、公开句柄展示、艾特搜索、公开榜单、关系链和 Console 用户排障。
  - 若验收无阻断，B9 转入维护线；若有真实缺口，按同一问题族成组修复，不扩大到联邦协议、邮箱通知系统或数据库主键迁移。
- `P3-10-B10 系统设置治理`
  - B9 运行态验收通过后进入方案与首批实现评审；先处理设置定义、默认值、覆盖值、风险等级、审计和低风险配置入口。
  - 不把部署密钥、宠物经济数值、高危资产设置或安全会话策略直接搬进 Console。
