...
- **产品下一顺位**：`从既有功能缺口中裁决一个长期边界清楚的完整专题`
...
- **当前结论**：
  ...
  - F4-M 与 F4-N 均已完成 A-D 批并关闭。F4-N 的 Main 原子资产事务、Log 幂等审计投影、逐笔屏蔽抑制通知、Pencil、正式 Web、多身份 PC / mobile、临时数据清理和严格 migration verify 形成闭环；下一顺位等待批准进入 F4-O-A 只读候选审计。
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-N 均已关闭，下一顺位等待批准进入 F4-O-A 只读候选审计。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-N 已关闭，功能完成线当前停在下一专题候选审计批准门。
...
13. [F4-M 论坛内容版本完整性与作者恢复](/features/forum-content-version-recovery-design) 与 [F4-N 论坛内容赞赏](/features/forum-content-reward)均已完成 A-D 批并关闭；下一顺位为 F4-O-A 只读候选审计，在裁决单一专题并补齐权威设计前不进入代码。
