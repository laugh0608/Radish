...
- **最近结论**：
  ...
  - 下一主线切到 `P3-2 PublicId 最小试点方案`；`P3-2-A` 已完成外部 ID 契约审计，首批最小试点对象收敛为 `Post`，不启动全量迁移
...
- `45%`：`P3-2` 外部 ID 契约与 `PublicId` 最小试点方案，继续保持 `LongId` 字符串安全过渡边界
- `20%`：公开内容增长基础后续评估，聚焦动态 sitemap、详情结构化数据与 SSR / SSG 是否值得进入下一批
- `15%`：代码热区拆分候选评估，优先看超大公开页面、超大 Service 与 Flutter 大页面
...
- `P3-2`：按 `Post` 最小试点方案推进实现前评估；保持 `VoId / postId` 旧链路兼容，先补 `Post.PublicId` 并行契约、forum canonical、通知 `extData` 与窗口参数双字段解析
- 后续候选顺序：代码热区拆分、用户留存轻闭环、公开内容增长后续专题
...
- 第三开发阶段大功能开发：需先完成 `P3-2 PublicId` 最小试点方案，不直接启动全量迁移
