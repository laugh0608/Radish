...
- **产品下一顺位**：`完成服务端后推进 Pencil + 正式 Web 搜索工作区，再执行成组验收；Reaction / 置顶继续后置`
...
- **当前结论**：
  ...
  - F4-C-A 已完成 Chat 数据、索引、成员 ACL、搜索文本、跨库查询、快照 cursor、消息定位、PC / mobile 入口和失败恢复设计；工程下一顺位进入 F4-C-B 服务端实现。
...
- 当前已进入发布后常态开发与 F4；一对一私聊和 F4-B 通知中心深化均已关闭，当前只推进 F4-C 聊天历史搜索与消息定位，不并行展开其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊与 F4-B 通知中心深化已关闭，功能完成线当前进入 F4-C-B 服务端权威检索契约。
...
3. F4-C-A 现状审计和 [聊天历史搜索与消息定位设计](/features/chat-message-search-design) 已完成，固定采用派生 `SearchText`、服务端成员 ACL、跨库字面量包含、快照 cursor 和现有消息窗口定位协议。
4. 下一批 F4-C-B 完成 migration、回填、仓储 / Service / API / `@radish/http` 契约和 SQLite / PostgreSQL 定向测试；随后依次推进 F4-C-C Pencil + 正式 Web 搜索工作区、F4-C-D 双账号成组验收。消息 Reaction、置顶及移动系统通知继续后置。
