...
- **最近结论**：
  ...
  - `P3-10-A` 已完成初版任务归属、历史功能规划回拉和源码复核；当前进入 `P3-10-B / C`，优先准备 Web 首页信息流、用户 PublicId、评论实时 / 神评稳定性的开发批次。
...
- `40%`：Web 首页信息流和 `/discover` 首批改造，重点是内容来源、卡片形态、移动 / PC 浏览器体验、公开 head 与登录后轻互动边界。
- `25%`：用户 PublicId、评论实时 / 神评稳定性、Token 不活跃过期等长期契约治理。
- `15%`：Web UI 改造与去 WebOS 化迁移图，重点是首页、详情页、个人页和评论区。
- `10%`：历史功能规划回拉只作为当前批次输入，不再单独占用默认主线。
...
- `P3-10-B1 / C1 Web 首页信息流`
  - 将 `/discover` 从公开导航聚合页推进为可持续浏览的信息流入口。
  - 首批先定内容来源、卡片形态、瀑布流 / 分栏布局、公开 head、移动布局和登录后轻互动边界。
- `P3-10-B3 User PublicId`
  - 优先处理公开主页、分享链接、通知回流和未来个人圈子会触达的用户公开标识。
  - 保留 LongId 兼容读取，不做数据库主键迁移。
- `P3-10-B4 / B5 评论实时与神评稳定性`
  - 设计 post group、评论实时事件、typing 状态、前端去重和双用户验证入口。
  - 明确神评 / 沙发稳定窗口、并列展示、替换阈值和奖励幂等。
- `P3-9 主路径维护回拉`
