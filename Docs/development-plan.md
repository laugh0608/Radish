...
- **当前子阶段**：`F4 既有功能持续完成`
- **工程第一顺位**：`F4-I-B 内容治理案件、证据与动作服务端权威契约`
- **产品下一顺位**：`收束举报聚合、不可变证据、治理决定、目标 / 用户动作和当前状态的一致性`
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-H 均已关闭，F4-I-A 已完成。下一顺位执行 F4-I-B 服务端权威契约，不并行实施其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-H 已关闭，功能完成线当前进入 F4-I-B 内容治理服务端权威契约。
...
9. [F4-I 内容治理案件、证据与动作一致性](/features/content-moderation-case-evidence-action-design) 已完成 A 批候选复核与权威设计；下一顺位进入 F4-I-B，完成 Main migration、案件 / 证据 / 当前状态的权威服务端契约、五类目标动作适配和定向回归，不重启主动生产证据采集。
