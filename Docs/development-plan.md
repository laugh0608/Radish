...
- **当前子阶段**：`F3 i18n 专题验收收口`
- **工程第一顺位**：`F3-D Console 管理态双语矩阵`
- **产品下一顺位**：`F4 反馈驱动的新功能`
...
- **当前结论**：
  ...
  - F1 商城、F2 主题以及 F3-A 至 F3-C9 已完成；F3-D 的 Public / Private / Auth 和 Console 认证 / 无权限运行态矩阵已通过，当前只剩管理员会话下的 Console 受权内页双语 PC / mobile 证据。该证据补齐前不提前启动 F4；补齐后由真实使用观察和反馈归因选择下一项完整功能专题。
...
- 当前已进入发布后常态开发；先完成 F3-D Console 管理态验收并关闭 i18n 专题，再进入 F4 反馈归因与选题。
...
1. 在本地管理员会话下完成 F3-D Console Dashboard、用户、治理、订单、应用、个人资料与 Hangfire 的 `zh / en × PC / mobile` 只读代表矩阵，补齐无权限、not-found、长文本、表格和格式化证据。
2. 证据通过后关闭 F3，并按 [发布后维护与功能完成线](/planning/post-release-maintenance-feature-completion)进入 F4：观察登录、内容参与、聊天、通知和 Console 管理的真实使用，完成反馈归因与优先级整理。
3. 只从具有明确用户路径、真实使用价值和长期维护边界的候选中选择一个完整功能专题；P0/P1 可中断功能专题，P2/P3 按同类问题成组维护。
