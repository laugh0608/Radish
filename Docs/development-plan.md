...
- **产品下一顺位**：`冻结 approved testing Gateway、fail-closed build、privacy / entitlement / export compliance preflight 与 internal-only 授权清单；External TestFlight / App Store 无限期后置`
...
- 当前已进入发布后常态开发与 F4；一对一私聊、F4-B 至 F4-Q、F4-S 及 F4-R Web 页面族 / 主题退出门禁均已关闭，`v26.8.1-release` 正式 tag 与五镜像发布完成。Flutter Native P1–P5、P6 Android 双 AVD 开发门禁与 P7-C iOS Simulator 开发门禁均已关闭；P6-C 真机验收转为 Android 分发前门禁，P7-D readiness 已完成，项目所有者将近期 iOS 目标限制为 Internal TestFlight。P7-D1 minimal readiness 为当前第一顺位；External TestFlight / App Store 与账号删除、完整 Flutter UGC、StoreKit / IAP 无限期后置。未单独授权前不读取签名材料，不连接或安装真机，不创建 archive 或外部分发状态。生产部署与长期运维由项目所有者独立负责，不进入当前开发顺位。
...
17. [F4-R 家族 UI 统一接入与产品视觉重构](/features/family-ui-convergence-design)的 Web 基线已关闭；[Flutter Native 产品化与 UI 重构](/features/flutter-native-product-ui-design)已完成 P0–P5，[P5-E 成组静态门禁](/records/f4-flutter-native-p5e-grouped-static-gate-implementation-2026-08-27)关闭首轮静态门禁。P6-A / B 已完成 Android 候选装配和双 AVD 运行态，项目所有者允许以 `Android UI AVD RC Go` 进入下一阶段；P7-A / B 建立 iOS 平台 owner，P7-C 以 iPhone / iPad 真实运行给出 `Simulator Go`。P7-D readiness 后，项目所有者进一步确认近期只做 Internal TestFlight：D1 minimal readiness、D2 development-signed 真机、D3 TestFlight Internal Only 依次推进，D4 external / App Store 无限期后置。
