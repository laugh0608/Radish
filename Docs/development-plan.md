...
- `35%`：`P3-10` 后续产品 / 治理增量选择，优先推进边界清楚、能降低真实风险、不会扩成大专题的候选项。
...
- `P3-10 后置治理候选：前端敏感日志脱敏`
  - 当前优先从 [密码传输与请求签名临时评审](/guide/password-transport-and-request-signature) 回拉前端敏感日志脱敏，排查 `paymentPassword`、`password`、`currentPassword`、`newPassword`、`confirmPassword` 等字段是否被对象日志输出。
  - 只做日志参数移除 / 脱敏和必要测试；不启动浏览器通用签名、字段级加密、支付口令慢哈希迁移、幂等体系、安全会话或资产风控扩面。
