...
- **最近结论**：
  ...
  - 当前切到 `P3-8-D`：移动 Web 公开视图验收矩阵、根路径 `/` 默认入口切换、历史 `?demo` 入口移除、公开个人页分享入口、Flutter 公开商品只读详情、轻量 forum 通知列表、公开商城列表、论坛详情评论发布 / 回复、原生公开详情分享入口、公开文档阅读链路、公开主页链接 / 来源返回复核、登录态商城订单 / 背包只读入口、纯 Web forum 公开详情到工作台参与回流和 shop 登录态购买意图回流已完成；后续继续从纯 Web / Flutter 真实主路径缺口中选择下一批可验证任务
...
- `45%`：Flutter 真实移动主路径补齐，已补公开商品只读详情、轻量 forum 通知列表、公开商城列表、论坛详情评论发布 / 回复、公开详情分享入口、docs 文档内链阅读、公开主页链接 / 来源返回复核和登录态商城订单 / 背包只读入口，后续继续从移动阅读、复访和跨端回流缺口中选择下一批任务
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 根路径 `/` 默认入口切换、`?demo` 清理、`/u/:id` 显式分享入口、移动 Web 公开视图矩阵阶段收口、Flutter 公开商品只读详情、轻量 forum 通知列表、公开商城列表、论坛详情评论发布 / 回复、公开详情分享入口、docs 文档内链阅读、原生公开主页链接 / 来源返回复核、登录态商城订单 / 背包只读入口、纯 Web forum 公开详情到工作台参与回流和 shop 登录态购买意图回流已完成；下一批继续从 Flutter 真实移动主路径或跨端回流高信号缺口中选择，不直接启动完整移动商城、完整通知中心或 WebOS 新功能
