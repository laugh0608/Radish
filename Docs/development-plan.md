...
- **最近结论**：
  ...
  - 当前切到 `P3-8-D`：移动 Web 公开视图验收矩阵、根路径 `/` 默认入口切换、历史 `?demo` 入口移除、公开个人页分享入口与 Flutter 公开商品只读详情小闭环已完成；下一批候选矩阵推荐推进 `Flutter 轻量 forum 通知列表`
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 根路径 `/` 默认入口切换、`?demo` 清理、`/u/:id` 显式分享入口、移动 Web 公开视图矩阵阶段收口和 Flutter 公开商品只读详情已完成；下一批优先推进 `Flutter 轻量 forum 通知列表`，纯 Web 登录后轻量链路与购买 / 订单 / 背包只在维护线暴露阻断时回拉
