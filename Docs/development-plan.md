...
- **当前阶段**：`截至 2026-03-27，社区相关主功能与首版范围内的大部分业务 / 体验主线已完成首轮闭环，但这仍不等于整体产品已经完全达到首版 dev 条件。当前已按“所有纳入首版范围的核心功能主线至少完成 70%”重新定义首版 dev 边界：一方面继续补齐尚未完成或尚未复核的全局主线，另一方面把国风视觉基线 / 主题切换与 radish.client i18n 纳入首版范围，并把最小 CI 门禁与 Docker 镜像构建链作为工程门槛。当前最新一次用于合并的 `master` PR 已完成 `Repo Hygiene`、`Frontend Lint`、`Baseline Quick` 并成功合并到 `master`，`dev` 也已同步 `master` 合并结果。radish.client i18n 已完成桌面壳层、应用注册、商城主链路、论坛高频讨论链路与边缘页、文档应用主链，以及聊天、通知中心、个人中心的高频模块首轮覆盖；通知中心 / 个人中心与桌面壳层残余语言 / 主题烟雾验证也已完成。通知中心已于 `2026-03-23` 完成一轮真实首版 Smoke，认证 / OIDC / Gateway 基础入口也已于同日完成真实首版 Smoke，当前均已转入等待总回归确认。Console 与社区主链的多项边界已完成收口，首版最小 Docker 资产与交付口径也已于 `2026-03-25` 完成本轮收口。最新于 `2026-03-26`，WebOS 桌面与应用容器、论坛基础、社区 P0、Console V1，以及 `radish.client` 国风视觉基线 / 主题切换 / i18n 又完成了一轮首版烟雾联调，用户确认“全部都没啥问题，可以收口了”。当前又已确认 `GHCR` 后端镜像真实产物可被 `docker pull` 获取，且 `frontend` 已接入统一 GHCR 推送规则。当前结论已收束为：首版 `dev` 可发内部开发版；真实外部反代域名 / Auth 证书 / OIDC 回调链路复核已明确后置到 `CI/CD` 完善且具备 Docker 镜像推送能力后再正式执行；当前下一阶段主线转为完成 `frontend` GHCR 首次真实产物验证，并继续收口上线前交付复核口径。详见：[首版 dev 边界](/planning/dev-first-scope)。`
- **并行治理尾项**：
  - 身份语义 Phase 4：协议输出收敛
...
13. **M15 首轮最小 CI 门禁已完成真实落地**
   ...
   - 当前结论为：首次 CI/CD 已完成真实合并闭环，并已支撑首版 `dev` 进入“可发内部开发版”状态；后端 `GHCR` 镜像真实产物已完成验证，后续重点转为验证 `frontend` GHCR 首次真实产物，并在条件具备后再执行上线前交付复核
...
18. **首版 `dev` 总回归记录已补，验证基线最新结果已回写**
   ...
   - 当前结论为：业务 / 体验主线 Smoke 与验证基线均已完成本轮收口，首版剩余重点进一步收束到内部开发版后观察、`frontend` GHCR 首次真实产物验证，以及真实外部交付复核准备
...
- `M13` 首轮验证入口回归观察与补漏
- `M15` 最小 CI 已完成首轮真实闭环，当前已完成 `GHCR` 后端镜像真实产物验证，并进入 `frontend` 统一镜像推送接入后的首次产物验证；真实外部交付复核后置到条件具备后执行
