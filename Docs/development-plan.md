...
- `35%`：`P3-4` 用户留存轻闭环阶段性收尾，整理 forum / docs / shop 回流矩阵覆盖面、验证留痕和剩余观察项
- `20%`：`P3-2` `Post.PublicId` 首批实现后的兼容观察；只处理真实暴露的回流断点，不扩全量迁移
- `20%`：公开内容增长基础后续评估，聚焦动态 sitemap、详情结构化数据与详情首包可见性是否值得进入下一批
...
- `P3-4`：forum / docs / shop 留存回流矩阵已完成一轮主动验收；下一步先做阶段性收尾判断，后续只从真实使用暴露的公开分享 / 复访断点中选择小闭环
