...
- **最近结论**：
  ...
  - 当前切到 `P3-8-D`：移动 Web 公开视图验收矩阵、根路径 `/` 默认入口切换、历史 `?demo` 入口移除、公开个人页分享入口、Flutter 公开商品只读详情、轻量 forum 通知列表、公开商城列表与论坛详情评论发布 / 回复已完成；后续继续从纯 Web / Flutter 真实主路径缺口中选择下一批可验证任务
...
- `45%`：移动 Web 公开视图验收矩阵，覆盖公开直达、窄屏阅读、来源返回、分享入口和只读边界
- `25%`：根据矩阵暴露的真实主路径缺口做纯 Web 小闭环修复，优先根路径 `/` 默认入口切换、公开访问、移动阅读、跨端回流、购买 / 订单 / 背包和运行日志高信号问题
- `15%`：Flutter 真实移动主路径补齐，已补公开商品只读详情、轻量 forum 通知列表、公开商城列表和论坛详情评论发布 / 回复，后续仍只选择一个高价值小批次
...
- `P3-8-D 多端主路径缺口补齐与验收矩阵`
  ...
  - 根路径 `/` 默认入口切换、`?demo` 清理、`/u/:id` 显式分享入口、移动 Web 公开视图矩阵阶段收口、Flutter 公开商品只读详情、轻量 forum 通知列表、公开商城列表和论坛详情评论发布 / 回复已完成；下一批仍只选择一个高价值任务进入实现，纯 Web 登录后轻量链路与购买 / 订单 / 背包只在维护线暴露阻断时回拉
