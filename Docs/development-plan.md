...
- **当前结论**：
  ...
  - F1 商城、F2 主题与 F3 i18n 已完成；F3-D 的 Public / Private / Auth、Console 认证 / 无权限和管理员受权内页均已通过双语 PC / mobile 代表矩阵。当前进入 F4，由真实使用观察和反馈归因选择下一项完整功能专题。
  - F4-A 首轮仓库盘点只确认一条发布后生产 UX 证据：首次管理员入口门禁不一致。该 `P2` 已在 `dev` 修复；其他六链路样本均来自本地种子、受控验收或发布前设计反馈，当前不足以选择功能专题，继续补采生产证据。
...
- 当前已进入发布后常态开发与 F4；先整理真实使用证据和反馈归属，再确认一个完整功能专题，不按内部页面清单任意选题。
...
1. 继续 F4-A，按 [首批真实使用证据整理与反馈归因记录](/records/f4-a-first-real-usage-evidence-attribution-2026-07-18) 补采生产六链路脱敏聚合与可追溯反馈，排除种子、smoke、自动化和维护操作。
2. 将单个已验证 `P0/P1` 直接纳入维护线，把 `P2/P3` 按共同成因成组；没有原文或可追溯来源的反馈只作为线索。
3. 只有两个独立真实使用信号相互印证，且能明确用户路径、长期价值、数据 / 接口 / 页面边界、停止线和验证口径时，才建立一个完整功能专题并进入代码实现。
