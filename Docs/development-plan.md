...
- **当前子阶段**：`F4 既有功能持续完成`
- **工程第一顺位**：`通知中心深化专题：现状审计与设计边界`
- **产品下一顺位**：`通知偏好、聚合 / 容量与可靠目标回流；聊天搜索 / Reaction / 置顶继续候选后置`
...
- **当前结论**：
  ...
  - F1 商城、F2 主题、F3 i18n 与一对一私聊批次 A-D 均已完成；私聊已通过 SQLite / PostgreSQL 定向回归及双账号 `zh / en × PC / mobile` 运行态矩阵并关闭。下一顺位进入通知中心深化专题的现状审计与设计。
...
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n 与一对一私聊已关闭，功能完成线当前进入通知中心深化专题设计。
...
2. 下一专题先完成通知中心深化的现状审计与设计，围绕通知偏好、聚合 / 容量治理、可靠目标回流、权限和失败恢复形成完整边界后再实施。
3. 聊天搜索 / Reaction / 置顶及移动系统通知继续后置，不与通知专题设计并行扩展。
