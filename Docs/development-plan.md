...
- **当前结论**：
  ...
  - F1 商城、F2 主题与 F3 i18n 已完成；F3-D 的 Public / Private / Auth、Console 认证 / 无权限和管理员受权内页均已通过双语 PC / mobile 代表矩阵。一对一私聊与会话管理已确认为下一专题并完成设计，当前进入数据与访问边界实现。
...
- 当前已进入发布后常态开发与 F4；下一专题固定为正式 Web 一对一私聊与会话管理，不再并行展开其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题与 F3 i18n 已关闭，功能完成线当前进入一对一私聊的数据、权限与接口实现。
...
1. 按 [一对一私聊与会话管理设计](/features/chat-direct-conversation-design) 推进批次 A：`DirectConversation`、Chat schema migration、统一成员 ACL、持久化发送幂等和 Chat 附件访问。
2. 批次 A 通过 SQLite / PostgreSQL 与未授权访问测试后，再推进会话请求生命周期和正式 Web 页面；不得提前开放公开个人页“发消息”。
3. 私聊专题完成后再评估通知深化、聊天搜索 / Reaction / 置顶或其他功能，不在当前批次并行扩展。
