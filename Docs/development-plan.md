...
- **产品下一顺位**：`更新 PC / mobile Pencil 后实现消息回应 UI、picker、revision Store 与重连追平；成组验收留给 F4-D-D`
...
- **当前结论**：
  ...
  - F4-D-A / B 已完成 Chat 专属 Reaction 设计与服务端权威契约：`CanReact`、目标状态 operation ledger、消息 revision、完整 Hub 快照、撤回一致性、30 天清理和首批不通知边界均已固定并落地；下一批进入 Pencil 与正式 Web。
...
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊、F4-B 通知中心深化与 F4-C 聊天历史搜索已关闭，功能完成线当前进入 F4-D-C Pencil 与正式 Web。
...
4. [F4-D 聊天消息 Reaction](/features/chat-message-reaction-design) 的 A / B 批已完成设计与服务端权威契约；下一批先更新 PC / mobile Pencil，再实现正式 Web / WebOS 共用的消息回应、picker、revision Store 与重连追平。消息置顶、逐条已读及移动系统通知继续后置。
