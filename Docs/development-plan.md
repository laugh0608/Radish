...
- 当前已进入发布后常态开发与 F4；一对一私聊、F4-B 通知中心深化、F4-C 聊天历史搜索、F4-D 消息 Reaction 和 F4-E 消息置顶均已关闭，F4-F-A / B 轻量阅读回执审计、设计和服务端权威契约已完成，当前只推进 F4-F-C 正式 Web 与 WebOS 共用页面，不并行展开其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B / C / D / E 已关闭，功能完成线当前进入 F4-F-C 轻量阅读回执正式 Web 与 WebOS 共用页面。
...
6. [F4-F 聊天轻量阅读回执](/features/chat-message-read-receipt-design) 已完成 A / B 批现状审计、权威设计和服务端契约；下一顺位进入 C 批，先更新 Pencil PC / mobile，再实现活跃阅读面、精确 REST 游标、Store、Direct 已读边界和普通 Private 读者详情。移动系统通知继续后置。
