...
- **当前结论**：
  ...
  - F4-I-C 已完成 Pencil 与正式页面：Console `P02 / P07`、`/moderation` Case 工作台、私域 `/me/reports`、权限分离、冲突草稿保留、双语和目标失效摘要均已收口，旧四个治理 HTTP 入口已退役；下一顺位进入 F4-I-D 成组运行态验收。
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-H 均已关闭，F4-I-A-C 已完成。下一顺位执行 F4-I-D 多角色、五目标成组验收，不并行实施其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-H 已关闭，功能完成线当前进入 F4-I-D 内容治理专题验收与关闭。
...
9. [F4-I 内容治理案件、证据与动作一致性](/features/content-moderation-case-evidence-action-design) 已完成 A-C 批设计、服务端权威契约、Pencil 与正式页面；下一顺位进入 F4-I-D，以举报者、被举报者、只读治理员、审核员和动作执行员覆盖五类目标及 `zh / en × PC / mobile`，完成清理、数据库完整性和专题关闭。
