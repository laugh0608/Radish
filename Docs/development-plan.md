...
- **当前子阶段**：`F4 既有功能持续完成`
- **工程第一顺位**：`F4-D-D 聊天消息 Reaction：成组验收`
- **产品下一顺位**：`执行双账号 zh / en × PC / mobile、权限、并发、重连与 WebOS 成组验收并清理临时数据`
...
- **当前结论**：
  ...
  - F4-D-A / B / C 已完成 Chat 专属 Reaction 设计、服务端权威契约、PC / mobile Pencil 与正式 Web：`CanReact`、目标状态 operation ledger、消息 revision、完整 Hub 快照、撤回一致性、30 天清理、picker、重连追平和首批不通知边界均已落地；下一批进入成组验收。
...
- 当前已进入发布后常态开发与 F4；一对一私聊、F4-B 通知中心深化和 F4-C 聊天历史搜索均已关闭，当前只推进 F4-D 聊天消息 Reaction，不并行展开其他候选功能。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊、F4-B 通知中心深化与 F4-C 聊天历史搜索已关闭，功能完成线当前进入 F4-D-D 成组验收。
...
4. [F4-D 聊天消息 Reaction](/features/chat-message-reaction-design) 的 A / B / C 批已完成设计、服务端权威契约、Pencil 与正式 Web；下一批执行双账号 `zh / en × PC / mobile`、权限、并发、重连与 WebOS 成组验收。消息置顶、逐条已读及移动系统通知继续后置。
