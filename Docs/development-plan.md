...
- **当前阶段**：`截至 2026-03-20，社区相关主功能已基本完成首轮闭环，但这不等于整体产品已经达到首版 dev 条件。当前已按“所有纳入首版范围的核心功能主线至少完成 70%”重新定义首版 dev 边界：一方面继续补齐尚未完成或尚未复核的全局主线，另一方面把国风视觉基线 / 主题切换与 radish.client i18n 纳入首版范围，并把 CI/CD 与 Docker 镜像首次跑通作为工程门槛。当前 radish.client i18n 已从待启动进入首轮覆盖阶段：桌面壳层、应用注册、商城主链路与论坛高频讨论链路已完成首批资源化，剩余聊天与其他业务页继续补齐。详见：[首版 dev 边界](/planning/dev-first-scope)。`
- **并行治理尾项**：
  - 身份语义 Phase 4：协议输出收敛
...
4. **体验主线纳入首版，但要收敛范围**
   ...
   - 当前 `radish.client` i18n 已完成桌面壳层、商城主链路与论坛高频讨论链路首轮覆盖，下一步继续补聊天与其余业务页。
...
13. **`radish.client` i18n 首轮高频覆盖已推进**
   ...
   - 当前结论为：`radish.client` i18n 不再处于“待启动”状态，但聊天与剩余论坛 / 文档 / 通知 / 个人中心细节页仍待继续补齐
...
- `radish.client` 国风视觉基线已完成规范冻结与桌面壳层首批落地，`i18n` 也已完成桌面壳层、商城主链路与论坛高频讨论链路首轮覆盖；后续继续推进剩余业务页适配与主题 token 收口
- `M13` 首轮验证入口回归观察与补漏
