...
- **产品下一顺位**：`完成 PostAnswer 生命周期、版本、治理、通知与采纳审计闭环`
...
- **当前结论**：
  ...
  - F4-M 与 F4-N 均已完成 A-D 批并关闭。F4-O-A 已从问答、投票、抽奖、圈子和匿名公开聊天中裁决“论坛问答回答生命周期与治理闭环”为唯一专题，固定 Answer PublicId / Revision、附件、分页、CAS、采纳事件、治理申诉、可靠通知和 A-D 批次；等待批准进入 F4-O-B。
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-N 均已关闭，F4-O-A 权威设计已完成，下一顺位等待批准进入 F4-O-B。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-N 已关闭，功能完成线当前停在 F4-O-B 服务端批准门。
...
14. [F4-O 论坛问答回答生命周期与治理闭环](/features/forum-answer-lifecycle-governance-design)已完成 A 批候选审计与权威设计；等待批准进入 B 批服务端、Main migration、治理通知与 HTTP 契约。
