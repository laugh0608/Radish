...
- `30%`：WebOS 功能迁移图收口，继续判断消息、个人状态、经验、资产和既有工作台能力的端归属。
- `25%`：首批高价值纯 Web 迁移能力推进；通知复访入口已落地，下一组优先评估个人状态 / 经验 / 资产入口。
- `5%`：通知复访入口 Gateway PC / 移动 smoke 补验，服务启动后覆盖登录恢复和目标分流。
...
- `P3-10-B7 WebOS 功能迁移图收口`
  ...
  - 通知复访入口已作为首批纯 Web 迁移落地；下一组优先评估个人状态 / 经验 / 资产入口，不把全量 WebOS 搬迁作为前置门禁。
