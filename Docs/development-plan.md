...
- `P3-10 后置安全治理评审：支付口令哈希升级`
  - 前端敏感日志脱敏已完成；下一候选建议从 [密码传输与请求签名临时评审](/guide/password-transport-and-request-signature) 回拉支付口令慢哈希升级评审。
  - 只评审支付口令存储 / 验证兼容升级、版本字段、旧口令验证后写回、测试和回滚口径；不直接启动浏览器通用签名、字段级加密、幂等体系、安全会话、完整钱包、经济扩展或资产风控扩面。
