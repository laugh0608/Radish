...
- **当前主线**：`P3-7-C2 ChatApp 聊天工作台首批热区拆分`
- **最近结论**：
  ...
  - `P3-7-C` 已完成近期任务重评估，下一批一天级任务按 `WikiApp -> ChatApp` 热区治理推进；`P3-7-C1` WikiApp 首批拆分已完成
...
- `40%`：`P3-7-C2` ChatApp 聊天工作台首批热区拆分，继续治理第三阶段点名的工作台代码热区
...
- `P3-7-C2 ChatApp 聊天工作台首批热区拆分`
  - 以行为等价拆分为主，不新增 Chat P2 backlog 功能
  - 优先保证通知 / 桌面入口跳转、目标消息定位、频道加载和窗口参数契约不回退
  - 验证覆盖 `radish.client` 类型检查与聊天 / 桌面导航相关 node tests
