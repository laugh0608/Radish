...
- **当前阶段**：`截至 2026-03-25，社区相关主功能已基本完成首轮闭环，但这不等于整体产品已经达到首版 dev 条件。当前已按“所有纳入首版范围的核心功能主线至少完成 70%”重新定义首版 dev 边界：一方面继续补齐尚未完成或尚未复核的全局主线，另一方面把国风视觉基线 / 主题切换与 radish.client i18n 纳入首版范围，并把最小 CI 门禁与 Docker 镜像构建链作为工程门槛。当前 master PR 已接通首轮仓库质量检查，radish.client i18n 已完成桌面壳层、应用注册、商城主链路、论坛高频讨论链路与边缘页、文档应用主链，以及聊天、通知中心、个人中心的高频模块首轮覆盖；通知中心 / 个人中心与桌面壳层残余语言 / 主题烟雾验证也已完成，其中通知中心已在 `2026-03-23` 完成一轮真实首版 Smoke，并从“待联调复核”转入“等待总回归确认”。同日，`radish-client / radish-console / radish-scalar` 的登录、回调、登出与 Gateway / Scalar 入口也已完成一轮真实首版 Smoke，当前“认证 / OIDC / Gateway 基础入口”已可视为完成收口，转入总回归前维护状态。最新一轮又补齐了 Console 与社区主链的几项收尾边界：`console.access` 已收口为“入口标记 + 真实后台能力联动”，分类与标签已拆分为独立管理模块，内容治理 / 胡萝卜 / 经验等级后台首版已纳入 Console，论坛公开个人主页与帖子详情 / 评论头像展示也已对齐当前实现。同时，首版最小 Docker 资产与交付口径也已完成本轮收口：`Radish.Api / Radish.Auth / Radish.Gateway / Frontend` Dockerfile、前端静态托管脚本，以及 `Deploy/docker-compose.yml / docker-compose.dev.yml / docker-compose.prod.yml` 已落地，并已完成一轮镜像 build 级验证、Compose dev 运行态 Smoke、Gateway 容器内 HTTP / HTTPS 模式切换，以及 `RADISH_PUBLIC_URL -> OpenIddict__Server__Issuer -> 官方 OIDC 客户端回调地址` 的配置链对齐。后续重点转向更大范围首版联调复核与总回归，而不再把 Docker 交付口径留作未完成项。详见：[首版 dev 边界](/planning/dev-first-scope)。`
- **并行治理尾项**：
  - 身份语义 Phase 4：协议输出收敛
...
13. **M15 首轮最小 CI 门禁已完成真实落地**
   - GitHub Actions 已新增 `Repo Quality` 工作流，并收口为仅在 `master` Pull Request 与手动触发时运行
   ...
   - 当前结论为：首次 CI/CD 已不再处于“待启动”状态；后续重点从“有没有入口”转为“继续稳定门禁、补首版镜像构建链”
...
- `M12` 收官后的首版 dev 边界冻结与全局主线补齐
- 按 [首版 dev 边界](/planning/dev-first-scope) 补齐未完成主线与待复核主线
- `radish.client` 国风视觉基线已完成规范冻结与桌面壳层首批落地，`i18n` 也已完成桌面壳层、商城主链路、论坛高频讨论链路与边缘页、文档应用主链，以及聊天 / 通知中心 / 个人中心高频模块的首轮覆盖；通知中心 / 个人中心与桌面壳层首轮烟雾验证已完成，其中通知中心已进一步完成真实首版 Smoke，本轮又补了桌面壳层手工联调、欢迎 App 主题 / 内容收口与长文案资源化，以及论坛发帖创作器 / 评论讨论面板的体验重排，后续转入更大范围联调复核与残余深层主题维护
- `M13` 首轮验证入口回归观察与补漏
- `M15` 首轮最小 CI 门禁回归维护与 Docker 镜像构建链运行态复核
