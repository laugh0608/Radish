...
- **当前结论**：
  ...
  - F4-K-A 已完成候选审计与权威设计：相较匿名公开聊天和论坛作者版本恢复，用户屏蔽更直接补齐关注、私信、通知和关系分发的用户安全缺口；专题固定 Main `UserBlock`、双向交互隔离、关注事务、Direct 历史只读、通知抑制、旧字段迁移、正式页面和 A-D 批次。下一顺位进入 F4-K-B。
...
- 当前已进入发布后常态开发与 F4；一对一私聊及 F4-B 至 F4-J 均已关闭。F4-K-A 已选定用户屏蔽与关系交互隔离，下一顺位执行 F4-K-B 服务端权威契约。
- 发布后只保持一个主要功能专题在进行；`P0/P1` 用户问题可以中断，P2/P3 按同类问题成组维护。
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊及 F4-B 至 F4-J 已关闭，功能完成线当前进入 F4-K-B。
...
11. [F4-K 用户屏蔽与关系交互隔离](/features/user-block-relationship-isolation-design) 已完成候选复核与权威设计；下一顺位进入 F4-K-B，完成 Main 真相源、关系策略、关注事务、Direct 迁移兼容和通知抑制。
