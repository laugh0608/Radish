...
- 当前已进入发布后常态开发；先处理首次管理员统一入口门禁，再启动商城商品效力与权益履约专题。
...
1. 以独立维护批次统一首次管理员初始化入口门禁，覆盖公开、Workbench、私域和 OIDC 回调路由契约。
2. 观察首批真实生产使用；P0/P1 可中断功能专题，P2/P3 按同类问题成组维护。
3. 进入 [发布后维护与功能完成线](/planning/post-release-maintenance-feature-completion)，优先完成商城商品效力与权益履约，再推进主题和 i18n 完成度。
