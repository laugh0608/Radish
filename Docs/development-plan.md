...
- **产品下一顺位**：`先固定 Reaction 的数据、权限、通知和正式 Web 边界；置顶、逐条已读与移动系统通知继续后置`
...
- 长期维护线处理反馈、安全、依赖、迁移和部署；F1 商城、F2 主题、F3 i18n、一对一私聊、F4-B 通知中心深化与 F4-C 聊天历史搜索已关闭，功能完成线当前进入 F4-D-A 现状审计与专题设计。
...
4. 下一批 F4-D-A 交叉审计现有 Reaction、Chat 消息与权限、通知聚合、实时同步、正式 Web / WebOS 和验证资产，形成独立专题设计后再进入实现。消息置顶、逐条已读及移动系统通知继续后置。
