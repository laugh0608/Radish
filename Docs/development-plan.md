...
- **当前阶段**：`截至 2026-03-19，社区相关主功能已基本完成首轮闭环，但这不等于整体产品已经达到首版 dev 条件。当前已按“所有纳入首版范围的核心功能主线至少完成 70%”重新定义首版 dev 边界：一方面继续补齐尚未完成或尚未复核的全局主线，另一方面把国风视觉基线 / 主题切换与 radish.client i18n 纳入首版范围，并把 CI/CD 与 Docker 镜像首次跑通作为工程门槛。详见：[首版 dev 边界](/planning/dev-first-scope)。`
- **并行治理尾项**：
  - 身份语义 Phase 4：协议输出收敛
...
- `radish.client` 国风视觉基线已完成规范冻结与桌面壳层首批落地，后续继续推进主题切换 / i18n 与高频页面适配
- `M13` 首轮验证入口回归观察与补漏
