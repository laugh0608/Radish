...
### 文件头校验测试（Magic Number）

| 测试用例 | 说明 | 预期结果 |
|---------|------|---------|
| 9.1 伪装图片 | 文本文件改扩展名为 .jpg | 400 Bad Request，提示文件内容与扩展名不匹配 |
| 9.2 伪装 PDF | HTML 文件改扩展名为 .pdf | 400 Bad Request，提示文件内容与扩展名不匹配 |
| 9.3 真实 PNG 声称 JPG | PNG 文件但文件名是 .jpg | 200 OK（因为都是允许的图片格式）|

**说明**：Magic Number 检查通过读取文件头字节来验证文件真实类型，防止恶意文件通过修改扩展名绕过类型检查。
